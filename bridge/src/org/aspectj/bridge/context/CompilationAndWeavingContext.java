...
public class CompilationAndWeavingContext {
  ...
  @NotNull
  private static final ThreadLocal<Stack<ContextStackEntry>> contextMap = new ThreadLocal<Stack<ContextStackEntry>>();

  // single thread mode stack
  @NotNull
  private static final Stack<ContextStackEntry> contextStack = new Stack<ContextStackEntry>();

  // formatters, by phase id
  @NotNull
  private static final Map<Integer, ContextFormatter> formatterMap = new HashMap<Integer, ContextFormatter>();
  @NotNull
  private static final ContextFormatter defaultFormatter = new DefaultFormatter();
  ...
  public static void registerFormatter(int phaseId, @NotNull ContextFormatter aFormatter) {
  ...
  public static String getCurrentContext() {
    final Stack<ContextStackEntry> contextStack = getContextStack();
    final Stack<String> explanationStack = new Stack<String>();
  ...
  @NotNull
  public static ContextToken enteringPhase(int phaseId, @NotNull Object data) {
    final Stack<ContextStackEntry> contextStack = getContextStack();
    final ContextTokenImpl nextToken = nextToken();
  ...
  public static void leavingPhase(ContextToken aToken) {
    ...
    while (!contextStack.isEmpty()) {
      final ContextStackEntry entry = contextStack.pop();
  ...
  @NotNull
  private static ContextFormatter getFormatter(@NotNull ContextStackEntry entry) {
    final Integer key = new Integer(entry.phaseId);
  ...
  private static class ContextStackEntry {
    ...
    public int phaseId;
    @NotNull
    private final WeakReference<Object> dataRef;

    public ContextStackEntry(@NotNull ContextTokenImpl ct, int phase, @NotNull WeakReference<Object> data) {
