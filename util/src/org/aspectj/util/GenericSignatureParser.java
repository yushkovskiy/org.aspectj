...
public final class GenericSignatureParser {
   ...
   */
  @NotNull
  public GenericSignature.ClassSignature parseAsClassSignature(@NotNull String sig) {
    this.inputString = sig;
    tokenStream = tokenize(sig);
    tokenIndex = 0;
    final GenericSignature.ClassSignature classSig = new GenericSignature.ClassSignature();
    // FormalTypeParameters-opt
    if (maybeEat("<")) {
      final List<FormalTypeParameter> formalTypeParametersList = new ArrayList<FormalTypeParameter>();
      do {
    ...
    final List<ClassTypeSignature> superIntSigs = new ArrayList<ClassTypeSignature>();
   ...
   */
  @NotNull
  public MethodTypeSignature parseAsMethodSignature(@NotNull String sig) {
    this.inputString = sig;
    tokenStream = tokenize(sig);
    tokenIndex = 0;
    ...
    // Parameters
    eat("(");
    final List<TypeSignature> paramList = new ArrayList<TypeSignature>();
    while (!maybeEat(")")) {
      final FieldTypeSignature fsig = parseFieldTypeSignature(true);
      if (fsig != null) {
        paramList.add(fsig);
      } else {
    ...
    final TypeSignature[] params = new TypeSignature[paramList.size()];
    ...
    // throws
    final List<FieldTypeSignature> throwsList = new ArrayList<FieldTypeSignature>();
    while (maybeEat("^")) {
      final FieldTypeSignature fsig = parseFieldTypeSignature(false);
      throwsList.add(fsig);
    }
    final FieldTypeSignature[] throwsSigs = new FieldTypeSignature[throwsList.size()];
   ...
   */
  @Nullable
  public FieldTypeSignature parseAsFieldSignature(@NotNull String sig) {
    this.inputString = sig;
    tokenStream = tokenize(sig);
    tokenIndex = 0;
   ...
   */
  @NotNull
  public String[] tokenize(@NotNull String signatureString) {
  ...
  private FormalTypeParameter parseFormalTypeParameter() {
    final FormalTypeParameter ftp = new FormalTypeParameter();
    // Identifier
    ...
    final List<FieldTypeSignature> optionalBounds = new ArrayList<FieldTypeSignature>();
    while (maybeEat(":")) {
  ...
  private ArrayTypeSignature parseArrayTypeSignature() {
    ...
    final FieldTypeSignature fieldType = parseFieldTypeSignature(true);
    if (fieldType != null) {
  ...
  private ClassTypeSignature parseClassTypeSignature() {
    ...
    while (!maybeEat(";")) {
      if (maybeEat(".")) {
        ...
        final List<SimpleClassTypeSignature> nestedTypeList = new ArrayList<SimpleClassTypeSignature>();
        do {
          ret.append(".");
          final SimpleClassTypeSignature sig = parseSimpleClassTypeSignature();
        ...
        ret.append("<");
        final TypeArgument[] tArgs = maybeParseTypeArguments();
  ...
  private SimpleClassTypeSignature parseSimpleClassTypeSignature() {
    final String identifier = eatIdentifier();
    final TypeArgument[] tArgs = maybeParseTypeArguments();
    if (tArgs != null) {
  ...
  private TypeArgument parseTypeArgument() {
    ...
    final FieldTypeSignature sig = parseFieldTypeSignature(false);
  ...
  private TypeArgument[] maybeParseTypeArguments() {
    if (maybeEat("<")) {
      final List<TypeArgument> typeArgs = new ArrayList<TypeArgument>();
      do {
        final TypeArgument arg = parseTypeArgument();
        typeArgs.add(arg);
      } while (!maybeEat(">"));
      final TypeArgument[] tArgs = new TypeArgument[typeArgs.size()];
  ...
  private TypeVariableSignature parseTypeVariableSignature() {
    final TypeVariableSignature tv = new TypeVariableSignature(eatIdentifier());
    eat(";");
    return tv;
  }

  private boolean maybeEat(@NotNull String token) {
  ...
  private void eat(@NotNull String token) {
    if (!tokenStream[tokenIndex].equals(token)) {
