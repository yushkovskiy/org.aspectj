...
public final class TestUtil {
  ...
  static {
    {
      final String[] paths = {"sp:aspectjrt.path", "sp:aspectjrt.jar", "../lib/test/aspectjrt.jar",
    ...
    }
    {
      ...
      if (!isLibDir(file)) {
        final File cur = new File(".").getAbsoluteFile();
    ...
    for (final Iterator iter = LIB_RPATHS.keySet().iterator(); iter.hasNext(); ) {
      final String key = (String) iter.next();
      final String path = LIB_RPATHS.getProperty(key);
      File file = null;
      URL url = null;
      try {
        file = libFile(path);
  ...
  public static boolean parseBoolean(final String input, boolean iaxOnError) {
    ...
    final String lc = input.trim().toLowerCase();
  ...
  public static File libFile(String rpath) {
    ...
    final File result = new File(LIB_DIR, rpath);
    if (result.exists()) {
      return result;
    }
  ...
  public static void assertSetEquals(String msg, Collection expected, Collection found) {
    ...
    final Set results1 = new HashSet(found);
    results1.removeAll(expected);

    final Set results2 = new HashSet(expected);
  ...
  public static Object runMethod(String classPath, String className, String methodName, Object[] args) {
    ...
    final ClassLoader loader = new TestCaseClassLoader(classPath);
  ...
  public static void assertMultiLineStringEquals(String message, String s1, String s2) {
    try {
      final BufferedReader r1 = new BufferedReader(new StringReader(s1));
      final BufferedReader r2 = new BufferedReader(new StringReader(s2));
  ...
  public static boolean sameDirectoryContents(final IMessageHandler handler, final File expectedBaseDir,
    ...
    final String[] paths = FileUtil.listFiles(expectedBaseDir);
    boolean result = true;
  ...
  public static boolean sameFiles(IMessageHandler handler, File expectedBaseDir, File actualBaseDir, String path) {
    final File actualFile = new File(actualBaseDir, path);
    final File expectedFile = new File(expectedBaseDir, path);
  ...
  public static void loadTestsReflectively(TestSuite sink, String sourceName, boolean ignoreError) {
    ...
    try {
      final ClassLoader loader = sink.getClass().getClassLoader();
      final Class sourceClass = loader.loadClass(sourceName);
      if (!Modifier.isPublic(sourceClass.getModifiers())) {
        errorSuite(sink, sourceName, "not public class");
        return;
      }
      final Method suiteMethod = sourceClass.getMethod("suite", new Class[0]);
      final int mods = suiteMethod.getModifiers();
      ...
      final Object result = suiteMethod.invoke(null, new Object[0]);
      final Test test = (Test) result;
      if (!(test instanceof TestSuite)) {
        sink.addTest(test);
      } else {
        final TestSuite source = (TestSuite) test;
        final Enumeration tests = source.tests();
  ...
  private static String readNonBlankLine(BufferedReader r) throws IOException {
    ...
    final int commentLoc = l.indexOf("//");
    if (-1 != commentLoc) {
  ...
  private static boolean doSameFile(IMessageHandler handler, File expectedBaseDir, File actualBaseDir, File expectedFile,
    ...
    if (!LangUtil.isEmpty(actualLines) && !LangUtil.isEmpty(expectedLines)) {
      ...
      // JDiff
      final CanonicalLine[][] clines = new CanonicalLine[][]{expectedLines, actualLines};
      final FileLine[][] flines = new FileLine[2][];
      for (int i = 0; i < clines.length; i++) {
        final CanonicalLine[] cline = clines[i];
        final FileLine[] fline = new FileLine[cline.length];
      ...
      final Diff.change edits = new Diff(flines[0], flines[1]).diff_2(false);
      if ((null == edits) || (0 == (edits.inserted + edits.deleted))) {
        ...
        final DiffNormalOutput out = new DiffNormalOutput(flines[0], flines[1]);
        out.setOut(writer);
        ...
        final String message = "diff between " + path + " in expected dir " + expectedBaseDir + " and actual dir "
  ...
  private static void errorSuite(TestSuite sink, String sourceName, String err) {
    final String message = "bad " + sourceName + ": " + err;
  ...
  private abstract static class Lineator implements ILineator {
    ...
    public CanonicalLine[] getLines(IMessageHandler handler, File file, File basedir) throws IOException {
      ...
      final LineStream capture = new LineStream();
      try {
      ...
      final String missed = capture.getMissed();
      if (!LangUtil.isEmpty(missed)) {
        MessageUtil.warn(handler, "NormalizedCompareFiles missed input: " + missed);
        return null;
      } else {
        final String[] lines = capture.getLines();
        final CanonicalLine[] result = new CanonicalLine[lines.length];
  ...
  public static class ClassLineator extends Lineator {
    ...
    protected void lineate(PrintStream sink, IMessageHandler handler, File basedir, File file) throws IOException {
      final String name = FileUtil.fileToClassName(basedir, file);
    ...
    private static void disassemble(IMessageHandler handler, File basedir, String name, PrintStream out) throws IOException {
      ...
      Throwable thrown = null;
      final String basedirPath = FileUtil.normalizedPath(basedir);
      // XXX use reflection utilities to invoke dissassembler?
      try {
        // XXX need test to detect when this is refactored
        final Class c = Class.forName("org.aspectj.weaver.bcel.LazyClassGen");
        final Method m = c.getMethod("disassemble", new Class[]{String.class, String.class, PrintStream.class});
