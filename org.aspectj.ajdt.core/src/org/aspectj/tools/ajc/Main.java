...
public final class Main {
   ...
   */
  public static int bareMain(@NotNull String[] args, boolean useSystemExit,
                             @Nullable List<String> fails, @Nullable List<String> errors,
                             @Nullable List<String> warnings, @Nullable List<String> infos) {
   ...
   */
  @NotNull
  public static String renderExceptionForUser(@NotNull Throwable thrown) {
    final String m = thrown.getMessage();
  ...
  public Main() {
    ...
    try {
      final String value = System.getProperty("aspectj.multithreaded");
  ...
  public void runMain(@NotNull String[] args, boolean useSystemExit) {
    ...
    try {
      ...
      final IMessage outOfMemoryMessage = new Message(OUT_OF_MEMORY_MSG, null, true);
   ...
   */
  public void run(@NotNull String[] args, @NotNull IMessageHolder holder) {
    PrintStream logStream = null;
    FileOutputStream fos = null;
    final String logFileName = parmInArgs("-log", args);
    if (null != logFileName) {
    ...
    try {
      ...
      if (ae.isSilent()) {
        ...
        final IMessage message = ae.getIMessage();
        final Throwable thrown = ae.getThrown();
        if (null == thrown) { // toss AbortException wrapper
          ...
          final String mssg = MessageUtil.MESSAGE_MOST.renderToString(message);
   ...
   */
  public void setHolder(@Nullable IMessageHolder holder) {
    clientHolder = holder;
  }
   ...
   */
  public void setCompletionRunner(@Nullable Runnable runner) {
   ...
   */
  protected void systemExit(@NotNull IMessageHolder messages) {
   ...
   */
  protected static void outMessage(@NotNull String message) { // XXX coordinate with MessagePrinter
   ...
   */
  protected boolean report(boolean pass, @NotNull IMessageHolder holder) {
    lastFails = holder.numMessages(IMessage.FAIL, true);
    final boolean result = (0 == lastFails);
    ...
    if (holder == ourHandler) {
      ...
      final int warnings = holder.numMessages(IMessage.WARNING, false);
      final StringBuffer sb = new StringBuffer();
      appendNLabel(sb, "fail|abort", lastFails);
      appendNLabel(sb, "error", lastErrors);
      appendNLabel(sb, "warning", warnings);
      if (0 < sb.length()) {
        final PrintStream out = (0 < (lastErrors + lastFails) ? System.err : System.out);
   ...
   */
  protected static void fail(@NotNull IMessageHandler handler, @NotNull String message, @Nullable Throwable thrown) {
    handler.handleMessage(new Message(message, IMessage.FAIL, thrown, null));
  }

  // for unit testing...
  void setController(@NotNull CommandController controller) {
   ...
   */
  private static void readMessages(@Nullable IMessageHolder holder, @Nullable Kind kind, boolean orGreater, @Nullable List<String> sink) {
    if ((null == sink) || (null == holder)) {
      return;
    }
    final IMessage[] messages = holder.getMessages(kind, orGreater);
  ...
  @Nullable
  private static String parmInArgs(@Nullable String flag, @Nullable String[] args) {
    final int loc = 1 + (null == args ? -1 : Arrays.asList(args).indexOf(flag));
    return ((0 == loc) || (args.length <= loc) ? null : args[loc]);
  }

  private static boolean flagInArgs(@NotNull String flag, @Nullable String[] args) {
   ...
   * content
   */
  private static void appendNLabel(@NotNull StringBuffer sink, @Nullable String label, int numItems) {
    if (0 == numItems) {
      return;
    }
    if (0 < sink.length()) {
   ...
   */
  @Nullable
  private IMessageHolder checkForCustomMessageHolder(@NotNull String[] args) {
  ...
  public static class MessagePrinter implements IMessageHandler {
     ...
     */
    @NotNull
    public static String render(@NotNull IMessage message) {
      ...
      final boolean toString = (LangUtil.isEmpty(text));
      if (toString) {
      ...
      String context = null;
      if (null != loc) {
        final File file = loc.getSourceFile();
        if (null != file) {
    ...
    public boolean handleMessage(@NotNull IMessage message) {
      if (null != message) {
        final PrintStream out = getStreamFor(message.getKind());
        if (null != out) {
     ...
     */
    @Nullable
    protected PrintStream getStreamFor(@Nullable Kind kind) {
  ...
  public static class LogModeMessagePrinter extends MessagePrinter {
    ...
    public LogModeMessagePrinter(boolean verbose, @NotNull PrintStream logStream) {
      super(verbose);
  ...
  public static class CommandController {
    ...
    /**
     * @see #init(String[], org.aspectj.bridge.IMessageHandler)
     */
    @NotNull
     ...
     */
    @NotNull
    public String[] init(@NotNull String[] args, @NotNull IMessageHandler sink) {
      running = true;
      // String[] unused;
      if (!LangUtil.isEmpty(args)) {
        final String[][] options = LangUtil.copyStrings(OPTIONS);
        /* unused = */
        LangUtil.extractOptions(args, options);
        incremental = (null != options[0][0]);
        if (null != options[1][0]) {
          final File file = new File(options[1][1]);
          if (!file.exists()) {
    ...
    protected void fileCheckDelay() {
      ...
      final long targetTime = System.currentTimeMillis() + delay;
      // long curTime;
