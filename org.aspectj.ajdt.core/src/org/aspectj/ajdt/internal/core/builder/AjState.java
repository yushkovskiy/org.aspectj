...
public class AjState implements CompilerConfigurationChangeFlags, TypeDelegateResolver {
   ...
   */
  public boolean prepareForNextBuild(@NotNull AjBuildConfig newBuildConfig) {
    ...
    if ((newBuildConfig.getChanged() & PROJECTSOURCEFILES_CHANGED) == 0) {
      ...
      final Set<File> oldFiles = new HashSet<File>(buildConfig.getFiles());
      final Set<File> newFiles = new HashSet<File>(newBuildConfig.getFiles());
    ...
    final Set<BinarySourceFile> oldBinaryFiles = new HashSet<BinarySourceFile>(buildConfig.getBinaryFiles());
    final Set<BinarySourceFile> newBinaryFiles = new HashSet<BinarySourceFile>(newBuildConfig.getBinaryFiles());
    ...
    final boolean couldStillBeIncremental = processDeletedFiles(deletedFiles);
  ...
  Collection<File> getModifiedFiles(long lastBuildTime) {
    ...
    final List<File> modifiedFiles = buildConfig.getModifiedFiles();

    if (modifiedFiles == null) {
      // do not know, so need to go looking
      // not our job to account for new and deleted files
      for (final Iterator<File> i = buildConfig.getFiles().iterator(); i.hasNext(); ) {
  ...
  Collection<BinarySourceFile> getModifiedBinaryFiles(long lastBuildTime) {
    ...
    // not our job to account for new and deleted files
    for (final Iterator<BinarySourceFile> i = buildConfig.getBinaryFiles().iterator(); i.hasNext(); ) {
      final AjBuildConfig.BinarySourceFile bsfile = i.next();
      final File file = bsfile.binSrc;
      if (!file.exists()) {
        continue;
      }
  ...
  private int classFileChangedInDirSinceLastBuildRequiringFullBuild(File dir, int pathid) {
    ...
    final AjState state = IncrementalStateManager.findStateManagingOutputLocation(dir);
    if (listenerDefined()) {
      if (state != null) {
    ...
    if (state == null) {
      ...
      final CompilationResultDestinationManager crdm = buildConfig.getCompilationResultDestinationManager();
      if (crdm != null) {
        final int i = crdm.discoverChangesSince(dir, lastSuccessfulBuildTime);
    ...
    final List<File> classFiles = FileUtil.listClassFiles(dir);

    for (final Iterator<File> iterator = classFiles.iterator(); iterator.hasNext(); ) {
      final File classFile = iterator.next();
      if (CHECK_STATE_FIRST && state != null) {
        ...
        if (state.isAspect(classFile)) {
          final boolean hasStructuralChanges = state.hasStructuralChangedSince(classFile, lastSuccessfulBuildTime);
        ...
        final long modTime = classFile.lastModified();
  ...
  private boolean isTypeWeReferTo(File file) {
    ...
    final int finalSeparator = fpath.lastIndexOf(File.separator);
    final String baseDir = fpath.substring(0, finalSeparator);
    final String theFile = fpath.substring(finalSeparator + 1);
  ...
  private boolean hasStructuralChangedSince(File file, long lastSuccessfulBuildTime) {
    ...
    final Long l = structuralChangesSinceLastFullBuild.get(file.getAbsolutePath());
    long strucModTime = -1;
    if (l != null) {
  ...
  private boolean hasAnyStructuralChangesSince(long lastSuccessfulBuildTime) {
    final Set<Map.Entry<String, Long>> entries = structuralChangesSinceLastFullBuild.entrySet();
    for (final Iterator<Map.Entry<String, Long>> iterator = entries.iterator(); iterator.hasNext(); ) {
      final Map.Entry<String, Long> entry = iterator.next();
      final Long l = entry.getValue();
      if (l != null) {
        final long lvalue = l.longValue();
  ...
  private boolean pathChange(AjBuildConfig previousConfig, AjBuildConfig newConfig) {
    final int changes = newConfig.getChanged();

    if ((changes & (CLASSPATH_CHANGED | ASPECTPATH_CHANGED | INPATH_CHANGED | OUTPUTDESTINATIONS_CHANGED | INJARS_CHANGED)) != 0) {
      final List<File> oldOutputLocs = getOutputLocations(previousConfig);

      final Set<String> alreadyAnalysedPaths = new HashSet<String>();

      final List<String> oldClasspath = previousConfig.getClasspath();
      final List<String> newClasspath = newConfig.getClasspath();
      if (stateListener != null) {
      ...
      final List<File> oldAspectpath = previousConfig.getAspectpath();
      final List<File> newAspectpath = newConfig.getAspectpath();
      if (changedAndNeedsFullBuild(oldAspectpath, newAspectpath, true, oldOutputLocs, alreadyAnalysedPaths, PATHID_ASPECTPATH)) {
        return true;
      }

      final List<File> oldInPath = previousConfig.getInpath();
      final List<File> newInPath = newConfig.getInpath();
      if (changedAndNeedsFullBuild(oldInPath, newInPath, false, oldOutputLocs, alreadyAnalysedPaths, PATHID_INPATH)) {
        return true;
      }

      final List<File> oldInJars = previousConfig.getInJars();
      final List<File> newInJars = newConfig.getInJars();
      ...
      final List<String> modifiedCpElements = newConfig.getClasspathElementsWithModifiedContents();
      for (final Iterator<String> iterator = modifiedCpElements.iterator(); iterator.hasNext(); ) {
        final File cpElement = new File(iterator.next());
        if (cpElement.exists() && !cpElement.isDirectory()) {
          if (cpElement.lastModified() > lastSuccessfulBuildTime) {
            return true;
          }
        } else {
          final int classFileChanges = classFileChangedInDirSinceLastBuildRequiringFullBuild(cpElement, PATHID_CLASSPATH);
   ...
   */
  private static List<File> getOutputLocations(AjBuildConfig config) {
    final List<File> outputLocs = new ArrayList<File>();
    ...
    if (config.getCompilationResultDestinationManager() != null) {
      final List<File> dirs = config.getCompilationResultDestinationManager().getAllOutputLocations();
  ...
  private static File getOutputLocationFor(AjBuildConfig config, File aResourceFile) {
    if (config.getCompilationResultDestinationManager() != null) {
      final File outputLoc = config.getCompilationResultDestinationManager().getOutputLocationForResource(aResourceFile);
      if (outputLoc != null) {
        return outputLoc;
      }
    }
  ...
  private boolean changedAndNeedsFullBuild(List oldPath, List newPath, boolean checkClassFiles, List<File> outputLocs,
    ...
    for (int i = 0; i < oldPath.size(); i++) {
      ...
      final Object o = oldPath.get(i); // String on classpath, File on other paths
      File f = null;
      ...
      if (checkClassFiles && f.exists() && f.isDirectory()) {
        ...
        for (final Iterator<File> iterator = outputLocs.iterator(); !foundMatch && iterator.hasNext(); ) {
        ...
        if (!foundMatch) {
          if (!alreadyAnalysedPaths.contains(f.getAbsolutePath())) { // Do not check paths more than once
            ...
            final int classFileChanges = classFileChangedInDirSinceLastBuildRequiringFullBuild(f, pathid);
            if (classFileChanges == CLASS_FILE_CHANGED_THAT_NEEDS_FULL_BUILD) {
              return true;
            }
          }
  ...
  private boolean classpathChangedAndNeedsFullBuild(List<String> oldPath, List<String> newPath, boolean checkClassFiles,
    ...
    for (int i = 0; i < oldPath.size(); i++) {
      ...
      final File f = new File(oldPath.get(i));
      if (f.exists() && !f.isDirectory() && (f.lastModified() >= lastSuccessfulBuildTime)) {
        return true;
      }
      if (checkClassFiles && f.exists() && f.isDirectory()) {
        ...
        if (!foundMatch) {
          if (!alreadyAnalysedPaths.contains(f.getAbsolutePath())) { // Do not check paths more than once
            alreadyAnalysedPaths.add(f.getAbsolutePath());
            final int classFileChanges = classFileChangedInDirSinceLastBuildRequiringFullBuild(f, PATHID_CLASSPATH);
            if (classFileChanges == CLASS_FILE_CHANGED_THAT_NEEDS_FULL_BUILD) {
              return true;
            }
          }
  ...
  private void removeAllResultsOfLastBuild() {
    ...
    for (final Iterator<List<ClassFile>> iter = this.inputClassFilesBySource.values().iterator(); iter.hasNext(); ) {
      final List<ClassFile> cfs = iter.next();
      for (ClassFile cf : cfs) {
        cf.deleteFromFileSystem(buildConfig);
      }
    }
    for (final Iterator<File> iterator = classesFromName.values().iterator(); iterator.hasNext(); ) {
      final File f = iterator.next();
      new ClassFile("", f).deleteFromFileSystem(buildConfig);
    }
    final Set<Map.Entry<String, File>> resourceEntries = resources.entrySet();
    for (final Iterator<Map.Entry<String, File>> iter = resourceEntries.iterator(); iter.hasNext(); ) {
      final Map.Entry<String, File> resourcePair = iter.next();
      final File sourcePath = resourcePair.getValue();
  ...
  private void deleteClassFiles() {
    ...
    for (File deletedFile : deletedFiles) {
      ...
      final List<ClassFile> cfs = this.fullyQualifiedTypeNamesResultingFromCompilationUnit.get(deletedFile);
  ...
  private void deleteBinaryClassFiles() {
    ...
    for (BinarySourceFile deletedFile : deletedBinaryFiles) {
      final List<ClassFile> cfs = this.inputClassFilesBySource.get(deletedFile.binSrc.getPath());
      for (final Iterator<ClassFile> iterator = cfs.iterator(); iterator.hasNext(); ) {
  ...
  private UnwovenClassFile createUnwovenClassFile(AjBuildConfig.BinarySourceFile bsf) {
    ...
    try {
      ...
      final IMessage msg = new Message("can't read class file " + bsf.binSrc.getPath(), new SourceLocation(bsf.binSrc, 0), false);
      buildManager.handler.handleMessage(msg);
    }
    return ucf;
  }
  ...
  private static void removeFromClassFilesIfPresent(String className, List<ClassFile> classFiles) {
  ...
  private void recordFQNsResultingFromCompilationUnit(File sourceFile, InterimCompilationResult icr) {
    ...
    final UnwovenClassFile[] types = icr.unwovenClassFiles();
  ...
  private void recordWhetherCompilationUnitDefinedAspect(File sourceFile, CompilationResult cr) {
    ...
    if (cr != null) {
      final Map compiledTypes = cr.compiledTypes;
      if (compiledTypes != null) {
        for (final Iterator<char[]> iterator = compiledTypes.keySet().iterator(); iterator.hasNext(); ) {
          final char[] className = iterator.next();
          final String typeName = new String(className).replace('/', '.');
          if (typeName.indexOf(BcelWeaver.SYNTHETIC_CLASS_POSTFIX) == -1) {
            final ResolvedType rt = world.resolve(typeName);
            if (rt.isMissing()) {
  ...
  private void recordClassFile(UnwovenClassFile thisTime, File lastTime) {
    if (simpleStrings == null) {
      ...
      // by the world.
      final ResolvedType rType = world.resolve(thisTime.getClassName());
      if (!rType.isMissing()) {
        try {
          final ClassFileReader reader = new ClassFileReader(thisTime.getBytes(), null);
    ...
    final CompactTypeStructureRepresentation existingStructure = this.resolvedTypeStructuresFromLastBuild
        .get(thisTime.getClassName());
    final ResolvedType newResolvedType = world.resolve(thisTime.getClassName());
    if (!newResolvedType.isMissing()) {
      try {
        ...
        try {
          final String s = System.getProperty("aspectj.debug377096", "false");
          if (s.equalsIgnoreCase("true")) {
            final String location = System.getProperty("java.io.tmpdir", "/tmp");
            final String name = thisTime.getClassName();
            final File f = File.createTempFile(location + File.separator + name, ".class");
    ...
    final byte[] newBytes = thisTime.getBytes();
    try {
      final ClassFileReader reader = new ClassFileReader(newBytes, lastTime.getAbsolutePath().toCharArray());
...
//						System.err.println("Detected a structural change in " + thisTime.getFilename());
  ...
  private boolean hasStructuralChanges(ClassFileReader reader, CompactTypeStructureRepresentation existingType) {
    ...
    final IBinaryAnnotation[] newAnnos = reader.getAnnotations();
    if (newAnnos == null || newAnnos.length == 0) {
      if (existingType.annotations != null && existingType.annotations.length != 0) {
        return true;
      }
    } else {
      final IBinaryAnnotation[] existingAnnos = existingType.annotations;
    ...
    // //}
    // }
    final IBinaryField[] existingFs = existingType.binFields;
    if (newFields.length != existingFs.length) {
      return true;
    }
    new_field_loop:
    for (int i = 0; i < newFields.length; i++) {
      final IBinaryField field = newFields[i];
      final char[] fieldName = field.getName();
      for (int j = 0; j < existingFs.length; j++) {
        if (CharOperation.equals(existingFs[j].getName(), fieldName)) {
          final IBinaryField existing = existingFs[j];
          ...
          final char[] existingGSig = existing.getGenericSignature();
          final char[] fieldGSig = field.getGenericSignature();
    ...
    // // }
    // }
    final IBinaryMethod[] existingMs = existingType.binMethods;
    if (newMethods.length != existingMs.length) {
      return true;
    }
    new_method_loop:
    for (int i = 0; i < newMethods.length; i++) {
      final IBinaryMethod method = newMethods[i];
      ...
      for (int j = 0; j < existingMs.length; j++) {
        if (CharOperation.equals(existingMs[j].getSelector(), methodName)) {
          ...
          if (!CharOperation.equals(method.getMethodDescriptor(), existingMs[j].getMethodDescriptor())) {
            ...
            // matching sigs
            final IBinaryMethod existing = existingMs[j];
            ...
            final char[] existingGSig = existing.getGenericSignature();
            final char[] methodGSig = method.getGenericSignature();
    ...
    final IBinaryNestedType[] binaryNestedTypes = reader.getMemberTypes();
    final IBinaryNestedType[] existingBinaryNestedTypes = existingType.getMemberTypes();
    ...
    if (binaryNestedTypes != null) {
      final int bnLength = binaryNestedTypes.length;
      if (existingBinaryNestedTypes.length != bnLength) {
        return true;
      }
      for (int m = 0; m < bnLength; m++) {
        final IBinaryNestedType bnt = binaryNestedTypes[m];
        final IBinaryNestedType existingBnt = existingBinaryNestedTypes[m];
  ...
  private static String stringify(IBinaryNestedType[] binaryNestedTypes) {
  ...
  private static String stringify(IBinaryMethod m) {
    final StringBuilder buf = new StringBuilder();
  ...
  private static Object stringify(IBinaryField f) {
    final StringBuilder buf = new StringBuilder();
  ...
  private static String stringify(char[][] arrayOfCharArrays) {
    final StringBuilder buf = new StringBuilder();
  ...
  private static String stringify(IBinaryAnnotation[] annotations) {
    final StringBuilder buf = new StringBuilder();
   ...
   */
  private static boolean exceptionClausesDiffer(IBinaryMethod lastMethod, IBinaryMethod newMethod) {
    final char[][] previousExceptionTypeNames = lastMethod.getExceptionTypeNames();
    final char[][] newExceptionTypeNames = newMethod.getExceptionTypeNames();
    final int pLength = previousExceptionTypeNames.length;
    final int nLength = newExceptionTypeNames.length;
    if (pLength != nLength) {
      return true;
    }
    if (pLength == 0) {
  ...
  private static String stringifySet(Set<?> l) {
    final StringBuilder sb = new StringBuilder();
    sb.append("{");
    for (final Iterator<?> iter = l.iterator(); iter.hasNext(); ) {
      final Object el = iter.next();
      sb.append(el);
      if (iter.hasNext()) {
        sb.append(",");
      }
   ...
   * @return
   */
  private static ClassFile getClassFileFor(UnwovenClassFile ucf) {
  ...
  private static class ClassFile {
    ...
    public void deleteFromFileSystem(AjBuildConfig buildConfig) {
      ...
      final File dir = locationOnDisk.getParentFile();
      if (dir != null) {
        final File[] weaverGenerated = dir.listFiles(new FilenameFilter() {
          @Override
