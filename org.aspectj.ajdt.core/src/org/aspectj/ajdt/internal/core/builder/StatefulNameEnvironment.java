...
public class StatefulNameEnvironment implements INameEnvironment {
  ...
  public StatefulNameEnvironment(INameEnvironment baseEnvironment, Map classesFromName, AjState state) {
    ...
    for (final Iterator i = classesFromName.keySet().iterator(); i.hasNext(); ) {
  ...
  private NameEnvironmentAnswer findType(String name) {
    ...
    final IBinaryType seenOnPreviousBuild = state.checkPreviousBuild(name);
    ...
    if (this.inflatedClassFilesCache.containsKey(name)) {
      ...
      final File fileOnDisk = (File) classesFromName.get(name);
      ...
      try {
        ...
        final byte[] bytes = FileUtil.readAsByteArray(fileOnDisk);
        final NameEnvironmentAnswer ret = new NameEnvironmentAnswer(new ClassFileReader(bytes, fileOnDisk.getAbsolutePath()
  ...
  public NameEnvironmentAnswer findType(char[] typeName, char[][] packageName) {
    final NameEnvironmentAnswer ret = findType(new String(CharOperation.concatWith(packageName, typeName, '.')));
    if (ret != null)
      return ret;
  ...
  public NameEnvironmentAnswer findType(char[][] compoundName) {
    final NameEnvironmentAnswer ret = findType(new String(CharOperation.concatWith(compoundName, '.')));
    if (ret != null)
      return ret;
  ...
  public boolean isPackage(char[][] parentPackageName, char[] packageName) {
    ...
    final String fullPackageName = new String(CharOperation.concatWith(parentPackageName, packageName, '.'));
  ...
  public void update(Map classNameToFileMap, Set added) {
    for (final Iterator i = added.iterator(); i.hasNext(); ) {
      final String className = (String) i.next();
      addAllPackageNames(className);
    }
