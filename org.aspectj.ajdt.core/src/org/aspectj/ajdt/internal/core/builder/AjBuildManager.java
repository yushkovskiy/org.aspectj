...
import org.aspectj.ajdt.internal.compiler.*;
...
public final class AjBuildManager implements IOutputClassFileNameProvider, IBinarySourceProvider, ICompilerAdapterFactory {
  @NotNull
   ...
   */
  public static void enableRuntimeVersionCheck(@Nullable Main caller) {
  ...
  public boolean batchBuild(@NotNull AjBuildConfig buildConfig, @NotNull IMessageHandler baseHandler) throws IOException, AbortException {
    return performBuild(buildConfig, baseHandler, true);
  }

  public boolean incrementalBuild(@NotNull AjBuildConfig buildConfig, @NotNull IMessageHandler baseHandler) throws IOException, AbortException {
   ...
   */
  @NotNull
  public CompilationUnit[] getCompilationUnits(@NotNull String[] filenames) {
  ...
  public void performCompilation(@NotNull Collection<File> files) {
    ...
    int idx = 0;
    for (final Iterator<File> fIterator = files.iterator(); fIterator.hasNext(); ) {
      final File f = fIterator.next();
    ...
    if (environment == null || environmentNeedsRebuilding) {
      final List<String> cps = buildConfig.getFullClasspath();
      Dump.saveFullClasspath(cps);
      final String[] classpaths = new String[cps.size()];
...
//    );
  ...
  public IIntermediateResultsRequestor getInterimResultRequestor() {
    return new IIntermediateResultsRequestor() {
      @Override
      public void acceptResult(@NotNull InterimCompilationResult result) {
        if (progressListener != null) {
          compiledCount++;
  ...
  public ICompilerRequestor getBatchRequestor() {
    return new ICompilerRequestor() {
      @Override
      public void acceptResult(@NotNull CompilationResult unitResult) {
        // end of compile, must now write the results to the output destination
        // this is either a jar file or a file in a directory
        final boolean hasErrors = unitResult.hasErrors();
        if (!hasErrors || proceedOnError()) {
          final Collection<ClassFile> classFiles = unitResult.compiledTypes.values();
          final boolean shouldAddAspectName = (buildConfig.getOutxmlName() != null);
          for (final Iterator<ClassFile> iter = classFiles.iterator(); iter.hasNext(); ) {
            final ClassFile classFile = iter.next();
            String filename = new String(classFile.fileName());
            final String classname = filename.replace('/', '.');
            filename = filename.replace('/', File.separatorChar) + ".class";

            try {
              if (buildConfig.getOutputJar() == null) {
                final String outfile = writeDirectoryEntry(unitResult, classFile, filename);
                ...
                if (environmentSupportsIncrementalCompilation) {
                  if (!classname.endsWith("$ajcMightHaveAspect")) {
                    final ResolvedType type = getBcelWorld().resolve(classname);
                    if (type.isAspect()) {
              ...
              final IMessage message = EclipseAdapterUtils.makeErrorMessage(new String(unitResult.fileName),
        ...
        if (unitResult.hasProblems() || unitResult.hasTasks()) {
          final IProblem[] problems = unitResult.getAllProblems();
          for (int i = 0; i < problems.length; i++) {
            final IMessage message = EclipseAdapterUtils.makeMessage(unitResult.compilationUnit, problems[i], getBcelWorld(),
                progressListener);
            handler.handleMessage(message);
          }
        }
      ...
      private String writeDirectoryEntry(CompilationResult unitResult, ClassFile classFile, String filename)
        ...
        try {
          final BufferedOutputStream os = FileUtil.makeOutputStream(new File(outFile));
          os.write(classFile.getBytes());
          os.close();
        } catch (FileNotFoundException fnfe) {
          final IMessage msg = new Message("unable to write out class file: '" + filename + "' - reason: " + fnfe.getMessage(),
      ...
      private void writeZipEntry(@NotNull ClassFile classFile, @NotNull String name) throws IOException {
        name = name.replace(File.separatorChar, '/');
        final ZipEntry newEntry = new ZipEntry(name); // ??? get compression scheme right
      ...
      private void addAspectName(@NotNull String name, @NotNull char[] fileContainingAspect) {
        final BcelWorld world = getBcelWorld();
  ...
  public void setProgressListener(@Nullable IProgressListener progressListener) {
  ...
  @Override
  @NotNull
  public String getOutputClassFileName(@NotNull char[] eclipseClassFileName, @NotNull CompilationResult result) {
    String filename = new String(eclipseClassFileName);
    filename = filename.replace('/', File.separatorChar) + ".class";
    File destinationPath = buildConfig.getOutputDir();
    if (buildConfig.getCompilationResultDestinationManager() != null) {
      final File f = new File(new String(result.getFileName()));
  ...
  @Override
  @NotNull
  public ICompilerAdapter getAdapter(@NotNull org.aspectj.org.eclipse.jdt.internal.compiler.Compiler forCompiler) {
    // complete compiler config and return a suitable adapter...
    populateCompilerOptionsFromLintSettings(forCompiler);
    final AjProblemReporter pr = new AjProblemReporter(DefaultErrorHandlingPolicies.proceedWithAllProblems(), forCompiler.options,
        getProblemFactory());

    forCompiler.problemReporter = pr;

    final AjLookupEnvironment le = new AjLookupEnvironment(forCompiler, forCompiler.options, pr, environment);
    final EclipseFactory factory = new EclipseFactory(le, this);
    le.factory = factory;
    pr.factory = factory;
    ...
    if (getBcelWorld().shouldPipelineCompilation()) {
      final IMessage message = MessageUtil.info("Pipelining compilation");
      handler.handleMessage(message);
  ...
  public void setState(@NotNull AjState buildState) {
    state = buildState;
  }
  ...
  @NotNull
  static String makeClasspathString(@Nullable AjBuildConfig buildConfig) {
    if (buildConfig == null || buildConfig.getFullClasspath() == null) {
      return "";
    }
    final StringBuilder buf = new StringBuilder();
    boolean first = true;
    for (final Iterator it = buildConfig.getFullClasspath().iterator(); it.hasNext(); ) {
      if (first) {
        first = false;
      } else {
   ...
   */
  private boolean performBuild(@Nullable AjBuildConfig buildConfig, @NotNull IMessageHandler baseHandler, boolean isFullBuild) throws IOException,
      AbortException {
    boolean ret = true;
   ...
   */
  private boolean openOutputStream(@NotNull File outJar) {
    try {
      final OutputStream os = FileUtil.makeOutputStream(buildConfig.getOutputJar());
      zos = new JarOutputStream(os, getWeaver().getManifest(true));
    } catch (IOException ex) {
      final IMessage message = new Message("Unable to open outjar " + outJar.getPath() + "(" + ex.getMessage() + ")",
  ...
  private void closeOutputStream(@NotNull File outJar) {
    try {
      if (zos != null) {
        zos.close();
        if (buildConfig.getCompilationResultDestinationManager() != null) {
      ...
      final IMessage message = new Message("Unable to write outjar " + outJar.getPath() + "(" + ex.getMessage() + ")",
          new SourceLocation(outJar, 0), true);
      handler.handleMessage(message);
    }
  }
  ...
  private void copyResourcesToDestination() throws IOException {
    ...
    for (final Iterator i = buildConfig.getInJars().iterator(); i.hasNext(); ) {
      final File inJar = (File) i.next();
      copyResourcesFromJarFile(inJar);
    }

    for (final Iterator i = buildConfig.getInpath().iterator(); i.hasNext(); ) {
      final File inPathElement = (File) i.next();
    ...
    if (buildConfig.getSourcePathResources() != null) {
      for (final Iterator i = buildConfig.getSourcePathResources().keySet().iterator(); i.hasNext(); ) {
        final String resource = (String) i.next();
        final File from = buildConfig.getSourcePathResources().get(resource);
  ...
  private void copyResourcesFromJarFile(@NotNull File jarFile) throws IOException {
  ...
  private void copyResourcesFromDirectory(@NotNull File dir) throws IOException {
    if (!COPY_INPATH_DIR_RESOURCES) {
      return;
    }
    // Get a list of all files (i.e. everything that isnt a directory)
    final File[] files = FileUtil.listFiles(dir, new FileFilter() {
      @Override
      public boolean accept(@NotNull File f) {
        final boolean accept = !(f.isDirectory() || f.getName().endsWith(".class"));
        return accept;
      }
    });
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      final String filename = files[i].getAbsolutePath().substring(dir.getAbsolutePath().length() + 1);
      copyResourcesFromFile(files[i], filename, dir);
    }
  }

  private void copyResourcesFromFile(@NotNull File f, @NotNull String filename, @NotNull File src) throws IOException {
   ...
   */
  private void writeDirectory(@NotNull String directory, @NotNull File srcloc) throws IOException {
    if (state.hasResource(directory)) {
      final IMessage msg = new Message("duplicate resource: '" + directory + "'", IMessage.WARNING, null, new SourceLocation(
          srcloc, 0));
      handler.handleMessage(msg);
      return;
    }
    if (zos != null) {
      final ZipEntry newEntry = new ZipEntry(directory);
      zos.putNextEntry(newEntry);
      zos.closeEntry();
  ...
  private void writeResource(@NotNull String filename, @NotNull byte[] content, @NotNull File srcLocation) throws IOException {
    if (state.hasResource(filename)) {
      final IMessage msg = new Message("duplicate resource: '" + filename + "'", IMessage.WARNING, null, new SourceLocation(
          srcLocation, 0));
      handler.handleMessage(msg);
      return;
    }
    if (filename.equals(buildConfig.getOutxmlName())) {
      ignoreOutxml = true;
      final IMessage msg = new Message("-outxml/-outxmlfile option ignored because resource already exists: '" + filename + "'",
          IMessage.WARNING, null, new SourceLocation(srcLocation, 0));
      handler.handleMessage(msg);
    }
    if (zos != null) {
      final ZipEntry newEntry = new ZipEntry(filename); // ??? get compression scheme right
      zos.putNextEntry(newEntry);
      zos.write(content);
      zos.closeEntry();
    } else {
      ...
      try {
        final File outputLocation = new File(destDir, filename);
        final OutputStream fos = FileUtil.makeOutputStream(outputLocation);
        fos.write(content);
        fos.close();
        if (buildConfig.getCompilationResultDestinationManager() != null) {
        ...
        final IMessage msg = new Message("unable to copy resource to output folder: '" + filename + "' - reason: "
  ...
  private void writeManifest() throws IOException {
    final Manifest manifest = getWeaver().getManifest(false);
    if (manifest != null && zos == null) {
      ...
      final File outputLocation = new File(outputDir, MANIFEST_NAME);
      final OutputStream fos = FileUtil.makeOutputStream(outputLocation);
      manifest.write(fos);
      fos.close();
      if (buildConfig.getCompilationResultDestinationManager() != null) {
  ...
  private boolean acceptResource(@NotNull String resourceName, boolean fromFile) {
  ...
  private void writeOutxmlFile() throws IOException {
    ...
    final String filename = buildConfig.getOutxmlName();
    // System.err.println("? AjBuildManager.writeOutxmlFile() outxml=" + filename);

    final Map<File, List<String>> outputDirsAndAspects = findOutputDirsForAspects();
    final Set<Map.Entry<File, List<String>>> outputDirs = outputDirsAndAspects.entrySet();
    for (final Iterator<Map.Entry<File, List<String>>> iterator = outputDirs.iterator(); iterator.hasNext(); ) {
      final Map.Entry<File, List<String>> entry = iterator.next();
      final File outputDir = entry.getKey();
      final List<String> aspects = entry.getValue();
      final ByteArrayOutputStream baos = getOutxmlContents(aspects);
      if (zos != null) {
        final ZipEntry newEntry = new ZipEntry(filename);
        ...
        final OutputStream fos = FileUtil.makeOutputStream(outputFile);
  ...
  @NotNull
  private static ByteArrayOutputStream getOutxmlContents(List aspectNames) {
    final ByteArrayOutputStream baos = new ByteArrayOutputStream();
    final PrintStream ps = new PrintStream(baos);
    ps.println("<aspectj>");
    ps.println("<aspects>");
    if (aspectNames != null) {
      for (final Iterator i = aspectNames.iterator(); i.hasNext(); ) {
  ...
  private Map<File, List<String>> findOutputDirsForAspects() {
    final Map<File, List<String>> outputDirsToAspects = new HashMap<File, List<String>>();
    final Map<String, char[]> aspectNamesToFileNames = state.getAspectNamesToFileNameMap();
    if (buildConfig.getCompilationResultDestinationManager() == null
      ...
      if (aspectNamesToFileNames != null) {
        final Set<String> keys = aspectNamesToFileNames.keySet();
      ...
      final List outputDirs = buildConfig.getCompilationResultDestinationManager().getAllOutputLocations();
      for (final Iterator iterator = outputDirs.iterator(); iterator.hasNext(); ) {
        final File outputDir = (File) iterator.next();
        outputDirsToAspects.put(outputDir, new ArrayList<String>());
      }
      if (aspectNamesToFileNames != null) {
        final Set<Map.Entry<String, char[]>> entrySet = aspectNamesToFileNames.entrySet();
        for (final Iterator<Map.Entry<String, char[]>> iterator = entrySet.iterator(); iterator.hasNext(); ) {
          final Map.Entry<String, char[]> entry = iterator.next();
          final String aspectName = entry.getKey();
          final char[] fileName = entry.getValue();
          final File outputDir = buildConfig.getCompilationResultDestinationManager().getOutputLocationForClass(
   ...
   */
  private void setupModel(@NotNull AjBuildConfig config) {
    ...
    final CompilationResultDestinationManager crdm = config.getCompilationResultDestinationManager();
    final AsmManager structureModel = AsmManager.createNewStructureModel(crdm == null ? Collections.EMPTY_MAP : crdm.getInpathMap());
    // AsmManager.getDefault().getRelationshipMap().clear();
    final IHierarchy model = structureModel.getHierarchy();
   ...
   */
  private void initBcelWorld(@NotNull IMessageHandler handler) throws IOException {
    final List cp = buildConfig.getFullClasspath(); // pr145693
    // buildConfig.getBootclasspath();
    // cp.addAll(buildConfig.getClasspath());
    final BcelWorld bcelWorld = new BcelWorld(cp, handler, null);
    ...
    final BcelWeaver bcelWeaver = new BcelWeaver(bcelWorld);
    ...
    for (final Iterator i = buildConfig.getAspectpath().iterator(); i.hasNext(); ) {
      final File f = (File) i.next();
      if (!f.exists()) {
        final IMessage message = new Message("invalid aspectpath entry: " + f.getName(), null, true);
        handler.handleMessage(message);
      } else {
    ...
    for (File inJar : buildConfig.getInJars()) {
      final List<UnwovenClassFile> unwovenClasses = bcelWeaver.addJarFile(inJar, outputDir, false);
    ...
    for (File inPathElement : buildConfig.getInpath()) {
      if (!inPathElement.isDirectory()) {
        ...
        final List<UnwovenClassFile> unwovenClasses = bcelWeaver.addJarFile(inPathElement, outputDir, true);
        ...
        final File[] binSrcs = FileUtil.listFiles(inPathElement, binarySourceFilter);
        for (int j = 0; j < binSrcs.length; j++) {
          final UnwovenClassFile ucf = bcelWeaver.addClassFile(binSrcs[j], inPathElement, outputDir);
          final List<UnwovenClassFile> ucfl = new ArrayList<UnwovenClassFile>();
          ucfl.add(ucf);
    ...
    final ResolvedType joinPoint = bcelWorld.resolve("org.aspectj.lang.JoinPoint");
  ...
  private void setBuildConfig(@NotNull AjBuildConfig buildConfig) {
   ...
   */
  @Nullable
  private String checkRtJar(@NotNull AjBuildConfig buildConfig) {
    // omitting dev info
    ...
    String ret = null;
    for (final Iterator it = buildConfig.getFullClasspath().iterator(); it.hasNext(); ) {
      final File p = new File((String) it.next());
      ...
      if (p.isFile() && p.getName().startsWith("aspectjrt") && p.getName().endsWith(".jar")) {
        try {
          String version = null;
          final Manifest manifest = new JarFile(p).getManifest();
          if (manifest == null) {
            ret = "no manifest found in " + p.getAbsolutePath() + ", expected " + Version.text;
            continue;
          }
          final Attributes attr = manifest.getAttributes("org/aspectj/lang/");
          if (null != attr) {
   ...
   */
  private void populateCompilerOptionsFromLintSettings(@NotNull org.aspectj.org.eclipse.jdt.internal.compiler.Compiler forCompiler) {
    final BcelWorld world = this.state.getBcelWorld();
    final IMessage.Kind swallowedExceptionKind = world.getLint().swallowedExceptionInCatchBlock.getKind();
  ...
  private static class AjBuildContexFormatter implements ContextFormatter {
    ...
    @Override
    @NotNull
    public String formatEntry(int phaseId, @NotNull Object data) {
      ...
      final AjBuildConfig config = (AjBuildConfig) data;
      final List classpath = config.getClasspath();
      sb.append("with classpath: ");
      for (final Iterator iter = classpath.iterator(); iter.hasNext(); ) {
