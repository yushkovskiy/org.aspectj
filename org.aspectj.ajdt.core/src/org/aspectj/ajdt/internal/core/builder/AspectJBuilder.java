...
public class AspectJBuilder extends JavaBuilder implements ICompilerAdapterFactory {
  ...
  public ICompilerAdapter getAdapter(Compiler forCompiler) {
    final Map javaOptions = forCompiler.options.getMap();
    // TODO get aspectj options from project and add into map before...
    final AjCompilerOptions ajOptions = new AjCompilerOptions(javaOptions);
    ...
    final AjLookupEnvironment le = new AjLookupEnvironment(forCompiler, forCompiler.options, pr, nameEnvironment);
    final EclipseFactory eFactory = new EclipseFactory(le, myBcelWorld, ajOptions.xSerializableAspects);
    le.factory = eFactory;
  ...
  private static void setLintProperties(BcelWorld world, AjCompilerOptions options) {
    final Properties p = new Properties();
    final Lint lintSettings = world.getLint();
    final Map map = options.getMap();
  ...
  private static class UnhandledMessageHandler implements IMessageHandler {
    ...
    public boolean handleMessage(@NotNull IMessage message) throws AbortException {
      try {
        final IMarker marker = project.createMarker(IJavaModelMarker.JAVA_MODEL_PROBLEM_MARKER);
  ...
  private static class OutputFileNameProvider implements IOutputClassFileNameProvider {
    ...
    public String getOutputClassFileName(char[] eclipseClassFileName, CompilationResult result) {
      ...
      final String filename = new String(eclipseClassFileName);
      final IPath out = outputLocation.append(filename);
