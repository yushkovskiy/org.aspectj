...
public class EclipseClassPathManager extends ClassPathManager {
  ...
  public ClassFile find(UnresolvedType type) {
    ...
    final char[][] cname = CharOperation.splitOn('.', name.toCharArray());
    final NameEnvironmentAnswer answer = nameEnv.findType(cname);
    if (answer == null || !answer.isBinaryType()) {
      return null;
    } else {
      final IBinaryType binType = answer.getBinaryType();
      // XXX - but better than the alternative hacks
      if (binType instanceof ClassFileReader) {
        final ClassFileReader cfr = (ClassFileReader) binType;
   ...
   */
  public static List getAllClassFiles() {
  ...
  public String toString() {
    final StringBuilder buf = new StringBuilder("EclipseClassPathManager: ");
