...
public class EmacsStructureModelManager {
  ...
  public void externalizeModel(AsmManager model) {
    ...
    try {
      ...
      final Set fileSet = model.getHierarchy().getFileMapEntrySet();
      for (final Iterator it = fileSet.iterator(); it.hasNext(); ) {
        final IProgramElement peNode = (IProgramElement) ((Map.Entry) it.next()).getValue();
  ...
  private static void dumpStructureToFile(IProgramElement node) throws IOException {
    final String s = node.getKind().toString();
    ...
    final String sourceName = node.getSourceLocation().getSourceFile().getAbsolutePath();
    final String fileName = sourceName.substring(0, sourceName.lastIndexOf(".")) + EXTERN_FILE_SUFFIX;
  ...
  private static class SExpressionPrinter {
    ...
    private void printDecls(IProgramElement node) {
      ...
      for (final Iterator it = node.getChildren().iterator(); it.hasNext(); ) {
        ...
        final Object nodeObject = it.next();
        // throw new RuntimeException("unimplemented");
        // if (nodeObject instanceof IProgramElement) {
        final IProgramElement child = (IProgramElement) nodeObject;
    ...
    private void printDecl(IProgramElement node, boolean recurse) {
      if (node == null || node.getSourceLocation() == null)
        return;
      final String kind = node.getKind().toString().toLowerCase();
      print("(");
      ...
      // HACK:
      final String displayName = node.toString().replace('\"', ' ');
      ...
      if (!recurse) {
        ...
        print(") ");
        print("(");
        final Iterator it3 = node.getChildren().iterator();
        if (it3.hasNext()) {
          while (it3.hasNext()) {
            // this ignores relations on the compile unit
            final Object nodeObject = it3.next();
            if (nodeObject instanceof IProgramElement) {
              final IProgramElement currNode = (IProgramElement) nodeObject;
    ...
    private static String subst(String n, String o, String in) {
      final int pos = in.indexOf(o);
      if (pos == -1)
        return in;
