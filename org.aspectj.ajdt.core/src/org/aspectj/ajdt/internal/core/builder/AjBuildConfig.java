...
public final class AjBuildConfig implements CompilerConfigurationChangeFlags {
  ...
  public void setCompilationResultDestinationManager(@Nullable CompilationResultDestinationManager mgr) {
  ...
  public void setXmlFiles(@NotNull List<File> xmlfiles) {
  ...
  public void setInPath(@NotNull List<File> dirsOrJars) {
    inPath = dirsOrJars;
    ...
    for (final Iterator<File> iter = dirsOrJars.iterator(); iter.hasNext(); ) {
      final File inpathElement = iter.next();
      if (inpathElement.isDirectory()) {
        final File[] files = FileUtil.listFiles(inpathElement, filter);
  ...
  public void setSourceRoots(@NotNull List<File> sourceRootDir) {
  ...
  public List<String> getFullClasspath() {
    ...
    for (final Iterator<File> i = inJars.iterator(); i.hasNext(); ) {
      full.add((i.next()).getAbsolutePath());
    }
    for (final Iterator<File> i = inPath.iterator(); i.hasNext(); ) {
      full.add((i.next()).getAbsolutePath());
    }
    for (final Iterator<File> i = aspectpath.iterator(); i.hasNext(); ) {
      full.add((i.next()).getAbsolutePath());
    }
  ...
  public void setAspectpath(@NotNull List<File> aspectpath) {
  ...
  // options...
  public void setLintMode(@NotNull String lintMode) {
    ...
    if (AJLINT_IGNORE.equals(lintMode)) {
      ...
      if (lintMode.indexOf("=") != -1) {
        ...
        final StringTokenizer tokenizer = new StringTokenizer(lintMode, ",");
        while (tokenizer.hasMoreElements()) {
          final String option = tokenizer.nextToken();
          final int equals = option.indexOf("=");
          if (equals != -1) {
            final String key = option.substring(0, equals);
            final String value = option.substring(equals + 1);
    ...
    if (lintValue != null || lintOptionsMap != null) {
      final Map<String, String> lintOptions = new HashMap<String, String>();
  ...
  static void join(Collection local, Collection global) {
    for (final Iterator iter = global.iterator(); iter.hasNext(); ) {
  ...
  private void setOption(String optionKey, String lintValue, Map<String, String> lintOptionsAccumulator) {
    if (lintOptionsMap != null && lintOptionsMap.containsKey(optionKey)) {
      final String v = lintOptionsMap.get(lintOptionsMap);
  ...
  public static class BinarySourceFile {
    ...
    public boolean equals(@Nullable Object obj) {
      if (obj != null && (obj instanceof BinarySourceFile)) {
        final BinarySourceFile other = (BinarySourceFile) obj;
