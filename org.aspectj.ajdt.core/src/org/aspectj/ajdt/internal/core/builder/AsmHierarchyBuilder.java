...
public class AsmHierarchyBuilder extends ASTVisitor {
  ...
  private void internalBuild(CompilationUnitDeclaration unit, AsmManager structureModel) {
    ...
    try {
      ...
      final IProgramElement cuNode;
      {
        ...
        final int startLine = getStartLine(unit);
        final int endLine = getEndLine(unit);
        final SourceLocation sourceLocation = new SourceLocation(file, startLine, endLine);
      ...
      if (addToNode != null && addToNode.getChildren() != null) {
        for (final ListIterator itt = addToNode.getChildren().listIterator(); itt.hasNext(); ) {
          final IProgramElement child = (IProgramElement) itt.next();
          final ISourceLocation childLoc = child.getSourceLocation();
          if (null == childLoc) {
  ...
  private static IProgramElement findOrCreateChildSourceFolder(String sourceFolder, AsmManager structureModel) {
    final IProgramElement root = structureModel.getHierarchy().getRoot();
    // Check if already there
    IProgramElement sourceFolderNode = null;
    final List<IProgramElement> kids = root.getChildren();
    for (final Iterator<IProgramElement> iterator = kids.iterator(); iterator.hasNext(); ) {
      final IProgramElement child = (IProgramElement) iterator.next();
  ...
  private IProgramElement genAddToNode(File sourceFile, CompilationUnitDeclaration unit, AsmManager structureModel) {
    final IProgramElement addToNode;
    {
      ...
      final CompilationResultDestinationManager manager = buildConfig.getCompilationResultDestinationManager();
      final String sourceFolder = (manager == null ? null : manager.getSourceFolderForFile(sourceFile));

      final ImportReference unitPackage = unit.currentPackage;
      ...
      if (structureModel != null && structureModel.getHierarchy().getRoot() != null && rootForSource.getChildren() != null) {
        for (final Iterator<IProgramElement> it = rootForSource.getChildren().iterator(); it.hasNext(); ) {
          final IProgramElement currNode = (IProgramElement) it.next();
  ...
  public boolean visit(TypeDeclaration typeDeclaration, CompilationUnitScope scope) {
    final String name = new String(typeDeclaration.name);
    ...
    if (typeDeclaration.annotations != null) {
      for (int i = 0; i < typeDeclaration.annotations.length; i++) {
        final Annotation annotation = typeDeclaration.annotations[i];
        if (Arrays.equals(annotation.type.getTypeBindingPublic(scope).signature(),
          ...
          final String[] temp = (new String(annotation.resolvedType.constantPoolName())).split("/");
          if (temp.length > 1) {
            final char[][] path = new char[temp.length][];
            ...
            // ProgramElement
            final ImportReference importRef = new ImportReference(path, new long[]{0}, false, 0);
            final IProgramElement ceNode = new ProgramElement(activeStructureModel, importRef.toString(),
            ...
            final ProgramElement imports = getImportReferencesRoot();// (ProgramElement) ((IProgramElement)
    ...
    final IProgramElement peNode = new ProgramElement(activeStructureModel, name, kind, makeLocation(typeDeclaration), typeModifiers,
        null, null);
  ...
  public void endVisit(TypeDeclaration typeDeclaration, CompilationUnitScope scope) {
    ...
    if (packageDecl != null) {
      final int dotIndex = packageDecl.toString().lastIndexOf('.');
      final String packageString = packageDecl.toString();
      final ProgramElement packageDeclaration = new ProgramElement(activeStructureModel, packageString,
          IProgramElement.Kind.PACKAGE_DECLARATION, makeLocation(packageDecl), 0, null, null);
      final StringBuilder packageSourceDeclaration = new StringBuilder();
      ...
      stack.pop();
      final ProgramElement containingTypeElement = (ProgramElement) stack.peek();
  ...
  public boolean visit(TypeDeclaration memberTypeDeclaration, ClassScope scope) {
    final String name = new String(memberTypeDeclaration.name);

    IProgramElement.Kind kind = IProgramElement.Kind.CLASS;
    final int typeDeclarationKind = TypeDeclaration.kind(memberTypeDeclaration.modifiers);
    ...
    if (memberTypeDeclaration.annotations != null) {
      for (int i = 0; i < memberTypeDeclaration.annotations.length; i++) {
        final Annotation annotation = memberTypeDeclaration.annotations[i];
        if (Arrays.equals(annotation.type.getTypeBindingPublic(scope).signature(),
            "Lorg/aspectj/lang/annotation/Aspect;".toCharArray())) {
          kind = IProgramElement.Kind.ASPECT;
          if (!(memberTypeDeclaration instanceof AspectDeclaration)) {
    ...
    final IProgramElement peNode = new ProgramElement(activeStructureModel, name, kind, makeLocation(memberTypeDeclaration),
  ...
  public boolean visit(TypeDeclaration memberTypeDeclaration, BlockScope scope) {
    ...
    if (memberTypeDeclaration.allocation != null && memberTypeDeclaration.allocation.type != null) {
      ...
      final int dollar = fullName.indexOf('$');
    ...
    final IProgramElement peNode = new ProgramElement(activeStructureModel, fullName, kind, makeLocation(memberTypeDeclaration),
    ...
    if (peNode.getBytecodeSignature() == null && memberTypeDeclaration.binding != null
      ...
      final StringTokenizer st = new StringTokenizer(new String(memberTypeDeclaration.binding.constantPoolName()), "/");
  ...
  private static String genSourceSignature(TypeDeclaration typeDeclaration) {
  ...
  public boolean visit(MethodDeclaration methodDeclaration, ClassScope scope) {
    ...
    if (methodDeclaration instanceof InterTypeDeclaration) {
      ...
      final ResolvedMember sig = itd.getSignature();
    ...
    final List namedPointcuts = genNamedPointcuts(methodDeclaration);
    ...
    if (methodDeclaration instanceof DeclareDeclaration) {
      final DeclareDeclaration dDeclaration = (DeclareDeclaration) methodDeclaration;
      final Declare decl = dDeclaration.declareDecl;
      if (decl instanceof DeclareParents) {
        final TypePatternList tpl = ((DeclareParents) decl).getParents();
      ...
      if (decl instanceof DeclareAnnotation) {
        final DeclareAnnotation da = (DeclareAnnotation) decl;
        final ResolvedType annotationType = da.getAnnotationType();
    ...
    if (methodDeclaration.returnType != null) {
      ...
      if (peNode.getKind().equals(IProgramElement.Kind.INTER_TYPE_FIELD)) {
        ...
        final InterTypeFieldDeclaration itfd = (InterTypeFieldDeclaration) methodDeclaration;
  ...
  private static ResolvedMember getPointcutDeclaration(ReferencePointcut rp, MethodDeclaration declaration) {
    final EclipseFactory factory = ((AjLookupEnvironment) declaration.scope.environment()).factory;
    final World world = factory.getWorld();
    UnresolvedType onType = rp.onType;
    if (onType == null) {
      if (declaration.binding != null) {
        final Member member = factory.makeResolvedMember(declaration.binding);
        onType = member.getDeclaringType();
      } else {
        return null;
      }
    }
    final ResolvedMember[] members = onType.resolve(world).getDeclaredPointcuts();
    if (members != null) {
  ...
  private static String genSourceSignature(MethodDeclaration methodDeclaration) {
    ...
    final String methodName = methodDeclaration.selector == null ? "null" : new String(methodDeclaration.selector);
  ...
  protected void genBytecodeInfo(MethodDeclaration methodDeclaration, IProgramElement peNode) {
    if (methodDeclaration.binding != null) {
      try {
        final EclipseFactory factory = ((AjLookupEnvironment) methodDeclaration.scope.environment()).factory;
        final Member member = factory.makeResolvedMember(methodDeclaration.binding);
  ...
  public boolean visit(ImportReference importRef, CompilationUnitScope scope) {
    ...
    final String currPackageImport = importRef.toString();
    final String stackPackageName = stack.peek().getPackageName();
    if (stackPackageName.equals(currPackageImport)) {
      packageDecl = importRef;
    } else {

      final ProgramElement peNode = new ProgramElement(activeStructureModel, new String(importRef.toString()),
      ...
      final IProgramElement containingTypeElement = (IProgramElement) stack.peek();
      final ProgramElement imports = getImportReferencesRoot();
  ...
  private ProgramElement getImportReferencesRoot() {
    final IProgramElement element = (IProgramElement) stack.peek();
    final boolean hasPackageDeclaration = (element.getChildren().get(0)).getKind().isPackageDeclaration();
  ...
  private static String genSourceSignature(ImportReference importreference) {
    final StringBuffer output = new StringBuffer();
...
//			peNode.setCorrespondingType(fieldDeclaration.binding.type.debugName());
      ...
      if (fieldDeclaration.type.resolvedType != null) {
        final char[] cs = fieldDeclaration.type.resolvedType.readableName();
  ...
  protected String generateJavadocComment(ASTNode astNode) {
    ...
    if (startIndex == -1) {
      ...
      while (comment.length > 2) {
        final int star = CharOperation.indexOf('*', comment);
        if (star == -1) {
          return null;
        }
   ...
   *
   */
  protected static String genSourceSignature(FieldDeclaration fieldDeclaration) {
    final StringBuffer output = new StringBuffer();
  ...
  public boolean visit(ConstructorDeclaration constructorDeclaration, ClassScope scope) {
    ...
    final StringBuilder argumentsSignature = new StringBuilder();
    ...
    argumentsSignature.append(")");
    final IProgramElement peNode = new ProgramElement(activeStructureModel, new String(constructorDeclaration.selector),
    ...
    if (constructorDeclaration.binding != null) {
      ...
      String memberBytecodeSignature = "";
      try {
        final EclipseFactory factory = ((AjLookupEnvironment) constructorDeclaration.scope.environment()).factory;
        final Member member = factory.makeResolvedMember(constructorDeclaration.binding);
        memberName = member.getName();
        memberBytecodeSignature = member.getSignature();
      } catch (BCException bce) { // bad type name
        memberName = "<undefined>";
  ...
  private static String genSourceSignature(ConstructorDeclaration constructorDeclaration) {
    final StringBuffer output = new StringBuffer();
    ASTNode.printModifiers(constructorDeclaration.modifiers, output);

    // Append Type Parameters if any
    final TypeParameter[] types = constructorDeclaration.typeParameters();
    if (types != null && types.length != 0) {
      output.append("<");
      for (int i = 0; i < types.length; i++) {
        if (i > 0) {
  ...
  public boolean visit(Initializer initializer, MethodScope scope) {
    ...
    final IProgramElement peNode = new ProgramElement(activeStructureModel, "...", IProgramElement.Kind.INITIALIZER,
