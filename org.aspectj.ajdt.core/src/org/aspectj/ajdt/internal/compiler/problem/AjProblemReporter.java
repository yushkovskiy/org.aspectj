...
public class AjProblemReporter extends ProblemReporter {
  ...
  public void unhandledException(TypeBinding exceptionType, ASTNode location) {
    if (!factory.getWorld().getDeclareSoft().isEmpty()) {
      final Shadow callSite = factory.makeShadow(location, referenceContext);
      final Shadow enclosingExec = factory.makeShadow(referenceContext);
...
//				DeclareSoft d = (DeclareSoft) i.next();
        ...
        final ResolvedType throwException = factory.fromEclipse((ReferenceBinding) exceptionType);
        final FuzzyBoolean isExceptionTypeOrSubtype = d.getException().matchesInstanceof(throwException);
        if (!isExceptionTypeOrSubtype.alwaysTrue())
          continue;

        if (callSite != null) {
          final FuzzyBoolean match = d.getPointcut().match(callSite);
          if (match.alwaysTrue()) {
        ...
        if (enclosingExec != null) {
          final FuzzyBoolean match = d.getPointcut().match(enclosingExec);
          if (match.alwaysTrue()) {
...
//				DeclareSoft d = (DeclareSoft) i.next();
  ...
  private static boolean isPointcutDeclaration(MethodBinding binding) {
    return CharOperation.prefixEquals(PointcutDeclaration.mangledPrefix, binding.selector);
  }

  private static boolean isIntertypeDeclaration(MethodBinding binding) {
  ...
  public void inheritedMethodReducesVisibility(SourceTypeBinding type, MethodBinding concreteMethod,
    ...
    for (final Iterator i = onTypeX.getInterTypeMungersIncludingSupers().iterator(); i.hasNext(); ) {
      final ConcreteTypeMunger m = (ConcreteTypeMunger) i.next();
      final ResolvedMember sig = m.getSignature();
  ...
  public void abstractMethodMustBeImplemented(SourceTypeBinding type, MethodBinding abstractMethod) {
    ...
    final List<ConcreteTypeMunger> mungers = onTypeX.getInterTypeMungersIncludingSupers();
    for (ConcreteTypeMunger m : mungers) {
      final ResolvedMember sig = m.getSignature();
      if (sig != null && !Modifier.isAbstract(sig.getModifiers())) {
        final ResolvedMember abstractMember = factory.makeResolvedMember(abstractMethod);
        if (abstractMember.getName().startsWith("ajc$interMethodDispatch")) {
          final ResolvedType dType = factory.getWorld().resolve(sig.getDeclaringType(), false);
  ...
  public void disallowedTargetForAnnotation(Annotation annotation) {
    ...
    if (annotation.recipient instanceof MethodBinding) {
      final MethodBinding binding = (MethodBinding) annotation.recipient;
      final String name = new String(binding.selector);
      if (name.startsWith("ajc$")) {
        final long metaTagBits = annotation.resolvedType.getAnnotationTagBits(); // could be forward reference
  ...
  public void abstractMethodInAbstractClass(SourceTypeBinding type, AbstractMethodDeclaration methodDecl) {
    ...
    final String abstractMethodName = new String(methodDecl.selector);
    if (abstractMethodName.startsWith("ajc$pointcut")) {
      // This will already have been reported, see: PointcutDeclaration.postParse()
      return;
    }
    final String[] arguments = new String[]{new String(type.sourceName()), abstractMethodName};
  ...
  public void methodMustOverride(AbstractMethodDeclaration method, long complianceLevel) {
    ...
    if (new String(method.selector).startsWith("ajc$"))
      return;
    final ResolvedMember possiblyErroneousRm = factory.makeResolvedMember(method.binding);

    final ResolvedType onTypeX = factory.fromEclipse(method.binding.declaringClass);
    ...
    while (supertypeToLookAt != null) {
      final List<ConcreteTypeMunger> itMungers = supertypeToLookAt.getInterTypeMungers();
      for (final Iterator<ConcreteTypeMunger> i = itMungers.iterator(); i.hasNext(); ) {
        final ConcreteTypeMunger m = (ConcreteTypeMunger) i.next();
        ...
        final UnresolvedType dType = sig.getDeclaringType();
        if (dType == null)
          continue;
        final ResolvedType resolvedDeclaringType = dType.resolve(factory.getWorld());
        final ResolvedMember rm = AjcMemberMaker.interMethod(sig, m.getAspectType(), resolvedDeclaringType.isInterface());
  ...
  private static String typesAsString(boolean isVarargs, TypeBinding[] types, boolean makeShort) {
    final StringBuilder buffer = new StringBuilder(10);
    for (int i = 0, length = types.length; i < length; i++) {
      if (i != 0)
        buffer.append(", "); //$NON-NLS-1$
      TypeBinding type = types[i];
      final boolean isVarargType = isVarargs && i == length - 1;
      if (isVarargType)
  ...
  public void unusedPrivateType(TypeDeclaration typeDecl) {
    ...
    if (typeDecl.enclosingType != null && (typeDecl.enclosingType instanceof AspectDeclaration)) {
      final AspectDeclaration ad = (AspectDeclaration) typeDecl.enclosingType;
      if (ad.concreteName != null) {
        final List declares = ad.concreteName.declares;
        for (final Iterator iter = declares.iterator(); iter.hasNext(); ) {
          final Object dec = iter.next();
          if (dec instanceof DeclareParents) {
            final DeclareParents decp = (DeclareParents) dec;
            final TypePattern[] newparents = decp.getParents().getTypePatterns();
            for (int i = 0; i < newparents.length; i++) {
              final TypePattern pattern = newparents[i];
              final UnresolvedType ut = pattern.getExactType();
              if (ut == null)
                continue;
  ...
  public void unusedPrivateField(FieldDeclaration fieldDecl) {
    if (fieldDecl != null && fieldDecl.binding != null && fieldDecl.binding.declaringClass != null) {
      final ReferenceBinding type = fieldDecl.binding.declaringClass;
      ...
      for (final Iterator i = weaverType.getInterTypeMungersIncludingSupers().iterator(); i.hasNext(); ) {
        final ConcreteTypeMunger m = (ConcreteTypeMunger) i.next();
        final ResolvedType theAspect = m.getAspectType();
        if (!checked.contains(theAspect)) {
          final TypeBinding tb = factory.makeTypeBinding(m.getAspectType());
          ...
          if (tb instanceof SourceTypeBinding) { // BinaryTypeBinding is also a SourceTypeBinding ;)
            final IPrivilegedHandler privilegedHandler = ((SourceTypeBinding) tb).privilegedHandler;
            if (privilegedHandler != null) {
              ...
              final String fname = new String(fieldDecl.name);
              final Collection/* ResolvedMember */privvies = ((ReferenceType) theAspect).getPrivilegedAccesses();
              // On an incremental compile the information is in the bcel delegate
              if (privvies != null) {
                for (final Iterator iterator = privvies.iterator(); iterator.hasNext(); ) {
                  final ResolvedMember priv = (ResolvedMember) iterator.next();
  ...
  public void caseExpressionMustBeConstant(Expression expression) {
    if (expression instanceof QualifiedNameReference) {
      final QualifiedNameReference qnr = (QualifiedNameReference) expression;
  ...
  public void unusedArgument(LocalDeclaration localDecl) {
    ...
    final String argType = new String(localDecl.type.resolvedType.signature());
    ...
    if (localDecl instanceof Argument) {
      final Argument arg = (Argument) localDecl;
      if (arg.binding != null && arg.binding.declaringScope != null) {
        final ReferenceContext context = arg.binding.declaringScope.referenceContext();
