...
public class InterTypeFieldDeclaration extends InterTypeDeclaration {
  ...
  public void resolve(ClassScope upperScope) {
    ...
    final EclipseFactory world = EclipseFactory.fromScopeLookupEnvironment(upperScope);
    final ResolvedMember sig = munger.getSignature();
    final UnresolvedType aspectType = world.fromBinding(upperScope.referenceContext.binding);
    ...
    if (initialization != null && initialization instanceof ArrayInitializer) {
      ...
      final ArrayAllocationExpression aae = new ArrayAllocationExpression();
      aae.initializer = (ArrayInitializer) initialization;
      final ArrayBinding arrayType = (ArrayBinding) world.makeTypeBinding(sig.getReturnType());
    ...
    if (initialization == null) {
      ...
      final MethodBinding writeMethod = world.makeMethodBinding(AjcMemberMaker.interFieldSetDispatcher(sig, aspectType),
      ...
      final MethodBinding writeMethod = world.makeMethodBinding(
  ...
  public EclipseTypeMunger build(ClassScope classScope) {
    ...
    final ResolvedMember sigtemp = world.makeResolvedMemberForITD(binding, onTypeBinding, interTypeScope.getRecoveryAliases());
    final UnresolvedType returnType = sigtemp.getReturnType();
    // if (returnType.isParameterizedType() || returnType.isGenericType()) returnType = returnType.getRawType();
    final ResolvedMember sig = new ResolvedMemberImpl(Member.FIELD, declaringType, declaredModifiers, returnType, new String(
        declaredSelector), UnresolvedType.NONE);
    sig.setTypeVariables(sigtemp.getTypeVariables());

    final NewFieldTypeMunger myMunger = new NewFieldTypeMunger(sig, null, typeVariableAliases);
    ...
    final ResolvedMember me = myMunger.getInitMethod(aspectType);
  ...
  private void generateDispatchMethod(EclipseFactory world, ResolvedMember sig, UnresolvedType aspectType, ClassScope classScope,
    ...
    final NewFieldTypeMunger fieldMunger = (NewFieldTypeMunger) munger;
    ...
    if (world.getItdVersion() == 1) {
      ...
      if (!onTypeBinding.isInterface()) {
        final FieldBinding[] existingFields = onTypeBinding.fields();
        for (int f = 0; f < existingFields.length; f++) {
          final FieldBinding fieldBinding = existingFields[f];
    ...
    final FieldBinding classField = world.makeFieldBinding(
    ...
    if (isGetter) {
      if (onTypeBinding.isInterface()) {
        final UnresolvedType declaringTX = sig.getDeclaringType();
        final ResolvedType declaringRTX = world.getWorld().resolve(declaringTX, munger.getSourceLocation());
        final MethodBinding readMethod = world.makeMethodBinding(
  ...
  private static void generateClassReadBody(MethodBinding binding, FieldBinding field, CodeStream codeStream) {
    if (field.isPrivate() || !field.canBeSeenBy(binding.declaringClass.fPackage)) {
      ...
      if (handler == null) {
        // one is now required!
        final ReferenceBinding typebinding = binding.declaringClass;
        if (typebinding instanceof ReferenceBinding) {
          final SourceTypeBinding sourceBinding = (SourceTypeBinding) typebinding;
      ...
      final PrivilegedFieldBinding fBinding = (PrivilegedFieldBinding) handler.getPrivilegedAccessField(field, null);
