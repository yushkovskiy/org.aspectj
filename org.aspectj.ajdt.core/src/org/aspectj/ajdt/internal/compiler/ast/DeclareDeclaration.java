...
public class DeclareDeclaration extends AjMethodDeclaration {
  ...
  public void addAtAspectJAnnotations() {
    ...
    if (declareDecl instanceof DeclareAnnotation) {
      final DeclareAnnotation da = (DeclareAnnotation) declareDecl;
      final String patternString = da.getPatternAsString();
      final String annString = da.getAnnotationString();
      final String kind = da.getKind().toString();
      ...
      final DeclareErrorOrWarning dd = (DeclareErrorOrWarning) declareDecl;
      ...
      final DeclareParents dp = (DeclareParents) declareDecl;
      final String childPattern = dp.getChild().toString();
      final Collection parentPatterns = dp.getParents().getExactTypes();
      final StringBuilder parents = new StringBuilder();
      for (final Iterator iter = parentPatterns.iterator(); iter.hasNext(); ) {
        final UnresolvedType urt = ((UnresolvedType) iter.next());
      ...
      final DeclarePrecedence dp = (DeclarePrecedence) declareDecl;
      final String precedenceList = dp.getPatterns().toString();
      ...
      final DeclareSoft ds = (DeclareSoft) declareDecl;
  ...
  public Declare build(ClassScope classScope) {
    ...
    final EclipseScope scope = new EclipseScope(new FormalBinding[0], classScope);
  ...
  public void postParse(TypeDeclaration typeDec) {
    ...
    if (declareDecl != null) {
      final String suffix = declareDecl.getNameSuffix();
      if (suffix.length() != 0) {
        sb.append("_");
        sb.append(suffix);
      }
