...
public class PseudoTokens extends ASTNode {
  ...
  public Pointcut parsePointcut(Parser parser) {
    final PatternParser patternParser = new PatternParser(tokenSource);
    try {
      final Pointcut ret = patternParser.parsePointcut();
      checkEof(parser);
      return ret;
  ...
  private void reportError(Parser parser, ParserException pe) {
    final IHasPosition tok = pe.getLocation();
    final int start;
    final int end;
    if (tok == IToken.EOF) {
      start = sourceEnd + 1;
  ...
  public PerClause parsePerClause(Parser parser) {
    final PatternParser patternParser = new PatternParser(tokenSource);
    try {
      final PerClause ret = patternParser.maybeParsePerClause();
      checkEof(parser);
...
//	
  public Declare parseDeclare(Parser parser) {
    final PatternParser patternParser = new PatternParser(tokenSource);
    try {
      final Declare ret = patternParser.parseDeclare();
      checkEof(parser);
      return ret;
    } catch (ParserException pe) {
      reportError(parser, pe);
  ...
  public Declare parseAnnotationDeclare(Parser parser) {
    final PatternParser patternParser = new PatternParser(tokenSource);
    try {
      final Declare ret = patternParser.parseDeclareAnnotation();
      checkEof(parser);
      return ret;
    } catch (ParserException pe) {
      reportError(parser, pe);
