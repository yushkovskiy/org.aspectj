...
public class ValidateAtAspectJAnnotationsVisitor extends ASTVisitor {
  ...
  private void checkTypeDeclaration(TypeDeclaration typeDecl) {
    ...
    if (!(typeDecl instanceof AspectDeclaration)) {
      if (ajAnnotations.hasAspectAnnotation) {
        ...
        final TypeReference parentRef = typeDecl.superclass;
        if (parentRef != null) {
          final TypeBinding parentBinding = parentRef.resolvedType;
          if (parentBinding instanceof SourceTypeBinding) {
            final SourceTypeBinding parentSTB = (SourceTypeBinding) parentBinding;
            if (parentSTB.scope != null) {
              final TypeDeclaration parentDecl = parentSTB.scope.referenceContext;
  ...
  private boolean insideAspect() {
    if (typeStack.empty())
      return false;
    final TypeDeclaration typeDecl = (TypeDeclaration) typeStack.peek();
    return isAspect(typeDecl);
  }

  private static boolean isAspect(TypeDeclaration typeDecl) {
    if (typeDecl instanceof AspectDeclaration)
      return true;
  ...
  private void validateAspectDeclaration(TypeDeclaration typeDecl) {
    ...
    final SourceTypeBinding binding = typeDecl.binding;
    if (binding != null) {
    ...
    final Annotation aspectAnnotation = ajAnnotations.aspectAnnotation;

    final int[] pcLoc = new int[2];
    final String perClause = getStringLiteralFor("value", aspectAnnotation, pcLoc);
    // AspectDeclaration aspectDecl = new
    // AspectDeclaration(typeDecl.compilationResult);

    try {
      if (perClause != null && !perClause.equals("")) {
        final ISourceContext context = new EclipseSourceContext(unit.compilationResult, pcLoc[0]);
        final Pointcut pc = new PatternParser(perClause, context).maybeParsePerClause();
        final FormalBinding[] bindings = new FormalBinding[0];
        if (pc != null)
  ...
  private void validateAdvice(MethodDeclaration methodDeclaration) {
    ...
    if (ajAnnotations.adviceKind == AdviceKind.AfterThrowing) {
      final int[] throwingLocation = new int[2];
      final String thrownFormal = getStringLiteralFor("throwing", ajAnnotations.adviceAnnotation, throwingLocation);
    ...
    if (ajAnnotations.adviceKind == AdviceKind.AfterReturning) {
      final int[] throwingLocation = new int[2];
      final String returningFormal = getStringLiteralFor("returning", ajAnnotations.adviceAnnotation, throwingLocation);
   ...
   */
  private static List toArgumentNames(Argument[] arguments) {
  ...
  private void resolveAndSetPointcut(MethodDeclaration methodDeclaration, Annotation adviceAnn) {
    final int[] pcLocation = new int[2];
    ...
    try {
      ...
      final ISourceContext context = new EclipseSourceContext(unit.compilationResult, pcLocation[0] + 1);
      if (pointcutExpression == null) {
        methodDeclaration.scope.problemReporter().signalError(methodDeclaration.sourceStart,
            methodDeclaration.sourceEnd, "the advice annotation must specify a pointcut value");
        return;
      }
      final PatternParser pp = new PatternParser(pointcutExpression, context);
      final Pointcut pc = pp.parsePointcut();
      pp.checkEof();
      final FormalBinding[] bindings = buildFormalAdviceBindingsFrom(methodDeclaration);
      pc.resolve(new EclipseScope(bindings, methodDeclaration.scope));
      final EclipseFactory factory = EclipseFactory.fromScopeLookupEnvironment(methodDeclaration.scope);
      // now create a ResolvedPointcutDefinition,make an attribute out of
      // it, and add it to the method
      final UnresolvedType[] paramTypes = new UnresolvedType[bindings.length];
      for (int i = 0; i < paramTypes.length; i++)
        paramTypes[i] = bindings[i].getType();
      final ResolvedPointcutDefinition resPcutDef = new ResolvedPointcutDefinition(factory.fromBinding(((TypeDeclaration) typeStack
          .peek()).binding), methodDeclaration.modifiers, "anonymous", paramTypes, pc);
      final AjAttribute attr = new AjAttribute.PointcutDeclarationAttribute(resPcutDef);
  ...
  private static void ensureVoidReturnType(MethodDeclaration methodDeclaration) {
    boolean returnsVoid = true;
    if ((methodDeclaration.returnType instanceof SingleTypeReference)) {
      final SingleTypeReference retType = (SingleTypeReference) methodDeclaration.returnType;
  ...
  private static String getStringLiteralFor(String memberName, Annotation inAnnotation, int[] location) {
    if (inAnnotation instanceof SingleMemberAnnotation && memberName.equals("value")) {
      final SingleMemberAnnotation sma = (SingleMemberAnnotation) inAnnotation;
      if (sma.memberValue instanceof StringLiteral) {
        final StringLiteral sv = (StringLiteral) sma.memberValue;
        ...
        final Binding b = ((NameReference) sma.memberValue).binding;
    ...
    if (!(inAnnotation instanceof NormalAnnotation))
      return null;
    final NormalAnnotation ann = (NormalAnnotation) inAnnotation;
    final MemberValuePair[] mvps = ann.memberValuePairs;
    if (mvps == null)
      return null;
    for (int i = 0; i < mvps.length; i++) {
      if (CharOperation.equals(memberName.toCharArray(), mvps[i].name)) {
        if (mvps[i].value instanceof StringLiteral) {
          final StringLiteral sv = (StringLiteral) mvps[i].value;
          location[0] = sv.sourceStart;
          location[1] = sv.sourceEnd;
          return new String(sv.source());
        }
  ...
  private void convertToPointcutDeclaration(MethodDeclaration methodDeclaration, ClassScope scope) {
    ...
    final PointcutDeclaration pcDecl = new PointcutDeclaration(unit.compilationResult);
    ...
    final int[] pcLocation = new int[2];
    final String pointcutExpression = getStringLiteralFor("value", ajAnnotations.pointcutAnnotation, pcLocation);
    try {
      final ISourceContext context = new EclipseSourceContext(unit.compilationResult, pcLocation[0]);
      ...
      final TypeDeclaration onType = (TypeDeclaration) typeStack.peek();
      ...
      if (pc != null) {
        // has an expression
        final EclipseScope eScope = new EclipseScope(bindings, methodDeclaration.scope);
        ...
        pc.resolve(eScope);
        final HasIfPCDVisitor ifFinder = new HasIfPCDVisitor();
  ...
  private static void copyAllFields(MethodDeclaration from, MethodDeclaration to) {
  ...
  private static void swap(TypeDeclaration inType, MethodDeclaration thisDeclaration, MethodDeclaration forThatDeclaration) {
