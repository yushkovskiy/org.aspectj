...
public class IfPseudoToken extends PseudoToken {
  ...
  private MethodDeclaration makeIfMethod(CompilationResult result, MethodDeclaration enclosingDec, TypeDeclaration containingTypeDec, int counter) {
    final MethodDeclaration ret = new IfMethodDeclaration(result, pointcut);
    ...
    if (enclosingDec != null && enclosingDec.annotations != null) {
        ...
        for (int a = 0; a < as.length && interestingAnnotation == null; a++) {
          if (as[a] instanceof NormalAnnotation) {
            ...
            if (tr instanceof SingleTypeReference) {
              ...
              final char[] qualifiedName = CharOperation.concatWith(((QualifiedTypeReference) tr).tokens, '.');
      ...
      if (interestingAnnotation != null) {
        final MemberValuePair[] memberValuePairs = interestingAnnotation.memberValuePairs;
    ...
    final StringBuilder ifSelector = new StringBuilder();
    ...
    if (nameSuffix == null || nameSuffix.length() == 0) {
      ...
      try {
        ...
        if (enclosingDec instanceof AdviceDeclaration) {
          ...
          final StringBuilder toHash = new StringBuilder(((PointcutDeclaration) enclosingDec).getPointcutText());
  ...
  private static Argument[] makeArguments(MethodDeclaration enclosingDec,
                                          TypeDeclaration containingTypeDec) {
    final Argument[] baseArguments = enclosingDec.arguments;
    ...
    for (int i = 0; i < len; i++) {
      final Argument a = baseArguments[i];
