 ...
 * @author Jim Hugunin
 */
public final class AdviceDeclaration extends AjMethodDeclaration {
  @Nullable
  ...
  @NotNull
  public final List<Proceed> proceedCalls = new ArrayList<>(2); // populated during Proceed.findEnclosingAround
  ...
  @NotNull
  public static Argument[] addTjpArguments(@NotNull Argument[] arguments, @NotNull TypeDeclaration containingTypeDec) {
  ...
  public AdviceDeclaration(@NotNull CompilationResult result, @NotNull AdviceKind kind) {
    super(result);
  ...
  public void resolveStatements() {
    ...
    final ClassScope upperScope = (ClassScope) scope.parent; // !!! safety

    modifiers = checkAndSetModifiers(modifiers, upperScope);
    final int bindingModifiers = (modifiers | (binding.modifiers & ExtraCompilerModifiers.AccGenericSignature));
    binding.modifiers = bindingModifiers;

    if (kind == AdviceKind.AfterThrowing && extraArgument != null) {
      final TypeBinding argTb = extraArgument.binding.type;
      final TypeBinding expectedTb = upperScope.getJavaLangThrowable();
    ...
    if (kind == AdviceKind.Around) {
      final ReferenceBinding[] exceptions = new ReferenceBinding[]{upperScope.getJavaLangThrowable()};
    ...
    if (kind == AdviceKind.Around) {
      ...
      for (int i = 0; i < n; i++) {
        final Proceed call = (Proceed) proceedCalls.get(i);
        if (call.inInner) {
      ...
      if (!proceedInInners) {
        ...
        final AccessForInlineVisitor v = new AccessForInlineVisitor((AspectDeclaration) upperScope.referenceContext, handler);
        final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.ACCESS_FOR_INLINE,
            selector);
  ...
  public void addAtAspectJAnnotations() {
    ...
    final String pointcutExpression = pointcutDesignator.getPointcut().toString();
    String extraArgumentName = "";
    if (extraArgument != null) {
      extraArgumentName = new String(extraArgument.name);
    }
    final String argNames = buildArgNameRepresentation();
  ...
  public void postParse(@NotNull TypeDeclaration typeDec) {
    final AspectDeclaration aspectDecl = (AspectDeclaration) typeDec;
    adviceSequenceNumberInType = aspectDecl.adviceCounter++;

    final StringBuffer stringifiedPointcut = new StringBuffer(30);
  ...
  @NotNull
  @Override
  public StringBuffer printBody(int indent, @NotNull StringBuffer output) {
    output.append(": ");
    if (pointcutDesignator != null) {
  ...
  @Override
  @NotNull
  public StringBuffer printReturnType(int indent, @NotNull StringBuffer output) {
  ...
  protected int generateInfoAttributes(@NotNull ClassFile classFile) {
    final List<IAttribute> l = new ArrayList<>(1);
  ...
  private void generateProceedMethod(@NotNull ClassScope classScope, @NotNull ClassFile classFile) {
    final MethodBinding binding = proceedMethodBinding;
    ...
    int index = 0;
    for (int i = 0; i < nargs - 1; i++) {
      final TypeBinding type = binding.parameters[i];
      codeStream.dup();
    ...
    // call run
    final ReferenceBinding closureType = (ReferenceBinding) binding.parameters[nargs - 1];
    final MethodBinding runMethod = closureType.getMethods("run".toCharArray())[0];
    codeStream.invoke(Opcodes.OPC_invokevirtual, runMethod, null);

    final TypeBinding returnType = binding.returnType;
  ...
  private void determineExtraArgumentFlags() {
    ...
    final ThisJoinPointVisitor tjp = new ThisJoinPointVisitor(this);
    extraArgumentFlags |= tjp.removeUnusedExtraArguments();
  }

  @NotNull
  private static TypeBinding[] resize(int newSize, @NotNull TypeBinding[] bindings) {
    final int len = bindings.length;
    final TypeBinding[] ret = new TypeBinding[newSize];
  ...
  private String buildArgNameRepresentation() {
    ...
    final int numArgsWeCareAbout = getDeclaredParameterCount();
  ...
  private int checkAndSetModifiers(int modifiers, @NotNull ClassScope scope) {
    if (modifiers == 0)
  ...
  @NotNull
  private static TypeReference toReference(@NotNull char[] typename) {
    if (CharOperation.contains('.', typename)) {
      final char[][] compoundName = CharOperation.splitOn('.', typename);
  ...
  @NotNull
  private static Argument makeFinalArgument(@NotNull String name, TypeReference typeRef) {
    final long pos = 0; // XXX encode start and end location
    return new Argument(name.toCharArray(), pos, typeRef, Modifier.FINAL);
  }

  @NotNull
  private static Argument[] extendArgumentsLength(@Nullable Argument[] args, int addedArguments) {
    if (args == null) {
      return new Argument[addedArguments];
    }
    final int len = args.length;
    final Argument[] ret = new Argument[len + addedArguments];
