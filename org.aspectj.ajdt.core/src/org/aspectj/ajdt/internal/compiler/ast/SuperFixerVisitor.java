  ...
  public boolean visit(TypeDeclaration localTypeDeclaration, BlockScope scope) {
    if (localTypeDeclaration.binding instanceof LocalTypeBinding) {
      if (((LocalTypeBinding) localTypeDeclaration.binding).isAnonymousType()) {
        ...
        final MethodBinding[] bindings = localTypeDeclaration.binding.methods;
        if (bindings != null) {
...
//				localTypeDeclaration.modifiers|=Modifier.PUBLIC;
  ...
  public void endVisit(MessageSend call, BlockScope scope) {
    ...
    final EclipseFactory factory = ((AjLookupEnvironment) method.scope.environment()).factory;
    if (depthCounter != 0 && targetClass.isInterface()) {// pr198196 - when calling MarkerInterface.super.XXX()
      if (call.isSuperAccess() && !call.binding.isStatic()) {
        final MethodScope currentMethodScope = scope.methodScope();
        final SourceTypeBinding sourceType = currentMethodScope.enclosingSourceType();
        final FieldBinding field = sourceType.addSyntheticFieldForInnerclass(targetClass);
      ...
      final MethodBinding superAccessBinding = new MethodBinding(ClassFileConstants.AccPublic, accessName, superBinding.returnType,
