...
public class Proceed extends MessageSend {
  ...
  public TypeBinding resolveType(BlockScope scope) {
    ...
    final AdviceDeclaration aroundDecl = findEnclosingAround(scope);
    ...
    if (arguments != null) {
      ...
      final Expression[] newArguments = new Expression[baseArgCount + 1];
    ...
    final int declaredParameterCount = aroundDecl.getDeclaredParameterCount();
...
//
    ...
    for (int i = 0, len = arguments.length; i < len; i++) {
      ...
      final TypeBinding argType = arg.resolveType(scope);
      if (argType != null) {
        final TypeBinding paramType = binding.parameters[i];
  ...
  private AdviceDeclaration findEnclosingAround(Scope scope) {
    ...
    if (scope instanceof MethodScope) {
      ...
      final ReferenceContext context = methodScope.referenceContext;
      if (context instanceof AdviceDeclaration) {
        final AdviceDeclaration adviceDecl = (AdviceDeclaration) context;
