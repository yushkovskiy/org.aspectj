...
public class PointcutDesignator extends ASTNode {
  ...
  public PointcutDesignator(Parser parser, PseudoTokens tokens) {
    ...
    final Pointcut pc = tokens.parsePointcut(parser);
  ...
  public boolean finishResolveTypes(final AbstractMethodDeclaration dec, MethodBinding method, final int baseArgumentCount,
    ...
    final EclipseFactory world = EclipseFactory.fromScopeLookupEnvironment(dec.scope);

    final TypeBinding[] parameters = method.parameters;
    final Argument[] arguments = dec.arguments;

    final FormalBinding[] bindings = new FormalBinding[baseArgumentCount];
    for (int i = 0, len = baseArgumentCount; i < len; i++) {
      ...
      if (parameters[i].isInterface() && parameters[i].isParameterizedType() && parameters[i].isMemberType()) {
        final TypeVariableBinding[] tvs = parameters[i].typeVariables();
    ...
    final EclipseScope scope = new EclipseScope(bindings, dec.scope);
  ...
  public String getPointcutDeclarationText() {
    ...
    final PseudoToken[] toks = tokens.tokens;
