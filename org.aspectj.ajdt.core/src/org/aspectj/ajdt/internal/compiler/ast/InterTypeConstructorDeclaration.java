...
public class InterTypeConstructorDeclaration extends InterTypeDeclaration {
  ...
  private void raiseNoFieldInitializersWarning() {
    ...
    final ISourceLocation location = new EclipseSourceLocation(scope.problemReporter().referenceContext.compilationResult(),
    ...
    for (int i = 0; i < this.annotations.length; i++) {
      if (new String(this.annotations[i].resolvedType.signature()).equals(SUPPRESSAJWARNINGS)) {
        if (this.annotations[i] instanceof MarkerAnnotation) {
          ...
          final SingleMemberAnnotation sma = (SingleMemberAnnotation) this.annotations[i];
          if (sma.memberValue instanceof ArrayInitializer) {
            final ArrayInitializer memberValue = (ArrayInitializer) sma.memberValue;
            for (int j = 0; j < memberValue.expressions.length; j++) {
              if (memberValue.expressions[j] instanceof StringLiteral) {
                final StringLiteral val = (StringLiteral) memberValue.expressions[j];
  ...
  private MethodDeclaration makePreMethod(ClassScope scope, ExplicitConstructorCall explicitConstructorCall) {
    ...
    final UnresolvedType aspectTypeX = world.fromBinding(binding.declaringClass);
    final UnresolvedType targetTypeX = world.fromBinding(onTypeBinding);

    final ArrayBinding objectArrayBinding = scope.createArrayType(scope.getJavaLangObject(), 1);

    final MethodDeclaration pre = new MethodDeclaration(compilationResult);
    ...
    final ResolvedMemberImpl preIntroducedConstructorRM = world.makeResolvedMember(binding);
    ...
    final InterTypeScope newParent = new InterTypeScope(scope, onTypeBinding);
    pre.scope.parent = newParent;

    pre.resolveStatements(); // newParent);

    final int nParams = pre.arguments.length;
    ...
    final ArrayInitializer init = new ArrayInitializer();
    ...
    for (int i = 0; i < nExprs; i++) {
      ...
      final ResolvedMember conversionMethod = AjcMemberMaker.toObjectConversionMethod(world
    ...
    for (int i = 0; i < nParams; i++) {
      final LocalVariableBinding binding = pre.arguments[i].binding;
      Expression arg = AstUtil.makeResolvedLocalVariableReference(binding);
      final ResolvedMember conversionMethod = AjcMemberMaker.toObjectConversionMethod(world.fromBinding(binding.type));
      if (conversionMethod != null) {
        arg = new KnownMessageSend(world.makeMethodBindingForCall(conversionMethod), new CastExpression(new NullLiteral(0,
            0), AstUtil.makeTypeReference(world.makeTypeBinding(AjcMemberMaker.CONVERSIONS_TYPE))),
            new Expression[]{arg});
    ...
    final ArrayAllocationExpression newArray = new ArrayAllocationExpression();
  ...
  public EclipseTypeMunger build(ClassScope classScope) {
    ...
    final ResolvedType declaringTypeX = world.fromEclipse(onTypeBinding);
    final ResolvedType aspectType = world.fromEclipse(classScope.referenceContext.binding);
    ...
    final ResolvedMemberImpl signature = world.makeResolvedMemberForITD(binding, onTypeBinding, interTypeScope.getRecoveryAliases());
    ...
    final ResolvedMember syntheticInterMember = AjcMemberMaker.interConstructor(declaringTypeX, signature, aspectType);

    final NewConstructorTypeMunger myMunger = new NewConstructorTypeMunger(signature, syntheticInterMember, null, null,
        typeVariableAliases);
    setMunger(myMunger);
