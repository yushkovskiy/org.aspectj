...
public abstract class InterTypeDeclaration extends AjMethodDeclaration {
  ...
  public void addAtAspectJAnnotations() {
    ...
    final Annotation ann = AtAspectJAnnotationFactory.createITDAnnotation(
  ...
  public void fixSuperCallsInBody() {
    ...
    final SuperFixerVisitor v = new SuperFixerVisitor(this, onTypeBinding);
  ...
  public void determineTypeVariableAliases() {
    if (onType != null) {
      ...
      if (onType instanceof ParameterizedSingleTypeReference) {
        final ParameterizedSingleTypeReference paramRef = (ParameterizedSingleTypeReference) onType;
        final TypeReference[] rb = paramRef.typeArguments;
        ...
        final ParameterizedQualifiedTypeReference paramRef = (ParameterizedQualifiedTypeReference) onType;
        typeVariableAliases = new ArrayList();
        for (int j = 0; j < paramRef.typeArguments.length; j++) {
          final TypeReference[] rb = paramRef.typeArguments[j];
  ...
  public void ensureScopeSetup() {
    ...
    final MethodScope scope = this.scope;

    TypeReference ot = onType;
    ReferenceBinding rb = null;

    if (ot instanceof ParameterizedQualifiedTypeReference) { // pr132349
      final ParameterizedQualifiedTypeReference pref = (ParameterizedQualifiedTypeReference) ot;
      if (pref.typeArguments != null && pref.typeArguments.length != 0) {
        ...
        for (int i = 0; i < pref.typeArguments.length; i++) {
          final TypeReference[] refs = pref.typeArguments[i];
          for (int j = 0; refs != null && j < refs.length; j++) {
            final TypeBinding tb = refs[j].getTypeBindingPublic(scope.parent);
        ...
        if (usingNonTypeVariableInITD) {
          ...
          final ReferenceBinding closestMatch = null;
    ...
    if (ot instanceof ParameterizedSingleTypeReference) {
      final ParameterizedSingleTypeReference pref = (ParameterizedSingleTypeReference) ot;
