 ...
 * @author Andy Clement
 */
public final class AspectDeclaration extends TypeDeclaration {
  @NotNull
  ...
  public AspectDeclaration(@NotNull CompilationResult compilationResult) {
  ...
  public void checkSpec(@NotNull ClassScope scope) {
    ...
    if (!isAbstract()) {
      final MethodBinding[] methods = binding.methods();
      for (int i = 0, len = methods.length; i < len; i++) {
        final MethodBinding m = methods[i];
        if (m.isConstructor()) {
    ...
    final ResolvedType myType = typeX;
    // if (myType == null) System.err.println("bad myType for: " + this);
    final ResolvedType superType = myType.getSuperclass();
  ...
  public void addAtAspectJAnnotations() {
    ...
    final Annotation atAspectAnnotation = AtAspectJAnnotationFactory.createAspectAnnotation(perClause.toDeclarationString(),
    ...
    final Annotation[] toAdd = new Annotation[isPrivileged ? 2 : 1];
    ...
    if (annotations == null) {
      ...
      final Annotation[] old = annotations;
    ...
    if (memberTypes != null) {
      for (int i = 0; i < memberTypes.length; i++) {
        if (memberTypes[i] instanceof IntertypeMemberClassDeclaration) {
          final EclipseTypeMunger m = ((IntertypeMemberClassDeclaration) memberTypes[i]).build(classScope);
          if (m != null) {
    ...
    if (methods != null) {
      for (int i = 0; i < methods.length; i++) {
        if (methods[i] instanceof InterTypeDeclaration) {
          final EclipseTypeMunger m = ((InterTypeDeclaration) methods[i]).build(classScope);
          if (m != null) {
            concreteName.typeMungers.add(m);
          }
        } else if (methods[i] instanceof DeclareDeclaration) {
          final Declare d = ((DeclareDeclaration) methods[i]).build(classScope);
          if (d != null) {
  ...
  protected void generateAttributes(ClassFile classFile) {
    ...
    if (binding.privilegedHandler != null) {
      ...
      final PrivilegedHandler privilegedHandler = (PrivilegedHandler) binding.privilegedHandler;
      final ResolvedMember[] members = privilegedHandler.getMembers();
      if (members.length > 0) {
    ...
    if (memberTypes != null) {
      for (int i = 0; i < memberTypes.length; i++) {
        if (memberTypes[i] instanceof IntertypeMemberClassDeclaration) {
          final IntertypeMemberClassDeclaration itdMemberClassDeclaration = (IntertypeMemberClassDeclaration) memberTypes[i];
          final AjAttribute attribute = itdMemberClassDeclaration.getAttribute();
          if (attribute != null) {
  ...
  private void generateInlineAccessMembers(ClassFile classFile) {
    ...
    for (final Iterator<Map.Entry<ResolvedMember, Binding>> i = accessForInline.entrySet().iterator(); i.hasNext(); ) {
      final Map.Entry<ResolvedMember, Binding> e = i.next();
  ...
  private void generatePerSupportMembers(ClassFile classFile) {
    ...
    if (perClause.getKind() == PerClause.SINGLETON) {
      ...
      final TypeBinding interfaceType = generatePerObjectInterface(classFile);
  ...
  private void generateMethod(ClassFile classFile, MethodBinding methodBinding, List additionalAttributes, BodyGenerator gen) {
    ...
    final MethodDeclaration md = AstUtil.makeMethodDeclaration(methodBinding);
    ...
    final boolean b = ((codeStream.generateAttributes & ClassFileConstants.ATTR_VARS) != 0 ? true : false); // pr148693
  ...
  private TypeBinding generatePerObjectInterface(ClassFile classFile) {
    final EclipseFactory world = EclipseFactory.fromScopeLookupEnvironment(this.scope);
    final UnresolvedType interfaceTypeX = AjcMemberMaker.perObjectInterfaceType(typeX);
    final HelperInterfaceBinding interfaceType = new HelperInterfaceBinding(this.binding, interfaceTypeX);
  ...
  private void generatePerTypeWithinAspectOfMethod(ClassFile classFile) {
    ...
    generateMethod(classFile, aspectOfMethod, new BodyGenerator() {
      ...
      public void generate(CodeStream codeStream) {
        ...
        final LocalVariableBinding theTypeVar = new LocalVariableBinding("type".toCharArray(), world
        ...
        final BranchLabel instanceFound = new BranchLabel(codeStream);

        final ExceptionLabel anythingGoesWrong = new ExceptionLabel(codeStream, world
        ...
        final LocalVariableBinding aspectInstanceVar = new LocalVariableBinding("instance".toCharArray(), world
  ...
  private void generatePerObjectAspectOfMethod(ClassFile classFile, final TypeBinding interfaceType) {
    ...
    generateMethod(classFile, aspectOfMethod, new BodyGenerator() {
      @Override
      public void generate(CodeStream codeStream) {
        final LocalVariableBinding objectVar = new LocalVariableBinding("object".toCharArray(), world
        ...
        final BranchLabel wrongType = new BranchLabel(codeStream);
        final BranchLabel returnIt = new BranchLabel(codeStream);
        codeStream.aload_0();
  ...
  private void generatePerTypeWithinHasAspectMethod(ClassFile classFile) {
    ...
    generateMethod(classFile, hasAspectMethod, new BodyGenerator() {
      @Override
      public void generate(CodeStream codeStream) {
        final LocalVariableBinding classVariable = new LocalVariableBinding("clazz".toCharArray(), world
            .makeTypeBinding(UnresolvedType.JL_CLASS), Modifier.PUBLIC, true);
        codeStream.record(classVariable);
        classVariable.recordInitializationStartPC(codeStream.position);
        final ExceptionLabel goneBang = new ExceptionLabel(codeStream, world.makeTypeBinding(UnresolvedType.JL_EXCEPTION));
        final BranchLabel noInstanceExists = new BranchLabel(codeStream);
        final BranchLabel leave = new BranchLabel(codeStream);
        goneBang.placeStart();
        codeStream.aload_0();
        codeStream.invoke(Opcodes.OPC_invokestatic,
  ...
  private void generatePerObjectBindMethod(ClassFile classFile, final TypeBinding interfaceType) {
    ...
    generateMethod(classFile, AjcMemberMaker.perObjectBind(world.fromBinding(binding)), new BodyGenerator() {
      ...
      public void generate(CodeStream codeStream) {
        ...
        final LocalVariableBinding aspectInstanceVar = new LocalVariableBinding("aspectInstance".toCharArray(), world
            .makeTypeBinding(UnresolvedType.OBJECT), Modifier.PUBLIC, true);
        codeStream.record(aspectInstanceVar);
  ...
  private void generatePerTypeWithinGetWithinTypeNameMethod(ClassFile classFile) {
    ...
    generateMethod(classFile,
        ...
        new BodyGenerator() {
          @Override
          public void generate(CodeStream codeStream) {
            final ExceptionLabel exc = new ExceptionLabel(codeStream, world.makeTypeBinding(UnresolvedType.JL_EXCEPTION));
            exc.placeStart();
            codeStream.aload_0();
  ...
  private void generatePerTypeWithinGetInstanceMethod(ClassFile classFile) {
    ...
    generateMethod(classFile, AjcMemberMaker.perTypeWithinGetInstance(world.fromBinding(binding)), new BodyGenerator() {
      ...
      public void generate(CodeStream codeStream) {
        ...
        codeStream.aconst_null();
        final TypeBinding methodTypeBinding = world.makeTypeBinding(UnresolvedType.forSignature("Ljava/lang/reflect/Method;"));
        codeStream.invoke(
        ...
        codeStream.astore_1();
        final LocalVariableBinding methodVariable = new LocalVariableBinding("method".toCharArray(), methodTypeBinding,
        ...
        codeStream.astore_2();
        final LocalVariableBinding aspectInstanceVariable = new LocalVariableBinding("aspectInstance".toCharArray(), world
  ...
  private void generatePerSingletonAspectOfMethod(ClassFile classFile) {
    ...
    generateMethod(classFile, aspectOfMethod, new BodyGenerator() {
      ...
      public void generate(CodeStream codeStream) {
        ...
        final FieldBinding fb = world.makeFieldBinding(AjcMemberMaker.perSingletonField(typeX));
        codeStream.fieldAccess(Opcodes.OPC_getstatic, fb, null);
        final BranchLabel isNonNull = new BranchLabel(codeStream);
  ...
  private void generatePerSingletonHasAspectMethod(ClassFile classFile) {
    ...
    generateMethod(classFile, hasAspectMethod, new BodyGenerator() {
      ...
      public void generate(CodeStream codeStream) {
        ...
        final BranchLabel isNull = new BranchLabel(codeStream);
        codeStream.ifnull(isNull);
        codeStream.iconst_1();
        codeStream.ireturn();
        isNull.place();
  ...
  private PerClause.Kind lookupPerClauseKind(ReferenceBinding binding) {
    ...
    if (binding instanceof BinaryTypeBinding) {
      ...
      final ParameterizedTypeBinding pBinding = (ParameterizedTypeBinding) binding;
      if (pBinding.type instanceof SourceTypeBinding) {
        final SourceTypeBinding sourceSc = (SourceTypeBinding) pBinding.type;
  ...
  private void buildPerClause(ClassScope scope) {
    ...
    if (perClause == null) {
      final PerClause.Kind kind = lookupPerClauseKind(binding.superclass);
      if (kind == null) {
        perClause = new PerSingleton();
      } else {
  ...
  private void mungeNewInnerClass(EclipseTypeMunger m, EclipseFactory world) {
    ...
    final NewMemberClassTypeMunger munger = (NewMemberClassTypeMunger) m.getMunger();
    ...
    final SourceTypeBinding aspectTypeBinding = (SourceTypeBinding) world.makeTypeBinding(m.getAspectType());
    ...
    final SourceTypeBinding targetSourceTypeBinding = (SourceTypeBinding) world.makeTypeBinding(munger.getTargetType());
    ...
    if (!(targetSourceTypeBinding instanceof BinaryTypeBinding)) {
      final ReferenceBinding[] existingMemberTypes = targetSourceTypeBinding.memberTypes();
      for (int i = 0; i < existingMemberTypes.length; i++) {
        final char[] compounded = CharOperation.concatWith(existingMemberTypes[i].compoundName, '.');
