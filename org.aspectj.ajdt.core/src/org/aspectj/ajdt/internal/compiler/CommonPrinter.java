...
public class CommonPrinter {
  ...
  protected StringBuilder printTypeReference(TypeReference tr) {
    if (tr instanceof Wildcard) {
      ...
      final ParameterizedSingleTypeReference pstr = (ParameterizedSingleTypeReference) tr;
      final ReferenceBinding tb = (ReferenceBinding) mscope.getType(pstr.token);
      output.append(CharOperation.concatWith(tb.compoundName, '.'));
      output.append('<');
      final TypeReference[] typeArguments = pstr.typeArguments;
      ...
      final ParameterizedQualifiedTypeReference pqtr = (ParameterizedQualifiedTypeReference) tr;
      output.append(CharOperation.concatWith(pqtr.tokens, '.'));
      output.append('<');
      final TypeReference[][] typeArguments = pqtr.typeArguments;
      // TODO don't support parameterized interim name components
      final TypeReference[] ofInterest = typeArguments[typeArguments.length - 1];
      ...
      final SingleTypeReference str = (SingleTypeReference) tr;
      final TypeBinding tb = mscope.getType(str.token);
      ...
      final QualifiedTypeReference qtr = (QualifiedTypeReference) tr;
  ...
  public StringBuilder printAnnotation(Annotation annotation) {
    ...
    final MemberValuePair[] mvps = annotation.memberValuePairs();
  ...
  public StringBuilder printExpression(Expression e) {
    ...
    try {
      ...
      if (e instanceof TypeReference) {
        ...
        final SingleNameReference snr = (SingleNameReference) e;
        if (snr.binding instanceof ReferenceBinding) {
          ...
          final ParameterizedFieldBinding pfb = (ParameterizedFieldBinding) snr.binding;
          output.append(pfb.name);
        } else if (snr.binding instanceof LocalVariableBinding) {
          final LocalVariableBinding lvb = (LocalVariableBinding) snr.binding;
          output.append(lvb.name);
        } else if (snr.binding instanceof FieldBinding) {
          final FieldBinding fb = (FieldBinding) snr.binding;
          final ReferenceBinding rb = fb.declaringClass;
          if (fb.isStatic()) {
            // qualify it
        ...
        final ArrayReference ar = (ArrayReference) e;
        ...
        final ThisReference tr = (ThisReference) e;
  ...
  protected StringBuilder printStatement(Statement statement, int indent, boolean applyIndent) {
    if (statement instanceof ReturnStatement) {
      ...
      printIndent(indent);
      final MessageSend ms = (MessageSend) statement;
      ...
      printIndent(indent);
      final IfStatement is = (IfStatement) statement;
  ...
  private StringBuilder printMessageSendStatement(MessageSend ms) {
    ...
    if (ms.typeArguments != null) {
      output.append('<');
      final int max = ms.typeArguments.length - 1;
      for (int j = 0; j < max; j++) {
    ...
    if (td.methods != null) {
      for (int i = 0; i < td.methods.length; i++) {
        if (td.methods[i] != null) {
          final AbstractMethodDeclaration amd = td.methods[i];
  ...
  protected StringBuilder printConstructorDeclaration(ConstructorDeclaration amd, int tab) {
    ...
    final TypeParameter[] typeParams = amd.typeParameters();
    if (typeParams != null) {
      output.append('<');
  ...
  protected StringBuilder printAllocationExpression(AllocationExpression ae) {
    ...
    if (ae.typeArguments != null) {
      output.append('<');
      final int max = ae.typeArguments.length - 1;
      for (int j = 0; j < max; j++) {
