...
public class InterimCompilationResult {
  ...
  public InterimCompilationResult(CompilationResult cr, List<UnwovenClassFile> ucfList) {
    ...
    for (int i = 0; i < ucfList.size(); i++) {
      final UnwovenClassFile element = ucfList.get(i);
      unwovenClassFiles[i] = element;
      final AjClassFile ajcf = new AjClassFile(element.getClassNameAsChars(), element.getBytes());
  ...
  public boolean equals(Object other) {
    ...
    final InterimCompilationResult ir = (InterimCompilationResult) other;
