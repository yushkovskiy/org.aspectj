...
public class WeaverMessageHandler implements IMessageHandler {
  ...
  public boolean handleMessage(@NotNull IMessage message) throws AbortException {
    ...
    final ISourceLocation sLoc = message.getSourceLocation();
    ...
    if (sLoc instanceof EclipseSourceLocation) {
      final EclipseSourceLocation esLoc = (EclipseSourceLocation) sLoc;
    ...
    if (sLoc instanceof SourceLocation) {
      final SourceLocation sl = (SourceLocation) sLoc;
    ...
    final int startPos = getStartPos(sLoc, problemSource);
    final int endPos = getEndPos(sLoc, problemSource);
    final int severity = message.isError() ? ProblemSeverities.Error : ProblemSeverities.Warning;
    ...
    final ReferenceContext referenceContext = findReferenceContextFor(problemSource);
    final CategorizedProblem problem = compiler.problemReporter.createProblem(filename, IProblem.Unclassified, new String[0],
        new String[]{message.getMessage()}, severity, startPos, endPos, sLoc != null ? sLoc.getLine() : 0,
        sLoc != null ? sLoc.getColumn() : 0);
    final IProblem[] seeAlso = buildSeeAlsoProblems(problem, message.getExtraSourceLocations(), problemSource,
    ...
    if (message instanceof LintMessage) {
      final String lintMessageName = ((LintMessage) message).getLintKind();
  ...
  private static int getStartPos(ISourceLocation sLoc, CompilationResult result) {
    int pos = 0;
    if (sLoc == null)
      return 0;
    final int line = sLoc.getLine();
    if (sLoc instanceof EclipseSourceLocation) {
  ...
  private static int getEndPos(ISourceLocation sLoc, CompilationResult result) {
    int pos = 0;
    if (sLoc == null)
      return 0;
  ...
  private IProblem[] buildSeeAlsoProblems(IProblem originalProblem, List sourceLocations, CompilationResult problemSource,
    ...
    final List<IProblem> ret = new ArrayList<IProblem>();

    for (int i = 0; i < sourceLocations.size(); i++) {
      final ISourceLocation loc = (ISourceLocation) sourceLocations.get(i);
      if (loc != null) {
        final DefaultProblem dp = new DefaultProblem(loc.getSourceFile().getPath().toCharArray(), "see also", 0, new String[]{},
    ...
    if (usedBinarySourceFileName) {
      final DefaultProblem dp = new DefaultProblem(problemSource.fileName, "see also", 0, new String[]{},
          ProblemSeverities.Ignore, 0, 0, 0, 0);
      ret.add(dp);
    }
    final IProblem[] retValue = (IProblem[]) ret.toArray(new IProblem[]{});
    return retValue;
  }
