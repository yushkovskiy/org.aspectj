...
public class ClassFileBasedByteCodeProvider
  ...
  public static UnwovenClassFile[] unwovenClassFilesFor(CompilationResult result,
    ...
    final ClassFile[] cfs = result.getClassFiles();
    final UnwovenClassFile[] ret = new UnwovenClassFile[result.compiledTypes.size()];
    int i = 0;
    for (final Iterator iterator = result.compiledTypes.keySet().iterator(); iterator.hasNext(); ) {
      final char[] className = (char[]) iterator.next();
      final ClassFile cf = (ClassFile) result.compiledTypes.get(className);
      // OPTIMIZE use char[] for classname
      final ClassFileBasedByteCodeProvider p = new ClassFileBasedByteCodeProvider(cf);
      final String fileName = nameProvider.getOutputClassFileName(cf.fileName(), result);
