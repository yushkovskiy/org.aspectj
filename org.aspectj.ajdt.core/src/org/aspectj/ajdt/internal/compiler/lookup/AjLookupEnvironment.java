 ...
 * @author Jim Hugunin
 */
public final class AjLookupEnvironment extends LookupEnvironment implements AnonymousClassCreationListener {
  @Nullable
  ...
  public AjLookupEnvironment(@NotNull ITypeRequestor typeRequestor, @NotNull CompilerOptions options,
                             @NotNull ProblemReporter problemReporter, @NotNull INameEnvironment nameEnvironment) {
  ...
  public void completeTypeBindings() {
    ...
    final ContextToken completeTypeBindingsToken = CompilationAndWeavingContext.enteringPhase(
    ...
    for (int i = lastCompletedUnitIndex + 1; i <= lastUnitIndex; i++) {
      final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.CHECK_AND_SET_IMPORTS,
    ...
    for (int i = lastCompletedUnitIndex + 1; i <= lastUnitIndex; i++) {
      final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.CONNECTING_TYPE_HIERARCHY,
          units[i].compilationResult.fileName);
    ...
    for (int i = lastCompletedUnitIndex + 1; i <= lastUnitIndex; i++) {
      final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.BUILDING_FIELDS_AND_METHODS,
          units[i].compilationResult.fileName);
    ...
    // them in the factory
    for (int i = lastCompletedUnitIndex + 1; i <= lastUnitIndex; i++) {
      final SourceTypeBinding[] b = units[i].scope.topLevelTypes;
    ...
    final List<SourceTypeBinding> aspectsToProcess = new ArrayList<SourceTypeBinding>();
    for (int i = lastCompletedUnitIndex + 1; i <= lastUnitIndex; i++) {
      final CompilationUnitScope cus = units[i].scope;
      final SourceTypeBinding[] stbs = cus.topLevelTypes;
      for (int j = 0; j < stbs.length; j++) {
        final SourceTypeBinding stb = stbs[j];
        typesToProcess.add(stb);
        final TypeDeclaration typeDeclaration = stb.scope.referenceContext;
    ...
    final boolean typeProcessingOrderIsImportant = declareParents.size() > 0 || declareAnnotationOnTypes.size() > 0; // DECAT
    ...
    if (typeProcessingOrderIsImportant) {
      ...
      final List<SourceTypeBinding> stb2 = new ArrayList<SourceTypeBinding>();
  ...
  public void doSupertypesFirst(@NotNull ReferenceBinding rb, @NotNull Collection<? extends ReferenceBinding> yetToProcess) {
    if (rb instanceof SourceTypeBinding) {
      ...
      final ParameterizedTypeBinding ptb = (ParameterizedTypeBinding) rb;
  ...
  public void warnOnAddedInterface(@NotNull ResolvedType type, @NotNull ResolvedType parent) {
    final World world = factory.getWorld();
    final ResolvedType serializable = world.getCoreType(UnresolvedType.SERIALIZABLE);
  ...
  @NotNull
  @Override
  public BinaryTypeBinding createBinaryTypeFrom(@NotNull IBinaryType binaryType, @NotNull PackageBinding packageBinding,
                                                boolean needFieldsAndMethods, @Nullable AccessRestriction accessRestriction) {

    if (inBinaryTypeCreationAndWeaving) {
      final BinaryTypeBinding ret = super.createBinaryTypeFrom(binaryType, packageBinding, needFieldsAndMethods, accessRestriction);
    ...
    try {
      ...
      if (pendingTypesToFinish.size() > 0) {
        ...
        while (!pendingTypesToFinish.isEmpty()) {
          final BinaryTypeBinding nextVictim = (BinaryTypeBinding) pendingTypesToFinish.remove(0);
   ...
   */
  @Override
  public void anonymousTypeBindingCreated(@NotNull LocalTypeBinding aBinding) {
   ...
   * @param yetToProcess
   */
  private void collectAllITDsAndDeclares(@NotNull SourceTypeBinding sourceType, @NotNull Collection<? extends ReferenceBinding> yetToProcess) {
    // Look at the supertype first
    final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.COLLECTING_ITDS_AND_DECLARES,
    ...
    for (int i = 0, length = memberTypes.length; i < length; i++) {
      final SourceTypeBinding rb = (SourceTypeBinding) memberTypes[i];
   ...
   */
  private void weaveIntertypes(@NotNull List<SourceTypeBinding> typesToProcess, @NotNull SourceTypeBinding typeToWeave,
    ...
    // Look at the supertype first
    final ReferenceBinding superType = typeToWeave.superclass();
    ...
    final ReferenceBinding[] interfaceTypes = typeToWeave.superInterfaces();
    for (int i = 0; i < interfaceTypes.length; i++) {
      final ReferenceBinding binding = interfaceTypes[i];
  ...
  private void addAdviceLikeDeclares(@NotNull ClassScope s) {
    final TypeDeclaration dec = s.referenceContext;

    if (dec instanceof AspectDeclaration) {
      final ResolvedType typeX = factory.fromEclipse(dec.binding);
      factory.getWorld().getCrosscuttingMembersSet().addAdviceLikeDeclares(typeX);
    }

    final SourceTypeBinding sourceType = s.referenceContext.binding;
    final ReferenceBinding[] memberTypes = sourceType.memberTypes;
    for (int i = 0, length = memberTypes.length; i < length; i++) {
  ...
  private void addCrosscuttingStructures(@NotNull ClassScope s) {
    final TypeDeclaration dec = s.referenceContext;
  ...
  private void resolvePointcutDeclarations(@NotNull ClassScope s) {
    final TypeDeclaration dec = s.referenceContext;
    final SourceTypeBinding sourceType = s.referenceContext.binding;
    boolean hasPointcuts = false;
    final AbstractMethodDeclaration[] methods = dec.methods;
    ...
    if (hasPointcuts || dec instanceof AspectDeclaration || couldBeAnnotationStyleAspectDeclaration(dec)) {
      final ReferenceType name = (ReferenceType) factory.fromEclipse(sourceType);
      final EclipseSourceType eclipseSourceType = (EclipseSourceType) name.getDelegate();
   ...
   */
  private static boolean couldBeAnnotationStyleAspectDeclaration(@NotNull TypeDeclaration dec) {
    final Annotation[] annotations = dec.annotations;
   ...
   */
  private static void processInterTypeMemberTypes(@NotNull ClassScope classScope) {
    final TypeDeclaration dec = classScope.referenceContext;
    if (dec instanceof AspectDeclaration) {
  ...
  private void buildInterTypeAndPerClause(@NotNull ClassScope s) {
    final TypeDeclaration dec = s.referenceContext;
    if (dec instanceof AspectDeclaration) {
    ...
    if (sourceType.superclass != null) {
      final ResolvedType parent = factory.fromEclipse(sourceType.superclass);
  ...
  private static boolean isAspect(@NotNull TypeDeclaration decl) {
    if ((decl instanceof AspectDeclaration)) {
      ...
      for (int i = 0; i < decl.annotations.length; i++) {
        final Annotation ann = decl.annotations[i];
        if (ann.type instanceof SingleTypeReference) {
          ...
          final QualifiedTypeReference qtr = (QualifiedTypeReference) ann.type;
  ...
  private void weaveInterTypeDeclarations(@NotNull CompilationUnitScope unit, List<ConcreteTypeMunger> typeMungers,
  ...
  private void weaveInterTypeDeclarations(SourceTypeBinding sourceType, List<ConcreteTypeMunger> typeMungers,
    ...
    final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.WEAVING_INTERTYPE_DECLARATIONS,
        sourceType.sourceName);
    ...
    final WeaverStateInfo info = onType.getWeaverState();
    ...
    if (mode < 2) {
      ...
      for (final Iterator i = dangerousInterfaces.entrySet().iterator(); i.hasNext(); ) {
        final Map.Entry entry = (Map.Entry) i.next();
        final ResolvedType interfaceType = (ResolvedType) entry.getKey();
        if (onType.isTopmostImplementor(interfaceType)) {
          factory.showMessage(IMessage.ERROR, onType + ": " + entry.getValue(), onType.getSourceLocation(), null);
        }
      }

      final boolean needOldStyleWarning = (info != null && info.isOldStyle());
      ...
      final List<DeclareParents> decpToRepeat = new ArrayList<DeclareParents>();
      final List<DeclareAnnotation> decaToRepeat = new ArrayList<DeclareAnnotation>();
      ...
      for (DeclareParents decp : declareParents) {
        if (!decp.isMixin()) {
          final boolean didSomething = doDeclareParents(decp, sourceType);
          if (didSomething) {
      ...
      for (DeclareAnnotation deca : declareAnnotationOnTypes) {
        final boolean didSomething = doDeclareAnnotations(deca, sourceType, true);
        if (didSomething) {
      ...
      final List<Object> forRemoval = new ArrayList<>();
      ...
      while ((anyNewAnnotations || anyNewParents) && (!decpToRepeat.isEmpty() || !decaToRepeat.isEmpty())) {
        ...
        for (DeclareAnnotation deca : decaToRepeat) {
          final boolean didSomething = doDeclareAnnotations(deca, sourceType, false);
          if (didSomething) {
            if (factory.pushinCollector != null) {
    ...
    if (mode == 0 || mode == 2) {
      for (final Iterator<ConcreteTypeMunger> i = typeMungers.iterator(); i.hasNext(); ) {
        final EclipseTypeMunger munger = (EclipseTypeMunger) i.next();
      ...
      for (final Iterator i = onType.getInterTypeMungers().iterator(); i.hasNext(); ) {
        final EclipseTypeMunger munger = (EclipseTypeMunger) i.next();
        if (munger.getMunger() == null || munger.getMunger().getKind() != ResolvedTypeMunger.InnerClass) {
          if (munger.munge(sourceType, onType)) {
            if (factory.pushinCollector != null) {
   ...
   */
  private void processTypeMungersFromExistingWeaverState(@NotNull SourceTypeBinding sourceType, @NotNull ResolvedType onType) {
    final List<ConcreteTypeMunger> previouslyAppliedMungers = onType.getWeaverState().getTypeMungers(onType);

    for (final Iterator<ConcreteTypeMunger> i = previouslyAppliedMungers.iterator(); i.hasNext(); ) {
      final ConcreteTypeMunger m = i.next();
      final EclipseTypeMunger munger = factory.makeEclipseTypeMunger(m);
      if (munger.munge(sourceType, onType)) {
  ...
  private boolean doDeclareParents(@NotNull DeclareParents declareParents, SourceTypeBinding sourceType) {
    final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.PROCESSING_DECLARE_PARENTS, sourceType.sourceName);
    final ResolvedType resolvedSourceType = factory.fromEclipse(sourceType);
    final List<ResolvedType> newParents = declareParents.findMatchingNewParents(resolvedSourceType, false);
    if (!newParents.isEmpty()) {
      for (final Iterator<ResolvedType> i = newParents.iterator(); i.hasNext(); ) {
        final ResolvedType parent = i.next();
        if (dangerousInterfaces.containsKey(parent)) {
          final ResolvedType onType = factory.fromEclipse(sourceType);
  ...
  @NotNull
  private static String stringifyTargets(long bits) {
    ...
    sb.append("{");
    for (final Iterator<String> iter = s.iterator(); iter.hasNext(); ) {
  ...
  private boolean doDeclareAnnotations(DeclareAnnotation decA, SourceTypeBinding sourceType, boolean reportProblems) {
    final ResolvedType rtx = factory.fromEclipse(sourceType);
    if (!decA.matches(rtx)) {
      return false;
    }
    ...
    final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.PROCESSING_DECLARE_ANNOTATIONS,
        sourceType.sourceName);
    ...
    if (aspectType instanceof ReferenceType) {
      final ReferenceType rt = (ReferenceType) aspectType;
      if (rt.isParameterizedType() || rt.isRawType()) {
        aspectType = rt.getGenericType();
      }
    }
    final TypeBinding tb = factory.makeTypeBinding(aspectType);
    ...
    final SourceTypeBinding stb = (SourceTypeBinding) tb;
    ...
    if (stb instanceof BinaryTypeBinding) {
      ...
      final char[] declareSelector = decA.getAnnotationMethod().toCharArray();

      ReferenceBinding rb = stb;
      final String declaringAspectName = decA.getDeclaringType().getRawName();
      while (rb != null && !new String(CharOperation.concatWith(rb.compoundName, '.')).equals(declaringAspectName)) {
        rb = rb.superclass();
      }
      final MethodBinding[] mbs = rb.getMethods(declareSelector);
      ...
      if (declaringBinding instanceof BinaryTypeBinding) {
        ...
        final TypeDeclaration typeDecl = ((SourceTypeBinding) declaringBinding).scope.referenceContext;
    ...
    if (sourceType instanceof BinaryTypeBinding) {
      ...
      final ResolvedType theTargetType = factory.fromEclipse(sourceType);
      final TypeBinding theAnnotationType = toAdd[0].resolvedType;
      ...
      final String sig = new String(theAnnotationType.signature());
      final UnresolvedType bcelAnnotationType = UnresolvedType.forSignature(sig);
      final String name = bcelAnnotationType.getName();
    ...
    final Annotation[] currentAnnotations = sourceType.scope.referenceContext.annotations;
    if (currentAnnotations != null) {
      for (int i = 0; i < currentAnnotations.length; i++) {
        final Annotation annotation = currentAnnotations[i];
        final String a = CharOperation.toString(annotation.type.getTypeName());
        final String b = CharOperation.toString(toAdd[0].type.getTypeName());
  ...
  private Annotation[] retrieveAnnotationFromBinaryTypeBinding(DeclareAnnotation decA, ReferenceBinding declaringBinding) {
    final ReferenceType rt = (ReferenceType) factory.fromEclipse(declaringBinding);
    final ResolvedMember[] methods = rt.getDeclaredMethods();
    ResolvedMember decaMethod = null;
    final String nameToLookFor = decA.getAnnotationMethod();
    for (int i = 0; i < methods.length; i++) {
    ...
    if (decaMethod != null) { // could assert this ...
      final AnnotationAJ[] axs = decaMethod.getAnnotations();
      if (axs != null) { // another error has occurred, dont crash here because of it
        final Annotation[] toAdd = new Annotation[1];
  ...
  private static Annotation createAnnotationFromBcelAnnotation(AnnotationAJ annX, int pos, EclipseFactory factory) {
    final String name = annX.getTypeName();
    final TypeBinding tb = factory.makeTypeBinding(annX.getType());
    // String theName = annX.getSignature().getBaseName();
    final char[][] typeName = CharOperation.splitOn('.', name.replace('$', '.').toCharArray()); // pr149293 - not bulletproof...
  ...
  private static Annotation createAnnotationCopy(Annotation ann) {
    final NormalAnnotation ann2 = new NormalAnnotation(ann.type, ann.sourceStart);
  ...
  private static boolean isAnnotationTargettingSomethingOtherThanAnnotationOrNormal(long abits) {
...
//	}
  ...
  private void addParent(SourceTypeBinding sourceType, ResolvedType parent) {
    ...
    if (parentBinding.isClass()) {
      ...
      final ReferenceBinding[] oldI = sourceType.superInterfaces;
      final ReferenceBinding[] newI;
      if (oldI == null) {
        newI = new ReferenceBinding[1];
        newI[0] = parentBinding;
      } else {
        final int n = oldI.length;
    ...
    if (sourceType instanceof BinaryTypeBinding) {
      ...
      final ReferenceTypeDelegate rtd = rt.getDelegate();
