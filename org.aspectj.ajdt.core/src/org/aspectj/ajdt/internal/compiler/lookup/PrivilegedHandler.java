...
public class PrivilegedHandler implements IPrivilegedHandler {
  ...
  public boolean definesPrivilegedAccessToField(FieldBinding field) {
    ...
    final ResolvedMember key = inAspect.factory.makeResolvedMember(field);
  ...
  public FieldBinding getPrivilegedAccessField(FieldBinding baseField, ASTNode location) {
    ...
    final ResolvedMember key = inAspect.factory.makeResolvedMember(baseField);
    if (accessors.containsKey(key))
      return (FieldBinding) accessors.get(key);
    final FieldBinding ret = new PrivilegedFieldBinding(inAspect, baseField);
    checkWeaveAccess(key.getDeclaringType(), location);
    if (!baseField.alwaysNeedsAccessMethod(true))
      accessors.put(key, ret);
    // 307120
    final ResolvedType rt = inAspect.factory.fromEclipse(baseField.declaringClass);
    if (rt != null) {
  ...
  public void notePrivilegedTypeAccess(ReferenceBinding type, ASTNode location) {
    final ResolvedMember key = new ResolvedMemberImpl(Member.STATIC_INITIALIZATION, inAspect.factory.fromEclipse(type), 0,
  ...
  private void checkWeaveAccess(UnresolvedType typeX, ASTNode location) {
    final World world = inAspect.factory.getWorld();
    final Lint.Kind check = world.getLint().typeNotExposedToWeaver;
    if (check.isEnabled()) {
  ...
  public ResolvedMember[] getMembers() {
    final Collection m = accessors.keySet();
    final int len = m.size();
    final ResolvedMember[] ret = new ResolvedMember[len];
    int index = 0;
    for (final Iterator i = m.iterator(); i.hasNext(); ) {
