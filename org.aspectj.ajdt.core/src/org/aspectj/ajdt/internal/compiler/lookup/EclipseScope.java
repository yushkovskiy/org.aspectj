...
public class EclipseScope implements IScope {
  ...
  public UnresolvedType lookupType(String name, IHasPosition location) {
    final char[][] splitName = WildTypePattern.splitNames(name, true);
    final TypeBinding b = scope.getType(splitName, splitName.length);
    ...
    if (referenceFromAnnotationStylePointcut) { // pr265360
      ...
      final char[] qualifiedPackageName = b.qualifiedPackageName();
      if (!CharOperation.equals(qualifiedPackageName, javaLang)) {
        final String packagePrefix = new String(qualifiedPackageName);
  ...
  private void computeImports() {
    ...
    final List importedNamesList = new ArrayList();
    final List importedPrefixesList = new ArrayList();
    ...
    final CompilationUnitScope cuScope = (CompilationUnitScope) currentScope;

    final String packageName = new String(CharOperation.concatWith(cuScope.currentPackageName, '.'));
    // System.err.println("package: " + packageName);
    if (packageName.length() > 0) {
      importedPrefixesList.add(packageName + ".");
    }

    final ImportBinding[] imports = cuScope.imports;
    for (int i = 0; i < imports.length; i++) {
      final ImportBinding importBinding = imports[i];
      final String importName = new String(CharOperation.concatWith(importBinding.compoundName, '.'));
    ...
    final TypeBinding[] topTypes = cuScope.topLevelTypes;
