...
public class InterTypeMemberFinder implements IMemberFinder {
  ...
  public FieldBinding getField(SourceTypeBinding sourceTypeBinding, char[] fieldName, InvocationSite site, Scope scope) {
    ...
    final int fieldLength = fieldName.length;

    for (int i = 0, len = interTypeFields.size(); i < len; i++) {
      final FieldBinding field = (FieldBinding) interTypeFields.get(i);
  ...
  private static FieldBinding resolveConflicts(SourceTypeBinding sourceTypeBinding, FieldBinding retField, FieldBinding field,
  ...
  // }
  // //
  private static boolean isVisible(MethodBinding m1, ReferenceBinding s) {
  ...
  public MethodBinding[] methods(SourceTypeBinding sourceTypeBinding) {
    final MethodBinding[] orig = sourceTypeBinding.methodsBase();
    // if (interTypeMethods.isEmpty()) return orig;

    final List ret = new ArrayList(Arrays.asList(orig));
    for (int i = 0, len = interTypeMethods.size(); i < len; i++) {
      final MethodBinding method = (MethodBinding) interTypeMethods.get(i);
      ret.add(method);
    }

    final ReferenceBinding[] interfaces = sourceTypeBinding.superInterfaces();
    for (int i = 0; i < interfaces.length; i++) {
      if (interfaces[i] instanceof SourceTypeBinding) {
        final SourceTypeBinding intSTB = (SourceTypeBinding) interfaces[i];
  ...
  private static void addPublicITDSFrom(SourceTypeBinding anInterface, List toAList) {
    if (anInterface.memberFinder != null) {
      final InterTypeMemberFinder finder = (InterTypeMemberFinder) anInterface.memberFinder;
      for (final Iterator iter = finder.interTypeMethods.iterator(); iter.hasNext(); ) {
        final MethodBinding aBinding = (MethodBinding) iter.next();
        if (Modifier.isPublic(aBinding.modifiers)) {
          toAList.add(aBinding);
        }
      }
    }
    final ReferenceBinding superType = anInterface.superclass;
  ...
  public MethodBinding[] getMethods(SourceTypeBinding sourceTypeBinding, char[] selector) {
    ...
    final MethodBinding[] orig = sourceTypeBinding.getMethodsBase(selector);
  ...
  public MethodBinding getExactMethod(SourceTypeBinding sourceTypeBinding, char[] selector, TypeBinding[] argumentTypes,
    ...
    final MethodBinding ret = sourceTypeBinding.getExactMethodBase(selector, argumentTypes, refScope);

    // An intertype declaration may override an inherited member (Bug#50776)
    for (int i = 0, len = interTypeMethods.size(); i < len; i++) {
      final MethodBinding im = (MethodBinding) interTypeMethods.get(i);
  ...
  public static boolean matches2(MethodBinding newBinding, MethodBinding existingBinding) {
    ...
    final int argCount = existingBinding.parameters.length;
    if (newBinding.parameters.length != argCount) {
      return false;
    }
    final TypeBinding[] toMatch = newBinding.parameters;
    ...
    if (!matches) {
      ...
      final TypeVariableBinding[] existingTVBs = existingBinding.typeVariables;
      final TypeVariableBinding[] newTVBs = newBinding.typeVariables;
      if (existingTVBs.length != 0 && existingTVBs.length == newTVBs.length) {
        ...
        for (int t = 0, max = existingTVBs.length; t < max; t++) {
          final char[] existingSig = existingTVBs[t].genericSignature();
          final char[] newSig = newTVBs[t].genericSignature();
        ...
        if (genericsMatch) {
          for (int a = 0; a < argCount; a++) {
            final char[] existingSig = existingBinding.parameters[a].genericTypeSignature();
            final char[] newSig = toMatch[a].genericTypeSignature();
            if (!CharOperation.equals(existingSig, newSig)) {
              genericsMatch = false;
              break;
            }
  ...
  public void addInterTypeMethod(MethodBinding binding) {
    ...
    if (isVisible(binding, sourceTypeBinding)) {
      final MethodBinding[] baseMethods = sourceTypeBinding.methods;
      for (int i = 0, len = baseMethods.length; i < len; i++) {
        final MethodBinding b = baseMethods[i];
  ...
  public boolean definesField(String name) {
    final List l = interTypeFields;
    if (l.size() > 0) {
      final char[] nameChars = name.toCharArray();
      for (int i = 0; i < l.size(); i++) {
        final FieldBinding fieldBinding = (FieldBinding) interTypeFields.get(i);
