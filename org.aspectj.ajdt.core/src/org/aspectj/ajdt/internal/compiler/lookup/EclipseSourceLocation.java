...
//                || (0 == result.fileName.length)) {
  ...
  public int getColumn() {
    if (-1 == column) {
      ...
      if (0 < lineNumber && lineNumber < result.lineSeparatorPositions.length) {
        final int lineStart = result.lineSeparatorPositions[lineNumber];
        final int col = startPos - lineStart; // 1-based
        if (0 <= col) {
          column = col;
        } else {
          column = 0;
  ...
  public String getContext() {
    if (null == context) {
      ...
      final IProblem[] problems = result.problems;
  ...
  public boolean equals(Object other) {
    ...
    final EclipseSourceLocation o = (EclipseSourceLocation) other;
    return
        getLine() == o.getLine() &&
