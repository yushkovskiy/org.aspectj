...
public class AspectDeclaration extends AjTypeDeclaration {
  ...
  static {
    final List temporary = new ArrayList();
  ...
  ASTNode clone0(AST target) {
    final AspectDeclaration result = new AspectDeclaration(target, perClause);
  ...
  public void setPerClause(ASTNode perClause) {
    ...
    final ASTNode oldChild = this.perClause;
  ...
  public List getAdvice() {
    // ajh02: method added
    final List bd = bodyDeclarations();
    final List advice = new ArrayList();
    for (final Iterator it = bd.listIterator(); it.hasNext(); ) {
      final Object decl = it.next();
