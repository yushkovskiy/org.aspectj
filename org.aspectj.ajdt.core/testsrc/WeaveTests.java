...
public class WeaveTests {
  ...
  public static void createClasses() throws IOException {
    ...
    for (int classcounter = 0; classcounter < NUMCLASSES; classcounter++) {
      ...
      final StringBuffer classfile = new StringBuffer(1000);
      ...
      try {
        ...
        final File f = new File(outputDir, ("C" + classcounter + ".java"));
    ...
    if (INCLUDE_TEST_CLASSES) {
      ...
      try {
        ...
        final File f = new File(outputDir, "TestGet.java");
        final FileOutputStream fos = new FileOutputStream(f);
        final BufferedOutputStream bos = new BufferedOutputStream(fos);
      ...
      try {
        ...
        final File f = new File(outputDir, "TestExecution.java");
        final FileOutputStream fos = new FileOutputStream(f);
        final BufferedOutputStream bos = new BufferedOutputStream(fos);
    ...
    final StringBuffer buildList = new StringBuffer(100);
    ...
    try {
      final File f = new File(outputDir, "build.lst");
      final FileOutputStream fos = new FileOutputStream(f);
      final BufferedOutputStream bos = new BufferedOutputStream(fos);
  ...
  private static void createAllAspects(String adviceType, int pointcut) {
    ...
    final StringBuffer aspectFile = new StringBuffer(1000);
    ...
    // Create the file
    try {
      final File f = new File(outputDir, (adviceName + ".aj"));
      final FileOutputStream fos = new FileOutputStream(f);
      final BufferedOutputStream bos = new BufferedOutputStream(fos);
  ...
  public static void compileClasses() throws IOException {
    ...
    for (int i = 0; i < ITERATIONS; i++) {
      final long time = performCompile();
      compileTimes[i] = time;
      average += time;
    }
  ...
  private static long performCompile() throws IOException {
    ...
    final String[] parsedArgs = RunWeaveTests.split(ajcargs);
  ...
  public static void weaveAspects(String adviceType) throws IOException {
    ...
    if (ALL_POINTCUT_TESTS || TEST_ONE) {
      ...
      for (int i = 0; i < ITERATIONS; i++) {
        final long time = weaveAllAspects(adviceType, WEAVE_FAST);
    ...
    if (ALL_POINTCUT_TESTS || TEST_ONE_SEARCH_ALL) {
      ...
      for (int i = 0; i < ITERATIONS; i++) {
        final long time = weaveAllAspects(adviceType, WEAVE_MED);
    ...
    if (ALL_POINTCUT_TESTS || TEST_ALL) {
      ...
      for (int i = 0; i < ITERATIONS; i++) {
        final long time = weaveAllAspects(adviceType, WEAVE_SLOW);
