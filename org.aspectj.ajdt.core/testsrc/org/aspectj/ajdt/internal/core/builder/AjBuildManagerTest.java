...
public class AjBuildManagerTest extends TestCase {
  ...
  public static void assertCompileMessagesValid(MessageHandler handler) {
    ...
    if (1 == numMessages) { // permit aspectjrt.jar warning
      final IMessage m = handler.getMessages(IMessage.WARNING, true)[0];
  ...
  public void testSimpleStructure() throws IOException /* , CoreException */ {
    ...
    final AjBuildManager manager = new AjBuildManager(messageWriter);
    final BuildArgParser parser = new BuildArgParser(messageWriter);
    final String javaClassPath = System.getProperty("java.class.path");
    final String sandboxName = Ajc.createEmptySandbox().getAbsolutePath();
    final AjBuildConfig buildConfig = parser.genBuildConfig(new String[]{"-d", sandboxName, "-classpath", javaClassPath,
        AjdtAjcTests.TESTDATA_PATH + "/src1/A.java",
        // EajcModuleTests.TESTDATA_PATH + "/src1/Hello.java",
    });
    final String err = parser.getOtherMessages(true);
