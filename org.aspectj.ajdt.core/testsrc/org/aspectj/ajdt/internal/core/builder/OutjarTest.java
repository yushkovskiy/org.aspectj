...
public class OutjarTest extends AjcTestCase {
  ...
  public void testOutjarInInjars() {
    final String[] args = new String[]{"-aspectpath", aspectjarName, "-injars", injarName, "-outjar", injarName};
    final Message error = new Message(WeaverMessages.format(WeaverMessages.OUTJAR_IN_INPUT_PATH));
    final Message fail = new Message("Usage:");
    final MessageSpec spec = new MessageSpec(null, null, newMessageList(error), newMessageList(fail), null);
...
//		System.out.println(result);
  ...
  public void testOutjarInInpath() {
    final String[] args = new String[]{"-aspectpath", aspectjarName, "-inpath", injarName, "-outjar", injarName};
    final Message error = new Message(WeaverMessages.format(WeaverMessages.OUTJAR_IN_INPUT_PATH));
    final Message fail = new Message("Usage:");
    final MessageSpec spec = new MessageSpec(null, null, newMessageList(error), newMessageList(fail), null);
    final CompilationResult result = ajc(baseDir, args);
//		System.out.println(result);
  ...
  public void testOutjarInAspectpath() {
    final String[] args = new String[]{"-aspectpath", aspectjarName, "-inpath", injarName, "-outjar", aspectjarName};
    final Message error = new Message(WeaverMessages.format(WeaverMessages.OUTJAR_IN_INPUT_PATH));
    final Message fail = new Message("Usage:");
    final MessageSpec spec = new MessageSpec(null, null, newMessageList(error), newMessageList(fail), null);
    final CompilationResult result = ajc(baseDir, args);
//		System.out.println(result);
  ...
  public void testOutjarDeletedOnError() {
    final String[] args = new String[]{"-aspectpath", aspectjarName, "-injars", injarName, "-outjar", outjarName};
    final Message error = new Message(WeaverMessages.format(WeaverMessages.CANT_FIND_TYPE_INTERFACES, "jar1.Parent"));
    final MessageSpec spec = new MessageSpec(null, newMessageList(error));
    final CompilationResult result = ajc(baseDir, args);
//		System.out.println(result);
    assertMessages(result, spec);
    final File outjar = new File(ajc.getSandboxDirectory(), outjarName);
