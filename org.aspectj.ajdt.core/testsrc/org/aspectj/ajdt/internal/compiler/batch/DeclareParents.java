...
public class DeclareParents extends AjcTestCase {
  ...
  public void testVerifyOrderOfProcessingIrrelevant1() {
    final File testBase = new File(baseDir, "TestA");
  ...
  public void test_interfaceMethodsImplemented() {
    final File testBase = new File(baseDir, "TestD");
  ...
  public void test_missingCtorAddedViaITD() {
    final File testBase = new File(baseDir, "TestE");
  ...
  public void runSourceAndBinaryTestcase(File testBase, String[] classes, String[] aspects, boolean expectErrors,
    ...
    final String[] sourceCompileCommandLine = new String[classes.length + aspects.length + 1];
    ...
    final String[] extraOption = new String[]{"-showWeaveInfo"};
    ...
    final List sourceWeaveMessages = getWeaveMessages(result);
    final int sourceWeaveMessagesCount = sourceWeaveMessages.size();
    final List sourceErrorMessages = result.getErrorMessages();
    final int sourceErrorMessagesCount = sourceErrorMessages.size();
    ...
    final List binaryWeaveMessages = getWeaveMessages(result);
    final int binaryWeaveMessagesCount = binaryWeaveMessages.size();
    final List binaryErrorMessages = result.getErrorMessages();
    final int binaryErrorMessagesCount = binaryErrorMessages.size();
    ...
    if (compareErrors) {
      for (final Iterator iter = binaryErrorMessages.iterator(); iter.hasNext(); ) {
        final IMessage binaryMessage = (IMessage) iter.next();
        IMessage correctSourceMessage = null;
        for (final Iterator iterator = sourceErrorMessages.iterator(); iterator.hasNext() && correctSourceMessage == null; ) {
          final IMessage sourceMessage = (IMessage) iterator.next();
      ...
      if (sourceErrorMessages.size() > 0) {
        for (final Iterator iter = sourceErrorMessages.iterator(); iter.hasNext(); ) {
          final IMessage srcMsg = (IMessage) iter.next();
    ...
    for (int i = 0; i < sourceWeaveMessages.size(); i++) {
      final IMessage m1 = (IMessage) sourceWeaveMessages.get(i);
      final IMessage m2 = (IMessage) binaryWeaveMessages.get(i);
      final String s1 = m1.getDetails();
      final String s2 = m2.getDetails();
  ...
  public static String[] mergeOptions(String[] input, String[] extras) {
    final String[] ret = new String[input.length + extras.length];
    System.arraycopy(input, 0, ret, 0, input.length);
    System.arraycopy(extras, 0, ret, input.length, extras.length);
    return ret;
  }

  private static List getWeaveMessages(CompilationResult result) {
    final List infoMessages = result.getInfoMessages();
    final List weaveMessages = new ArrayList();
    for (final Iterator iter = infoMessages.iterator(); iter.hasNext(); ) {
      final IMessage element = (IMessage) iter.next();
