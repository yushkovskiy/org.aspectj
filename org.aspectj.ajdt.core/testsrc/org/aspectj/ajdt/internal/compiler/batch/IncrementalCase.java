...
public class IncrementalCase { // XXX NOT bound to junit - bridge tests?
  ...
  public static void main(String[] args) throws IOException {
    final IncrementalCase me = new IncrementalCase();
...
//		boolean result;
  ...
  public boolean run(File srcBase, IMessageHandler handler)
    ...
    final File targetSrc = makeDir(targetBase, "src", handler);
    if (null == targetSrc) {
      return false;
    }
    final File targetClasses = makeDir(targetBase, "classes", handler);
...
//		final String canonicalFrom = srcBase.getCanonicalPath();
    ...
    final MessageHandler compilerMessages = new MessageHandler();
    final StringBuffer commandLine = new StringBuffer();
    for (int i = 1; result && (i < 10); i++) {
      final String fromSuffix = "." + i + "0.java";
   ...
   */
  protected static File getSandboxDir() throws IOException { // XXX util
    final File tempFile = File.createTempFile("IncrementalCase", ".txt");
...
//				System.getProperty("sun.boot.class.path"),
  ...
  static String normalizeFilename(String s) {        // XXX error-prone
  ...
  boolean haveAll(
    ...
    final BitSet foundSet = new BitSet(10);
    for (int i = 0; i < exp.length; i++) {
      final int expLine = exp[i];
      boolean found = false;
      for (int j = 0; !found && (j < messages.length); j++) {
        final ISourceLocation sl = messages[j].getSourceLocation();
   ...
   */
  static Definition[] getDefinitions(File srcBase) {
    final File file = new File(srcBase, Definition.PATH);
    ...
    final Definition[] result = new Definition[9];
...
//			Enumeration keys = props.keys();
    ...
    static String[] get(Properties props, String key) {
