...
public abstract class AjASTTestCase extends TestCase {
  ...
  protected static AjAST createAjAST(int astlevel) {
  ...
  protected static void checkJLS3(String source, ITypePatternTester tester) {
    final ASTParser parser = ASTParser.newParser(AST.JLS3);
    parser.setCompilerOptions(new HashMap());
    parser.setSource(source.toCharArray());
    final CompilationUnit cu2 = (CompilationUnit) parser.createAST(null);
    final AjASTVisitor visitor = tester.createVisitor();
    cu2.accept(visitor);
  ...
  /**
   * @param source               to parse and visit
   * @param expectedSourceRanges of TypePattern nodes encountered while visiting the AST
   */
  ...
  /**
   * @param source           to parse and visit
   * @param expectedCategory expected category of a TypeCategoryTypePattern node
  ...
  protected static List<SourceRange> getSourceRanges(int[][] sourceRanges) {
    final List<SourceRange> convertedRanges = new ArrayList<SourceRange>();
  ...
  class TypeCategoryTester implements ITypePatternTester {
    ...
    @Override
    public void testCondition(AjASTVisitor visitor) {
      final TypeCategoryTypeVisitor tcVisitor = (TypeCategoryTypeVisitor) visitor;
  ...
  class TypePatternSourceRangeTester implements ITypePatternTester {
    ...
    @Override
    public void testCondition(AjASTVisitor visitor) {
      final TypePatternSourceRangeVisitor sourceRangeVisitor = (TypePatternSourceRangeVisitor) visitor;

      final List<SourceRange> actualRanges = sourceRangeVisitor
          .getVisitedSourceRanges();
      final List<SourceRange> expectedRanges = getSourceRanges(expectedRawSourceRanges);
      ...
      for (int i = 0; i < actualRanges.size(); i++) {
        final SourceRange expected = expectedRanges.get(i);
        final SourceRange actual = actualRanges.get(i);
        assertTrue(
  ...
  class SourceRangeTester implements ITypePatternTester {
    ...
    public void testCondition(AjASTVisitor visitor) {
      ...
      final int s = ((SourceRangeVisitor) visitor).getStart();
      final int l = ((SourceRangeVisitor) visitor).getLength();
