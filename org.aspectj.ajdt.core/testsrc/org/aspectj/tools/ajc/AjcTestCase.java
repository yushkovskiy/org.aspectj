...
public class AjcTestCase extends TestCase {
  ...
  public static class Message {
    ...
    public Message(int line, String srcFile, String text, ISourceLocation[] seeAlso) {
      this.line = line;
      final StringBuilder srcFileName = new StringBuilder();
      if (srcFile != null) {
        final char[] chars = srcFile.toCharArray();
    ...
    public boolean matches(IMessage message) {
      ...
      if (seeAlsos != null) {
        final List extraLocations = message.getExtraSourceLocations();
    ...
    private static boolean hasAMatch(List srcLocations, ISourceLocation sLoc) {
      for (final Iterator iter = srcLocations.iterator(); iter.hasNext(); ) {
        final ISourceLocation thisLoc = (ISourceLocation) iter.next();
   ...
   */
  public void assertNoMessages(@Nullable CompilationResult result) {
   ...
   * Assert that no (non-informational) messages where produced during a compiler run.
   */
  public void assertNoMessages(@Nullable CompilationResult result, @NotNull String message) {
   ...
   */
  public void assertMessages(@Nullable CompilationResult result, @NotNull MessageSpec expected) {
   ...
   * Assert that messages in accordance with the <code>expected</code> message specification where produced during a compiler run.
   */
  public void assertMessages(@Nullable CompilationResult result, @NotNull String message, @NotNull MessageSpec expected) {
    if (result == null)
      fail("Attempt to compare null compilation results against expected.");
    final List missingFails = copyAll(expected.fails);
    final List missingInfos = copyAll(expected.infos);
    final List missingWarnings = copyAll(expected.warnings);
    final List missingErrors = copyAll(expected.errors);
    final List missingWeaves = copyAll(expected.weaves);
    final List<IMessage> extraFails = copyAll(result.getFailMessages());
    final List<IMessage> extraInfos = copyAll(result.getInfoMessages());
    final List<IMessage> extraWarnings = copyAll(result.getWarningMessages());
    final List<IMessage> extraErrors = copyAll(result.getErrorMessages());
    final List<IMessage> extraWeaves = copyAll(result.getWeaveMessages());
    ...
    final boolean infosEmpty = expected.isIgnoringInfoMessages() ? true : (missingInfos.isEmpty() && extraInfos.isEmpty());
    if (!(missingFails.isEmpty() && missingWarnings.isEmpty() && missingErrors.isEmpty() && missingWeaves.isEmpty()
      ...
      final StringBuffer failureReport = new StringBuffer(message);
      ...
      failureReport.append("\ncommand was: ajc");
      final String[] args = result.getArgs();
      for (int i = 0; i < args.length; i++) {
        failureReport.append(" ");
        failureReport.append(args[i]);
      }
      final String report = failureReport.toString();
   ...
   */
  @Nullable
  public CompilationResult ajc(@NotNull File baseDir, @NotNull String[] args) {
    try {
  ...
  public RunResult run(String className, String[] args, String vmargs, final String classpath, boolean useLTW, boolean useFullLTW) {
    ...
    final URLClassLoader sandboxLoader;
    final URLClassLoader testLoader = (URLClassLoader) getClass().getClassLoader();
    ...
    if (!useFullLTW && useLTW) {
       ...
       * Create a new AspectJ class loader using the existing test CLASSPATH and any missing Java 5 projects
			 */
      final URL[] testUrls = testLoader.getURLs();
      final URL[] java5Urls = getURLs(JAVA5_CLASSPATH_ENTRIES);
      final URL[] urls = new URL[testUrls.length + java5Urls.length];
      ...
      final ClassLoader aspectjLoader = new URLClassLoader(urls, parentLoader);
      final URL[] sandboxUrls = getURLs(cp.toString());
      ...
      final String absPath = directory.getAbsolutePath();
      final String javaagent = absPath + File.separator + ".." + File.separator + "aj-build" + File.separator + "dist" + File.separator + "tools" + File.separator + "lib" + File.separator + "aspectjweaver.jar";
      try {

        final String command = "java " + vmargs + " -classpath " + cp + " -javaagent:" + javaagent + " " + className;

        // Command is executed using ProcessBuilder to allow setting CWD for ajc sandbox compliance
        final ProcessBuilder pb = new ProcessBuilder(tokenizeCommand(command));
      ...
      final URL[] urls = getURLs(cp.toString());
    ...
    final StringBuilder command = new StringBuilder("java -classpath ");
...
//		}
    ...
    final ClassLoader contexClassLoader = Thread.currentThread().getContextClassLoader();
    try {
      try {
        final Class testerClass = sandboxLoader.loadClass("org.aspectj.testing.Tester");
        final Method setBaseDir = testerClass.getDeclaredMethod("setBASEDIR", new Class[]{File.class});
      ...
      final Class toRun = sandboxLoader.loadClass(className);
      final Method mainMethod = toRun.getMethod("main", new Class[]{String[].class});
...
//			}
  ...
  private static List<String> tokenizeCommand(String command) {
    final StringTokenizer st = new StringTokenizer(command, " ", false);
  ...
  private static RunResult createResultFromBufferReaders(String command,
                                                         BufferedReader stdInput, BufferedReader stdError) throws IOException {
    String line = "";
    final ByteArrayOutputStream baosOut = new ByteArrayOutputStream();
    final ByteArrayOutputStream baosErr = new ByteArrayOutputStream();

    final PrintWriter stdOutWriter = new PrintWriter(baosOut);
    final PrintWriter stdErrWriter = new PrintWriter(baosErr);
...
//	}
   ...
   */
  private static URLClassLoader createWeavingClassLoader(URL[] urls, ClassLoader parent) {
    URLClassLoader loader = null;

    try {
      final Class loaderClazz = Class.forName("org.aspectj.weaver.loadtime.WeavingURLClassLoader", false, parent);
      final Class[] parameterTypes = new Class[]{urls.getClass(), ClassLoader.class};
      final Object[] parameters = new Object[]{urls, parent};
      final Constructor constructor = loaderClazz.getConstructor(parameterTypes);
  ...
  private static URL[] getURLs(String classpath) {
    final StringTokenizer strTok = new StringTokenizer(classpath, File.pathSeparator);
    final URL[] urls = new URL[strTok.countTokens()];
    try {
  ...
  private String substituteSandbox(String classpath) {
    ...
    while (classpath.indexOf("$sandbox") != -1) {
      final int pos = classpath.indexOf("$sandbox");
      final String firstbit = classpath.substring(0, pos);
      final String endbit = classpath.substring(pos + 8);
  ...
  @NotNull
  private static String adaptToPlatform(@NotNull String s) {
    final String ret = s.replace(';', File.pathSeparatorChar);
    // ret = ret.replace(':',File.pathSeparatorChar);
    return ret;
  }

  private static List copyAll(List in) {
    if (in == Collections.EMPTY_LIST)
      return in;

    final List out = new ArrayList();
    for (final Iterator iter = in.iterator(); iter.hasNext(); ) {
      out.add(iter.next());
    }
    return out;
  }
   ...
   */
  private static void compare(List expected, List actual, List missingElements, List extraElements) {
    for (final Iterator expectedIter = expected.iterator(); expectedIter.hasNext(); ) {
      final Message expectedMessage = (Message) expectedIter.next();
      for (final Iterator actualIter = actual.iterator(); actualIter.hasNext(); ) {
        final IMessage actualMessage = (IMessage) actualIter.next();
  ...
  private static void addMissing(StringBuffer buff, String type, List messages) {
    if (!messages.isEmpty()) {
  ...
  private static void addExtra(StringBuffer buff, String type, List messages) {
    if (!messages.isEmpty()) {
  ...
  private static void getAnyJars(File dir, StringBuffer buff) {
