...
public class StandardPointcutExpressionImpl implements StandardPointcutExpression {
  ...
  private ShadowMatch matchesExecution(ResolvedMember aMember) {
    final Shadow s = StandardShadow.makeExecutionShadow(world, aMember, this.matchContext);
    final StandardShadowMatchImpl sm = getShadowMatch(s);
    sm.setSubject(aMember);
    sm.setWithinCode(null);
  ...
  public ShadowMatch matchesStaticInitialization(ResolvedType aType) {
    final Shadow s = StandardShadow.makeStaticInitializationShadow(world, aType, this.matchContext);
    final StandardShadowMatchImpl sm = getShadowMatch(s);
    sm.setSubject(null);
    sm.setWithinCode(null);
  ...
  public ShadowMatch matchesMethodCall(ResolvedMember aMethod, ResolvedMember withinCode) {
    final Shadow s = StandardShadow.makeCallShadow(world, aMethod, withinCode, this.matchContext);
    final StandardShadowMatchImpl sm = getShadowMatch(s);
    sm.setSubject(aMethod);
  ...
  private StandardShadowMatchImpl getShadowMatch(Shadow forShadow) {
    ...
    final StandardShadowMatchImpl sm = new StandardShadowMatchImpl(match, residueTest, state, parameters);
