...
public class JoinPointSignatureIterator implements Iterator<JoinPointSignature> {
  ...
  private final Member signaturesOfMember;
  private ResolvedMember firstDefiningMember;
  private final World world;
  private final List<JoinPointSignature> discoveredSignatures = new ArrayList<JoinPointSignature>();
  ...
  private final Set<ResolvedType> visitedSuperTypes = new HashSet<ResolvedType>();
  ...
  private void addSignaturesUpToFirstDefiningMember() {
    final ResolvedType originalDeclaringType = signaturesOfMember.getDeclaringType().resolve(world);
    final ResolvedType superType = originalDeclaringType.getSuperclass();
    ...
    if (world.isJoinpointArrayConstructionEnabled() && originalDeclaringType.isArray()) {
      final Member m = signaturesOfMember;
      final ResolvedMember rm = new ResolvedMemberImpl(m.getKind(), m.getDeclaringType(), m.getModifiers(), m.getReturnType(), m
  ...
  private boolean findSignaturesFromSupertypes() {
    ...
    if (superTypeIterator.hasNext()) {
      ...
      if (visitedSuperTypes.contains(superType)) {
        ...
        final ResolvedMemberImpl foundMember = (ResolvedMemberImpl) superType.lookupResolvedMember(firstDefiningMember, true,
            isProxy);
        if (foundMember != null && isVisibleTo(firstDefiningMember, foundMember)) {
          ...
          final ResolvedType resolvedDeclaringType = foundMember.getDeclaringType().resolve(world);
          ...
          if (!isProxy && superType.isParameterizedType() && (foundMember.backingGenericMember != null)) {
            final JoinPointSignature member = new JoinPointSignature(foundMember.backingGenericMember,
    ...
    if (yetToBeProcessedSuperMembers != null && !yetToBeProcessedSuperMembers.isEmpty()) {
      final SearchPair nextUp = yetToBeProcessedSuperMembers.remove(0);
  ...
  private void warnOnMissingType(ResolvedType missing) {
    if (missing instanceof MissingResolvedTypeWithKnownSignature) {
      // which it should be...
      final MissingResolvedTypeWithKnownSignature mrt = (MissingResolvedTypeWithKnownSignature) missing;
