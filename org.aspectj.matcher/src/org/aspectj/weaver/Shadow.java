...
public abstract class Shadow {
  ...
  public ResolvedType[] getArgumentTypesForArrayConstructionShadow() {
    final String s = signature.getDeclaringType().getSignature();
    int pos = s.indexOf("[");
    int dims = 1;
    while (pos < s.length()) {
      pos++;
    ...
    final ResolvedType intType = UnresolvedType.INT.resolve(this.getIWorld());
    if (dims == 1) {
      return new ResolvedType[]{intType};
    }
    final ResolvedType[] someInts = new ResolvedType[dims];
  ...
  public static final class Kind extends TypeSafeEnum {
    ...
    public String getSimpleName() {
      final int dash = getName().lastIndexOf('-');
      if (dash == -1) {
        return getName();
      } else {
  ...
  protected boolean checkMunger(ShadowMunger munger) {
    if (munger.mustCheckExceptions()) {
      for (final Iterator<ResolvedType> i = munger.getThrownExceptions().iterator(); i.hasNext(); ) {
  ...
  private boolean isDeclaredException(ResolvedType resolvedTypeX, Member member) {
    final ResolvedType[] excs = getIWorld().resolve(member.getExceptions(getIWorld()));
  ...
  private void sortMungers() {
    ...
    final List sorted = PartialOrder.sort(mungers);
  ...
  private void possiblyReportUnorderedAdvice(List sorted) {
    if (sorted != null && getIWorld().getLint().unorderedAdviceAtShadow.isEnabled() && mungers.size() > 1) {
      ...
      final Set<String> clashingAspects = new HashSet<String>();
      final int max = mungers.size();

      // Compare every pair of advice mungers
      for (int i = max - 1; i >= 0; i--) {
        for (int j = 0; j < i; j++) {
          final Object a = mungers.get(i);
          final Object b = mungers.get(j);

          // Make sure they are the right type
          if (a instanceof Advice && b instanceof Advice) {
            final Advice adviceA = (Advice) a;
            final Advice adviceB = (Advice) b;
            if (!adviceA.concreteAspect.equals(adviceB.concreteAspect)) {
              final AdviceKind adviceKindA = adviceA.getKind();
              final AdviceKind adviceKindB = adviceB.getKind();
              ...
              if (adviceKindA.getKey() < (byte) 6 && adviceKindB.getKey() < (byte) 6
                ...
                final Integer order = getIWorld().getPrecedenceIfAny(adviceA.concreteAspect, adviceB.concreteAspect);

                if (order != null && order.equals(new Integer(0))) {
                  final String key = adviceA.getDeclaringAspect() + ":" + adviceB.getDeclaringAspect();
                  final String possibleExistingKey = adviceB.getDeclaringAspect() + ":" + adviceA.getDeclaringAspect();
      ...
      for (final Iterator<String> iter = clashingAspects.iterator(); iter.hasNext(); ) {
        final String element = iter.next();
        final String aspect1 = element.substring(0, element.indexOf(":"));
        final String aspect2 = element.substring(element.indexOf(":") + 1);
  ...
  public String toResolvedString(World world) {
    ...
    sb.append(getKind());
    sb.append("(");
    final Member m = getSignature();
    if (m == null) {
      sb.append("<<missing signature>>");
    } else {
      final ResolvedMember rm = world.resolve(m);
      if (rm == null) {
        sb.append("<<unresolvableMember:").append(m).append(">>");
      } else {
        final String genString = rm.toGenericString();
        if (genString == null) {
  ...
  public static Set<Kind> toSet(int i) {
    final Set<Kind> results = new HashSet<Kind>();
    for (int j = 0; j < Shadow.SHADOW_KINDS.length; j++) {
      final Kind k = Shadow.SHADOW_KINDS[j];
      if (k.isSet(i)) {
        results.add(k);
      }
    }
