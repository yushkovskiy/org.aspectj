...
public class UnresolvedType implements Traceable, TypeVariableDeclaringElement {
  ...
  public static UnresolvedType[] forNames(String[] names) {
    final UnresolvedType[] ret = new UnresolvedType[names.length];
  ...
  public static UnresolvedType forGenericType(String name, TypeVariable[] tvbs, String genericSig) {
    ...
    final String sig = nameToSignature(name);
    final UnresolvedType ret = UnresolvedType.forSignature(sig);
  ...
  public static UnresolvedType forGenericTypeSignature(String sig, String declaredGenericSig) {
    ...
    final ClassSignature csig = new GenericSignatureParser().parseAsClassSignature(declaredGenericSig);

    final GenericSignature.FormalTypeParameter[] ftps = csig.formalTypeParameters;
    ret.typeVariables = new TypeVariable[ftps.length];
    for (int i = 0; i < ftps.length; i++) {
      final GenericSignature.FormalTypeParameter parameter = ftps[i];
      if (parameter.classBound instanceof GenericSignature.ClassTypeSignature) {
        final GenericSignature.ClassTypeSignature cts = (GenericSignature.ClassTypeSignature) parameter.classBound;
        ...
        final GenericSignature.TypeVariableSignature tvs = (GenericSignature.TypeVariableSignature) parameter.classBound;
        final UnresolvedTypeVariableReferenceType utvrt = new UnresolvedTypeVariableReferenceType(new TypeVariable(
  ...
  public static UnresolvedType forRawTypeName(String name) {
    final UnresolvedType ret = UnresolvedType.forName(name);
    ret.typeKind = TypeKind.RAW;
    return ret;
  }

  public static UnresolvedType forPrimitiveType(String signature) {
    final UnresolvedType ret = new UnresolvedType(signature);
  ...
  public static UnresolvedType[] add(UnresolvedType[] types, UnresolvedType end) {
    ...
    final UnresolvedType[] ret = new UnresolvedType[len + 1];
  ...
  public static UnresolvedType[] forSignatures(String[] sigs) {
    final UnresolvedType[] ret = new UnresolvedType[sigs.length];
  ...
  public UnresolvedType getOutermostType() {
    ...
    final String sig = getErasureSignature();
    final int dollar = sig.indexOf('$');
    if (dollar != -1) {
   ...
   */
  public ResolvedType resolve(@NotNull World world) {
  ...
  private static String signatureToName(String signature) {
    switch (signature.charAt(0)) {
      ...
      case 'J':
        return "long";
      case 'L':
        final String name = signature.substring(1, signature.length() - 1).replace('/', '.');
        return name;
      case 'T':
        final StringBuffer nameBuff2 = new StringBuffer();
        final int colon = signature.indexOf(";");
        final String tvarName = signature.substring(1, colon);
        nameBuff2.append(tvarName);
        return nameBuff2.toString();
      case 'P': // it's one of our parameterized type sigs
        final StringBuffer nameBuff = new StringBuffer();
...
//					throw new IllegalStateException("Should only happen inside <...>");
  ...
  public TypeVariable getTypeVariableNamed(String name) {
    final TypeVariable[] vars = getTypeVariables();
    if (vars == null || vars.length == 0) {
      return null;
    }
    for (int i = 0; i < vars.length; i++) {
      final TypeVariable aVar = vars[i];
  ...
  public static UnresolvedType[] readArray(DataInputStream s) throws IOException {
    ...
    final UnresolvedType[] types = new UnresolvedType[len];
    for (int i = 0; i < len; i++) {
