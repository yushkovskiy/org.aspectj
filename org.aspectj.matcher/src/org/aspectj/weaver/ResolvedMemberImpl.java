...
public class ResolvedMemberImpl extends MemberImpl implements IHasPosition, ResolvedMember {
  ...
  public static JoinPointSignature[] getJoinPointSignatures(Member joinPointSignature, World inAWorld) {
    ...
    // including the
    // first defining type
    final ResolvedType originalDeclaringType = joinPointSignature.getDeclaringType().resolve(inAWorld);
    final ResolvedMemberImpl firstDefiningMember = (ResolvedMemberImpl) joinPointSignature.resolve(inAWorld);
    ...
    final ResolvedType firstDefiningType = firstDefiningMember.getDeclaringType().resolve(inAWorld);
    ...
    final Set<ResolvedMember> memberSignatures = new HashSet<ResolvedMember>();
    ...
    if (shouldWalkUpHierarchyFor(firstDefiningMember)) {
      ...
      // member.
      final Iterator<ResolvedType> superTypeIterator = firstDefiningType.getDirectSupertypes();
      final List<ResolvedType> typesAlreadyVisited = new ArrayList<ResolvedType>();
      accumulateMembersMatching(firstDefiningMember, superTypeIterator, typesAlreadyVisited, memberSignatures, false);
    }

    final JoinPointSignature[] ret = new JoinPointSignature[memberSignatures.size()];
  ...
  public void write(CompressingDataOutputStream s) throws IOException {
    ...
    final String gsig = getGenericSignature();
    ...
    if (getSignature().equals(gsig)) {
      ...
      for (int i = 0; i < parameterTypes.length; i++) {
        if (s.canCompress()) {
          ...
          final UnresolvedType array_element = parameterTypes[i];
  ...
  public String getSignatureForAttribute() {
    ...
    sb.append("(");
    for (int i = 0; i < parameterTypes.length; i++) {
      final ResolvedType ptype = (ResolvedType) parameterTypes[i];
  ...
  public String getGenericSignature() {
    ...
    sb.append("(");
    for (int i = 0; i < parameterTypes.length; i++) {
      final UnresolvedType ptype = parameterTypes[i];
  ...
  public boolean isVisible(ResolvedType fromType) {
    final UnresolvedType declaringType = getDeclaringType();
    ResolvedType type = null;
    if (fromType.equals(declaringType)) {
      type = fromType;
    } else {
      final World world = fromType.getWorld();
  ...
  public ResolvedMemberImpl parameterizedWith(UnresolvedType[] typeParameters, ResolvedType newDeclaringType,
    ...
    final TypeVariable[] typeVariables = getDeclaringType().getTypeVariables();
    if (isParameterized && (typeVariables.length != typeParameters.length)) {
      throw new IllegalStateException("Wrong number of type parameters supplied");
    }
    final Map<String, UnresolvedType> typeMap = new HashMap<String, UnresolvedType>();
    ...
    final UnresolvedType[] genericParameterTypes = getGenericParameterTypes();
    for (int i = 0; i < parameterizedParameterTypes.length; i++) {
      parameterizedParameterTypes[i] = parameterize(genericParameterTypes[i], typeMap, isParameterized,
          newDeclaringType.getWorld());
    }
    final ResolvedMemberImpl ret = new ResolvedMemberImpl(getKind(), newDeclaringType, getModifiers(), parameterizedReturnType,
  ...
  public ResolvedMember parameterizedWith(Map<String, UnresolvedType> m, World w) {
    ...
    final UnresolvedType parameterizedReturnType = parameterize(getGenericReturnType(), m, true, w);
    final UnresolvedType[] parameterizedParameterTypes = new UnresolvedType[getGenericParameterTypes().length];
    final UnresolvedType[] genericParameterTypes = getGenericParameterTypes();
    for (int i = 0; i < parameterizedParameterTypes.length; i++) {
      parameterizedParameterTypes[i] = parameterize(genericParameterTypes[i], m, true, w);
    }
    final ResolvedMemberImpl ret = new ResolvedMemberImpl(getKind(), declaringType, getModifiers(), parameterizedReturnType,
        getName(), parameterizedParameterTypes, getExceptions(), this);
    ret.setTypeVariables(getTypeVariables());
    ret.setSourceContext(getSourceContext());
    ret.setPosition(getStart(), getEnd());
  ...
  public boolean matches(ResolvedMember aCandidateMatch, boolean ignoreGenerics) {
    final ResolvedMemberImpl candidateMatchImpl = (ResolvedMemberImpl) aCandidateMatch;
    if (!getName().equals(aCandidateMatch.getName())) {
      return false;
    }
    final UnresolvedType[] parameterTypes = getGenericParameterTypes();
    final UnresolvedType[] candidateParameterTypes = aCandidateMatch.getGenericParameterTypes();
  ...
  public String toDebugString() {
    ...
    final String modsStr = Modifier.toString(mods);
    if (modsStr.length() != 0) {
    ...
    if (typeVariables != null && typeVariables.length > 0) {
      ...
      for (int i = 0; i < typeVariables.length; i++) {
        ...
        final TypeVariable t = typeVariables[i];
    ...
    if (kind != FIELD) {
      r.append("(");
      final UnresolvedType[] params = getGenericParameterTypes();
      final boolean parameterNamesExist = showParameterNames && parameterNames != null && parameterNames.length == params.length;
      if (params.length != 0) {
