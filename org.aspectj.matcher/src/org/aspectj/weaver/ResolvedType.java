...
public abstract class ResolvedType extends UnresolvedType implements AnnotatedElement {
  ...
  private static final int MungersAnalyzed = 0x0004;
  private static final int HasParentMunger = 0x0008;
  private static final int TypeHierarchyCompleteBit = 0x0010;
  ...
  public Iterator<ResolvedType> getHierarchy(final boolean wantGenerics, final boolean wantDeclaredParents) {
    ...
    final Iterators.Getter<ResolvedType, ResolvedType> interfaceGetter = new Iterators.Getter<ResolvedType, ResolvedType>() {
      ...
      public Iterator<ResolvedType> get(ResolvedType type) {
        ...
        if (!wantDeclaredParents && type.hasNewParentMungers()) {
          ...
          final List<Integer> forRemoval = new ArrayList<Integer>();
          for (ConcreteTypeMunger munger : type.interTypeMungers) {
            if (munger.getMunger() != null) {
              final ResolvedTypeMunger m = munger.getMunger();
          ...
          if (forRemoval.size() > 0) {
            final ResolvedType[] interfaces2 = new ResolvedType[interfaces.length - forRemoval.size()];
            int p = 0;
            for (int ii = 0; ii < interfaces.length; ii++) {
    ...
    if (this.isInterface()) {
      ...
      final SuperInterfaceWalker superInterfaceWalker = new SuperInterfaceWalker(interfaceGetter);
      final Iterator<ResolvedType> superClassesIterator = new SuperClassWalker(this, superInterfaceWalker, wantGenerics);
  ...
  public List<ResolvedMember> getMethodsWithoutIterator(boolean includeITDs, boolean allowMissing, boolean genericsAware) {
    final List<ResolvedMember> methods = new ArrayList<ResolvedMember>();
    final Set<String> knowninterfaces = new HashSet<String>();
  ...
  public List<ResolvedType> getHierarchyWithoutIterator(boolean includeITDs, boolean allowMissing, boolean genericsAware) {
    final List<ResolvedType> types = new ArrayList<ResolvedType>();
  ...
  public ResolvedMember lookupField(Member field) {
    ...
    while (i.hasNext()) {
      final ResolvedMember resolvedMember = i.next();
  ...
  public ResolvedMember lookupMethod(Member m) {
    final List<ResolvedType> typesTolookat = new ArrayList<ResolvedType>();
    typesTolookat.add(this);
    int pos = 0;
    while (pos < typesTolookat.size()) {
      final ResolvedType type = typesTolookat.get(pos++);
      if (!type.isMissing()) {
        final ResolvedMember[] methods = type.getDeclaredMethods();
        if (methods != null) {
          for (int i = 0; i < methods.length; i++) {
            final ResolvedMember method = methods[i];
            if (matches(method, m)) {
              return method;
            }
      ...
      final ResolvedType superclass = type.getSuperclass();
      if (superclass != null) {
        typesTolookat.add(superclass);
      }
      // Queue any interfaces not already checked:
      final ResolvedType[] superinterfaces = type.getDeclaredInterfaces();
      if (superinterfaces != null) {
        for (int i = 0; i < superinterfaces.length; i++) {
          final ResolvedType interf = superinterfaces[i];
  ...
  public ResolvedPointcutDefinition findPointcut(String name) {
    for (final Iterator<ResolvedMember> i = getPointcuts(); i.hasNext(); ) {
      final ResolvedPointcutDefinition f = (ResolvedPointcutDefinition) i.next();
    ...
    if (!getOutermostType().equals(this)) {
      final ResolvedType outerType = getOutermostType().resolve(world);
      final ResolvedPointcutDefinition rpd = outerType.findPointcut(name);
      return rpd;
    }
  ...
  public final List<Declare> collectDeclares(boolean includeAdviceLike) {
    ...
    if (!this.isAbstract()) {
      ...
      while (typeIterator.hasNext()) {
        ...
        for (final Iterator<Declare> i = ty.getDeclares().iterator(); i.hasNext(); ) {
          final Declare dec = i.next();
          if (dec.isAdviceLike()) {
  ...
  public List<ShadowMunger> getDeclaredAdvice() {
    ...
    final Map<String, UnresolvedType> typeVariableMap = getAjMemberParameterizationMap();
      ...
      if (munger != null) {
        if (ajMembersNeedParameterization()) {
          ...
          if (munger instanceof Advice) {
            ...
            final UnresolvedType[] ptypes = methods[i].getGenericParameterTypes();
            final UnresolvedType[] newPTypes = new UnresolvedType[ptypes.length];
            for (int j = 0; j < ptypes.length; j++) {
              if (ptypes[j] instanceof TypeVariableReferenceType) {
                final TypeVariableReferenceType tvrt = (TypeVariableReferenceType) ptypes[j];
  ...
  public ResolvedMember lookupMemberNoSupers(Member member) {
    final ResolvedMember ret = lookupDirectlyDeclaredMemberNoSupers(member);
  ...
  public ResolvedType discoverActualOccurrenceOfTypeInHierarchy(ResolvedType lookingFor) {
    ...
    final ResolvedType superT = getSuperclass();
    if (superT.genericTypeEquals(lookingFor)) {
      return superT;
    }

    final ResolvedType[] superIs = getDeclaredInterfaces();
    for (int i = 0; i < superIs.length; i++) {
      final ResolvedType superI = superIs[i];
      if (superI.genericTypeEquals(lookingFor)) {
        return superI;
      }
      final ResolvedType checkTheSuperI = superI.discoverActualOccurrenceOfTypeInHierarchy(lookingFor);
  ...
  public ConcreteTypeMunger fillInAnyTypeParameters(ConcreteTypeMunger munger) {
    ...
    if (munger.isTargetTypeParameterized()) {
      ...
      final ResolvedType onTypeResolved = world.resolve(member.getDeclaringType());
      final ResolvedType onType = onTypeResolved.getGenericType();
      if (onType == null) {
      ...
      final ResolvedType actualTarget = discoverActualOccurrenceOfTypeInHierarchy(onType);
  ...
  public void addInterTypeMunger(ConcreteTypeMunger munger, boolean isDuringCompilation) {
    ...
    if (sig.getKind() == Member.METHOD) {
      ...
      if (!isDuringCompilation) {
        final ResolvedTypeMunger thisRealMunger = munger.getMunger();
        if (thisRealMunger instanceof NewFieldTypeMunger) {
          final NewFieldTypeMunger newFieldTypeMunger = (NewFieldTypeMunger) thisRealMunger;
          if (newFieldTypeMunger.version == NewFieldTypeMunger.VersionTwo) {
            final String thisRealMungerSignatureName = newFieldTypeMunger.getSignature().getName();
            for (ConcreteTypeMunger typeMunger : interTypeMungers) {
              if (typeMunger.getMunger() instanceof NewFieldTypeMunger) {
                if (typeMunger.getSignature().getKind() == Member.FIELD) {
                  final NewFieldTypeMunger existing = (NewFieldTypeMunger) typeMunger.getMunger();
    ...
    for (final Iterator<ConcreteTypeMunger> i = interTypeMungers.iterator(); i.hasNext(); ) {
      final ConcreteTypeMunger existingMunger = i.next();
      final boolean v2itds = munger.getSignature().getKind() == Member.FIELD && (munger.getMunger() instanceof NewFieldTypeMunger) && ((NewFieldTypeMunger) munger.getMunger()).version == NewFieldTypeMunger.VersionTwo;
...
//						i.remove();
  ...
  private boolean clashesWithExistingMember(ConcreteTypeMunger typeTransformer, Iterator<ResolvedMember> existingMembers) {
    final ResolvedMember typeTransformerSignature = typeTransformer.getSignature();
    ...
    // filled in
    // }
    final ResolvedTypeMunger rtm = typeTransformer.getMunger();
    boolean v2itds = true;
    ...
    while (existingMembers.hasNext()) {
      ...
      if (conflictingSignature(existingMember, typeTransformerSignature, v2itds)) {
        ...
        if (isVisible(existingMember.getModifiers(), this, typeTransformer.getAspectType())) {
          final int c = compareMemberPrecedence(typeTransformerSignature, existingMember);
          // System.err.println("   c: " + c);
          if (c < 0) {
            final ResolvedType typeTransformerTargetType = typeTransformerSignature.getDeclaringType().resolve(world);
            if (typeTransformerTargetType.isInterface()) {
              final ResolvedType existingMemberType = existingMember.getDeclaringType().resolve(world);
            ...
            final boolean sameReturnTypes = (existingMember.getReturnType().equals(typeTransformerSignature.getReturnType()));
            if (sameReturnTypes) {
              ...
              final ResolvedType declaringRt = existingMember.getDeclaringType().resolve(world);
              final WeaverStateInfo wsi = declaringRt.getWeaverState();
              if (wsi != null) {
                final List<ConcreteTypeMunger> mungersAffectingThisType = wsi.getTypeMungers(declaringRt);
                if (mungersAffectingThisType != null) {
                  for (final Iterator<ConcreteTypeMunger> iterator = mungersAffectingThisType.iterator(); iterator
              ...
              if (!isDuplicateOfPreviousITD) {
                ...
                if (!(typeTransformerSignature.getName().equals("<init>") && existingMember.isDefaultConstructor())) {
                  final String aspectName = typeTransformer.getAspectType().getName();
                  final ISourceLocation typeTransformerLocation = typeTransformer.getSourceLocation();
                  final ISourceLocation existingMemberLocation = existingMember.getSourceLocation();
                  final String msg = WeaverMessages.format(WeaverMessages.ITD_MEMBER_CONFLICT, aspectName,
                      existingMember);
  ...
  private boolean isDuplicateMemberWithinTargetType(ResolvedMember existingMember, ResolvedType targetType,
    ...
    final UnresolvedType declaringType = existingMember.getDeclaringType();
  ...
  private int compareMemberPrecedence(ResolvedMember m1, ResolvedMember m2) {
    ...
    if (Modifier.isProtected(m2.getModifiers()) && m2.getName().charAt(0) == 'c') {
      final UnresolvedType declaring = m2.getDeclaringType();
      if (declaring != null) {
    ...
    final ResolvedType t1 = m1.getDeclaringType().resolve(world);
    final ResolvedType t2 = m2.getDeclaringType().resolve(world);
  ...
  static class SuperClassWalker implements Iterator<ResolvedType> {
    ...
    private final SuperInterfaceWalker iwalker;
  ...
  static class SuperInterfaceWalker implements Iterator<ResolvedType> {
    ...
    private final Getter<ResolvedType, ResolvedType> ifaceGetter;
    ...
    public boolean hasNext() {
      if (delegate == null || !delegate.hasNext()) {
        ...
        do {
          final ResolvedType next = toPersue.remove();
          visited.add(next);
    ...
    @Override
    public ResolvedType next() {
      final ResolvedType next = delegate.next();
