...
public abstract class Advice extends ShadowMunger {
  ...
  private final boolean isAnnotationStyle;
  ...
  public static Advice makeCflowEntry(World world, Pointcut entry, boolean isBelow, Member stackField, int nFreeVars,
    ...
    final Advice ret = world.createAdviceMunger(isBelow ? AdviceKind.CflowBelowEntry : AdviceKind.CflowEntry, entry, stackField, 0,
        entry, inAspect);
  ...
  public static Advice makePerCflowEntry(World world, Pointcut entry, boolean isBelow, Member stackField, ResolvedType inAspect,
    ...
    final Advice ret = world.createAdviceMunger(isBelow ? AdviceKind.PerCflowBelowEntry : AdviceKind.PerCflowEntry, entry,
  ...
  public static Advice makePerObjectEntry(World world, Pointcut entry, boolean isThis, ResolvedType inAspect) {
    final Advice ret = world.createAdviceMunger(isThis ? AdviceKind.PerThisEntry : AdviceKind.PerTargetEntry, entry, null, 0, entry,
        inAspect);
  ...
  public static Advice makePerTypeWithinEntry(World world, Pointcut p, ResolvedType inAspect) {
    final Advice ret = world.createAdviceMunger(AdviceKind.PerTypeWithinEntry, p, null, 0, p, inAspect);
    ret.concreteAspect = inAspect;
    return ret;
  }
  ...
  public boolean match(Shadow shadow, World world) {
    if (super.match(shadow, world)) {
      ...
      if (hasExtraParameter() && kind == AdviceKind.AfterReturning) {
        final ResolvedType resolvedExtraParameterType = getExtraParameterType().resolve(world);
        final ResolvedType shadowReturnType = shadow.getReturnType().resolve(world);
        final boolean matches = (resolvedExtraParameterType.isConvertableFrom(shadowReturnType) && shadow.getKind()
            .hasReturnValue());
        ...
        final ResolvedType exceptionType = getExtraParameterType().resolve(world);
        if (!exceptionType.isCheckedException() || exceptionType.getName().equals("java.lang.Exception")) { // pr292239
          return true;
        }
        final UnresolvedType[] shadowThrows = shadow.getSignature().getExceptions(world);
        boolean matches = false;
        for (int i = 0; i < shadowThrows.length && !matches; i++) {
          final ResolvedType type = shadowThrows[i].resolve(world);
        ...
        if (shadow.getKind() == Shadow.PreInitialization) {
          ...
          if (getSignature().getReturnType().equals(UnresolvedType.VOID)) {
            if (!shadow.getReturnType().equals(UnresolvedType.VOID)) {
              final String s = shadow.toString();
              final String s2 = WeaverMessages.format(WeaverMessages.NON_VOID_RETURN, s);
            ...
            final ResolvedType shadowReturnType = shadow.getReturnType().resolve(world);
            final ResolvedType adviceReturnType = getSignature().getGenericReturnType().resolve(world);
            ...
            if (shadowReturnType.isParameterizedType() && adviceReturnType.isRawType()) { // Set
              ...
              // and
              // Set
              final ResolvedType shadowReturnGenericType = shadowReturnType.getGenericType(); // Set
              final ResolvedType adviceReturnGenericType = adviceReturnType.getGenericType(); // Set
  ...
  public String[] getBaseParameterNames(World world) {
    final String[] allNames = getSignature().getParameterNames(world);
    final int extras = getExtraParameterCount();
    if (extras == 0) {
      return allNames;
    }
    final String[] result = new String[getBaseParameterCount()];
    ...
    if (signature instanceof ResolvedMember) {
      ...
      if (getConcreteAspect().isAnnotationStyleAspect()) {
        ...
        if (pnames != null) {
          ...
          final AnnotationAJ[] annos = getSignature().getAnnotations();
          ...
          if (annos != null && (getKind() == AdviceKind.AfterThrowing || getKind() == AdviceKind.AfterReturning)) {
            for (int i = 0; i < annos.length && parameterToLookup == null; i++) {
              final AnnotationAJ anno = annos[i];
              final String annosig = anno.getType().getSignature();
  ...
  public ShadowMunger concretize(ResolvedType fromType, World world, PerClause clause) {
    ...
    final Advice munger = world.getWeavingSupport().createAdviceMunger(attribute, p, signature, fromType);
