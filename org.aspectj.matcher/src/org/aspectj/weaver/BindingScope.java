...
public class BindingScope extends SimpleScope {
  ...
  public UnresolvedType lookupType(String name, IHasPosition location) {
    ...
    if (enclosingType != null && !importsUpdated) {
      ...
      final String pkgName = enclosingType.getPackageName();
      if (pkgName != null && !pkgName.equals("")) {
        final String[] existingImports = getImportedPrefixes();
        final String pkgNameWithDot = pkgName.concat(".");
        boolean found = false;
        ...
        if (!found) {
          final String[] newImports = new String[existingImports.length + 1];
