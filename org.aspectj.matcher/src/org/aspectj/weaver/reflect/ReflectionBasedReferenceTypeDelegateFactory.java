...
public class ReflectionBasedReferenceTypeDelegateFactory {
  ...
  public static ReflectionBasedReferenceTypeDelegate createDelegate(ReferenceType forReferenceType, World inWorld,
    ...
    try {
      final Class c = Class.forName(forReferenceType.getName(), false, usingClassLoader);
      if (LangUtil.is15VMOrGreater()) {
        final ReflectionBasedReferenceTypeDelegate rbrtd = create15Delegate(forReferenceType, c, usingClassLoader, inWorld);
        if (rbrtd != null) {
  ...
  private static ReflectionBasedReferenceTypeDelegate create15Delegate(ReferenceType forReferenceType, Class forClass,
    ...
    try {
      final Class delegateClass = Class.forName("org.aspectj.weaver.reflect.Java15ReflectionBasedReferenceTypeDelegate");
      final ReflectionBasedReferenceTypeDelegate ret = (ReflectionBasedReferenceTypeDelegate) delegateClass.newInstance();
  ...
  private static GenericSignatureInformationProvider createGenericSignatureProvider(World inWorld) {
    if (LangUtil.is15VMOrGreater()) {
      try {
        final Class providerClass = Class.forName("org.aspectj.weaver.reflect.Java15GenericSignatureInformationProvider");
        final Constructor cons = providerClass.getConstructor(new Class[]{World.class});
        final GenericSignatureInformationProvider ret = (GenericSignatureInformationProvider) cons
  ...
  public static ResolvedMember createResolvedMethod(Method aMethod, World inWorld) {
    final ReflectionBasedResolvedMemberImpl ret = new ReflectionBasedResolvedMemberImpl(org.aspectj.weaver.Member.METHOD,
  ...
  public static ResolvedMember createResolvedAdviceMember(Method aMethod, World inWorld) {
    final ReflectionBasedResolvedMemberImpl ret = new ReflectionBasedResolvedMemberImpl(org.aspectj.weaver.Member.ADVICE,
        toResolvedType(aMethod.getDeclaringClass(), (IReflectionWorld) inWorld), aMethod.getModifiers(), toResolvedType(
        aMethod.getReturnType(), (IReflectionWorld) inWorld), aMethod.getName(), toResolvedTypeArray(
        aMethod.getParameterTypes(), inWorld), toResolvedTypeArray(aMethod.getExceptionTypes(), inWorld), aMethod);
    if (inWorld instanceof IReflectionWorld) {
  ...
  public static ResolvedMember createResolvedConstructor(Constructor aConstructor, World inWorld) {
    final ReflectionBasedResolvedMemberImpl ret = new ReflectionBasedResolvedMemberImpl(org.aspectj.weaver.Member.CONSTRUCTOR,
  ...
  public static ResolvedMember createResolvedField(Field aField, World inWorld) {
    final ReflectionBasedResolvedMemberImpl ret = new ReflectionBasedResolvedMemberImpl(org.aspectj.weaver.Member.FIELD,
  ...
  private static ResolvedType[] toResolvedTypeArray(Class[] classes, World inWorld) {
    final ResolvedType[] ret = new ResolvedType[classes.length];
