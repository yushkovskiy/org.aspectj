...
public class MemberImpl implements Member {
  ...
  public MemberImpl(MemberKind kind, UnresolvedType declaringType, int modifiers, String name, String erasedSignature) {
    ...
    if (kind == FIELD) {
      ...
      final Object[] returnAndParams = signatureToTypes(erasedSignature);
  ...
  private static Object[] signatureToTypes(String sig) {
    final boolean hasParameters = sig.charAt(1) != ')';
    if (hasParameters) {
      final List<UnresolvedType> l = new ArrayList<UnresolvedType>();
      int i = 1;
      final boolean hasAnyAnglies = sig.indexOf('<') != -1;
      while (true) {
        char c = sig.charAt(i);
        if (c == ')') {
        ...
        if (c == 'L' || c == 'P') {
          final int nextSemicolon = sig.indexOf(';', start);
          final int firstAngly = (hasAnyAnglies ? sig.indexOf('<', start) : -1);
          ...
          final int nextSemicolon = sig.indexOf(';', start);
          final String nextbit = sig.substring(start, nextSemicolon + 1);
      ...
      final UnresolvedType[] paramTypes = l.toArray(new UnresolvedType[l.size()]);
      final UnresolvedType returnType = UnresolvedType.forSignature(sig.substring(i + 1, sig.length()));
      return new Object[]{returnType, paramTypes};
    } else {
      final UnresolvedType returnType = UnresolvedType.forSignature(sig.substring(2));
  ...
  public static MemberImpl method(UnresolvedType declaring, int mods, String name, String signature) {
    final Object[] pair = signatureToTypes(signature);
  ...
  public int getModifiers(World world) {
    final ResolvedMember resolved = resolve(world);
    if (resolved == null) {
  ...
  public Collection<ResolvedType> getDeclaringTypes(World world) {
    final ResolvedType myType = getDeclaringType().resolve(world);
    final Collection<ResolvedType> ret = new HashSet<ResolvedType>();
    if (kind == CONSTRUCTOR) {
  ...
  private boolean walkUp(Collection<ResolvedType> acc, ResolvedType curr) {
    ...
    boolean b = false;
    for (final Iterator<ResolvedType> i = curr.getDirectSupertypes(); i.hasNext(); ) {
      b |= walkUp(acc, i.next());
    }
  ...
  private void reportDidntFindMember(World world) {
    ...
    final ResolvedType rType = getDeclaringType().resolve(world);
    if (rType.isMissing()) {
