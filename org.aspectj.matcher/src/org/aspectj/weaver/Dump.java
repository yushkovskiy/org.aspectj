...
public class Dump {
  ...
  private static boolean shouldDumpOnExit(IMessageHolder messageHolder) {
    ...
    final boolean result = (messageHolder == null) || messageHolder.hasAnyMessage(conditionKind, true);
  ...
  public static boolean setDumpDirectory(String directoryName) {
    ...
    final File newDirectory = new File(directoryName);
  ...
  private void dumpCompilerMessages(IMessageHolder messageHolder) {
    ...
    if (messageHolder != null) {
      for (final Iterator<IMessage> i = messageHolder.getUnmodifiableListView().iterator(); i.hasNext(); ) {
        final IMessage message = i.next();
  ...
  private void println(File file) {
    ...
    if (!file.exists()) {
      ...
      final int count = file.listFiles().length;
  ...
  static {
    final String exceptionName = System.getProperty("org.aspectj.weaver.Dump.exception", "true");
    if (!exceptionName.equals("false")) {
      setDumpOnException(true);
    }

    final String conditionName = System.getProperty(DUMP_CONDITION_PROPERTY);
    if (conditionName != null) {
      setDumpOnExit(conditionName);
    }

    final String directoryName = System.getProperty(DUMP_DIRECTORY_PROPERTY);
