...
public class StandardPointcutParser {
  ...
  public static Set<PointcutPrimitive> getAllSupportedPointcutPrimitives() {
    final Set<PointcutPrimitive> primitives = new HashSet<PointcutPrimitive>();
  ...
  public static StandardPointcutParser getPointcutParserSupportingAllPrimitives(World world) {
    final StandardPointcutParser p = new StandardPointcutParser(world);
    return p;
  }
  ...
  public static StandardPointcutParser getPointcutParserSupportingSpecifiedPrimitives(Set supportedPointcutKinds, World world) {
    final StandardPointcutParser p = new StandardPointcutParser(supportedPointcutKinds, world);
    return p;
  }
  ...
  private StandardPointcutParser(Set/* <PointcutPrimitives> */supportedPointcutKinds, World world) {
    ...
    for (final Iterator iter = supportedPointcutKinds.iterator(); iter.hasNext(); ) {
      final PointcutPrimitive element = (PointcutPrimitive) iter.next();
  ...
  protected Pointcut resolvePointcutExpression(String expression, Class<?> inScope, PointcutParameter[] formalParameters) {
    try {
      final PatternParser parser = new PatternParser(expression);
      parser.setPointcutDesignatorHandlers(pointcutDesignators, world);
      Pointcut pc = parser.parsePointcut();
      validateAgainstSupportedPrimitives(pc, expression);
      final IScope resolutionScope = buildResolutionScope((inScope == null ? Object.class : inScope), formalParameters);
  ...
  protected Pointcut concretizePointcutExpression(Pointcut pc, Class<?> inScope, PointcutParameter[] formalParameters) {
    ...
    final IntMap arity = new IntMap(formalParameters.length);
  ...
  public TypePatternMatcher parseTypePattern(String typePattern) throws IllegalArgumentException {
    try {
      final TypePattern tp = new PatternParser(typePattern).parseTypePattern();
      tp.resolve(world);
  ...
  IMessageHandler setCustomMessageHandler(IMessageHandler aHandler) {
    final IMessageHandler current = getWorld().getMessageHandler();
  ...
  private IScope buildResolutionScope(Class inScope, PointcutParameter[] formalParameters) {
    ...
    final FormalBinding[] formalBindings = new FormalBinding[formalParameters.length];
    ...
    if (inScope == null) {
      final SimpleScope ss = new SimpleScope(getWorld(), formalBindings);
      ss.setImportedPrefixes(new String[]{"java.lang.", "java.util."});
      return ss;
    } else {
      final ResolvedType inType = getWorld().resolve(inScope.getName());
      final ISourceContext sourceContext = new ISourceContext() {
        @Override
      ...
      final BindingScope bScope = new BindingScope(inType, sourceContext, formalBindings);
  ...
  private void validateAgainstSupportedPrimitives(Pointcut pc, String expression) {
    switch (pc.getPointcutKind()) {
      ...
      case Pointcut.ARGS:
        ...
        break;
      case Pointcut.CFLOW:
        final CflowPointcut cfp = (CflowPointcut) pc;
        if (cfp.isCflowBelow()) {
  ...
  private void validateKindedPointcut(KindedPointcut pc, String expression) {
    final Shadow.Kind kind = pc.getKind();
  ...
  private static String buildUserMessageFromParserException(String pc, ParserException ex) {
    ...
    msg.append("'");
    final IHasPosition location = ex.getLocation();
