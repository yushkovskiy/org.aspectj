...
public class TypeFactory {
  ...
  public static ReferenceType createParameterizedType(ResolvedType aBaseType, UnresolvedType[] someTypeParameters, World inAWorld) {
    ...
    final ResolvedType[] resolvedParameters = inAWorld.resolve(someTypeParameters);

    final ReferenceType existingType = ((ReferenceType) baseType).findDerivativeType(resolvedParameters);
  ...
  static UnresolvedType convertSigToType(String aSignature) {
    ...
    final int startOfParams = aSignature.indexOf('<');
    if (startOfParams == -1) {
      bound = UnresolvedType.forSignature(aSignature);
    } else {
      final int endOfParams = aSignature.lastIndexOf('>');
      final String signatureErasure = "L" + aSignature.substring(1, startOfParams) + ";";
      final UnresolvedType[] typeParams = createTypeParams(aSignature.substring(startOfParams + 1, endOfParams));
  ...
  public static UnresolvedType createTypeFromSignature(String signature) {
    ...
    if (firstChar == 'P') {
      ...
      if (startOfParams == -1) {
        ...
        final String signatureErasure = "L" + signature.substring(1);
        ...
        final StringBuffer erasureSig = new StringBuffer(signature);
        ...
        final String signatureErasure = erasureSig.toString();// "L" + erasureSig.substring(1);
        ...
        final int nestedTypePosition = signature.indexOf("$", endOfParams); // don't look for $ INSIDE the parameters
        ...
        final int firstAngleBracket = signature.indexOf('<');
      ...
      // ? extends ...
      final UnresolvedType upperBound = convertSigToType(signature.substring(1));
      final WildcardedUnresolvedType wildcardedUT = new WildcardedUnresolvedType(signature, upperBound, null);
      return wildcardedUT;
    } else if (firstChar == '-') {
      // ? super ...
      final UnresolvedType lowerBound = convertSigToType(signature.substring(1));
      final WildcardedUnresolvedType wildcardedUT = new WildcardedUnresolvedType(signature, null, lowerBound);
      return wildcardedUT;
    } else if (firstChar == 'T') {
      ...
      final UnresolvedType componentType = createTypeFromSignature(signature.substring(dims));
      ...
      if (leftAngleBracket == -1) {
        ...
        final String signatureErasure = erasureSig.toString();
  ...
  private static int locateMatchingEndAngleBracket(CharSequence signature, int startOfParams) {
    ...
    final int max = signature.length();
    while (idx < max) {
      final char ch = signature.charAt(++idx);
      if (ch == '<') {
        count++;
      } else if (ch == '>') {
        if (count == 1) {
  ...
  private static UnresolvedType[] createTypeParams(String typeParameterSpecification) {
    ...
    final List<UnresolvedType> types = new ArrayList<UnresolvedType>();
    while (remainingToProcess.length() != 0) {
      ...
      for (endOfSig = 0; (endOfSig < remainingToProcess.length()) && !sigFound; endOfSig++) {
        final char thisChar = remainingToProcess.charAt(endOfSig);
        switch (thisChar) {
          case '<':
            anglies++;
            hadAnglies = true;
          ...
          case '*':
            if (anglies == 0) {
              final int nextCharPos = endOfSig + 1;
              if (nextCharPos >= remainingToProcess.length()) {
                sigFound = true;
              } else {
                final char nextChar = remainingToProcess.charAt(nextCharPos);
    ...
    final UnresolvedType[] typeParams = new UnresolvedType[types.size()];
  ...
  public static UnresolvedType createUnresolvedParameterizedType(String baseTypeSignature, UnresolvedType[] arguments) {
    final StringBuffer parameterizedSig = new StringBuffer();
