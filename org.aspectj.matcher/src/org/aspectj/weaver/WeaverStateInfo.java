...
public class WeaverStateInfo {
  ...
  private static final byte[] key = {-51, 34, 105, 56, -34, 65, 45, 78, -26, 125, 114, 97, 98, 1, -1, -42};
  ...
  private void checkCompressionEnabled() {
    if (compressionEnabled == 0) {
      ...
      try {
        final String value = System.getProperty("aspectj.compression.weaverstateinfo", "false");
  ...
  public static final WeaverStateInfo read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    ...
    final boolean isReweavable = ((b & REWEAVABLE_BIT) != 0);
    if (isReweavable) {
      b = (byte) (b - REWEAVABLE_BIT);
    }

    final boolean isReweavableCompressed = ((b & REWEAVABLE_COMPRESSION_BIT) != 0);
    if (isReweavableCompressed) {
      b = (byte) (b - REWEAVABLE_COMPRESSION_BIT);
    }

    final boolean isReweavableDiff = ((b & REWEAVABLE_DIFF_BIT) != 0);
    if (isReweavableDiff) {
    ...
    switch (b) {
      ...
      case EXTENDED:
        ...
        final int n = s.readShort();
        final List l = new ArrayList();
        for (int i = 0; i < n; i++) {
          // conditional on version
          UnresolvedType aspectType = null;
          if (isCompressed) {
            final int cpIndex = s.readShort();
            final String signature = s.readUtf8(cpIndex);
          ...
          final ResolvedTypeMunger typeMunger = ResolvedTypeMunger.read(s, context);
          l.add(new Entry(aspectType, typeMunger));
        }
        final WeaverStateInfo wsi = new WeaverStateInfo(l, false, isReweavable, isReweavableCompressed, isReweavableDiff);
  ...
  public List<ConcreteTypeMunger> getTypeMungers(ResolvedType onType) {
    final World world = onType.getWorld();
    final List<ConcreteTypeMunger> ret = new ArrayList<ConcreteTypeMunger>();
    for (Entry entry : typeMungers) {
      final ResolvedType aspectType = world.resolve(entry.aspectType, true);
  ...
  private static void readAnyReweavableData(WeaverStateInfo wsi, VersionedDataInputStream s, boolean compressed)
    ...
    if (wsi.isReweavable()) {
      ...
      final int numberAspectsAffectingType = s.readShort();
      ...
      final int unwovenClassFileSize = s.readInt();
      byte[] classData = null;
      // the unwovenClassFile may have been compressed:
      if (wsi.reweavableCompressedMode) {
        classData = new byte[unwovenClassFileSize];
        final ZipInputStream zis = new ZipInputStream(s);
        final ZipEntry zen = zis.getNextEntry();
        int current = 0;
        int bytesToGo = unwovenClassFileSize;
        while (bytesToGo > 0) {
          final int amount = zis.read(classData, current, bytesToGo);
          current += amount;
          bytesToGo -= amount;
        }
        zis.closeEntry();
        ...
        classData = new byte[unwovenClassFileSize];
        final int bytesread = s.read(classData);
  ...
  public byte[] replaceKeyWithDiff(byte wovenClassFile[]) {
    ...
    if (reweavableDiffMode) {
      ...
      final DataOutputStream s = new DataOutputStream(arrayStream);

      final int endOfKey = findEndOfKey(wovenClassFile);
      final int startOfKey = endOfKey - key.length;
      ...
      final int oldLengthLocation = startOfKey - 4;
      final int oldLength = readInt(wovenClassFile, oldLengthLocation);
      wovenClassFile = deleteInArray(wovenClassFile, startOfKey, endOfKey); // delete the key

      final byte[] wovenClassFileUpToWSI = new byte[oldLengthLocation];
      ...
      final int newLength = oldLength - key.length + diff.length;
      final byte[] newLengthBytes = serializeInt(newLength);
  ...
  private static final byte[] insertArray(byte toInsert[], byte original[], int offset) {
    final byte[] result = new byte[original.length + toInsert.length];
  ...
  private static final int readInt(byte[] a, int offset) {
    final ByteArrayInputStream b = new ByteArrayInputStream(a, offset, 4);
  ...
  private static final byte[] deleteInArray(byte a[], int start, int end) {
    final int lengthToDelete = end - start;
    final byte[] result = new byte[a.length - lengthToDelete]; // make a new array
  ...
  byte[] generateDiff(byte[] wovenClassFile, byte[] unWovenClassFile) {
    ...
    int lookingAt = 10;
    final int shorterLength = (wovenClassFile.length < unWovenClassFile.length) ? wovenClassFile.length : unWovenClassFile.length;
    while (lookingAt < shorterLength && (wovenClassFile[lookingAt] == unWovenClassFile[lookingAt])) {
      lookingAt++;
    }
    final int lengthInCommon = lookingAt - 10;
    final byte[] diff = new byte[unWovenClassFile.length - 4 - lengthInCommon];
    ...
    final byte[] lengthInCommonBytes = serializeInt(lengthInCommon);
  ...
  private static void writeAnyReweavableData(WeaverStateInfo wsi, CompressingDataOutputStream s, boolean compress)
    ...
    if (wsi.isReweavable()) {
      ...
      final byte[] data = wsi.unwovenClassFile;
  ...
  public boolean isAspectAlreadyApplied(ResolvedType someAspect) {
    final String someAspectSignature = someAspect.getSignature();
