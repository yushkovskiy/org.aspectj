...
public class KindedPointcut extends Pointcut {
  ...
  public boolean couldEverMatchSameJoinPointsAs(KindedPointcut other) {
    ...
    final String myName = signature.getName().maybeGetSimpleName();
    final String yourName = other.signature.getName().maybeGetSimpleName();
  ...
  public FuzzyBoolean fastMatch(FastMatchInfo info) {
    ...
    if (info.world.optimizedMatching) {
      ...
      if ((kind == Shadow.MethodExecution || kind == Shadow.Initialization) && info.getKind() == null) {
        final boolean fastMatchingOnAspect = info.getType().isAspect();
        ...
        if (this.getSignature().isExactDeclaringTypePattern()) {
          final ExactTypePattern typePattern = (ExactTypePattern) this.getSignature().getDeclaringType();
          // Interface checks are more expensive, they could be anywhere...
          final ResolvedType patternExactType = typePattern.getResolvedExactType(info.world);
          if (patternExactType.isInterface()) {
            ResolvedType curr = info.getType();
            final Iterator<ResolvedType> hierarchyWalker = curr.getHierarchy(true, true);
            boolean found = false;
          ...
          final AnnotationTypePattern annotationTypePattern = ((AnyWithAnnotationTypePattern) getSignature().getDeclaringType())
              .getAnnotationPattern();
          if (annotationTypePattern instanceof ExactAnnotationTypePattern) {
            final ExactAnnotationTypePattern exactAnnotationTypePattern = (ExactAnnotationTypePattern) annotationTypePattern;
            if (exactAnnotationTypePattern.getAnnotationValues() == null
                || exactAnnotationTypePattern.getAnnotationValues().size() == 0) {
              final ResolvedType annotationType = exactAnnotationTypePattern.getAnnotationType().resolve(info.world);
  ...
  private void warnOnConfusingSig(Shadow shadow) {
    ...
    final UnresolvedType exactDeclaringType = signature.getDeclaringType().getExactType();

    final ResolvedType shadowDeclaringType = shadow.getSignature().getDeclaringType().resolve(world);
    ...
    final ResolvedMember rm = shadow.getSignature().resolve(world);
    ...
    final int shadowModifiers = rm.getModifiers();
    ...
    final SignaturePattern nonConfusingPattern = new SignaturePattern(signature.getKind(), signature.getModifiers(),
  ...
  public boolean equals(Object other) {
    ...
    final KindedPointcut o = (KindedPointcut) other;
  ...
  public static Pointcut read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    final Shadow.Kind kind = Shadow.Kind.read(s);
    final SignaturePattern sig = SignaturePattern.read(s, context);
    final KindedPointcut ret = new KindedPointcut(kind, sig);
  ...
  public void resolveBindings(IScope scope, Bindings bindings) {
    ...
    if (kind == Shadow.ConstructorExecution) { // Bug fix 60936
      if (signature.getDeclaringType() != null) {
        ...
        final UnresolvedType exactType = signature.getDeclaringType().getExactType();
    ...
    if ((kind == Shadow.FieldGet) || (kind == Shadow.FieldSet)) {
      ...
      final UnresolvedType returnType = signature.getReturnType().getExactType();
  ...
  public Pointcut concretize1(ResolvedType inAspect, ResolvedType declaringType, IntMap bindings) {
    final Pointcut ret = new KindedPointcut(kind, signature, bindings.getEnclosingAdvice());
  ...
  public Pointcut parameterizeWith(Map typeVariableMap, World w) {
    final Pointcut ret = new KindedPointcut(kind, signature.parameterizeWith(typeVariableMap, w), munger);
    ret.copyLocationFrom(this);
    return ret;
  }
