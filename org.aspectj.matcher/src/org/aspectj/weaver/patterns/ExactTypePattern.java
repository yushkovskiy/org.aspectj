...
public class ExactTypePattern extends TypePattern {
  ...
  protected boolean matchesSubtypes(ResolvedType type) {
    final boolean match = super.matchesSubtypes(type);
    if (match) {
      return match;
    }
    // are we dealing with array funkyness - pattern might be jlObject[]+ and type jlString[]
    if (type.isArray() && this.type.isArray()) {
      final ResolvedType componentType = type.getComponentType().resolve(type.getWorld());
      final UnresolvedType newPatternType = this.type.getComponentType();
      final ExactTypePattern etp = new ExactTypePattern(newPatternType, includeSubtypes, false);
  ...
  protected boolean couldEverMatchSameTypesAs(TypePattern other) {
    ...
    if (other instanceof WildTypePattern) {
      ...
      final String yourSimpleNamePrefix = owtp.getNamePatterns()[0].maybeGetSimpleName();
  ...
  public FuzzyBoolean matchesInstanceof(ResolvedType matchType) {
    ...
    final ResolvedType resolvedType = type.resolve(matchType.getWorld());
  ...
  public boolean equals(Object other) {
    ...
    final ExactTypePattern o = (ExactTypePattern) other;
  ...
  public static TypePattern readTypePattern150(VersionedDataInputStream s, ISourceContext context) throws IOException {
    ...
    final TypePattern ret = new ExactTypePattern(s.isAtLeast169() ? s.readSignatureAsUnresolvedType() : UnresolvedType.read(s), s
  ...
  public static TypePattern readTypePatternOldStyle(DataInputStream s, ISourceContext context) throws IOException {
    final TypePattern ret = new ExactTypePattern(UnresolvedType.read(s), s.readBoolean(), false);
    ret.readLocation(context, s);
    return ret;
  }
  ...
  public TypePattern parameterizeWith(Map<String, UnresolvedType> typeVariableMap, World w) {
    ...
    if (type.isTypeVariableReference()) {
      final TypeVariableReference t = (TypeVariableReference) type;
    ...
    final ExactTypePattern ret = new ExactTypePattern(newType, includeSubtypes, isVarArgs);
