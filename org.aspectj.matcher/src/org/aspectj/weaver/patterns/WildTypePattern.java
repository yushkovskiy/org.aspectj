...
public class WildTypePattern extends TypePattern {
  ...
  protected boolean couldEverMatchSameTypesAs(TypePattern other) {
    ...
    final UnresolvedType otherType = other.getExactType();
    ...
    if (other instanceof WildTypePattern) {
      final WildTypePattern owtp = (WildTypePattern) other;
      final String mySimpleName = namePatterns[0].maybeGetSimpleName();
      final String yourSimpleName = owtp.namePatterns[0].maybeGetSimpleName();
  ...
  public static char[][] splitNames(String s, boolean convertDollar) {
    final List<char[]> ret = new ArrayList<char[]>();
    int startIndex = 0;
    while (true) {
      ...
      final char[] name = s.substring(startIndex, breakIndex).toCharArray();
      ret.add(name);
  ...
  protected boolean matchesExactly(ResolvedType type, ResolvedType annotatedType) {
    final String targetTypeName = type.getName();
  ...
  private boolean matchesExactlyByName(String targetTypeName, boolean isAnonymous, boolean isNested) {
    ...
    if (namePatterns.length == 1) {
      ...
      for (int i = 0, len = knownMatches.length; i < len; i++) {
        final String knownMatch = knownMatches[i];
        ...
        if (targetTypeName.startsWith(knownMatch) && targetTypeName.length() > knownMatch.length()
          ...
          final int pos = lastIndexOfDotOrDollar(knownMatch);
    ...
    for (int i = 0, len = importedPrefixes.length; i < len; i++) {
      final String prefix = importedPrefixes[i];
  ...
  private boolean innerMatchesExactly(String s, boolean isAnonymous, boolean convertDollar /* isNested */) {
    ...
    final int namesLength = ret.size();
    final int patternsLength = namePatterns.length;
    ...
    if (ellipsisCount == 0) {
      ...
      while (patternsIndex < patternsLength) {
        final NamePattern p = namePatterns[patternsIndex++];
      ...
      final boolean b = outOfStar(namePatterns, ret.toArray(new char[ret.size()][]), 0, 0, patternsLength - ellipsisCount,
  ...
  public NamePattern extractName() {
    ...
    final int len = namePatterns.length;
    if (len == 1 && !annotationPattern.isAny()) {
      return null; // can't extract
    }
    final NamePattern ret = namePatterns[len - 1];
    final NamePattern[] newNames = new NamePattern[len - 1];
  ...
  public boolean maybeExtractName(String string) {
    final int len = namePatterns.length;
    final NamePattern ret = namePatterns[len - 1];
    final String simple = ret.maybeGetSimpleName();
  ...
  public String maybeGetCleanName() {
    ...
    for (int i = 0, len = namePatterns.length; i < len; i++) {
      final NamePattern p = namePatterns[i];
      final String simpleName = p.maybeGetSimpleName();
      if (simpleName == null) {
        return null;
      }
      if (i > 0) {
  ...
  public TypePattern parameterizeWith(Map<String, UnresolvedType> typeVariableMap, World w) {
    ...
    if (newNamePatterns.length == 1) {
      final String simpleName = newNamePatterns[0].maybeGetSimpleName();
      if (simpleName != null) {
        if (typeVariableMap.containsKey(simpleName)) {
          final String newName = ((ReferenceType) typeVariableMap.get(simpleName)).getName().replace('$', '.');
          final StringTokenizer strTok = new StringTokenizer(newName, ".");
    ...
    final WildTypePattern ret = new WildTypePattern(newNamePatterns, includeSubtypes, dim, isVarArgs, typeParameters
  ...
  public TypePattern resolveBindings(IScope scope, Bindings bindings, boolean allowBinding, boolean requireExactType) {
    if (isNamePatternStar()) {
      final TypePattern anyPattern = maybeResolveToAnyPattern(scope, bindings, allowBinding, requireExactType);
      if (anyPattern != null) {
        if (requireExactType) {
    ...
    final TypePattern bindingTypePattern = maybeResolveToBindingTypePattern(scope, bindings, allowBinding, requireExactType);
    ...
    final String fullyQualifiedName = maybeGetCleanName();
  ...
  private TypePattern maybeResolveToAnyPattern(IScope scope, Bindings bindings, boolean allowBinding, boolean requireExactType) {
    ...
    if (annotationPattern == AnnotationTypePattern.ANY) {
      ...
      annotationPattern = annotationPattern.resolveBindings(scope, bindings, allowBinding);
      final AnyWithAnnotationTypePattern ret = new AnyWithAnnotationTypePattern(annotationPattern);
  ...
  private TypePattern maybeResolveToBindingTypePattern(IScope scope, Bindings bindings, boolean allowBinding,
    ...
    if (simpleName != null) {
      ...
      if (formalBinding != null) {
        ...
        final BindingTypePattern binding = new BindingTypePattern(formalBinding, isVarArgs);
  ...
  private TypePattern resolveBindingsFromFullyQualifiedTypeName(String fullyQualifiedName, IScope scope, Bindings bindings,
    ...
    final String originalName = fullyQualifiedName;
  ...
  private ResolvedType lookupTypeInWorldIncludingPrefixes(World world, String typeName, String[] prefixes) {
    final ResolvedType ret = lookupTypeInWorld(world, typeName);
    if (!ret.isMissing()) {
      return ret;
    }
    ...
    if (canBeExact) {
      ...
      if (upperBound != null) {
        if (upperBound.isIncludeSubtypes()) {
          ...
          final ReferenceType upper = (ReferenceType) upperBound.getExactType().resolve(scope.getWorld());
        ...
        if (lowerBound.isIncludeSubtypes()) {
          ...
          final ReferenceType lower = (ReferenceType) lowerBound.getExactType().resolve(scope.getWorld());
  ...
  private TypePattern resolveParameterizedType(IScope scope, UnresolvedType aType, boolean requireExactType) {
    ...
    if (typeParameters.areAllExactWithNoSubtypesAllowed()) {
      final TypePattern[] typePats = typeParameters.getTypePatterns();
      final UnresolvedType[] typeParameterTypes = new UnresolvedType[typePats.length];
  ...
  private TypePattern resolveBindingsForTypeVariable(IScope scope, UnresolvedTypeVariableReferenceType tvrType) {
    final Bindings emptyBindings = new Bindings(0);
    if (upperBound != null) {
    ...
    if (additionalInterfaceBounds != null) {
      final TypePattern[] resolvedIfBounds = new TypePattern[additionalInterfaceBounds.length];
    ...
    if (upperBound == null && lowerBound == null && additionalInterfaceBounds == null) {
      ...
      if (canCreateExactTypePattern) {
        final TypeVariable tv = tvrType.getTypeVariable();
        if (upperBound != null) {
          tv.setSuperclass(upperBound.getExactType());
        }
        if (additionalInterfaceBounds != null) {
          final UnresolvedType[] ifBounds = new UnresolvedType[additionalInterfaceBounds.length];
  ...
  private boolean verifyTypeParameters(ResolvedType baseType, IScope scope, boolean requireExactType) {
    final ResolvedType genericType = baseType.getGenericType();
    if (genericType == null) {
    ...
    final TypePattern[] typeParamPatterns = typeParameters.getTypePatterns();
    for (int i = 0; i < typeParamPatterns.length; i++) {
      if (typeParamPatterns[i] instanceof WildTypePattern) {
        final WildTypePattern wtp = (WildTypePattern) typeParamPatterns[i];
    ...
    final TypeVariable[] tvs = genericType.getTypeVariables();
    if ((tvs.length < minRequiredTypeParameters) || (!foundEllipsis && minRequiredTypeParameters != tvs.length)) {
      ...
      final String msg = WeaverMessages.format(WeaverMessages.INCORRECT_NUMBER_OF_TYPE_ARGUMENTS, genericType.getName(),
    ...
    if (!boundscheckingoff) {
      final VerifyBoundsForTypePattern verification = new VerifyBoundsForTypePattern(scope, genericType, requireExactType,
    ...
    public void verify() {
      ...
      if (typeParameters.areAllExactWithNoSubtypesAllowed()) {
        for (int i = 0; i < tvs.length; i++) {
          final UnresolvedType ut = typeParamPatterns[i].getExactType();
          ...
          if (continueCheck && !tvs[i].canBeBoundTo(ut.resolve(scope.getWorld()))) {
            ...
            final String msg = WeaverMessages.format(WeaverMessages.VIOLATES_TYPE_VARIABLE_BOUNDS, parameterName,
  ...
  public boolean isStar() {
    final boolean annPatternStar = annotationPattern == AnnotationTypePattern.ANY;
  ...
  public String toString() {
    ...
    for (int i = 0, len = namePatterns.length; i < len; i++) {
      final NamePattern name = namePatterns[i];
      if (name == null) {
        buf.append(".");
      } else {
        if (i > 0) {
  ...
  public boolean equals(Object other) {
    ...
    final WildTypePattern o = (WildTypePattern) other;
    final int len = o.namePatterns.length;
  ...
  public static TypePattern readTypePattern150(VersionedDataInputStream s, ISourceContext context) throws IOException {
    ...
    final int len = s.readShort();
    final NamePattern[] namePatterns = new NamePattern[len];
    for (int i = 0; i < len; i++) {
      namePatterns[i] = NamePattern.read(s);
    }
    final boolean includeSubtypes = s.readBoolean();
    final int dim = s.readInt();
    final boolean varArg = s.readBoolean();
    final TypePatternList typeParams = TypePatternList.read(s, context);
    final WildTypePattern ret = new WildTypePattern(namePatterns, includeSubtypes, dim, varArg, typeParams);
    ...
    final int numIfBounds = s.readInt();
    if (numIfBounds > 0) {
  ...
  public static TypePattern readTypePatternOldStyle(VersionedDataInputStream s, ISourceContext context) throws IOException {
    ...
    final boolean includeSubtypes = s.readBoolean();
    final int dim = s.readInt();
    final WildTypePattern ret = new WildTypePattern(namePatterns, includeSubtypes, dim, false, null);
    ret.knownMatches = FileUtil.readStringArray(s);
    ret.importedPrefixes = FileUtil.readStringArray(s);
    ret.readLocation(context, s);
    return ret;
