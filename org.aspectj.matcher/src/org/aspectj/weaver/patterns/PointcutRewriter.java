...
public class PointcutRewriter {
  ...
  private boolean isDNFHelper(Pointcut pc, boolean canStillHaveOrs) {
    if (isAnd(pc)) {
      final AndPointcut ap = (AndPointcut) pc;
      return isDNFHelper(ap.getLeft(), false) && isDNFHelper(ap.getRight(), false);
    } else if (isOr(pc)) {
      if (!canStillHaveOrs) {
        return false;
      }
      final OrPointcut op = (OrPointcut) pc;
  ...
  private Pointcut distributeNot(Pointcut pc) {
    if (isNot(pc)) {
      final NotPointcut npc = (NotPointcut) pc;
      final Pointcut notBody = distributeNot(npc.getNegatedPointcut());
      if (isNot(notBody)) {
        // !!X => X
        return ((NotPointcut) notBody).getNegatedPointcut();
      } else if (isAnd(notBody)) {
        // !(X && Y) => !X || !Y
        final AndPointcut apc = (AndPointcut) notBody;
        final Pointcut newLeft = distributeNot(new NotPointcut(apc.getLeft(), npc.getStart()));
        final Pointcut newRight = distributeNot(new NotPointcut(apc.getRight(), npc.getStart()));
        return new OrPointcut(newLeft, newRight);
      } else if (isOr(notBody)) {
        // !(X || Y) => !X && !Y
        final OrPointcut opc = (OrPointcut) notBody;
        final Pointcut newLeft = distributeNot(new NotPointcut(opc.getLeft(), npc.getStart()));
        final Pointcut newRight = distributeNot(new NotPointcut(opc.getRight(), npc.getStart()));
      ...
      final Pointcut left = distributeNot(apc.getLeft());
      final Pointcut right = distributeNot(apc.getRight());
      return new AndPointcut(left, right);
    } else if (isOr(pc)) {
      final OrPointcut opc = (OrPointcut) pc;
      final Pointcut left = distributeNot(opc.getLeft());
      final Pointcut right = distributeNot(opc.getRight());
  ...
  private Pointcut pullUpDisjunctions(Pointcut pc) {
    if (isNot(pc)) {
      ...
      final Pointcut left = pullUpDisjunctions(apc.getLeft());
      final Pointcut right = pullUpDisjunctions(apc.getRight());
      if (isOr(left) && !isOr(right)) {
        // (A || B) && C => (A && C) || (B && C)
        final Pointcut leftLeft = ((OrPointcut) left).getLeft();
        final Pointcut leftRight = ((OrPointcut) left).getRight();
        ...
        // A && (B || C) => (A && B) || (A && C)
        final Pointcut rightLeft = ((OrPointcut) right).getLeft();
        final Pointcut rightRight = ((OrPointcut) right).getRight();
        return pullUpDisjunctions(new OrPointcut(new AndPointcut(left, rightLeft), new AndPointcut(left, rightRight)));
      } else if (isOr(right) && isOr(left)) {
        // (A || B) && (C || D) => (A && C) || (A && D) || (B && C) || (B && D)
        final Pointcut A = pullUpDisjunctions(((OrPointcut) left).getLeft());
        final Pointcut B = pullUpDisjunctions(((OrPointcut) left).getRight());
        final Pointcut C = pullUpDisjunctions(((OrPointcut) right).getLeft());
        final Pointcut D = pullUpDisjunctions(((OrPointcut) right).getRight());
        final Pointcut newLeft = new OrPointcut(new AndPointcut(A, C), new AndPointcut(A, D));
        final Pointcut newRight = new OrPointcut(new AndPointcut(B, C), new AndPointcut(B, D));
  ...
  public static Pointcut createAndsFor(Pointcut[] ps) {
    ...
    // otherwise ...
    final Pointcut[] subset = new Pointcut[ps.length - 1];
  ...
  private Pointcut simplifyAnds(Pointcut pc) {
    if (isNot(pc)) {
      final NotPointcut npc = (NotPointcut) pc;
      final Pointcut notBody = npc.getNegatedPointcut();
      if (isNot(notBody)) {
        // !!X => X
  ...
  private Pointcut simplifyAnd(AndPointcut apc) {
    final SortedSet<Pointcut> nodes = new TreeSet<Pointcut>(new PointcutEvaluationExpenseComparator());
    collectAndNodes(apc, nodes);
    // look for A and !A, or IfFalse
    for (final Iterator<Pointcut> iter = nodes.iterator(); iter.hasNext(); ) {
      final Pointcut element = iter.next();
      if (element instanceof NotPointcut) {
        final Pointcut body = ((NotPointcut) element).getNegatedPointcut();
    ...
    final Iterator<Pointcut> iter = nodes.iterator();
    Pointcut result = iter.next();
    while (iter.hasNext()) {
      final Pointcut right = iter.next();
  ...
  private Pointcut removeNothings(Pointcut pc) {
    if (isAnd(pc)) {
      final AndPointcut apc = (AndPointcut) pc;
      final Pointcut right = removeNothings(apc.getRight());
      final Pointcut left = removeNothings(apc.getLeft());
      ...
      final OrPointcut opc = (OrPointcut) pc;
      final Pointcut right = removeNothings(opc.getRight());
      final Pointcut left = removeNothings(opc.getLeft());
      if (left instanceof MatchesNothingPointcut && !(right instanceof MatchesNothingPointcut)) {
        return right;
