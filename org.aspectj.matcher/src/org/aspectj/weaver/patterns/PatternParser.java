...
public class PatternParser {
  ...
  private final ITokenSource tokenSource;
  ...
  public PerClause maybeParsePerClause() {
    final IToken tok = tokenSource.peek();
    if (tok == IToken.EOF) {
      return null;
    }
    if (tok.isIdentifier()) {
      final String name = tok.getString();
  ...
  private PerClause parsePerCflow(boolean isBelow) {
    parseIdentifier();
    eat("(");
    final Pointcut entry = parsePointcut();
    eat(")");
  ...
  private PerClause parsePerTypeWithin() {
    parseIdentifier();
    eat("(");
    final TypePattern withinTypePattern = parseTypePattern();
    eat(")");
  ...
  public Declare parseDeclare() {
    final int startPos = tokenSource.peek().getStart();

    eatIdentifier("declare");
    final String kind = parseIdentifier();
    final Declare ret;
    ...
    final int endPos = tokenSource.peek(-1).getEnd();
  ...
  public DeclareAnnotation parseDeclareAtType() {
    ...
    final TypePattern p = parseTypePattern();
  ...
  public DeclareAnnotation parseDeclareAtMethod(boolean isMethod) {
    final ISignaturePattern sp = parseCompoundMethodOrConstructorSignaturePattern(isMethod);// parseMethodOrConstructorSignaturePattern();
  ...
  public DeclareAnnotation parseDeclareAtField() {
    final ISignaturePattern compoundFieldSignaturePattern = parseCompoundFieldSignaturePattern();
    final DeclareAnnotation da = new DeclareAnnotation(DeclareAnnotation.AT_FIELD, compoundFieldSignaturePattern);
    return da;
  }

  public ISignaturePattern parseCompoundFieldSignaturePattern() {
    final int index = tokenSource.getIndex();
    try {
      ...
      final int nowAt = tokenSource.getIndex();
      tokenSource.setIndex(index);
      try {
        final ISignaturePattern fsp = parseFieldSignaturePattern();
        return fsp;
      } catch (Exception e2) {
  ...
  private boolean isEitherAndOrOr() {
    final String tokenstring = tokenSource.peek().getString();
  ...
  public DeclarePrecedence parseDominates() {
    final List<TypePattern> l = new ArrayList<TypePattern>();
    do {
  ...
  private Declare parseParents() {
    ...
    eat(":");
    allowHasTypePatterns = true;
    final TypePattern p = parseTypePattern(false, false);
    allowHasTypePatterns = false;
    final IToken t = tokenSource.next();
    if (!(t.getString().equals("extends") || t.getString().equals("implements"))) {
      throw new ParserException("extends or implements", t);
    }
    final boolean isExtends = t.getString().equals("extends");
    ...
    final DeclareParents decp = new DeclareParents(p, l, isExtends);
    return decp;
  }
  ...
  private Declare parseSoft() {
    final TypePattern p = parseTypePattern();
    eat(":");
    final Pointcut pointcut = parsePointcut();
  ...
  private Declare parseErrorOrWarning(boolean isError) {
    ...
    try {
      ...
      try {
        tokenSource.setIndex(index);
        final boolean oldValue = allowHasTypePatterns;
        ...
        eat(":");
        final String message = parsePossibleStringSequence(true);
  ...
  private Pointcut parseAtomicPointcut() {
    if (maybeEat("!")) {
      final int startPos = tokenSource.peek(-1).getStart();
      final Pointcut p = new NotPointcut(parseAtomicPointcut(), startPos);
      return p;
    }
    if (maybeEat("(")) {
      final Pointcut p = parsePointcut();
      eat(")");
      return p;
    }
    if (maybeEat("@")) {
      final int startPos = tokenSource.peek().getStart();
      final Pointcut p = parseAnnotationPointcut();
      final int endPos = tokenSource.peek(-1).getEnd();
      p.setLocation(sourceContext, startPos, endPos);
      return p;
    }
    final int startPos = tokenSource.peek().getStart();
    final Pointcut p = parseSinglePointcut();
    final int endPos = tokenSource.peek(-1).getEnd();
    p.setLocation(sourceContext, startPos, endPos);
    return p;
  }
  ...
  public Pointcut parseSinglePointcut() {
    final int start = tokenSource.getIndex();
    final IToken t = tokenSource.peek();
    ...
    if (kind.equals("execution") || kind.equals("call") || kind.equals("get") || kind.equals("set")) {
      ...
      eat("(");
      final TypePattern typePat = parseTypePattern(false, false);
      eat(")");
      ...
      eat("(");
      final SignaturePattern sig = parseConstructorSignaturePattern();
      eat(")");
  ...
  public Pointcut parseAnnotationPointcut() {
    ...
    final IToken t = tokenSource.peek();
    final String kind = parseIdentifier();
    final IToken possibleTypeVariableToken = tokenSource.peek();
    final String[] typeVariables = maybeParseSimpleTypeVariableList();
  ...
  private Pointcut parseAtAnnotationPointcut() {
    ...
    final ExactAnnotationTypePattern type = parseAnnotationNameOrVarTypePattern();
    eat(")");
    return new AnnotationPointcut(type);
  }

  private SignaturePattern parseConstructorSignaturePattern() {
    final SignaturePattern ret = parseMethodOrConstructorSignaturePattern();
  ...
  private Pointcut parseWithinCodePointcut() {
    // parseIdentifier();
    eat("(");
    final SignaturePattern sig = parseMethodOrConstructorSignaturePattern();
    eat(")");
  ...
  private Pointcut parseWithinPointcut() {
    // parseIdentifier();
    eat("(");
    final TypePattern type = parseTypePattern();
    eat(")");
  ...
  private Pointcut parseWithinAnnotationPointcut() {
    ...
    final AnnotationTypePattern type = parseAnnotationNameOrVarTypePattern();
    eat(")");
  ...
  private Pointcut parseArgsPointcut() {
    // parseIdentifier();
    final TypePatternList arguments = parseArgumentsPattern(false);
    return new ArgsPointcut(arguments);
  }

  private Pointcut parseArgsAnnotationPointcut() {
    parseIdentifier();
    final AnnotationPatternList arguments = parseArgumentsAnnotationPattern();
  ...
  private Pointcut parseReferencePointcut() {
    ...
    final String simpleName = name.maybeGetSimpleName();
    if (simpleName == null) {
  ...
  private Pointcut parseDesignatorPointcut(PointcutDesignatorHandler pcdHandler) {
    eat("(");
    int parenCount = 1;
    final StringBuffer pointcutBody = new StringBuffer();
    while (parenCount > 0) {
      if (maybeEat("(")) {
        parenCount++;
        pointcutBody.append("(");
    ...
    final ContextBasedMatcher pcExpr = pcdHandler.parse(pointcutBody.toString());
  ...
  private TypePattern parseAtomicTypePattern(boolean insideTypeParameters, boolean parameterAnnotationsPossible) {
    final AnnotationTypePattern ap = maybeParseAnnotationPattern(); // might be parameter annotation pattern or type annotation
    // pattern
    if (maybeEat("!")) {
      ...
      TypePattern p = null;
      final TypePattern tp = parseAtomicTypePattern(insideTypeParameters, parameterAnnotationsPossible);
    ...
    if (maybeEat("(")) {
      final int openParenPos = tokenSource.peek(-1).getStart();
      ...
      if ((p instanceof NotTypePattern) && !(ap instanceof AnyAnnotationTypePattern)) {
        ...
        final TypePattern tp = setAnnotationPatternForTypePattern(TypePattern.ANY, ap, parameterAnnotationsPossible);
        p = new AndTypePattern(tp, p);
      } else {
        p = setAnnotationPatternForTypePattern(p, ap, parameterAnnotationsPossible);
      }
      eat(")");
      final int closeParenPos = tokenSource.peek(-1).getStart();
      final boolean isVarArgs = maybeEat("...");
      if (isVarArgs) {
        p.setIsVarArgs(isVarArgs);
      }
      final boolean isIncludeSubtypes = maybeEat("+");
  ...
  public AnnotationTypePattern maybeParseSingleAnnotationPattern() {
    ...
    final int startIndex = tokenSource.getIndex();
    if (maybeEat("!")) {
      if (maybeEat("@")) {
        if (maybeEat("(")) {
          final TypePattern p = parseTypePattern();
          ...
          final TypePattern p = parseSingleTypePattern();
          if (maybeEatAdjacent("(")) {
    ...
    if (maybeEat("@")) {
      if (maybeEat("(")) {
        ...
        final int atPos = tokenSource.peek(-1).getStart();
        final TypePattern p = parseSingleTypePattern();
        if (maybeEatAdjacent("(")) {
          values = parseAnnotationValues();
          eat(")");
  ...
  public Map<String, String> parseAnnotationValues() {
    ...
    do {
      final String possibleKeyString = parseAnnotationNameValuePattern();
      ...
      if (maybeEat("=")) {
        // it was a key!
        final String valueString = parseAnnotationNameValuePattern();
        if (valueString == null) {
          throw new ParserException("expecting simple literal ", tokenSource.peek(-1));
        }
  ...
  public TypePattern parseSingleTypePattern(boolean insideTypeParameters) {
    ...
    if (maybeEatIdentifier("is")) {
      final int pos = tokenSource.getIndex() - 1;
      final TypePattern typeIsPattern = parseIsTypePattern();
      if (typeIsPattern != null) {
        return typeIsPattern;
      }
    ...
    final List<NamePattern> names = parseDottedNamePattern();
    ...
    final TypePatternList typeParameters = maybeParseTypeParameterList();
    final int endPos = tokenSource.peek(-1).getEnd();

    final boolean includeSubtypes = maybeEat("+");
  ...
  public TypePattern parseHasMethodTypePattern() {
    final int startPos = tokenSource.peek(-1).getStart();
    eat("(");
    final SignaturePattern sp = parseMethodOrConstructorSignaturePattern();
    eat(")");
    final int endPos = tokenSource.peek(-1).getEnd();
  ...
  public TypePattern parseIsTypePattern() {
    final int startPos = tokenSource.peek(-1).getStart(); // that will be the start of the 'typeIs'
    if (!maybeEatAdjacent("(")) {
      return null;
    }
    final IToken token = tokenSource.next();
    TypeCategoryTypePattern typeIsPattern = null;
    if (token.isIdentifier()) {
      final String category = token.getString();
  ...
  public TypePattern parseHasFieldTypePattern() {
    final int startPos = tokenSource.peek(-1).getStart();
    eat("(");
    final SignaturePattern sp = parseFieldSignaturePattern();
    eat(")");
    final int endPos = tokenSource.peek(-1).getEnd();
    final HasMemberTypePattern ret = new HasMemberTypePattern(sp);
    ret.setLocation(sourceContext, startPos, endPos);
  ...
  public TypePattern parseGenericsWildcardTypePattern() {
    final List<NamePattern> names = new ArrayList<NamePattern>();
  ...
  protected ExactAnnotationTypePattern parseAnnotationNameOrVarTypePattern() {
    ...
    if (maybeEat("(")) {
      final String formalName = parseIdentifier();
  ...
  private ExactAnnotationTypePattern parseSimpleAnnotationName() {
    ...
    final ExactAnnotationTypePattern p;
    final StringBuffer annotationName = new StringBuffer();
    annotationName.append(parseIdentifier());
    while (maybeEat(".")) {
      annotationName.append('.');
      annotationName.append(parseIdentifier());
    }
    final UnresolvedType type = UnresolvedType.forName(annotationName.toString());
  ...
  public List<NamePattern> parseDottedNamePattern() {
    ...
    while (true) {
      ...
      if (buf.length() == 0) {
        ...
        final NamePattern ret = new NamePattern(buf.toString());
        ret.setLocation(sourceContext, startPos, endPos);
        names.add(ret);
  ...
  public ModifiersPattern parseModifiersPattern() {
    ...
    while (true) {
      ...
      final IToken t = tokenSource.next();
      final int flag = ModifiersPattern.getModifierFlag(t.getString());
      if (flag == -1) {
        break;
      }
      if (isForbidden) {
  ...
  public TypePatternList parseArgumentsPattern(boolean parameterAnnotationsPossible) {
    final List<TypePattern> patterns = new ArrayList<TypePattern>();
    eat("(");
  ...
  public AnnotationPatternList parseArgumentsAnnotationPattern() {
    final List<AnnotationTypePattern> patterns = new ArrayList<AnnotationTypePattern>();
    eat("(");
    if (maybeEat(")")) {
  ...
  public ThrowsPattern parseOptionalThrowsPattern() {
    ...
    if (t.isIdentifier() && t.getString().equals("throws")) {
      tokenSource.next();
      final List<TypePattern> required = new ArrayList<TypePattern>();
      final List<TypePattern> forbidden = new ArrayList<TypePattern>();
      do {
        final boolean isForbidden = maybeEat("!");
  ...
  public SignaturePattern parseMethodOrConstructorSignaturePattern() {
    final int startPos = tokenSource.peek().getStart();
    final AnnotationTypePattern annotationPattern = maybeParseAnnotationPattern();
    final ModifiersPattern modifiers = parseModifiersPattern();
    ...
    final TypePatternList parameterTypes = parseArgumentsPattern(true);

    final ThrowsPattern throwsPattern = parseOptionalThrowsPattern();
    final SignaturePattern ret = new SignaturePattern(kind, modifiers, returnType, declaringType, name, parameterTypes,
  ...
  private boolean maybeEatNew(TypePattern returnType) {
    if (returnType instanceof WildTypePattern) {
      final WildTypePattern p = (WildTypePattern) returnType;
    ...
    final int start = tokenSource.getIndex();
    if (maybeEat(".")) {
      final String id = maybeEatIdentifier();
  ...
  public ISignaturePattern parseMaybeParenthesizedFieldSignaturePattern() {
    final boolean negated = tokenSource.peek().getString().equals("!") && tokenSource.peek(1).getString().equals("(");
    if (negated) {
      eat("!");
    }
  ...
  public ISignaturePattern parseMaybeParenthesizedMethodOrConstructorSignaturePattern(boolean isMethod) {
    ...
    if (maybeEat("(")) {
      ...
      final SignaturePattern sp = parseMethodOrConstructorSignaturePattern();
      final boolean isConstructorPattern = (sp.getKind() == Member.CONSTRUCTOR);
  ...
  public SignaturePattern parseFieldSignaturePattern() {
    ...
    final AnnotationTypePattern annotationPattern = maybeParseAnnotationPattern();
    final ModifiersPattern modifiers = parseModifiersPattern();
    final TypePattern returnType = parseTypePattern();
    TypePattern declaringType = parseTypePattern();
    final NamePattern name;
    ...
    final SignaturePattern ret = new SignaturePattern(Member.FIELD, modifiers, returnType, declaringType, name, TypePatternList.ANY,
  ...
  private static NamePattern tryToExtractName(TypePattern nextType) {
    if (nextType == TypePattern.ANY) {
      ...
      final WildTypePattern p = (WildTypePattern) nextType;
  ...
  public TypeVariablePatternList maybeParseTypeVariableList() {
    ...
    final List<TypeVariablePattern> typeVars = new ArrayList<TypeVariablePattern>();
    final TypeVariablePattern t = parseTypeVariable();
    typeVars.add(t);
    while (maybeEat(",")) {
      final TypeVariablePattern nextT = parseTypeVariable();
      typeVars.add(nextT);
    }
    eat(">");
    final TypeVariablePattern[] tvs = new TypeVariablePattern[typeVars.size()];
  ...
  public String[] maybeParseSimpleTypeVariableList() {
    ...
    final List<String> typeVarNames = new ArrayList<String>();
    do {
      typeVarNames.add(parseIdentifier());
    } while (maybeEat(","));
    eat(">", "',' or '>'");
    final String[] tvs = new String[typeVarNames.size()];
  ...
  public TypePatternList maybeParseTypeParameterList() {
    ...
    final List<TypePattern> typePats = new ArrayList<TypePattern>();
    do {
      final TypePattern tp = parseTypePattern(true, false);
      typePats.add(tp);
    } while (maybeEat(","));
    eat(">");
    final TypePattern[] tps = new TypePattern[typePats.size()];
  ...
  public TypeVariablePattern parseTypeVariable() {
    ...
    TypePattern lowerBound = null;
    final String typeVariableName = parseIdentifier();
    if (maybeEatIdentifier("extends")) {
  ...
  private TypePattern[] maybeParseAdditionalInterfaceBounds() {
    final List<TypePattern> boundsList = new ArrayList<TypePattern>();
    while (maybeEat("&")) {
      final TypePattern tp = parseTypePattern();
      boundsList.add(tp);
    }
    if (boundsList.size() == 0) {
      return null;
    }
    final TypePattern[] ret = new TypePattern[boundsList.size()];
  ...
  public String parsePossibleStringSequence(boolean shouldEnd) {
    ...
    final int currentIndex = tokenSource.getIndex();
  ...
  public String parseStringLiteral() {
    final IToken token = tokenSource.next();
    final String literalKind = token.getLiteralKind();
  ...
  public void eatIdentifier(String expectedValue) {
    final IToken next = tokenSource.next();
  ...
  public boolean maybeEatIdentifier(String expectedValue) {
    final IToken next = tokenSource.peek();
    if (next.getString().equals(expectedValue)) {
      tokenSource.next();
      return true;
    } else {
  ...
  private IToken nextToken() {
    if (pendingRightArrows != null) {
      final IToken ret = pendingRightArrows;
  ...
  public void checkEof() {
    final IToken last = tokenSource.next();
    if (last != IToken.EOF) {
