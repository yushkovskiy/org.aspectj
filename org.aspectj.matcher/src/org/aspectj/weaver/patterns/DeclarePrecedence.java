...
public class DeclarePrecedence extends Declare {
  ...
  public Declare parameterizeWith(Map typeVariableBindingMap, World w) {
    final DeclarePrecedence ret = new DeclarePrecedence(this.patterns.parameterizeWith(typeVariableBindingMap, w));
  ...
  public boolean equals(Object other) {
    ...
    final DeclarePrecedence o = (DeclarePrecedence) other;
  ...
  public static Declare read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    final Declare ret = new DeclarePrecedence(TypePatternList.read(s, context));
  ...
  public void resolve(IScope scope) {
    ...
    for (int i = 0; i < patterns.size(); i++) {
      final TypePattern pi = patterns.get(i);
      if (pi.isStar()) {
        if (seenStar) {
      ...
      final ResolvedType exactType = pi.getExactType().resolve(scope.getWorld());
      ...
      for (int j = 0; j < patterns.size(); j++) {
        ...
        final TypePattern pj = patterns.get(j);
        if (pj.isStar()) {
          continue;
        }
  ...
  private int matchingIndex(ResolvedType a) {
    ...
    for (int i = 0, len = patterns.size(); i < len; i++) {
      final TypePattern p = patterns.get(i);
      if (p.isStar()) {
        starMatch = i;
  ...
  public int compare(ResolvedType aspect1, ResolvedType aspect2) {
    ensureResolved();
    final int index1 = matchingIndex(aspect1);
    final int index2 = matchingIndex(aspect2);
