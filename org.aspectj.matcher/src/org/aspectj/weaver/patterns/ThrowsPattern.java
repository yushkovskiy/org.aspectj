...
public class ThrowsPattern extends PatternNode {
  ...
  public boolean equals(Object other) {
    ...
    final ThrowsPattern o = (ThrowsPattern) other;
    final boolean ret = o.required.equals(this.required) && o.forbidden.equals(this.forbidden);
    return ret;
  }
  ...
  public ThrowsPattern parameterizeWith(Map/* name -> resolved type */typeVariableMap, World w) {
    final ThrowsPattern ret = new ThrowsPattern(required.parameterizeWith(typeVariableMap, w), forbidden.parameterizeWith(
        typeVariableMap, w));
  ...
  public boolean matches(UnresolvedType[] tys, World world) {
    ...
    final ResolvedType[] types = world.resolve(tys);
  ...
  public static ThrowsPattern read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    final TypePatternList required = TypePatternList.read(s, context);
    final TypePatternList forbidden = TypePatternList.read(s, context);
    if (required.size() == 0 && forbidden.size() == 0) {
      return ANY;
    }
    final ThrowsPattern ret = new ThrowsPattern(required, forbidden);
