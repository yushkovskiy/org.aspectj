...
public class Bindings {
  ...
  private final BindingPattern[] bindings;
  ...
  public void register(BindingPattern binding, IScope scope) {
    final int index = binding.getFormalIndex();
    final BindingPattern existingBinding = bindings[index];
  ...
  public void checkEquals(Bindings other, IScope scope) {
    final BindingPattern[] b1 = this.bindings;
    final BindingPattern[] b2 = other.bindings;
  ...
  public String toString() {
    final StringBuffer buf = new StringBuffer("Bindings(");
  ...
  public int[] getUsedFormals() {
    ...
    final int[] ret = new int[bindings.length];
    int index = 0;
    for (int i = 0, len = bindings.length; i < len; i++) {
      if (bindings[i] != null) {
        ret[index++] = i;
    ...
    final int[] newRet = new int[index];
  ...
  public UnresolvedType[] getUsedFormalTypes() {
    final UnresolvedType[] ret = new UnresolvedType[bindings.length];
    int index = 0;
    for (int i = 0, len = bindings.length; i < len; i++) {
      if (bindings[i] != null) {
    ...
    final UnresolvedType[] newRet = new UnresolvedType[index];
    System.arraycopy(ret, 0, newRet, 0, index);
    // System.out.println("ret: " + index);
    return newRet;
  }
