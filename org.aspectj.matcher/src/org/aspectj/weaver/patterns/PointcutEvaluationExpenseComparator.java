...
public class PointcutEvaluationExpenseComparator implements Comparator<Pointcut> {
  ...
  public int compare(Pointcut p1, Pointcut p2) {
    ...
    final int result = getScore(p1) - getScore(p2);
    if (result == 0) {
      // they have the same evaluation expense, but are not 'equal'
      // sort by hashCode
      final int p1code = p1.hashCode();
      final int p2code = p2.hashCode();
      if (p1code == p2code) {
        return 0;
  ...
  private static int getScore(Pointcut p) {
    ...
    if (p instanceof KindedPointcut) {
      final KindedPointcut kp = (KindedPointcut) p;
      final Shadow.Kind kind = kp.getKind();
      if (kind == Shadow.AdviceExecution) {
        return ADVICEEXECUTION;
      } else if ((kind == Shadow.ConstructorCall) || (kind == Shadow.MethodCall)) {
        final TypePattern declaringTypePattern = kp.getSignature().getDeclaringType();
    ...
    if (p instanceof AndPointcut) {
      final int leftScore = getScore(((AndPointcut) p).getLeft());
      final int rightScore = getScore(((AndPointcut) p).getRight());
    ...
    if (p instanceof OrPointcut) {
      final int leftScore = getScore(((OrPointcut) p).getLeft());
      final int rightScore = getScore(((OrPointcut) p).getRight());
      if (leftScore > rightScore) {
        return leftScore;
      } else {
        return rightScore;
