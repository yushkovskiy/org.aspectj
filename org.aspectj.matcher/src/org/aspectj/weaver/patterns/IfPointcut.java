...
public class IfPointcut extends Pointcut {
  ...
  public static Pointcut read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    final boolean hasTestMethod = s.readBoolean();
    ...
    final IfPointcut ret = new IfPointcut(resolvedTestMethod, s.readByte());
  ...
  public boolean equals(Object other) {
    ...
    final IfPointcut o = (IfPointcut) other;
  ...
  protected Test findResidueInternal(Shadow shadow, ExposedState state) {
    ...
    try {
      ...
      final List<Var> args = new ArrayList<Var>();
      ...
      if (extraParameterFlags >= 0) {
        ...
        if (baseArgsCount > 0) {
          final ExposedState myState = new ExposedState(baseArgsCount);
          ...
          final UnresolvedType[] pTypes = (testMethod == null ? null : testMethod.getParameterTypes());
        ...
        for (int i = 0; i < testMethod.getParameterTypes().length; i++) {
          final String argSignature = testMethod.getParameterTypes()[i].getSignature();
          if (AjcMemberMaker.TYPEX_JOINPOINT.getSignature().equals(argSignature)) {
            ...
            if (state.size() == 0 || currentStateIndex > state.size()) {
              final String[] paramNames = testMethod.getParameterNames();
              final StringBuffer errorParameter = new StringBuffer();
              if (paramNames != null) {
  ...
  public Pointcut concretize1(ResolvedType inAspect, ResolvedType declaringType, IntMap bindings) {
    ...
    if (extraParameterFlags < 0 && testMethod == null) {
      ...
      final ResolvedPointcutDefinition def = bindings.peekEnclosingDefinition();
      if (def != null) {
        final ResolvedType aspect = inAspect.getWorld().resolve(def.getDeclaringType());
        for (final Iterator memberIter = aspect.getMethods(true, true); memberIter.hasNext(); ) {
          final ResolvedMember method = (ResolvedMember) memberIter.next();
          if (def.getName().equals(method.getName())
              && def.getParameterTypes().length == method.getParameterTypes().length) {
            boolean sameSig = true;
            for (int j = 0; j < method.getParameterTypes().length; j++) {
              final UnresolvedType argJ = method.getParameterTypes()[j];
    ...
    if (bindings.directlyInAdvice()) {
      final ShadowMunger advice = bindings.getEnclosingAdvice();
      ...
      final IntMap newBindings = IntMap.idMap(ret.baseArgsCount);
  ...
  public static IfPointcut makeIfFalsePointcut(State state) {
    final IfPointcut ret = new IfFalsePointcut();
    ret.state = state;
    return ret;
  }
  ...
  public static IfPointcut makeIfTruePointcut(State state) {
    final IfPointcut ret = new IfTruePointcut();
    ret.state = state;
    return ret;
  }
