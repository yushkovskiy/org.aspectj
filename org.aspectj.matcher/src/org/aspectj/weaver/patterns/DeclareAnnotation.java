...
public class DeclareAnnotation extends Declare {
  ...
  public boolean equals(Object obj) {
    ...
    final DeclareAnnotation other = (DeclareAnnotation) obj;
  ...
  public static Declare read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    ...
    final String annotationString = s.readUTF();
    if (s.getMajorVersion() >= WeaverVersionInfo.WEAVER_VERSION_AJ169) {
      // int numAnnotationMethods =
      s.readByte();
    }
    final String annotationMethod = s.readUTF();
    TypePattern tp = null;
  ...
  private void ensureAnnotationDiscovered() {
    ...
    final String annotationMethod = annotationMethods.get(0);
    for (final Iterator<ResolvedMember> iter = containingAspect.getMethods(true, true); iter.hasNext(); ) {
      final ResolvedMember member = iter.next();
      if (member.getName().equals(annotationMethod)) {
        final AnnotationAJ[] annos = member.getAnnotations();
        if (annos == null) {
  ...
  public ResolvedType getAnnotationType() {
    if (annotationType == null) {
      ...
      for (final Iterator<ResolvedMember> iter = containingAspect.getMethods(true, true); iter.hasNext(); ) {
        final ResolvedMember member = iter.next();
        if (member.getName().equals(annotationMethod)) {
          final ResolvedType[] annoTypes = member.getAnnotationTypes();
          if (annoTypes == null) {
            // if weaving broken code, this can happen
            return null;
          }
