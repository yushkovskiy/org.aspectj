...
public class TypePatternList extends PatternNode {
  private final TypePattern[] typePatterns;
  int ellipsisCount = 0;
  ...
  public String toString() {
    ...
    for (int i = 0, len = typePatterns.length; i < len; i++) {
      final TypePattern type = typePatterns[i];
      if (i > 0) {
        buf.append(", ");
      }
  ...
  public FuzzyBoolean matches(ResolvedType[] types, TypePattern.MatchKind kind, ResolvedType[][] parameterAnnotations) {
    final int nameLength = types.length;
    final int patternLength = typePatterns.length;
    ...
    if (ellipsisCount == 0) {
      ...
      while (patternIndex < patternLength) {
        final ResolvedType t = types[nameIndex];
      ...
      FuzzyBoolean finalReturn = FuzzyBoolean.YES;
      while (patternIndex < patternLength) {
        final TypePattern p = typePatterns[patternIndex++];
      ...
      final FuzzyBoolean b = outOfStar(typePatterns, types, 0, 0, patternLength - ellipsisCount, nameLength, ellipsisCount, kind,
  ...
  private static FuzzyBoolean inStar(final TypePattern[] pattern, final ResolvedType[] target, int pi, int ti, final int pLeft,
    ...
    while (true) {
      ...
      if (ff.maybeTrue()) {
        final FuzzyBoolean xx = outOfStar(pattern, target, pi + 1, ti + 1, pLeft - 1, tLeft - 1, starsLeft, kind,
            parameterAnnotations);
        if (xx.maybeTrue()) {
          return ff.and(xx);
        }
  ...
  public FuzzyBoolean matches(ResolvableTypeList types, TypePattern.MatchKind kind, ResolvedType[][] parameterAnnotations) {
    ...
    if (ellipsisCount == 0) {
      ...
      FuzzyBoolean finalReturn = FuzzyBoolean.YES;
      while (patternIndex < patternLength) {
        final ResolvedType t = types.getResolved(nameIndex);
        FuzzyBoolean ret = null;
        try {
          if (parameterAnnotations != null) {
            t.temporaryAnnotationTypes = parameterAnnotations[nameIndex];
  ...
  private static FuzzyBoolean outOfStar(final TypePattern[] pattern, ResolvableTypeList target, int pi, int ti, int pLeft,
    ...
    while (true) {
      ...
      FuzzyBoolean ret = null;
      final ResolvedType type = target.getResolved(ti);
      try {
        if (parameterAnnotations != null) {
  ...
  public TypePatternList parameterizeWith(Map typeVariableMap, World w) {
    final TypePattern[] parameterizedPatterns = new TypePattern[typePatterns.length];
  ...
  public TypePatternList resolveBindings(IScope scope, Bindings bindings, boolean allowBinding, boolean requireExactType) {
    for (int i = 0; i < typePatterns.length; i++) {
      final TypePattern p = typePatterns[i];
      if (p != null) {
  ...
  public TypePatternList resolveReferences(IntMap bindings) {
    ...
    final TypePattern[] ret = new TypePattern[len];
    for (int i = 0; i < len; i++) {
  ...
  public boolean equals(Object other) {
    ...
    final TypePatternList o = (TypePatternList) other;
  ...
  public static TypePatternList read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    ...
    final TypePattern[] arguments = new TypePattern[len];
    for (int i = 0; i < len; i++) {
      arguments[i] = TypePattern.read(s, context);
    }
    final TypePatternList ret = new TypePatternList(arguments);
    if (!s.isAtLeast169()) {
  ...
  public List<UnresolvedType> getExactTypes() {
    final List<UnresolvedType> ret = new ArrayList<UnresolvedType>();
    for (int i = 0; i < typePatterns.length; i++) {
      final UnresolvedType t = typePatterns[i].getExactType();
  ...
  public boolean areAllExactWithNoSubtypesAllowed() {
    for (int i = 0; i < typePatterns.length; i++) {
      final TypePattern array_element = typePatterns[i];
      if (!(array_element instanceof ExactTypePattern)) {
        return false;
      } else {
        final ExactTypePattern etp = (ExactTypePattern) array_element;
  ...
  public String[] maybeGetCleanNames() {
    final String[] theParamNames = new String[typePatterns.length];
    for (int i = 0; i < typePatterns.length; i++) {
      final TypePattern string = typePatterns[i];
