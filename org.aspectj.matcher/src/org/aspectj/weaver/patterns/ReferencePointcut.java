...
public class ReferencePointcut extends Pointcut {
  ...
  public static Pointcut read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    ...
    final ReferencePointcut ret = new ReferencePointcut(onType, s.readUTF(), TypePatternList.read(s, context));
  ...
  public void resolveBindings(IScope scope, Bindings bindings) {
    ...
    if (pointcutDef == null && onType == null) {
      while (true) {
        final UnresolvedType declaringType = searchType.getDeclaringType();
    ...
    final ResolvedType[] parameterTypes = scope.getWorld().resolve(pointcutDef.getParameterTypes());
    ...
    if (onType != null) {
      if (onType.isParameterizedType()) {
        ...
        final ResolvedType underlyingGenericType = ((ResolvedType) onType).getGenericType();
        final TypeVariable[] tVars = underlyingGenericType.getTypeVariables();
        final ResolvedType[] typeParams = ((ResolvedType) onType).getResolvedTypeParameters();
    ...
    for (int i = 0, len = arguments.size(); i < len; i++) {
      ...
      if (parameterTypes[i].isTypeVariableReference() && p.getExactType().isTypeVariableReference()) {
        final UnresolvedType One = ((TypeVariableReference) parameterTypes[i]).getTypeVariable().getFirstBound();
        final UnresolvedType Two = ((TypeVariableReference) p.getExactType()).getTypeVariable().getFirstBound();
  ...
  public Pointcut concretize1(ResolvedType searchStart, ResolvedType declaringType, IntMap bindings) {
    if (concretizing) {
      ...
      final Pointcut p = Pointcut.makeMatchesNothing(Pointcut.CONCRETE);
    ...
    try {
      ...
      if (onType != null) {
        ...
        if (onType.isTypeVariableReference()) {
          ...
          if (declaringType.isParameterizedType()) {
            final TypeVariable[] tvs = declaringType.getGenericType().getTypeVariables();
            final String typeVariableName = ((TypeVariableReference) onType).getTypeVariable().getName();
            for (int i = 0; i < tvs.length; i++) {
              if (tvs[i].getName().equals(typeVariableName)) {
                final ResolvedType realOnType = declaringType.getTypeParameters()[i].resolve(declaringType.getWorld());
                onType = realOnType;
      ...
      final boolean foundMatchingPointcut = (pointcutDec != null && Modifier.isPrivate(pointcutDec.getModifiers()));
      ...
      if (pointcutDec.isAbstract()) {
        // Thread.currentThread().dumpStack();
        final ShadowMunger enclosingAdvice = bindings.getEnclosingAdvice();
      ...
      final TypePatternList arguments = this.arguments.resolveReferences(bindings);

      final IntMap newBindings = new IntMap();
      for (int i = 0, len = arguments.size(); i < len; i++) {
        final TypePattern p = arguments.get(i);
        if (p == TypePattern.NO) {
          continue;
        }
        // we are allowed to bind to pointcuts which use subtypes as this is type safe
      ...
      if (searchStart.isParameterizedType()) {
        ...
        // the type parameters presented
        typeVariableMap = new HashMap<String, UnresolvedType>();
        final ResolvedType underlyingGenericType = searchStart.getGenericType();
        final TypeVariable[] tVars = underlyingGenericType.getTypeVariables();
        final ResolvedType[] typeParams = searchStart.getResolvedTypeParameters();
        for (int i = 0; i < tVars.length; i++) {
          typeVariableMap.put(tVars[i].getName(), typeParams[i]);
        }
      }
  ...
  public Pointcut parameterizeWith(Map<String, UnresolvedType> typeVariableMap, World w) {
    final ReferencePointcut ret = new ReferencePointcut(onType, name, arguments);
