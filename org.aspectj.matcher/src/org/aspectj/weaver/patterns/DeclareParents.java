...
public class DeclareParents extends Declare {
  ...
  public Declare parameterizeWith(Map typeVariableBindingMap, World w) {
    final DeclareParents ret = new DeclareParents(child.parameterizeWith(typeVariableBindingMap, w), parents.parameterizeWith(
  ...
  public boolean equals(Object other) {
    ...
    final DeclareParents o = (DeclareParents) other;
  ...
  public static Declare read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    final DeclareParents ret = new DeclareParents(TypePattern.read(s, context), TypePatternList.read(s, context), true);
  ...
  private ResolvedType maybeGetNewParent(ResolvedType targetType, TypePattern typePattern, World world, boolean reportErrors) {
    ...
    // isWildChild = (child instanceof WildTypePattern);
    final UnresolvedType iType = typePattern.getExactType();
    final ResolvedType parentType = iType.resolve(world);
    ...
    if (parentType.isParameterizedType() || parentType.isRawType()) {
      ...
      final boolean isOK = verifyNoInheritedAlternateParameterization(targetType, parentType, world);
      if (!isOK) {
        return null;
      }
    }
  ...
  private boolean verifyNoInheritedAlternateParameterization(ResolvedType typeToVerify, ResolvedType newParent, World world) {
    ...
    final ResolvedType newParentGenericType = newParent.getGenericType();
    final Iterator iter = typeToVerify.getDirectSupertypes();
    while (iter.hasNext()) {
      final ResolvedType supertype = (ResolvedType) iter.next();
      ...
      if (supertype.isParameterizedType()) {
        final ResolvedType generictype = supertype.getGenericType();
  ...
  public List<ResolvedType> findMatchingNewParents(ResolvedType onType, boolean reportErrors) {
    ...
    final List<ResolvedType> ret = new ArrayList<ResolvedType>();
    for (int i = 0; i < parents.size(); i++) {
      final ResolvedType t = maybeGetNewParent(onType, parents.get(i), onType.getWorld(), reportErrors);
      if (t != null) {
        ret.add(t);
      }
    }
