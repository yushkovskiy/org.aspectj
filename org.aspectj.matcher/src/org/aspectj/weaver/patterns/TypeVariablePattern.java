...
public class TypeVariablePattern extends PatternNode {
  ...
  private final String name; // eg. "T"
  private TypePattern upperBound; // default is object unless of the form T extends Bar
  private final TypePattern[] interfaceBounds; // additional upper bounds (must be interfaces) arising from
  // declarations of the form T extends Bar & IGoo, IDoo
  private final TypePattern lowerBound; // only set if type variable is of the form T super Bar
  ...
  public boolean equals(Object obj) {
    ...
    final TypeVariablePattern other = (TypeVariablePattern) obj;
  ...
  private String getExtendsClause() {
    if (upperBound instanceof ExactTypePattern) {
      final ExactTypePattern bound = (ExactTypePattern) upperBound;
  ...
  public static TypeVariablePattern read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    ...
    final TypePattern upperBound = TypePattern.read(s, context);
    TypePattern[] additionalInterfaceBounds = null;
    final int numInterfaceBounds = s.readInt();
    ...
    final boolean hasLowerBound = s.readBoolean();
