...
public class SignaturePattern extends PatternNode implements ISignaturePattern {
  ...
  private void checkForIncorrectTargetKind(PatternNode patternNode, IScope scope, boolean targetsOtherThanTypeAllowed,
    ...
    if (patternNode instanceof ExactAnnotationTypePattern) {
      final ResolvedType resolvedType = ((ExactAnnotationTypePattern) patternNode).getAnnotationType().resolve(scope.getWorld());
      if (targetsOtherThanTypeAllowed) {
        final AnnotationTargetKind[] targetKinds = resolvedType.getAnnotationTargetKinds();
        if (targetKinds == null) {
          return;
        }
      ...
      final TypePatternVisitor visitor = new TypePatternVisitor(scope, targetsOtherThanTypeAllowed,
          parameterTargettingAnnotationsAllowed);
      patternNode.traverse(visitor, null);
      if (visitor.containedIncorrectTargetKind()) {
        final Set<ExactAnnotationTypePattern> keys = visitor.getIncorrectTargetKinds().keySet();
        for (final Iterator<ExactAnnotationTypePattern> iter = keys.iterator(); iter.hasNext(); ) {
          final PatternNode node = iter.next();
          final AnnotationTargetKind[] targetKinds = visitor.getIncorrectTargetKinds().get(node);
  ...
  private void reportUnmatchedTargetKindMessage(AnnotationTargetKind[] annotationTargetKinds, PatternNode node, IScope scope,
    ...
    final StringBuffer targetNames = new StringBuffer("{");
    for (int i = 0; i < annotationTargetKinds.length; i++) {
      final AnnotationTargetKind targetKind = annotationTargetKinds[i];
  ...
  private class TypePatternVisitor extends AbstractPatternNodeVisitor {
    ...
    private final Map<ExactAnnotationTypePattern, AnnotationTargetKind[]> incorrectTargetKinds = new HashMap<ExactAnnotationTypePattern, AnnotationTargetKind[]>();
    ...
    public Object visit(ExactAnnotationTypePattern node, Object data) {
      final ResolvedType resolvedType = node.getAnnotationType().resolve(scope.getWorld());
      if (targetsOtherThanTypeAllowed) {
        final AnnotationTargetKind[] targetKinds = resolvedType.getAnnotationTargetKinds();
        if (targetKinds == null) {
          return data;
        }
        final List<AnnotationTargetKind> incorrectTargets = new ArrayList<AnnotationTargetKind>();
        ...
        final AnnotationTargetKind[] kinds = new AnnotationTargetKind[incorrectTargets.size()];
        ...
        if (parameterTargettingAnnotationsAllowed) {
          for (int i = 0; i < targetKinds.length; i++) {
            final AnnotationTargetKind annotationTargetKind = targetKinds[i];
    ...
    public Object visit(ExactTypePattern node, Object data) {
      final ExactAnnotationTypePattern eatp = new ExactAnnotationTypePattern(node.getExactType().resolve(scope.getWorld()), null);
  ...
  public SignaturePattern parameterizeWith(Map<String, UnresolvedType> typeVariableMap, World w) {
    final SignaturePattern ret = new SignaturePattern(kind, modifiers, returnType.parameterizeWith(typeVariableMap, w), declaringType
  ...
  public boolean matches(Member joinPointSignature, World world, boolean allowBridgeMethods) {
    ...
    final boolean wantsAnnotationMatch = wantToMatchAnnotationPattern();
    final JoinPointSignatureIterator candidateMatches = joinPointSignature.getJoinPointSignatures(world);
    while (candidateMatches.hasNext()) {
      final JoinPointSignature aSig = candidateMatches.next();
      // System.out.println(aSig);
      final FuzzyBoolean matchResult = matchesExactly(aSig, world, allowBridgeMethods, subjectMatch);
  ...
  private FuzzyBoolean matchesExactlyField(JoinPointSignature aField, World world) {
    ...
    final ResolvedType fieldDeclaringType = aField.getDeclaringType().resolve(world);
  ...
  private boolean parametersCannotMatch(JoinPointSignature methodJoinpoint) {
    ...
    final int patternParameterCount = parameterTypes.size();

    if (patternParameterCount == 0 || parameterTypes.ellipsisCount == 0) {
      final boolean equalCount = patternParameterCount == methodJoinpoint.getParameterTypes().length;
  ...
  private FuzzyBoolean matchesExactlyMethod(JoinPointSignature aMethod, World world, boolean subjectMatch) {
    ...
    if (!returnType.isStar()) {
      final boolean b = returnType.isBangVoid();
      if (b) {
        final String s = aMethod.getReturnType().getSignature();
    ...
    final ResolvableTypeList rtl = new ResolvableTypeList(world, aMethod.getParameterTypes());
  ...
  private boolean isMatchingParameterAnnotations() {
    if ((bits & CHECKED_FOR_PARAMETER_ANNOTATION_MATCHING) == 0) {
      ...
      for (int tp = 0, max = parameterTypes.size(); tp < max; tp++) {
        final TypePattern typePattern = parameterTypes.get(tp);
  ...
  private boolean isParameterAnnotationMatching(TypePattern tp) {
    if (tp instanceof OrTypePattern) {
      final OrTypePattern orAtp = (OrTypePattern) tp;
      ...
      final AndTypePattern andAtp = (AndTypePattern) tp;
      ...
      final NotTypePattern notAtp = (NotTypePattern) tp;
      return (isParameterAnnotationMatching(notAtp.getNegatedPattern()));
    } else {
      final AnnotationTypePattern atp = tp.getAnnotationPattern();
  ...
  private static boolean isParameterAnnotationMatching(AnnotationTypePattern tp) {
    if (tp instanceof OrAnnotationTypePattern) {
      ...
      final AndAnnotationTypePattern andAtp = (AndAnnotationTypePattern) tp;
      return (isParameterAnnotationMatching(andAtp.getLeft()) || isParameterAnnotationMatching(andAtp.getRight()));
    } else if (tp instanceof NotAnnotationTypePattern) {
      final NotAnnotationTypePattern notAtp = (NotAnnotationTypePattern) tp;
  ...
  private FuzzyBoolean matchesExactlyConstructor(JoinPointSignature aConstructor, World world) {
    ...
    final ResolvedType[] resolvedParameters = world.resolve(aConstructor.getParameterTypes());
  ...
  private boolean matchesVarArgs(JoinPointSignature aMethodOrConstructor, World inAWorld) {
    ...
    final TypePattern lastPattern = parameterTypes.get(parameterTypes.size() - 1);
    final boolean canMatchVarArgsSignature = lastPattern.isStar() || lastPattern.isVarArgs() || (lastPattern == TypePattern.ELLIPSIS);
  ...
  private FuzzyBoolean matchesAnnotations(ResolvedMember member, World world) {
    ...
    if (member.isAnnotatedElsewhere() && member.getKind() == Member.FIELD) {
      ...
      final List<ConcreteTypeMunger> mungers = member.getDeclaringType().resolve(world).getInterTypeMungers();
  ...
  public boolean declaringTypeMatchAllowingForCovariance(Member member, UnresolvedType shadowDeclaringType, World world,
    ...
    final ResolvedType onType = shadowDeclaringType.resolve(world);
    ...
    final Collection<ResolvedType> declaringTypes = member.getDeclaringTypes(world);

    final boolean checkReturnType = true;
    ...
    for (ResolvedType type : declaringTypes) {
      if (declaringType.matchesStatically(type)) {
        ...
        final UnresolvedType returnTypeX = rm.getReturnType();
        final ResolvedType returnType = returnTypeX.resolve(world);
  ...
  public boolean equals(Object other) {
    ...
    final SignaturePattern o = (SignaturePattern) other;
  ...
  public static SignaturePattern read(VersionedDataInputStream s, ISourceContext context) throws IOException {
    ...
    final MemberKind kind = MemberKind.read(s);
    final ModifiersPattern modifiers = ModifiersPattern.read(s);
    final TypePattern returnType = TypePattern.read(s, context);
    final TypePattern declaringType = TypePattern.read(s, context);
    final NamePattern name = NamePattern.read(s);
    final TypePatternList parameterTypes = TypePatternList.read(s, context);
    final ThrowsPattern throwsPattern = ThrowsPattern.read(s, context);
  ...
  public List<ExactTypePattern> getExactDeclaringTypes() {
    if (declaringType instanceof ExactTypePattern) {
      final List<ExactTypePattern> l = new ArrayList<ExactTypePattern>();
