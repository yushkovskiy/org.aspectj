...
public class TypeVariable {
  ...
  public TypeVariable resolve(World world) {
    ...
    if (declaringElement != null) {
      ...
      if (declaringElementKind == TYPE) {
        final UnresolvedType declaring = (UnresolvedType) declaringElement;
        final ReferenceType rd = (ReferenceType) declaring.resolve(world);
        final TypeVariable[] tVars = rd.getTypeVariables();
        ...
        final ResolvedMember declaring = (ResolvedMember) declaringElement;
        final TypeVariable[] tvrts = declaring.getTypeVariables();
    ...
    if (superclass != null) {
      final ResolvedType rt = superclass.resolve(world);
  ...
  public String getSignature() {
    ...
    if (superInterfaces.length != 0) {
      for (int i = 0; i < superInterfaces.length; i++) {
        sb.append(":");
        final UnresolvedType iBound = superInterfaces[i];
  ...
  public String getSignatureForAttribute() {
    ...
    if (superInterfaces.length != 0) {
      for (int i = 0; i < superInterfaces.length; i++) {
        sb.append(":");
        final ResolvedType iBound = (ResolvedType) superInterfaces[i];
  ...
  public static TypeVariable read(VersionedDataInputStream s) throws IOException {
    ...
    final UnresolvedType ubound = UnresolvedType.read(s);
    final int iboundcount = s.readInt();
    ...
    final TypeVariable newVariable = new TypeVariable(name, ubound, ibounds);
    return newVariable;
  }
