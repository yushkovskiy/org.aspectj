...
public class CrosscuttingMembers {
  ...
  public void addDeclare(Declare declare) {
    ...
    if (declare instanceof DeclareErrorOrWarning) {
      final ShadowMunger m = new Checker((DeclareErrorOrWarning) declare);
      ...
      final DeclareSoft d = (DeclareSoft) declare;
      // Ordered so that during concretization we can check the related
      // munger
      final ShadowMunger m = Advice.makeSoftener(world, d.getPointcut(), d.getException(), inAspect, d);
      m.setDeclaringType(d.getDeclaringType());
      final Pointcut concretePointcut = d.getPointcut().concretize(inAspect, d.getDeclaringType(), 0, m);
  ...
  public void exposeType(UnresolvedType typeToExpose) {
    ...
    final String signatureToLookFor = typeToExpose.getSignature();
    for (final Iterator<ConcreteTypeMunger> iterator = typeMungers.iterator(); iterator.hasNext(); ) {
      final ConcreteTypeMunger cTM = iterator.next();
      final ResolvedTypeMunger rTM = cTM.getMunger();
      if (rTM != null && rTM instanceof ExposeTypeMunger) {
        final String exposedType = ((ExposeTypeMunger) rTM).getExposedTypeSignature();
  ...
  public void addPrivilegedAccesses(Collection<ResolvedMember> accessedMembers) {
    final int version = inAspect.getCompilerVersion();
    for (ResolvedMember member : accessedMembers) {
      ...
      if (resolvedMember == null) {
        ...
        final UnresolvedType unresolvedDeclaringType = member.getDeclaringType().getRawType();
        final UnresolvedType resolvedDeclaringType = resolvedMember.getDeclaringType().getRawType();
        if (!unresolvedDeclaringType.equals(resolvedDeclaringType)) {
          resolvedMember = member;
        }
      }
      final PrivilegedAccessMunger privilegedAccessMunger = new PrivilegedAccessMunger(resolvedMember,
          version >= WeaverVersionInfo.WEAVER_VERSION_AJ169);
      final ConcreteTypeMunger concreteTypeMunger = world.getWeavingSupport().concreteTypeMunger(privilegedAccessMunger, inAspect);
  ...
  public boolean replaceWith(CrosscuttingMembers other, boolean careAboutShadowMungers) {
    ...
    if (careAboutShadowMungers) {
      ...
      final Set<ShadowMunger> theseShadowMungers = new HashSet<ShadowMunger>();
      final Set<ShadowMunger> theseInlinedAroundMungers = new HashSet<ShadowMunger>();
      for (ShadowMunger munger : shadowMungers) {
        if (munger instanceof Advice) {
          final Advice adviceMunger = (Advice) munger;
      ...
      final Set<ShadowMunger> tempSet = new HashSet<ShadowMunger>();
      tempSet.addAll(other.shadowMungers);
      final Set<ShadowMunger> otherShadowMungers = new HashSet<ShadowMunger>();
      final Set<ShadowMunger> otherInlinedAroundMungers = new HashSet<ShadowMunger>();
      ...
      if (!changed) {
        for (ShadowMunger munger : shadowMungers) {
          final int i = other.shadowMungers.indexOf(munger);
          final ShadowMunger otherMunger = other.shadowMungers.get(i);
          if (munger instanceof Advice) {
    ...
    // of shadowMungers
    final Set<Object> theseTypeMungers = new HashSet<Object>();
    final Set<Object> otherTypeMungers = new HashSet<Object>();
    if (!careAboutShadowMungers) {
      for (final Iterator<ConcreteTypeMunger> iter = typeMungers.iterator(); iter.hasNext(); ) {
        final Object o = iter.next();
        if (o instanceof ConcreteTypeMunger) {
          final ConcreteTypeMunger typeMunger = (ConcreteTypeMunger) o;
      ...
      for (final Iterator<ConcreteTypeMunger> iter = other.typeMungers.iterator(); iter.hasNext(); ) {
        final Object o = iter.next();
        if (o instanceof ConcreteTypeMunger) {
          final ConcreteTypeMunger typeMunger = (ConcreteTypeMunger) o;
          if (!typeMunger.existsToSupportShadowMunging()) {
    ...
    if (theseTypeMungers.size() != otherTypeMungers.size()) {
      ...
      for (final Iterator<Object> iter = theseTypeMungers.iterator(); iter.hasNext() && !foundInequality; ) {
        final Object thisOne = iter.next();
    ...
    if (!declareParents.equals(other.declareParents)) {
      ...
      if (!careAboutShadowMungers) {
        ...
        final Set<DeclareParents> trimmedThis = new HashSet<DeclareParents>();
        for (final Iterator<DeclareParents> iterator = declareParents.iterator(); iterator.hasNext(); ) {
          final DeclareParents decp = iterator.next();
          if (!decp.isMixin()) {
            trimmedThis.add(decp);
          }
        }
        final Set<DeclareParents> trimmedOther = new HashSet<DeclareParents>();
        for (final Iterator<DeclareParents> iterator = other.declareParents.iterator(); iterator.hasNext(); ) {
          final DeclareParents decp = iterator.next();
          if (!decp.isMixin()) {
  ...
  private boolean equivalent(Set<ShadowMunger> theseInlinedAroundMungers, Set<ShadowMunger> otherInlinedAroundMungers) {
    ...
    for (final Iterator<ShadowMunger> iter = theseInlinedAroundMungers.iterator(); iter.hasNext(); ) {
      final Advice thisAdvice = (Advice) iter.next();
      boolean foundIt = false;
      for (final Iterator<ShadowMunger> iterator = otherInlinedAroundMungers.iterator(); iterator.hasNext(); ) {
        final Advice otherAdvice = (Advice) iterator.next();
