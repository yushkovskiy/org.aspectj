...
public class WithinTestCase extends PatternsTestCase {
  ...
  public void testMatch() throws IOException {
    final Shadow getOutFromArrayList = new TestShadow(Shadow.FieldGet, TestUtils
    ...
    final Shadow getOutFromEntry = new TestShadow(Shadow.FieldGet, TestUtils
  ...
  public Pointcut makePointcut(String pattern) {
    final Pointcut pointcut0 = Pointcut.fromString(pattern);

    final Bindings bindingTable = new Bindings(0);
    final IScope scope = new SimpleScope(world, FormalBinding.NONE);

    pointcut0.resolveBindings(scope, bindingTable);
    final Pointcut pointcut1 = pointcut0;
  ...
  private void checkMatch(Pointcut p, Shadow s, FuzzyBoolean shouldMatch) throws IOException {
    final FuzzyBoolean doesMatch = p.match(s);
