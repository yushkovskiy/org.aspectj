...
public class TypePatternListTestCase extends PatternsTestCase {
  ...
  private void checkStaticMatch(String pattern, String[] names, FuzzyBoolean shouldMatchStatically) {
    ...
    final TypePatternList p = makeArgumentsPattern(pattern);
    final ResolvedType[] types = new ResolvedType[names.length];
    ...
    final FuzzyBoolean result = p.matches(types, TypePattern.STATIC);
    final String msg = "matches statically " + pattern + " to " + Arrays.asList(types);
  ...
  private TestScope makeTestScope() {
    final TestScope scope = new TestScope(NO_STRINGS, NO_STRINGS, world);
    return scope;
  }
  ...
  public void stupidCheck(String pattern, boolean[] matches) {
    ...
    for (int j = 0; j < len; j++) {
      ...
      final ResolvedType[] types = new ResolvedType[j];
      for (int i = 0; i < j; i++) {
  ...
  public void testSerialization() throws IOException {
    final String[] patterns = new String[]{"( ..,  ..,  .., int)", "( ..,  .., int,  ..)", "( ..,  .., int, int)",
  ...
  private void checkSerialization(String string) throws IOException {
    final TypePatternList p = makeArgumentsPattern(string);
    ...
    final TypePatternList newP = TypePatternList.read(in, null);
