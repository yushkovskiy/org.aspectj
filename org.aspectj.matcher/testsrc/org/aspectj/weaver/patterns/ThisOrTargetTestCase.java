...
public class ThisOrTargetTestCase extends TestCase {
  ...
  public void testMatchJP() throws Exception {
    ...
    final PointcutExpression thisEx = parser.parsePointcutExpression("this(Exception)");
    final PointcutExpression thisIOEx = parser.parsePointcutExpression("this(java.io.IOException)");

    final PointcutExpression targetEx = parser.parsePointcutExpression("target(Exception)");
    final PointcutExpression targetIOEx = parser.parsePointcutExpression("target(java.io.IOException)");
  ...
  public void testBinding() throws Exception {
    ...
    final PointcutParser parser = PointcutParser
        .getPointcutParserSupportingAllPrimitivesAndUsingSpecifiedClassloaderForResolution(this.getClass().getClassLoader());
    final PointcutParameter ex = parser.createPointcutParameter("ex", Exception.class);
    final PointcutParameter ioEx = parser.createPointcutParameter("ioEx", IOException.class);

    final PointcutExpression thisEx = parser.parsePointcutExpression("this(ex)", Exception.class, new PointcutParameter[]{ex});

    final PointcutExpression targetIOEx = parser.parsePointcutExpression("target(ioEx)", Exception.class,
        new PointcutParameter[]{ioEx});

    final Method toString = Object.class.getMethod("toString", new Class[0]);

    ShadowMatch sMatch = thisEx.matchesMethodCall(toString, toString);
    final Exception exceptionParameter = new Exception();
    final IOException ioExceptionParameter = new IOException();
