...
public abstract class RuntimeParamAnnos extends Attribute {
  ...
  private final List<AnnotationGen[]> parameterAnnotations;
  ...
  private void inflate() {
    try {
      final DataInputStream dis = new DataInputStream(new ByteArrayInputStream(annotation_data));
      final int numParameters = dis.readUnsignedByte();
      for (int i = 0; i < numParameters; i++) {
        final int numAnnotations = dis.readUnsignedShort();
        final AnnotationGen[] annotations = new AnnotationGen[numAnnotations];
  ...
  protected void writeAnnotations(DataOutputStream dos) throws IOException {
    if (!inflated) {
      ...
      for (int i = 0; i < parameterAnnotations.size(); i++) {
        final AnnotationGen[] annotations = parameterAnnotations.get(i);
