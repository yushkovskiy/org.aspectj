...
public class TypeAnnotationGen {
  ...
  public static TypeAnnotationGen read(DataInputStream dis, ConstantPool cpool, boolean isVisible) throws IOException {
    final TypeAnnotationGen typeAnno = new TypeAnnotationGen(cpool);
    ...
    switch (typeAnno.targetType) {
      ...
      case METHOD_TYPE_PARAMETER:
        ...
        break;
      case CLASS_EXTENDS:
        final int superTypeIndex = dis.readUnsignedShort();
    ...
    final int typepathlength = dis.readUnsignedByte();
    if (typepathlength == 0) {
  ...
  public static int[] readLocalVarTarget(DataInputStream dis) throws IOException {
    final int tableLength = dis.readUnsignedShort();
    final int[] table = new int[tableLength * 3];
    int count = 0;
