...
public class TypeAnnotationsTest extends BcelTestCase {
  ...
  public void testClassTypeParameter() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnClassTypeParameter");
    final RuntimeVisTypeAnnos rvta = (RuntimeVisTypeAnnos) getAttribute(jc.getAttributes(), Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS);
    assertTrue(rvta.areVisible());
    final TypeAnnotationGen[] tas = rvta.getTypeAnnotations();
  ...
  public void testMethodTypeParameter() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnMethodTypeParameter");
    final Method m = getMethod(jc, "m");
    final RuntimeVisTypeAnnos rvta = (RuntimeVisTypeAnnos) getAttribute(m.getAttributes(), Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS);
    assertTrue(rvta.areVisible());
    final TypeAnnotationGen[] tas = rvta.getTypeAnnotations();
  ...
  public void testSuperinterface() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnSuperinterface1");
    final RuntimeVisTypeAnnos rvta = (RuntimeVisTypeAnnos) getAttribute(jc.getAttributes(), Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS);
    assertTrue(rvta.areVisible());
    final TypeAnnotationGen[] tas = rvta.getTypeAnnotations();
    assertEquals(1, tas.length);
    final TypeAnnotationGen ta = tas[0];
    checkTypeAnnotationClassExtends(ta, 0, "@Anno");
  }

  public void testSupertypes() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnSupertypes");
    final RuntimeVisTypeAnnos rvta = (RuntimeVisTypeAnnos) getAttribute(jc.getAttributes(), Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS);
    assertTrue(rvta.areVisible());
    final TypeAnnotationGen[] tas = rvta.getTypeAnnotations();
  ...
  public void testClassTypeParameterBound() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnClassTypeParameterBound");
    final RuntimeVisTypeAnnos rvta = (RuntimeVisTypeAnnos) getAttribute(jc.getAttributes(), Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS);
    assertTrue(rvta.areVisible());
    final TypeAnnotationGen[] tas = rvta.getTypeAnnotations();
    assertEquals(3, tas.length);
  ...
  public void testMethodTypeParameterBound() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnMethodTypeParameterBound");
    final Method m = getMethod(jc, "m");
    final RuntimeVisTypeAnnos rvta = (RuntimeVisTypeAnnos) getAttribute(m.getAttributes(), Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS);
    assertTrue(rvta.areVisible());
    final TypeAnnotationGen[] tas = rvta.getTypeAnnotations();
  ...
  public void testField() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnField");
    final Field f = getField(jc, "f1");
    final RuntimeVisTypeAnnos rvta = (RuntimeVisTypeAnnos) getAttribute(f.getAttributes(), Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS);
    assertTrue(rvta.areVisible());
  ...
  public void testMethodReturn() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnMethodReturn");
    final TypeAnnotationGen[] tas = getTypeAnnotations(getMethod(jc, "m"), true);
    checkTypeAnnotationMethodReturn(tas[0], "@Anno");
    checkTypePath(tas[0], TypeAnnotationGen.NO_TYPE_PATH);
  }

  public void testMethodReceiver() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnMethodReceiver");
    final TypeAnnotationGen[] tas = getTypeAnnotations(getMethod(jc, "m"), true);
  ...
  public void testMethodFormalParameter() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnMethodFormalParameter");
    final TypeAnnotationGen[] tas = getTypeAnnotations(getMethod(jc, "m"), true);
  ...
  public void testThrows() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnThrows");
    final TypeAnnotationGen[] tas = getTypeAnnotations(getMethod(jc, "m"), true);
  ...
  public void testLocalVariable() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnLocalVariable");
    // TODO I think the attribute should be on the code for the method, not the method
    final TypeAnnotationGen[] tas = getTypeAnnotations(getMethod(jc, "m").getAttributes(), true);
    assertEquals(1, tas.length);
    checkTypeAnnotationLocalVariable(tas[0], new int[]{11, 8, 1}, "@Anno");
  }

  public void testResourceVariable() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnResourceVariable");
    // TODO I think the attribute should be on the code for the method, not the method
    final TypeAnnotationGen[] tas = getTypeAnnotations(getMethod(jc, "m").getAttributes(), true);
    assertEquals(2, tas.length);
  ...
  public void testExceptionParameter() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnExceptionParameter");
    // TODO I think the attribute should be on the code for the method, not the method
    final TypeAnnotationGen[] tas = getTypeAnnotations(getMethod(jc, "m").getAttributes(), true);
    assertEquals(2, tas.length);
  ...
  public void testInstanceOf() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnInstanceOf");
    // TODO I think the attribute should be on the code for the method, not the method
    final TypeAnnotationGen[] tas = getTypeAnnotations(getMethod(jc, "m").getAttributes(), true);
    assertEquals(2, tas.length);
  ...
  public void testNew() throws Exception {
    final JavaClass jc = getClassFromJava8Jar("TypeAnnoOnNew");
    // TODO I think the attribute should be on the code for the method, not the method
    final TypeAnnotationGen[] tas = getTypeAnnotations(getMethod(jc, "m").getAttributes(), true);
  ...
  private TypeAnnotationGen[] getTypeAnnotations(Attribute[] attrs, boolean visible) {
    final RuntimeTypeAnnos rvta = (RuntimeTypeAnnos) getAttribute(attrs, visible ? Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS : Constants.ATTR_RUNTIME_INVISIBLE_TYPE_ANNOTATIONS);
    return rvta.getTypeAnnotations();
  }

  private TypeAnnotationGen[] getTypeAnnotations(Method m, boolean visible) {
    final RuntimeTypeAnnos rvta = (RuntimeTypeAnnos) getAttribute(m.getAttributes(), visible ? Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS : Constants.ATTR_RUNTIME_INVISIBLE_TYPE_ANNOTATIONS);
    return rvta.getTypeAnnotations();
  }

  private TypeAnnotationGen[] getTypeAnnotations(Field f, boolean visible) {
    final RuntimeTypeAnnos rvta = (RuntimeTypeAnnos) getAttribute(f.getAttributes(), visible ? Constants.ATTR_RUNTIME_VISIBLE_TYPE_ANNOTATIONS : Constants.ATTR_RUNTIME_INVISIBLE_TYPE_ANNOTATIONS);
    return rvta.getTypeAnnotations();
  }
