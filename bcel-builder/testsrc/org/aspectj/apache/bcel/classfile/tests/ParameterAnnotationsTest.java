...
public class ParameterAnnotationsTest extends BcelTestCase {
  ...
  public void testParameterAnnotations_builtOK() {
    final ClassGen clg = createClassGen("HelloWorld");
    final ConstantPool cpg = clg.getConstantPool();
    final InstructionList il = new InstructionList();

    buildClassContentsWithAnnotatedMethods(clg, cpg, il, true);

    final int i = clg.getMethods().length;
    assertTrue("Class should have 2 methods but has " + i, i == 2);

    final Method mainMethod = clg.getMethods()[0];
    final AnnotationGen[] annos = mainMethod.getAnnotationsOnParameter(0);
  ...
  public void testParameterAnnotations_savedAndLoadedOK() throws ClassNotFoundException {
    ...
    final JavaClass jc = getClassFrom("temp5", "HelloWorld");
  ...
  public void testParameterAnnotations_loadedThenModifiedThenSavedAndLoadedOK() throws ClassNotFoundException {
    final JavaClass jc = getClassFrom("testcode.jar", "AnnotatedParameters");

    final ClassGen clg = new ClassGen(jc);
    final ConstantPool cpg = clg.getConstantPool();
    ...
    final MethodGen mg = new MethodGen(m, clg.getClassName(), cpg);
    ...
    final JavaClass jc2 = getClassFrom("temp2", "AnnotatedParameters");

    m = jc2.getMethods()[2];
    final AnnotationGen[] p1annotations = m.getAnnotationsOnParameter(0);
    final AnnotationGen[] p2annotations = m.getAnnotationsOnParameter(1);
  ...
  public void testParameterAnnotations_loadedThenModifiedWithInvisibleAnnotationThenSavedAndLoadedOK() throws ClassNotFoundException {
    ...
    final JavaClass jc2 = getClassFrom("temp3", "AnnotatedParameters");
