...
public class DefaultCacheKeyResolver implements CacheKeyResolver {
  ...
  public String createClassLoaderScope(ClassLoader cl, List<String> aspects) {
    ...
    final List<String> hashableStrings = new LinkedList<String>();
    final StringBuilder hashable = new StringBuilder(256);
    ...
    for (final Iterator<String> it = hashableStrings.iterator(); it.hasNext(); ) {
      final String url = it.next();
      hashable.append(url);
    }
    String hash = null;
    final byte[] bytes = hashable.toString().getBytes();
    hash = crc(bytes);
