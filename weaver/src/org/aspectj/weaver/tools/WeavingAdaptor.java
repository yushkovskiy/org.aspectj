...
public class WeavingAdaptor implements IMessageContext {
  ...
  public void addURL(URL url) {
    final File libFile = new File(url.getPath());
    try {
  ...
  public byte[] weaveClass(String name, byte[] bytes, boolean mustWeave) throws IOException {
    ...
    try {
      ...
      final boolean debugOn = !messageHandler.isIgnoring(Message.DEBUG);
        ...
        if (couldWeave(name, bytes)) {
          if (accept(name, bytes)) {
            ...
            if (cache != null && !mustWeave) {
              ...
              final CachedClassEntry entry = cache.get(cacheKey, original_bytes);
              if (entry != null) {
  ...
  protected void setMessageHandler(IMessageHandler mh) {
    if (mh instanceof ISupportsMessageContext) {
      final ISupportsMessageContext smc = (ISupportsMessageContext) mh;
  ...
  protected void ensureDelegateInitialized(String name, byte[] bytes) {
    if (delegateForCurrentClass == null) {
      final BcelWorld world = (BcelWorld) weaver.getWorld();
  ...
  protected void dump(String name, byte[] b, boolean before) {
    ...
    dir.mkdirs();
    final String fileName = dirName + File.separator + className + ".class";
    try {
  ...
  private List<String> getFullAspectPath(ClassLoader loader) {
    ...
    for (; loader != null; loader = loader.getParent()) {
      if (loader instanceof WeavingClassLoader) {
        final URL[] urls = ((WeavingClassLoader) loader).getAspectURLs();
        list.addAll(0, FileUtil.makeClasspath(urls));
      }
    }
    return list;
  ...
  private boolean shouldWeaveName(String name) {
    if ("osj".indexOf(name.charAt(0)) != -1) {
      if ((weavingSpecialTypes & INITIALIZED) == 0) {
        ...
        // initialize it
        final Properties p = weaver.getWorld().getExtraConfiguration();
        if (p != null) {
  ...
  private byte[] getWovenBytes(String name, byte[] bytes) throws IOException {
    final WeavingClassFileProvider wcp = new WeavingClassFileProvider(name, bytes);
    weaver.weave(wcp);
    return wcp.getBytes();
  }
  ...
  private void registerAspectLibraries(List aspectPath) {
    ...
    for (final Iterator i = aspectPath.iterator(); i.hasNext(); ) {
      final String libName = (String) i.next();
  ...
  private void addAspectLibrary(String aspectLibraryName) {
    final File aspectLibrary = new File(aspectLibraryName);
  ...
  private static List<String> makeClasspath(String cp) {
    ...
    if (cp != null) {
      final StringTokenizer tok = new StringTokenizer(cp, File.pathSeparator);
  ...
  protected class WeavingAdaptorMessageHolder extends MessageHandler {
    ...
    public List<IMessage> getUnmodifiableListView() {
      ...
      final List<IMessage> allMessages = new ArrayList<IMessage>();
        ...
        public void acceptResult(@NotNull IUnwovenClassFile result) {
          if (wovenClass == null) {
            ...
            if (SimpleCacheFactory.isEnabled()) {
              final SimpleCache lacache = SimpleCacheFactory.createSimpleCache();
