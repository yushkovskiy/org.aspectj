...
public class SimpleAOPParser {
  ...
  public static Definition parse(final URL url) throws Exception {
    ...
    final LightXMLParser xml = new LightXMLParser();
    xml.parseFromReader(new InputStreamReader(in));
    final SimpleAOPParser sap = new SimpleAOPParser();
    traverse(sap, xml);
  ...
  private void startElement(String qName, Map attrMap) throws Exception {
    if (ASPECT_ELEMENT.equals(qName)) {
      final String name = (String) attrMap.get(NAME_ATTRIBUTE);
      final String scopePattern = replaceXmlAnd((String) attrMap
          .get(SCOPE_ATTRIBUTE));
      final String requiredType = (String) attrMap.get(REQUIRES_ATTRIBUTE);
      if (!isNull(name)) {
      ...
      final String options = (String) attrMap.get(OPTIONS_ATTRIBUTE);
      if (!isNull(options)) {
      ...
      final String name = (String) attrMap.get(NAME_ATTRIBUTE);
      final String extend = (String) attrMap.get(EXTEND_ATTRIBUTE);
      final String precedence = (String) attrMap.get(PRECEDENCE_ATTRIBUTE);
      final String perclause = (String) attrMap.get(PERCLAUSE_ATTRIBUTE);
      if (!isNull(name)) {
      ...
      final String name = (String) attrMap.get(NAME_ATTRIBUTE);
      final String expression = (String) attrMap.get(EXPRESSION_ATTRIBUTE);
      ...
      final String typePattern = getWithinAttribute(attrMap);
      ...
      final String anno = (String) attrMap.get(ANNONATION_TAG);
      if (!isNull(anno)) {
      ...
      final String pointcut = (String) attrMap.get(POINTCUT_ELEMENT);
      final String adviceClass = (String) attrMap.get("invokeClass");
      final String adviceMethod = (String) attrMap.get("invokeMethod");
  ...
  private static void traverse(SimpleAOPParser sap, LightXMLParser xml)
    ...
    final ArrayList childrens = xml.getChildrens();
    for (int i = 0; i < childrens.size(); i++) {
      final LightXMLParser child = (LightXMLParser) childrens.get(i);
      traverse(sap, child);
    }
