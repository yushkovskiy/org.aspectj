...
public class DocumentParser extends DefaultHandler {
  ...
  private static XMLReader getXMLReader() throws SAXException, ParserConfigurationException {
    ...
    try {
      xmlReader = XMLReaderFactory.createXMLReader();
		...
		/* .. and ignore "System property ... not set" and then try this instead */ catch (SAXException ex) {
  ...
  public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
    if (ASPECT_ELEMENT.equals(qName)) {
      ...
      final String scopePattern = replaceXmlAnd(attributes.getValue(SCOPE_ATTRIBUTE));
      final String requiredType = attributes.getValue(REQUIRES_ATTRIBUTE);
      if (!isNull(name)) {
      ...
      final String options = attributes.getValue(OPTIONS_ATTRIBUTE);
      if (!isNull(options)) {
      ...
      final String name = attributes.getValue(NAME_ATTRIBUTE);
      final String extend = attributes.getValue(EXTEND_ATTRIBUTE);
      final String precedence = attributes.getValue(PRECEDENCE_ATTRIBUTE);
      final String perclause = attributes.getValue(PERCLAUSE_ATTRIBUTE);
      if (!isNull(name)) {
      ...
      final String name = attributes.getValue(NAME_ATTRIBUTE);
      final String expression = attributes.getValue(EXPRESSION_ATTRIBUTE);
      ...
      final String methodSig = attributes.getValue("method");
      final String fieldSig = attributes.getValue("field");
      final String typePat = attributes.getValue("type");
      final String anno = attributes.getValue("annotation");
      if (isNull(anno)) {
      ...
      final String pointcut = attributes.getValue(POINTCUT_ELEMENT);
      final String adviceClass = attributes.getValue("invokeClass");
      final String adviceMethod = attributes.getValue("invokeMethod");
      ...
      final String typePattern = getWithinAttribute(attributes);
      ...
      final String beforeAndAfter = attributes.getValue(DUMP_BEFOREANDAFTER_ATTRIBUTE);
      if (isTrue(beforeAndAfter)) {
        definition.setDumpBefore(true);
      }
      final String perWeaverDumpDir = attributes.getValue(DUMP_PERCLASSLOADERDIR_ATTRIBUTE);
