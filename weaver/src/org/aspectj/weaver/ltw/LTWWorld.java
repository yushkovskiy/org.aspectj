...
public class LTWWorld extends BcelWorld implements IReflectionWorld {
  ...
  protected ReferenceTypeDelegate resolveDelegate(ReferenceType ty) {
    ...
    final ReferenceTypeDelegate bootstrapLoaderDelegate = resolveIfBootstrapDelegate(ty);
  ...
  private ReferenceTypeDelegate resolveReflectionTypeDelegate(ReferenceType ty, ClassLoader resolutionLoader) {
    final ReferenceTypeDelegate res = ReflectionBasedReferenceTypeDelegateFactory.createDelegate(ty, this, resolutionLoader);
    return res;
  }
  ...
  private static Class makeConcurrentMapClass() {
    final String[] betterChoices = {"java.util.concurrent.ConcurrentHashMap",
    ...
    if (isLocallyDefined(ret.getName())) {
      if (typeCompletionInProgress) {
        ...
        while (typesForCompletion.size() != 0) {
          final ResolvedType rt = (ResolvedType) typesForCompletion.get(0);
  ...
  protected ResolvedType completeNonLocalType(ResolvedType ret) {
    ...
    final ReferenceTypeDelegate rtd = resolveReflectionTypeDelegate((ReferenceType) toResolve, getClassLoader());
