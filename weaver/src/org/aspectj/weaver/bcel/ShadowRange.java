  ...
  void extractInstructionsInto(LazyMethodGen freshMethod, IntMap remap, boolean addReturn) {
    ...
    for (InstructionHandle oldIh = start.getNext(); oldIh != end; oldIh = oldIh.getNext()) {
      ...
      if (freshI instanceof InstructionBranch) {
        ...
        final InstructionBranch oldBranch = (InstructionBranch) oldI;
        final InstructionBranch freshBranch = (InstructionBranch) freshI;
        final InstructionHandle oldTarget = oldBranch.getTarget();
        oldTarget.removeTargeter(oldBranch);
        oldTarget.addTargeter(freshBranch);
        if (freshBranch instanceof InstructionSelect) {
          final InstructionSelect oldSelect = (InstructionSelect) oldI;
          final InstructionSelect freshSelect = (InstructionSelect) freshI;
          final InstructionHandle[] oldTargets = freshSelect.getTargets();
      ...
      for (InstructionTargeter source : oldIh.getTargetersCopy()) {
        if (source instanceof LocalVariableTag) {
          final Shadow.Kind kind = getKind();
          if (kind == Shadow.AdviceExecution || kind == Shadow.ConstructorExecution || kind == Shadow.MethodExecution
            ...
            final LocalVariableTag sourceLocalVariableTag = (LocalVariableTag) source;
      ...
      if (freshI.isLocalVariableInstruction() || freshI instanceof RET) {
        ...
        final int oldIndex = freshI.getIndex();
    ...
    for (InstructionHandle newIh = freshBody.getStart(); newIh != freshBody.getEnd(); newIh = newIh.getNext()) {
      final Iterator<InstructionTargeter> tIter = newIh.getTargeters().iterator();
      while (tIter.hasNext()) {
        final InstructionTargeter source = tIter.next();
        if (source instanceof LocalVariableTag) {
          final LocalVariableTag lvt = (LocalVariableTag) source;
    ...
    try {
      for (InstructionHandle oldIh = start.getNext(); oldIh != end; ) {
        final InstructionHandle next = oldIh.getNext();
        body.delete(oldIh);
        oldIh = next;
      }
