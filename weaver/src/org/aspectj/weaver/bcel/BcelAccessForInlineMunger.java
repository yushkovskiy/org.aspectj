...
public class BcelAccessForInlineMunger extends BcelTypeMunger {
  ...
  public ResolvedMember getMatchingSyntheticMember(Member member) {
    final ResolvedMember rm = inlineAccessors.get(member.getName());// + member.getSignature());
...
//				+ (rm == null ? "" : rm.getName()));
  ...
  private void openAroundAdvice(LazyMethodGen aroundAdvice) {
    ...
    final InstructionHandle end = aroundAdvice.getBody().getEnd();
    final ConstantPool cpg = aroundAdvice.getEnclosingClass().getConstantPool();
    final InstructionFactory factory = aroundAdvice.getEnclosingClass().getFactory();
    ...
    while (curr != end) {
      ...
      if ((inst instanceof InvokeInstruction)) {
        final InvokeInstruction invoke = (InvokeInstruction) inst;
        final ResolvedType callee = aspectGen.getWorld().resolve(UnresolvedType.forName(invoke.getClassName(cpg)));

        // look in the whole method list and not just declared for super calls and alike
        final List<ResolvedMember> methods = callee.getMethodsWithoutIterator(false, true, false);
        for (ResolvedMember resolvedMember : methods) {
          if (invoke.getName(cpg).equals(resolvedMember.getName())
            ...
            if ("<init>".equals(invoke.getName(cpg))) {
              ...
              final ResolvedType memberType = aspectGen.getWorld().resolve(resolvedMember.getDeclaringType());
              if (!aspectType.equals(memberType) && memberType.isAssignableFrom(aspectType)) {
                ...
                final ResolvedMember accessor = createOrGetInlineAccessorForSuperDispatch(resolvedMember);
                final InvokeInstruction newInst = factory.createInvoke(aspectType.getName(), accessor.getName(),
                ...
                final ResolvedMember accessor = createOrGetInlineAccessorForMethod(resolvedMember);
                final InvokeInstruction newInst = factory.createInvoke(aspectType.getName(), accessor.getName(),
                    BcelWorld.makeBcelType(accessor.getReturnType()),
        ...
        final FieldInstruction invoke = (FieldInstruction) inst;
        final ResolvedType callee = aspectGen.getWorld().resolve(UnresolvedType.forName(invoke.getClassName(cpg)));
        for (int i = 0; i < callee.getDeclaredJavaFields().length; i++) {
          final ResolvedMember resolvedMember = callee.getDeclaredJavaFields()[i];
          if (invoke.getName(cpg).equals(resolvedMember.getName())
              && invoke.getSignature(cpg).equals(resolvedMember.getSignature()) && !resolvedMember.isPublic()) {
  ...
  private ResolvedMember createOrGetInlineAccessorForMethod(ResolvedMember resolvedMember) {
    final String accessorName = NameMangler.inlineAccessMethodForMethod(resolvedMember.getName(), resolvedMember.getDeclaringType(),
        aspectType);
    final String key = accessorName;// new StringBuilder(accessorName).append(resolvedMember.getSignature()).toString();
...
//		System.err.println(key + " accessor=" + inlineAccessor);
    if (inlineAccessor == null) {
      ...
      final InstructionFactory factory = aspectGen.getFactory();
      final LazyMethodGen method = makeMethodGen(aspectGen, inlineAccessor);
      method.makeSynthetic();
      final List<AjAttribute> methodAttributes = new ArrayList<AjAttribute>();
      ...
      for (int i = 0, max = inlineAccessor.getParameterTypes().length; i < max; i++) {
        final UnresolvedType ptype = inlineAccessor.getParameterTypes()[i];
        final Type type = BcelWorld.makeBcelType(ptype);
  ...
  private ResolvedMember createOrGetInlineAccessorForSuperDispatch(ResolvedMember resolvedMember) {
    final String accessor = NameMangler.superDispatchMethod(aspectType, resolvedMember.getName());

    final String key = accessor;
    ResolvedMember inlineAccessor = inlineAccessors.get(key);
    ...
    if (inlineAccessor == null) {
      ...
      for (int i = 0; i < inlineAccessor.getParameterTypes().length; i++) {
        final UnresolvedType typeX = inlineAccessor.getParameterTypes()[i];
  ...
  private ResolvedMember createOrGetInlineAccessorForFieldGet(ResolvedMember resolvedMember) {
    final String accessor = NameMangler.inlineAccessMethodForFieldGet(resolvedMember.getName(), resolvedMember.getDeclaringType(),
        aspectType);
    final String key = accessor;
    ResolvedMember inlineAccessor = inlineAccessors.get(key);
  ...
  private ResolvedMember createOrGetInlineAccessorForFieldSet(ResolvedMember resolvedMember) {
    final String accessor = NameMangler.inlineAccessMethodForFieldSet(resolvedMember.getName(), resolvedMember.getDeclaringType(),
        aspectType);
    final String key = accessor;
    ResolvedMember inlineAccessor = inlineAccessors.get(key);
