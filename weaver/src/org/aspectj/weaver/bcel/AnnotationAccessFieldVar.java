...
class AnnotationAccessFieldVar extends BcelVar {
  ...
  public AnnotationAccessFieldVar(AnnotationAccessVar aav, ResolvedType annoFieldOfInterest, String name) {
    ...
    this.name = name;
    final String sig = annoFieldOfInterest.getSignature();
    if (sig.length() == 1) {
      switch (sig.charAt(0)) {
        case 'I':
  ...
  public void appendLoadAndConvert(InstructionList il, InstructionFactory fact, ResolvedType toType) {
    ...
    final String annotationOfInterestSignature = annoAccessor.getType().getSignature();
    // So we have an entity that has an annotation on and within it is the value we want
    final Member holder = annoAccessor.getMember();
    final AnnotationAJ[] annos = holder.getAnnotations();
    for (AnnotationAJ anno : annos) {
      final AnnotationGen annotation = ((BcelAnnotation) anno).getBcelAnnotation();
      boolean foundValueInAnnotationUsage = false;
      if (annotation.getTypeSignature().equals(annotationOfInterestSignature)) {
        final ResolvedMember[] annotationFields = toType.getWorld()
        ...
        final List<NameValuePair> nvps = annotation.getValues();
        for (NameValuePair nvp : nvps) {
          ...
          final ElementValue o = nvp.getValue();
          if (o.getElementValueType() != elementValueType) {
            continue;
          }
          if (o instanceof EnumElementValue) {
            final EnumElementValue v = (EnumElementValue) o;
            final String s = v.getEnumTypeString();
            final ResolvedType rt = toType.getWorld().resolve(UnresolvedType.forSignature(s));
            if (rt.equals(toType)) {
            ...
            final SimpleElementValue v = (SimpleElementValue) o;
        ...
        if (!foundValueInAnnotationUsage) {
          for (ResolvedMember annotationField : annotationFields) {
            ...
            if (annotationField.getType().getSignature().equals("I")) {
              final int ivalue = Integer.parseInt(annotationField.getAnnotationDefaultValue());
              il.append(fact.createConstant(ivalue));
              foundValueInAnnotationUsage = true;
              break;
            } else if (annotationField.getType().getSignature().equals("Ljava/lang/String;")) {
              final String svalue = annotationField.getAnnotationDefaultValue();
              il.append(fact.createConstant(svalue));
              foundValueInAnnotationUsage = true;
              break;
            } else {
              final String dvalue = annotationField.getAnnotationDefaultValue();
              // form will be LBLAHBLAHBLAH;X where X is the field within X
              final String typename = dvalue.substring(0, dvalue.lastIndexOf(';') + 1);
              final String field = dvalue.substring(dvalue.lastIndexOf(';') + 1);
              final ResolvedType rt = toType.getWorld().resolve(UnresolvedType.forSignature(typename));
