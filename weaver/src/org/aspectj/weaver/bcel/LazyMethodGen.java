...
public final class LazyMethodGen implements Traceable {
  ...
  public void addParameterAnnotation(int parameterNumber, AnnotationAJ anno) {
    ...
    if (memberView == null) {
      if (newParameterAnnotations == null) {
        // time to create it
        final int pcount = getArgumentTypes().length;
        ...
        final AnnotationAJ[] currentAnnoArray = newParameterAnnotations[parameterNumber];
        final AnnotationAJ[] newAnnoArray = new AnnotationAJ[currentAnnoArray.length + 1];
  ...
  public ResolvedType[] getAnnotationTypes() {
    ...
    if (memberView == null && newAnnotations != null && newAnnotations.size() != 0) {
      ...
      final ResolvedType[] annotationTypes = new ResolvedType[newAnnotations.size()];
  ...
  private void initialize() {
    ...
    final MethodGen gen = new MethodGen(savedMethod, enclosingClass.getName(), enclosingClass.getConstantPool(), true);
  ...
  private void unpackHandlers(MethodGen gen) {
    final CodeExceptionGen[] exns = gen.getExceptionHandlers();
    if (exns != null) {
      final int len = exns.length;
      // if (len > 0) hasExceptionHandlers = true;
      int priority = len - 1;
      for (int i = 0; i < len; i++, priority--) {
        final CodeExceptionGen exn = exns[i];

        final InstructionHandle start = Range.genStart(body, getOutermostExceptionStart(exn.getStartPC()));
        final InstructionHandle end = Range.genEnd(body, getOutermostExceptionEnd(exn.getEndPC()));
        // this doesn't necessarily handle overlapping correctly!!!
        final ExceptionRange er = new ExceptionRange(body, exn.getCatchType() == null ? null : BcelWorld.fromBcel(exn
  ...
  public String toString() {
    final BcelObjectType bot = enclosingClass.getBcelObjectType();
    final WeaverVersionInfo weaverVersion = (bot == null ? WeaverVersionInfo.CURRENT : bot.getWeaverVersionAttribute());
    return toLongString(weaverVersion);
  }

  public String toShortString() {
    final String access = org.aspectj.apache.bcel.classfile.Utility.accessToString(getAccessFlags());
    ...
    {
      final int len = declaredExceptions != null ? declaredExceptions.length : 0;
      if (len > 0) {
        buf.append(" throws ");
  ...
  public void print(PrintStream out, WeaverVersionInfo weaverVersion) {
    ...
    final InstructionList body = getBody();
    if (body == null) {
      out.println(";");
      return;
    }
  ...
  private void printAspectAttributes(PrintStream out, WeaverVersionInfo weaverVersion) {
    ...
    final List<AjAttribute> as = Utility.readAjAttributes(getClassName(), attributes.toArray(new Attribute[]{}), context, null, weaverVersion,
  ...
  private class BodyPrinter {
    ...
    // label assignment
    void assignLabels() {
      final LinkedList<ExceptionRange> exnTable = new LinkedList<ExceptionRange>();
      String pendingLabel = null;
      // boolean hasPendingTargeters = false;
      int lcounter = 0;
      for (InstructionHandle ih = body.getStart(); ih != null; ih = ih.getNext()) {
        final Iterator<InstructionTargeter> tIter = ih.getTargeters().iterator();
        while (tIter.hasNext()) {
          final InstructionTargeter t = tIter.next();// targeters
          // [
          // i
          // ]
          // ;
          if (t instanceof ExceptionRange) {
            ...
            final ExceptionRange r = (ExceptionRange) t;
            if (r.getStart() == ih) {
      ...
      int ecounter = 0;
      for (final Iterator i = exnTable.iterator(); i.hasNext(); ) {
        final ExceptionRange er = (ExceptionRange) i.next();
        final String exceptionLabel = "E" + ecounter++;
    ...
    void print() {
      ...
      for (InstructionHandle ih = body.getStart(); ih != null; ih = ih.getNext()) {
        if (Range.isRangeHandle(ih)) {
          final Range r = Range.getRange(ih);
          ...
          final int line = getLineNumber(ih, currLine);
          if (line != currLine) {
            currLine = line;
    ...
    String getRangeString(Range r, Map<InstructionHandle, String> labelMap) {
      if (r instanceof ExceptionRange) {
        final ExceptionRange er = (ExceptionRange) r;
    ...
    void printInstruction(InstructionHandle h, int depth) {
      ...
      final Instruction inst = h.getInstruction();
      if (inst.isConstantPoolInstruction()) {
        ...
        final InstructionSelect sinst = (InstructionSelect) inst;
        out.println(Constants.OPCODE_NAMES[sinst.opcode].toUpperCase());
        final int[] matches = sinst.getMatchs();
        final InstructionHandle[] targets = sinst.getTargets();
        final InstructionHandle defaultTarget = sinst.getTarget();
        ...
        final InstructionBranch brinst = (InstructionBranch) inst;
        ...
        final int index = inst.getIndex();
        final LocalVariableTag tag = getLocalVariableTag(h, index);
        if (tag != null) {
          out.print("     // ");
  ...
  static LocalVariableTag getLocalVariableTag(InstructionHandle ih, int index) {
    for (InstructionTargeter t : ih.getTargeters()) {
      if (t instanceof LocalVariableTag) {
        final LocalVariableTag lvt = (LocalVariableTag) t;
  ...
  public void addExceptionHandler(InstructionHandle start, InstructionHandle end, InstructionHandle handlerStart,
    ...
    final InstructionHandle start1 = Range.genStart(body, start);
    final InstructionHandle end1 = Range.genEnd(body, end);

    final ExceptionRange er = new ExceptionRange(body, (catchType == null ? null : BcelWorld.fromBcel(catchType)), highPriority);
  ...
  public MethodGen pack() {
    ...
    final MethodGen gen = new MethodGen(flags, getReturnType(), getArgumentTypes(), null, // getArgumentNames(),
    ...
    if (newParameterAnnotations != null) {
      for (int i = 0; i < newParameterAnnotations.length; i++) {
        final AnnotationAJ[] annos = newParameterAnnotations[i];
    ...
    if (memberView != null && memberView.getAnnotations() != null && memberView.getAnnotations().length != 0) {
      final AnnotationAJ[] ans = memberView.getAnnotations();
      for (int i = 0, len = ans.length; i < len; i++) {
        final AnnotationGen a = ((BcelAnnotation) ans[i]).getBcelAnnotation();
    ...
    if (isSynthetic) {
      ...
      if (!hasAttribute("Synthetic")) {
        ...
        final ConstantPool cpg = gen.getConstantPool();
        final int index = cpg.addUtf8("Synthetic");
  ...
  private boolean inAspect() {
    final BcelObjectType objectType = enclosingClass.getBcelObjectType();
  ...
  public void packBody(MethodGen gen) {
    final InstructionList fresh = gen.getInstructionList();
    final Map<InstructionHandle, InstructionHandle> map = copyAllInstructionsExceptRangeInstructionsInto(fresh);
    ...
    final LinkedList<ExceptionRange> exceptionList = new LinkedList<ExceptionRange>();

    final Map<LocalVariableTag, LVPosition> localVariables = new HashMap<LocalVariableTag, LVPosition>();

    int currLine = -1;
    final int lineNumberOffset = (fromFilename == null) ? 0 : getEnclosingClass().getSourceDebugExtensionOffset(fromFilename);
    ...
    while (oldInstructionHandle != null) {
      if (map.get(oldInstructionHandle) == null) {
        ...
        final Instruction oldInstruction = oldInstructionHandle.getInstruction();
        final Instruction newInstruction = newInstructionHandle.getInstruction();
        ...
        for (InstructionTargeter targeter : oldInstructionHandle.getTargeters()) {
          if (targeter instanceof LineNumberTag) {
            final int line = ((LineNumberTag) targeter).getLineNumber();
            if (line != currLine) {
              gen.addLineNumber(newInstructionHandle, line + lineNumberOffset);
              currLine = line;
            }
          } else if (targeter instanceof LocalVariableTag) {
            final LocalVariableTag lvt = (LocalVariableTag) targeter;
            final LVPosition p = localVariables.get(lvt);
            // If we don't know about it, create a new position and
            // store
            // If we do know about it - update its end position
            if (p == null) {
              final LVPosition newp = new LVPosition();
  ...
  private void createNewLocalVariables(MethodGen gen) {
    ...
    if (!getName().startsWith("<")) {
      int slot = 0;
      final InstructionHandle start = gen.getInstructionList().getStart();
      final InstructionHandle end = gen.getInstructionList().getEnd();
      // Add a 'this' if non-static
      if (!isStatic()) {
        final String cname = this.enclosingClass.getClassName();
        if (cname == null) {
          return; // give up for now
        }
        final Type enclosingType = BcelWorld.makeBcelType(UnresolvedType.forName(cname));
        gen.addLocalVariable("this", enclosingType, slot++, start, end);
      }
      // Add entries for the method arguments
      final String[] paramNames = (memberView == null ? null : memberView.getParameterNames());
      if (paramNames != null) {
  ...
  public void optimizedPackBody(MethodGen gen) {
    final InstructionList theBody = getBody();
    ...
    final Map<LocalVariableTag, LVPosition> localVariables = new HashMap<LocalVariableTag, LVPosition>();
    final LinkedList<ExceptionRange> exceptionList = new LinkedList<ExceptionRange>();
    final Set<InstructionHandle> forDeletion = new HashSet<InstructionHandle>();
    final Set<BranchHandle> branchInstructions = new HashSet<BranchHandle>();
    // OPTIMIZE sort out in here: getRange()/insertHandler() and type of
    // exceptionList
    while (iHandle != null) {
      final Instruction inst = iHandle.getInstruction();
      ...
      if (inst == Range.RANGEINSTRUCTION) {
        final Range r = Range.getRange(iHandle);
        if (r instanceof ExceptionRange) {
          final ExceptionRange er = (ExceptionRange) r;
  ...
  private void addLocalVariables(MethodGen gen, Map<LocalVariableTag, LVPosition> localVariables) {
    ...
    final InstructionHandle methodStart = gen.getInstructionList().getStart();
    final InstructionHandle methodEnd = gen.getInstructionList().getEnd();
    ...
    final Type[] argTypes = gen.getArgumentTypes();
    if (argTypes != null) {
    ...
    final Map<InstructionHandle, Set<Integer>> duplicatedLocalMap = new HashMap<InstructionHandle, Set<Integer>>();
    for (LocalVariableTag tag : localVariables.keySet()) {
      // have we already added one with the same slot number and start
      // location?
      // if so, just continue.
      final LVPosition lvpos = localVariables.get(tag);
      final InstructionHandle start = (tag.getSlot() < paramSlots ? methodStart : lvpos.start);
      final InstructionHandle end = (tag.getSlot() < paramSlots ? methodEnd : lvpos.end);
  ...
  private void addExceptionHandlers(MethodGen gen, Map<InstructionHandle, InstructionHandle> map,
    ...
    for (ExceptionRange r : exnList) {
      ...
      final InstructionHandle rMappedStart = remap(r.getRealStart(), map);
      final InstructionHandle rMappedEnd = remap(r.getRealEnd(), map);
      final InstructionHandle rMappedHandler = remap(r.getHandler(), map);
  ...
  private void handleBranchInstruction(Map<InstructionHandle, InstructionHandle> map, Instruction oldInstruction,
    ...
    final InstructionBranch oldBranchInstruction = (InstructionBranch) oldInstruction;
    final InstructionBranch newBranchInstruction = (InstructionBranch) newInstruction;
    final InstructionHandle oldTarget = oldBranchInstruction.getTarget(); // old
    // target
    ...
    if (oldBranchInstruction instanceof InstructionSelect) {
      ...
      final InstructionHandle[] oldTargets = ((InstructionSelect) oldBranchInstruction).getTargets();
      final InstructionHandle[] newTargets = ((InstructionSelect) newBranchInstruction).getTargets();
  ...
  private void handleBranchInstruction(BranchHandle branchHandle, Set<InstructionHandle> handlesForDeletion) {
    final InstructionBranch branchInstruction = (InstructionBranch) branchHandle.getInstruction();
    ...
    if (branchInstruction instanceof InstructionSelect) {
      // Either LOOKUPSWITCH or TABLESWITCH
      final InstructionSelect iSelect = (InstructionSelect) branchInstruction;
      final InstructionHandle[] targets = iSelect.getTargets();
  ...
  private Map<InstructionHandle, InstructionHandle> copyAllInstructionsExceptRangeInstructionsInto(InstructionList intoList) {
    ...
    for (InstructionHandle ih = getBody().getStart(); ih != null; ih = ih.getNext()) {
      ...
      final Instruction copy = Utility.copyInstruction(inst);
  ...
  private static InstructionHandle remap(InstructionHandle handle, Map<InstructionHandle, InstructionHandle> map) {
    while (true) {
      final InstructionHandle ret = map.get(handle);
      if (ret == null) {
  ...
  static void insertHandler(ExceptionRange fresh, LinkedList<ExceptionRange> l) {
    ...
    for (final ListIterator<ExceptionRange> iter = l.listIterator(); iter.hasNext(); ) {
      final ExceptionRange r = iter.next();
...
//	}
  private static void assertTargetedBy(InstructionHandle target, InstructionTargeter targeter, String from) {
    final Iterator tIter = target.getTargeters().iterator();
    while (tIter.hasNext()) {
  ...
  private static void assertTargets(InstructionTargeter targeter, InstructionHandle target, String from) {
    if (targeter instanceof Range) {
      ...
      final InstructionBranch bi = (InstructionBranch) targeter;
      if (bi.getTarget() == target) {
        return;
      }
      if (targeter instanceof InstructionSelect) {
        final InstructionSelect sel = (InstructionSelect) targeter;
        final InstructionHandle[] itargets = sel.getTargets();
        for (int k = itargets.length - 1; k >= 0; k--) {
  ...
  private static Range getRangeAndAssertExactlyOne(InstructionHandle ih, String from) {
    ...
    while (tIter.hasNext()) {
      final InstructionTargeter ts = tIter.next();
