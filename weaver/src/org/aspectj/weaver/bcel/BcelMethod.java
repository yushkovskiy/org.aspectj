...
class BcelMethod extends ResolvedMemberImpl {
  ...
  BcelMethod(@NotNull BcelObjectType declaringType, @NotNull Method method) {
   ...
   */
  BcelMethod(@NotNull BcelObjectType declaringType, @NotNull Method method, List<AjAttribute> attributes) {
    super(method.getName().equals("<init>") ? CONSTRUCTOR : (method.getName().equals("<clinit>") ? STATIC_INITIALIZATION
        : METHOD), declaringType.getResolvedTypeX(), declaringType.isInterface() ? method.getModifiers()
        | Modifier.INTERFACE : method.getModifiers(), method.getName(), method.getSignature());
    this.method = method;
  ...
  public int getLineNumberOfFirstInstruction() {
    final LineNumberTable lnt = method.getLineNumberTable();
    if (lnt == null) {
      return -1;
    }
    final LineNumber[] lns = lnt.getLineNumberTable();
  ...
  public void determineParameterNames() {
    ...
    final LocalVariableTable varTable = method.getLocalVariableTable();
    final int len = getArity();
    if (varTable == null) {
      // do we have an annotation with the argNames value specified...
      final AnnotationAJ[] annos = getAnnotations();
      if (annos != null && annos.length != 0) {
        final AnnotationAJ[] axs = getAnnotations();
        for (int i = 0; i < axs.length; i++) {
          final AnnotationAJ annotationX = axs[i];
          final String typename = annotationX.getTypeName();
          if (typename.charAt(0) == 'o') {
            if (typename.equals("org.aspectj.lang.annotation.Pointcut")
              ...
              if (a != null) {
                final List<NameValuePair> values = a.getValues();
                for (NameValuePair nvPair : values) {
                  if (nvPair.getNameString().equals("argNames")) {
                    final String argNames = nvPair.getValue().stringifyValue();
                    final StringTokenizer argNameTokenizer = new StringTokenizer(argNames, " ,");
                    final List<String> argsList = new ArrayList<String>();
                    while (argNameTokenizer.hasMoreTokens()) {
                      argsList.add(argNameTokenizer.nextToken());
                    }
                    final int requiredCount = getParameterTypes().length;
      ...
      final UnresolvedType[] paramTypes = getParameterTypes();
      final String[] paramNames = new String[len];
      int index = Modifier.isStatic(modifiers) ? 0 : 1;
      for (int i = 0; i < len; i++) {
        final LocalVariable lv = varTable.getLocalVariable(index);
        if (lv == null) {
          paramNames[i] = "arg" + i;
        } else {
  ...
  public String getAnnotationDefaultValue() {
    final Attribute[] attrs = method.getAttributes();
    for (int i = 0; i < attrs.length; i++) {
      final Attribute attribute = attrs[i];
      if (attribute.getName().equals("AnnotationDefault")) {
        final AnnotationDefault def = (AnnotationDefault) attribute;
  ...
  public String[] getAttributeNames(boolean onlyIncludeAjOnes) {
    final Attribute[] as = method.getAttributes();
  ...
  public ISourceLocation getSourceLocation() {
    ...
    if ((ret == null || ret.getLine() == 0) && hasDeclarationLineNumberInfo()) {
      ...
      final ISourceContext isc = getSourceContext();
      if (isc != null) {
  ...
  public void addAnnotation(AnnotationAJ annotation) {
    ...
    if ((bitflags & HAS_ANNOTATIONS) == 0) {
      ...
      final AnnotationAJ[] ret = new AnnotationAJ[len + 1];
      System.arraycopy(annotations, 0, ret, 0, len);
      ret[len] = annotation;
      annotations = ret;
      final ResolvedType[] newAnnotationTypes = new ResolvedType[len + 1];
  ...
  public void addParameterAnnotation(int param, AnnotationAJ anno) {
    ...
    final int existingCount = parameterAnnotations[param].length;
    if (existingCount == 0) {
      final AnnotationAJ[] annoArray = new AnnotationAJ[1];
      annoArray[0] = anno;
      parameterAnnotations[param] = annoArray;
    } else {
      final AnnotationAJ[] newAnnoArray = new AnnotationAJ[existingCount + 1];
  ...
  public boolean isEquivalentTo(Object other) {
    ...
    final BcelMethod o = (BcelMethod) other;
  ...
  public boolean isDefaultConstructor() {
    final boolean mightBe = !hasDeclarationLineNumberInfo() && name.equals("<init>") && parameterTypes.length == 0;
    if (mightBe) {
  ...
  private void unpackJavaAttributes() {
    final ExceptionTable exnTable = method.getExceptionTable();
    checkedExceptions = (exnTable == null) ? UnresolvedType.NONE : UnresolvedType.forNames(exnTable.getExceptionNames());
  }

  private void unpackAjAttributes(World world) {
    associatedShadowMunger = null;
    final ResolvedType resolvedDeclaringType = getDeclaringType().resolve(world);
    final WeaverVersionInfo wvinfo = bcelObjectType.getWeaverVersionAttribute();
  ...
  private void ensureAnnotationsRetrieved() {
    ...
    final AnnotationGen[] annos = method.getAnnotations();
    if (annos.length == 0) {
      annotationTypes = ResolvedType.NONE;
      annotations = AnnotationAJ.EMPTY_ARRAY;
    } else {
      final int annoCount = annos.length;
  ...
  private void ensureParameterAnnotationsRetrieved() {
    ...
    final AnnotationGen[][] pAnns = method.getParameterAnnotations();
  ...
  private void unpackGenericSignature() {
    ...
    final String gSig = method.getGenericSignature();
    if (gSig != null) {
      final GenericSignature.MethodTypeSignature mSig = new GenericSignatureParser().parseAsMethodSignature(gSig);// method
      // .
      // getGenericSignature
      // ());
      ...
      for (int i = 0; i < typeVariables.length; i++) {
        final GenericSignature.FormalTypeParameter methodFtp = mSig.formalTypeParameters[i];
        try {
      ...
      final GenericSignature.FormalTypeParameter[] parentFormals = bcelObjectType.getAllFormals();
      final GenericSignature.FormalTypeParameter[] formals = new GenericSignature.FormalTypeParameter[parentFormals.length
      ...
      final GenericSignature.TypeSignature returnTypeSignature = mSig.returnType;
      try {
      ...
      final GenericSignature.TypeSignature[] paramTypeSigs = mSig.parameters;
  ...
  private void workOutIfSynthetic() {
    ...
    final JavaClass jc = bcelObjectType.getJavaClass();
    ...
    if (jc.getMajor() < 49/* Java5 */) {
      ...
      final String[] synthetics = getAttributeNames(false);
      if (synthetics != null) {
