...
public final class LazyClassGen {
  ...
  void addInlinedSourceFileInfo(String fullpath, int highestLineNumber) {
    final Object o = inlinedFiles.get(fullpath);
    if (o != null) {
      final InlinedSourceFileInfo info = (InlinedSourceFileInfo) o;
  ...
  public static void disassemble(String path, String name, PrintStream out) throws IOException {
    ...
    final BcelWorld world = new BcelWorld(path);

    final UnresolvedType ut = UnresolvedType.forName(name);
    ut.setNeedsModifiableDelegate(true);
    final LazyClassGen clazz = new LazyClassGen(BcelWorld.getBcelObjectType(world.resolve(ut)));
    clazz.print(out);
    out.println();
  }
  ...
  public LazyClassGen(BcelObjectType myType) {
    ...
    if (implementsSerializable(getType())) {
      ...
      final ResolvedMember[] methods = getType().getDeclaredMethods();
      ...
      if (!getType().isInterface() && !hasSerialVersionUIDField && world.isAddSerialVerUID()) {
        ...
        final FieldGen fg = new FieldGen(Constants.ACC_PRIVATE | Constants.ACC_FINAL | Constants.ACC_STATIC, BasicType.LONG,
    ...
    final ResolvedMember[] methods = myType.getDeclaredMethods();
    for (int i = 0; i < methods.length; i++) {
    ...
    final ResolvedMember[] fields = myType.getDeclaredFields();
    for (int i = 0; i < fields.length; i++) {
  ...
  public static boolean hasSerialVersionUIDField(ResolvedType type) {
    ...
    for (int i = 0; i < fields.length; i++) {
      final ResolvedMember field = fields[i];
  ...
  public String getInternalFileName() {
    final String str = getInternalClassName();
...
//		}
  ...
  private void writeBack(BcelWorld world) {
    ...
    final boolean sourceDebugExtensionSupportSwitchedOn = false;
    ...
    for (int i = 0; i < len; i++) {
      final BcelField gen = fields.get(i);
  ...
  private void fixupGenericSignatureAttribute() {
    ...
    if (!needAttribute) {
      if (myType != null) {
        final ResolvedType[] interfaceRTXs = myType.getDeclaredInterfaces();
        for (int i = 0; i < interfaceRTXs.length; i++) {
          final ResolvedType typeX = interfaceRTXs[i];
        ...
        if (extraSuperInterfaces != null) {
          for (int i = 0; i < extraSuperInterfaces.length; i++) {
            final ResolvedType interfaceType = extraSuperInterfaces[i];
      ...
      if (myType == null) {
        final ResolvedType superclassRTX = superclass;
        ...
        // check the supertype
        final ResolvedType superclassRTX = getSuperClass();
        if (superclassRTX.isGenericType() || superclassRTX.isParameterizedType()) {
          needAttribute = true;
        }
      }
    ...
    if (needAttribute) {
      ...
      if (myType != null) {
        final TypeVariable[] tVars = myType.getTypeVariables();
        if (tVars.length > 0) {
          signature.append("<");
          for (int i = 0; i < tVars.length; i++) {
            final TypeVariable variable = tVars[i];
      ...
      // now the supertype
      final String supersig = getSuperClass().getSignatureForAttribute();
      signature.append(supersig);
      if (myType != null) {
        final ResolvedType[] interfaceRTXs = myType.getDeclaredInterfaces();
        for (int i = 0; i < interfaceRTXs.length; i++) {
          final String s = interfaceRTXs[i].getSignatureForAttribute();
          signature.append(s);
        }
        if (extraSuperInterfaces != null) {
          for (int i = 0; i < extraSuperInterfaces.length; i++) {
            final String s = extraSuperInterfaces[i].getSignatureForAttribute();
            signature.append(s);
          }
        }
      }
  ...
  public byte[] getJavaClassBytesIncludingReweavable(BcelWorld world) {
    ...
    final WeaverStateInfo wsi = myType.getWeaverState();// getOrCreateWeaverStateInfo();
  ...
  public void addInterface(ResolvedType newInterface, ISourceLocation sourceLocation) {
    ...
    if (extraSuperInterfaces == null) {
      ...
      final ResolvedType[] x = new ResolvedType[extraSuperInterfaces.length + 1];
  ...
  private List<LazyClassGen> getClassGens() {
    final List<LazyClassGen> ret = new ArrayList<LazyClassGen>();
    ret.add(this);
  ...
  public List<UnwovenClassFile.ChildClass> getChildClasses(BcelWorld world) {
    ...
    final List<UnwovenClassFile.ChildClass> ret = new ArrayList<UnwovenClassFile.ChildClass>();
    for (LazyClassGen clazz : classGens) {
      final byte[] bytes = clazz.getJavaClass(world).getBytes();
  ...
  public void print(PrintStream out) {
    final List<LazyClassGen> classGens = getClassGens();
    for (final Iterator<LazyClassGen> iter = classGens.iterator(); iter.hasNext(); ) {
      final LazyClassGen element = iter.next();
  ...
  private void printOne(PrintStream out) {
    ...
    final int size = myGen.getInterfaces().length;
    ...
    final Field[] fields = myGen.getFields();
    for (int i = 0, len = fields.length; i < len; i++) {
      out.print("  ");
      out.println(fields[i]);
    }
    final List<LazyMethodGen> methodGens = getMethodGens();
    for (final Iterator<LazyMethodGen> iter = methodGens.iterator(); iter.hasNext(); ) {
      final LazyMethodGen gen = iter.next();
      // we skip empty clinits
      if (isEmptyClinit(gen)) {
        continue;
      }
  ...
  public LazyMethodGen getStaticInitializer() {
    ...
    final LazyMethodGen clinit = new LazyMethodGen(Modifier.STATIC, Type.VOID, "<clinit>", new Type[0], NO_STRINGS, this);
  ...
  public LazyMethodGen getAjcPreClinit() {
    ...
    final LazyMethodGen ajcPreClinit = new LazyMethodGen(Modifier.PRIVATE | Modifier.STATIC, Type.VOID,
  ...
  public Field getTjpField(BcelShadow shadow, final boolean isEnclosingJp) {
    ...
    final LazyMethodGen encMethod = shadow.getEnclosingMethod();
    ...
    if (tjpFieldsCounter == -1) {
      ...
      if (!world.isOverWeaving()) {
        ...
        final List<BcelField> existingFields = getFieldGens();
    ...
    final FieldGen fGen = new FieldGen(modifiers, jpType, "ajc$tjp_" + tjpFieldsCounter++, getConstantPool());
    addField(fGen);
  ...
  public Field getAnnotationCachingField(BcelShadow shadow, ResolvedType toType, boolean isWithin) {
    ...
    final CacheKey cacheKey = new CacheKey(shadow, toType, isWithin);
    ...
    if (field == null) {
      ...
      final FieldGen annotationCacheField = new FieldGen(Modifier.PRIVATE | Modifier.STATIC, typeForAnnotation, sb.toString(), cp);
  ...
  static class CacheKey {
    ...
    public boolean equals(Object other) {
      ...
      final CacheKey oCacheKey = (CacheKey) other;
  ...
  private void addAjcInitializers() {
    ...
    if (serialVersionUIDRequiresInitialization) {
      final InstructionList[] ilSVUID = new InstructionList[1];
      ...
      if (il == null) {
        ...
        final InstructionList[] newIl = new InstructionList[il.length + ilSVUID.length];
  ...
  private InstructionList[] initializeAllTjps() {
    final Vector<InstructionList> lists = new Vector<InstructionList>();

    InstructionList list = initInstructionList();
    lists.add(list);

    final List<Map.Entry<BcelShadow, Field>> entries = new ArrayList<Map.Entry<BcelShadow, Field>>(tjpFields.entrySet());
    ...
    for (final Iterator<Map.Entry<BcelShadow, Field>> i = entries.iterator(); i.hasNext(); ) {
      final Map.Entry<BcelShadow, Field> entry = i.next();
    ...
    final InstructionList[] listArrayModel = new InstructionList[1];
  ...
  private void initializeTjp(InstructionFactory fact, InstructionList list, Field field, BcelShadow shadow) {
    ...
    final boolean isFastSJPAvailable = shadow.getWorld().isTargettingRuntime1_6_10()
        && !enclosingStaticTjpType.equals(field.getType());

    final Member sig = shadow.getSignature();
    ...
    final String signatureMakerName = SignatureUtils.getSignatureMakerName(sig);
    final ObjectType signatureType = new ObjectType(SignatureUtils.getSignatureType(sig));
    ...
    if (world.isTargettingAspectJRuntime12()) { // TAG:SUPPORTING12: We
      ...
      final BcelWorld w = shadow.getWorld();
  ...
  private void makeSyntheticAndTransientIfNeeded(FieldGen field) {
    if (field.getName().startsWith(NameMangler.PREFIX) && !field.getName().startsWith("ajc$interField$")
      ...
      if (!hasSyntheticAttribute(field.getAttributes())) {
        ...
        final ConstantPool cpg = myGen.getConstantPool();
        final int index = cpg.addUtf8("Synthetic");
        final Attribute synthetic = new Synthetic(index, 0, new byte[0], cpg);
  ...
  public boolean hasAnnotation(UnresolvedType t) {
    ...
    final AnnotationGen[] agens = myGen.getAnnotations();
    if (agens == null) {
      return false;
    }
    for (int i = 0; i < agens.length; i++) {
      final AnnotationGen gen = agens[i];
  ...
  private boolean implementsSerializable(ResolvedType aType) {
    ...
    final ResolvedType[] interfaces = aType.getDeclaredInterfaces();
    ...
    final ResolvedType superType = aType.getSuperclass();
  ...
  public String allocateField(String prefix) {
    ...
    final List<BcelField> fs = getFieldGens();
    for (BcelField field : fs) {
      if (field.getName().startsWith(prefix)) {
        try {
          final int num = Integer.parseInt(field.getName().substring(prefix.length()));
