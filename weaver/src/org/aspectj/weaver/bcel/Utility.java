...
public class Utility {
  ...
  public static List<AjAttribute> readAjAttributes(String classname, Attribute[] as, ISourceContext context, World w,
    ...
    final List<AjAttribute> attributes = new ArrayList<AjAttribute>();

    // first pass, look for version
    final List<Unknown> forSecondPass = new ArrayList<Unknown>();
    for (int i = as.length - 1; i >= 0; i--) {
      final Attribute a = as[i];
      if (a instanceof Unknown) {
        final Unknown u = (Unknown) a;
    ...
    for (int i = forSecondPass.size() - 1; i >= 0; i--) {
      final Unknown a = forSecondPass.get(i);
      final String name = a.getName();
      final AjAttribute attr = AjAttribute.read(version, name, a.getBytes(), context, w, dataDecompressor);
      if (attr != null) {
        attributes.add(attr);
      }
    }
  ...
  public static Instruction createInvoke(InstructionFactory fact, BcelWorld world, Member signature) {
    final short kind;
    final int signatureModifiers = signature.getModifiers();
  ...
  public static Instruction createInstanceof(InstructionFactory fact, ReferenceType t) {
    final int cpoolEntry = (t instanceof ArrayType) ? fact.getConstantPool().addArrayClass((ArrayType) t) : fact.getConstantPool()
  ...
  public static void appendConversion(InstructionList il, InstructionFactory fact, ResolvedType fromType, ResolvedType toType) {
    ...
    if (toType.equals(UnresolvedType.VOID)) {
      ...
      final Type to = BcelWorld.makeBcelType(toType);
      if (toType.isPrimitiveType()) {
        final String name = toType.toString() + "Value";
      ...
      final Type from = BcelWorld.makeBcelType(fromType);
      ...
      final Type from = BcelWorld.makeBcelType(fromType);
      final Type to = BcelWorld.makeBcelType(toType);
      final String name = validBoxing.get(toType.getSignature() + fromType.getSignature());
      if (toType.isPrimitiveType()) {
        il.append(fact.createInvoke("org.aspectj.runtime.internal.Conversions", name, to, new Type[]{Type.OBJECT},
            Constants.INVOKESTATIC));
      } else {
      ...
      final Type to = BcelWorld.makeBcelType(toType);
      try {
        final Instruction i = fact.createCast(from, to);
        if (i != null) {
          il.append(i);
        } else {
    ...
    switch (value) {
      ...
      default:
        if (value <= Byte.MAX_VALUE && value >= Byte.MIN_VALUE) {
          ...
          final int ii = fact.getClassGen().getConstantPool().addInteger(value);
  ...
  public static JavaClass makeJavaClass(String filename, byte[] bytes) {
    try {
      testingParseCounter++;
      final ClassParser parser = new ClassParser(new ByteArrayInputStream(bytes), filename);
      return parser.parse();
    } catch (IOException e) {
  ...
  public static void replaceInstruction(InstructionHandle ih, InstructionList replacementInstructions,
    ...
    final InstructionList il = enclosingMethod.getBody();
    final InstructionHandle fresh = il.append(ih, replacementInstructions);
  ...
  public static Instruction copyInstruction(Instruction i) {
    if (i instanceof InstructionSelect) {
      final InstructionSelect freshSelect = (InstructionSelect) i;

      // Create a new targets array that looks just like the existing one
      final InstructionHandle[] targets = new InstructionHandle[freshSelect.getTargets().length];
  ...
  public static int getSourceLine(InstructionHandle ih) {// ,boolean
    ...
    while (lookahead++ < 100) {
      ...
      while (tIter.hasNext()) {
        final InstructionTargeter t = tIter.next();
        if (t instanceof LineNumberTag) {
          return ((LineNumberTag) t).getLineNumber();
        }
      }
  ...
  public static BcelVar[] pushAndReturnArrayOfVars(ResolvedType[] proceedParamTypes, InstructionList il, InstructionFactory fact,
                                                   LazyMethodGen enclosingMethod) {
    final int len = proceedParamTypes.length;
    final BcelVar[] ret = new BcelVar[len];

    for (int i = len - 1; i >= 0; i--) {
      final ResolvedType typeX = proceedParamTypes[i];
      final Type type = BcelWorld.makeBcelType(typeX);
      final int local = enclosingMethod.allocateLocal(type);
  ...
  public static boolean isConstantPushInstruction(Instruction i) {
    final long ii = Constants.instFlags[i.opcode];
  ...
  public static boolean isSuppressing(Member member, String lintkey) {
    final boolean isSuppressing = Utils.isSuppressing(member.getAnnotations(), lintkey);
    if (isSuppressing) {
      return true;
    }
    final UnresolvedType type = member.getDeclaringType();
  ...
  public static List<Lint.Kind> getSuppressedWarnings(AnnotationAJ[] anns, Lint lint) {
    ...
    final List<Lint.Kind> suppressedWarnings = new ArrayList<Lint.Kind>();
    boolean found = false;
    for (int i = 0; !found && i < anns.length; i++) {
      ...
      if (UnresolvedType.SUPPRESS_AJ_WARNINGS.getSignature().equals(
        ...
        final List<NameValuePair> vals = ((BcelAnnotation) anns[i]).getBcelAnnotation().getValues();
        if (vals == null || vals.isEmpty()) { // (1)
          suppressedWarnings.addAll(lint.allKinds());
        } else { // (2)
          // We know the value is an array value
          final ArrayElementValue array = (ArrayElementValue) (vals.get(0)).getValue();
          final ElementValue[] values = array.getElementValuesArray();
          for (int j = 0; j < values.length; j++) {
            // We know values in the array are strings
            final SimpleElementValue value = (SimpleElementValue) values[j];
            final Lint.Kind lintKind = lint.getLintKind(value.getValueString());
            if (lintKind != null) {
  ...
  public static Attribute bcelAttribute(AjAttribute a, ConstantPool pool) {
    final int nameIndex = pool.addUtf8(a.getNameString());
    final byte[] bytes = a.getBytes(new BcelConstantPoolWriter(pool));
