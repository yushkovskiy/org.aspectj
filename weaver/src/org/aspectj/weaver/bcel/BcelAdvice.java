...
class BcelAdvice extends Advice {
  ...
  private static Member simplify(AdviceKind kind, Member adviceSignature) {
    if (adviceSignature != null) {
      final UnresolvedType adviceDeclaringType = adviceSignature.getDeclaringType();
      ...
      if (kind != AdviceKind.Around
        ...
        if (adviceSignature instanceof BcelMethod) {
          final BcelMethod bm = (BcelMethod) adviceSignature;
          if (bm.getMethod() != null && bm.getMethod().getAnnotations() != null) {
            return adviceSignature;
          }
          final ResolvedMemberImpl simplermember = new ResolvedMemberImpl(bm.getKind(), bm.getDeclaringType(),
  ...
  public ShadowMunger concretize(ResolvedType fromType, World world, PerClause clause) {
    ...
    final ShadowMunger ret = super.concretize(fromType, world, clause);
    if (!world.areAllLintIgnored()) {
      clearLintSuppressions(world, this.suppressedLintKinds);
    }
    final IfFinder ifinder = new IfFinder();
    ret.getPointcut().accept(ifinder, null);
    final boolean hasGuardTest = ifinder.hasIf && getKind() != AdviceKind.Around;
    final boolean isAround = getKind() == AdviceKind.Around;
  ...
  public ShadowMunger parameterizeWith(ResolvedType declaringType, Map<String, UnresolvedType> typeVariableMap) {
    final Pointcut pc = getPointcut().parameterizeWith(typeVariableMap, declaringType.getWorld());
  ...
  public boolean match(Shadow shadow, World world) {
    if (world.areAllLintIgnored()) {
      ...
      suppressLintWarnings(world);
      final boolean ret = super.match(shadow, world);
      clearLintSuppressions(world, this.suppressedLintKinds);
      return ret;
    }
  }
  ...
  public void specializeOn(Shadow shadow) {
    ...
    if ((getExtraParameterFlags() & ThisJoinPoint) != 0) {
      final boolean hasGuardTest = runtimeTest != Literal.TRUE && getKind() != AdviceKind.Around;
      final boolean isAround = getKind() == AdviceKind.Around;
  ...
  private boolean canInline(Shadow s) {
    ...
    final BcelObjectType boType = BcelWorld.getBcelObjectType(concreteAspect);
    if (boType == null) {
  ...
  public boolean implementOn(Shadow s) {
    ...
    if (!shadow.getWorld().isIgnoringUnusedDeclaredThrownException() && !getThrownExceptions().isEmpty()) {
      final Member member = shadow.getSignature();
      if (member instanceof BcelMethod) {
        ...
        final ResolvedMember resolvedMember = shadow.getSignature().resolve(shadow.getWorld());
        if (resolvedMember instanceof BcelMethod && shadow.getEnclosingShadow() instanceof BcelShadow) {
          final Member enclosingMember = shadow.getEnclosingShadow().getSignature();
    ...
    if (getKind() == AdviceKind.Before) {
      ...
      final UnresolvedType catchType = hasExtraParameter() ? getExtraParameterType() : UnresolvedType.THROWABLE;
      ...
      final LazyClassGen enclosingClass = shadow.getEnclosingClass();
  ...
  private void removeUnnecessaryProblems(BcelMethod method, int problemLineNumber) {
    final ISourceContext sourceContext = method.getSourceContext();
  ...
  private Collection<ResolvedType> collectCheckedExceptions(UnresolvedType[] excs) {
    ...
    final Collection<ResolvedType> ret = new ArrayList<ResolvedType>();
    final World world = concreteAspect.getWorld();
    final ResolvedType runtimeException = world.getCoreType(UnresolvedType.RUNTIME_EXCEPTION);
    final ResolvedType error = world.getCoreType(UnresolvedType.ERROR);

    for (int i = 0, len = excs.length; i < len; i++) {
      final ResolvedType t = world.resolve(excs[i], true);
      if (t.isMissing()) {
  ...
  public Collection<ResolvedType> getThrownExceptions() {
    if (thrownExceptions == null) {
      ...
      if (concreteAspect != null && concreteAspect.getWorld() != null && // null tests for test harness
        ...
        final World world = concreteAspect.getWorld();
        final ResolvedMember m = world.resolve(signature);
        if (m == null) {
  ...
  InstructionList getAdviceInstructions(BcelShadow s, BcelVar extraArgVar, InstructionHandle ifNoAdvice) {
    final BcelShadow shadow = s;
    final InstructionFactory fact = shadow.getFactory();
    final BcelWorld world = shadow.getWorld();
    ...
    if (hasExtraParameter() && getKind() == AdviceKind.AfterReturning) {
      final UnresolvedType extraParameterType = getExtraParameterType();
    ...
    final InstructionHandle ifYesAdvice = il.getStart();
    ...
    if (shadow.getKind() == Shadow.MethodExecution && getKind() == AdviceKind.Before) {
      ...
      final LocalVariableTable lvt = shadow.getEnclosingMethod().getMemberView().getMethod().getLocalVariableTable();
      if (lvt != null) {
        final LocalVariable[] lvTable = lvt.getLocalVariableTable();
        for (int i = 0; i < lvTable.length; i++) {
          final LocalVariable lv = lvTable[i];
          if (lv.getStartPC() == 0) {
  ...
  public InstructionList getAdviceArgSetup(BcelShadow shadow, BcelVar extraVar, InstructionList closureInstantiation) {
    ...
    // pr121385
    final boolean x = this.getDeclaringAspect().resolve(world).isAnnotationStyleAspect();
    ...
    for (int i = 0, len = exposedState.size(); i < len; i++) {
      ...
      final BcelVar v = (BcelVar) exposedState.get(i);
      ...
      if (v == null) {
        ...
        final UnresolvedType desiredTy = getBindingParameterTypes()[i];
  ...
  public Member getOriginalSignature() {
    final Member sig = getSignature();
    if (sig instanceof ResolvedMember) {
      final ResolvedMember rsig = (ResolvedMember) sig;
  ...
  public int compareTo(@NotNull Object other) {
    ...
    final BcelAdvice o = (BcelAdvice) other;
    ...
    if (kind.isCflow()) {
      ...
      final boolean isBelow = (kind == AdviceKind.CflowBelowEntry);
    ...
    final int ret = concreteAspect.getWorld().compareByPrecedence(concreteAspect, o.concreteAspect);
    if (ret != 0) {
      return ret;
    }

    final ResolvedType declaringAspect = getDeclaringAspect().resolve(world);
    final ResolvedType o_declaringAspect = o.getDeclaringAspect().resolve(world);
  ...
  public BcelVar[] getExposedStateAsBcelVars(boolean isAround) {
    ...
    final int len = exposedState.vars.length;
    final BcelVar[] ret = new BcelVar[len];
    for (int i = 0; i < len; i++) {
