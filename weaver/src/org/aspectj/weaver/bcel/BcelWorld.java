...
public class BcelWorld extends World implements Repository {
  ...
  public static Type[] makeBcelTypes(String[] types) {
    ...
    final Type[] ret = new Type[types.length];
  ...
  public static Member makeFieldJoinPointSignature(LazyClassGen cg, FieldInstruction fi) {
    final ConstantPool cpg = cg.getConstantPool();
  ...
  public static BcelObjectType getBcelObjectType(ResolvedType concreteAspect) {
    ...
    final ReferenceTypeDelegate rtDelegate = ((ReferenceType) concreteAspect).getDelegate();
  ...
  public BcelWorld(List classPath, @NotNull IMessageHandler handler, ICrossReferenceHandler xrefHandler) {
  ...
  public BcelWorld(ClassPathManager cpm, @NotNull IMessageHandler handler, ICrossReferenceHandler xrefHandler) {
    classPath = cpm;
    setMessageHandler(handler);
    setCrossReferenceHandler(xrefHandler);
    // Tell BCEL to use us for resolving any classes
   ...
   * @param xrefHandler
   */
  public BcelWorld(ClassLoader loader, @NotNull IMessageHandler handler, ICrossReferenceHandler xrefHandler) {
    classPath = null;
  ...
  public IRelationship.Kind determineRelKind(ShadowMunger munger) {
    final AdviceKind ak = ((Advice) munger).getKind();
  ...
  public BcelObjectType buildBcelDelegate(ReferenceType type, JavaClass jc, boolean artificial, boolean exposedToWeaver) {
    final BcelObjectType ret = new BcelObjectType(type, jc, artificial, exposedToWeaver);
    return ret;
  }
  ...
  public BcelObjectType addSourceObjectType(String classname, JavaClass jc, boolean artificial) {
    ...
    final String signature = UnresolvedType.forName(jc.getClassName()).getSignature();

    final ResolvedType fromTheMap = typeMap.get(signature);
    ...
    if (nameTypeX == null) {
      if (jc.isGeneric() && isInJava5Mode()) {
        final ReferenceType rawType = ReferenceType.fromTypeX(UnresolvedType.forRawTypeName(jc.getClassName()), this);
        ret = buildBcelDelegate(rawType, jc, artificial, true);
        final ReferenceType genericRefType = new ReferenceType(UnresolvedType.forGenericTypeSignature(signature,
  ...
  public BcelObjectType addSourceObjectType(String classname, byte[] bytes, boolean artificial) {
    BcelObjectType ret = null;
    final String signature = UnresolvedType.forName(classname).getSignature();
    final ResolvedType fromTheMap = typeMap.get(signature);
    ...
    if (nameTypeX == null) {
      final JavaClass jc = Utility.makeJavaClass(classname, bytes);
      if (jc.isGeneric() && isInJava5Mode()) {
      ...
      final Object o = nameTypeX.getDelegate();
  ...
  public Member makeJoinPointSignatureFromMethod(LazyMethodGen mg, MemberKind kind) {
    final Member ret = mg.getMemberView();
    if (ret == null) {
  ...
  public Member makeJoinPointSignatureForArrayConstruction(LazyClassGen cg, InstructionHandle handle) {
    ...
    if (i.opcode == Constants.ANEWARRAY) {
      ...
      final Type ot = i.getType(cpg);
      UnresolvedType ut = fromBcel(ot);
      ut = UnresolvedType.makeArray(ut, 1);
      retval = MemberImpl.method(ut, Modifier.PUBLIC, UnresolvedType.VOID, "<init>", new ResolvedType[]{INT});
    } else if (i instanceof MULTIANEWARRAY) {
      final MULTIANEWARRAY arrayInstruction = (MULTIANEWARRAY) i;
      UnresolvedType ut = null;
      final short dimensions = arrayInstruction.getDimensions();
      final ObjectType ot = arrayInstruction.getLoadClassType(cpg);
      if (ot != null) {
        ut = fromBcel(ot);
        ut = UnresolvedType.makeArray(ut, dimensions);
      } else {
        final Type t = arrayInstruction.getType(cpg);
        ut = fromBcel(t);
      }
      final ResolvedType[] parms = new ResolvedType[dimensions];
      ...
      final Type ot = i.getType();
      final UnresolvedType ut = fromBcel(ot);
      retval = MemberImpl.method(ut, Modifier.PUBLIC, UnresolvedType.VOID, "<init>", new ResolvedType[]{INT});
    } else {
  ...
  public Member makeJoinPointSignatureForMethodInvocation(LazyClassGen cg, InvokeInstruction ii) {
    final ConstantPool cpg = cg.getConstantPool();
    final String name = ii.getName(cpg);
    final String declaring = ii.getClassName(cpg);
    UnresolvedType declaringType = null;

    final String signature = ii.getSignature(cpg);

    final int modifier = (ii instanceof INVOKEINTERFACE) ? Modifier.INTERFACE
    ...
    if (ii.opcode == Constants.INVOKESTATIC) {
      final ResolvedType appearsDeclaredBy = resolve(declaring);
      // look for the method there
      for (final Iterator<ResolvedMember> iterator = appearsDeclaredBy.getMethods(true, true); iterator.hasNext(); ) {
        final ResolvedMember method = iterator.next();
  ...
  public void validateType(UnresolvedType type) {
    final ResolvedType result = typeMap.get(type.getSignature());
    if (result == null) {
  ...
  public void reportCheckerMatch(Checker checker, Shadow shadow) {
    final IMessage iMessage = new Message(checker.getMessage(shadow), shadow.toString(), checker.isError() ? IMessage.ERROR
  ...
  public void setXmlFiles(List<File> xmlFiles) {
    ...
    for (File xmlfile : xmlFiles) {
      try {
        final Definition d = DocumentParser.parse(xmlfile.toURI().toURL());
  ...
  public boolean hasUnsatisfiedDependency(ResolvedType aspectType) {
    ...
    final String aspectName = aspectType.getName();
    if (!aspectRequiredTypesProcessed.contains(aspectName)) {
      final String requiredTypeName = aspectRequiredTypes.get(aspectName);
      if (requiredTypeName == null) {
        aspectRequiredTypesProcessed.add(aspectName);
        return false;
      } else {
        final ResolvedType rt = resolve(UnresolvedType.forName(requiredTypeName));
        if (!rt.isMissing()) {
          aspectRequiredTypesProcessed.add(aspectName);
  ...
  protected ReferenceTypeDelegate resolveDelegate(ReferenceType ty) {
    final String name = ty.getName();
    ensureAdvancedConfigurationProcessed();
    final JavaClass jc = lookupJavaClass(classPath, name);
    if (jc == null) {
      // Anyone else to ask?
      if (typeDelegateResolvers != null) {
        for (TypeDelegateResolver tdr : typeDelegateResolvers) {
          final ReferenceTypeDelegate delegate = tdr.getDelegate(ty);
          if (delegate != null) {
            return delegate;
          }
        }
  ...
  protected void weaveInterTypeDeclarations(ResolvedType onType) {
    ...
    final List<DeclareParents> declareParentsList = getCrosscuttingMembersSet().getDeclareParents();
    if (onType.isRawType()) {
    ...
    for (final Iterator<DeclareParents> i = declareParentsList.iterator(); i.hasNext(); ) {
      final DeclareParents decp = i.next();
  ...
  static UnresolvedType[] fromBcel(Type[] ts) {
    final UnresolvedType[] ret = new UnresolvedType[ts.length];
  ...
  private void reportWeavingMessage(ShadowMunger munger, Shadow shadow) {
    ...
    final AdviceKind aKind = advice.getKind();
    ...
    final String description = advice.getKind().toString();
    final String advisedType = shadow.getEnclosingType().getName();
    final String advisingType = advice.getConcreteAspect().getName();
    Message msg = null;
    if (advice.getKind().equals(AdviceKind.Softener)) {
      ...
      final String joinPointDescription = shadow.toString();
      msg = WeaveMessage
  ...
  private boolean areTheSame(ISourceLocation locA, ISourceLocation locB) {
    ...
    final File fA = locA.getSourceFile();
    final File fB = locA.getSourceFile();
    if (fA == null) {
      return fB == null;
    }
    if (fB == null) {
  ...
  private String beautifyLocation(ISourceLocation isl) {
    final StringBuilder nice = new StringBuilder();
    if (isl == null || isl.getSourceFile() == null || isl.getSourceFile().getName().indexOf("no debug info available") != -1) {
      ...
      final int binary = isl.getSourceFile().getPath().lastIndexOf('!');
      if (binary != -1 && binary < takeFrom) {
        // we have been woven by a binary aspect
        final String pathToBinaryLoc = isl.getSourceFile().getPath().substring(0, binary + 1);
  ...
  private static List<String> getPathEntries(String s) {
    ...
    final StringTokenizer tok = new StringTokenizer(s, File.pathSeparator);
  ...
  private JavaClass lookupJavaClass(ClassPathManager classPath, String name) {
    if (classPath == null) {
      try {
        ...
        final JavaClass jc = delegate.loadClass(name);
    ...
    try {
      ...
      final ClassParser parser = new ClassParser(file.getInputStream(), file.getPath());
  ...
  private boolean applyDeclareParents(DeclareParents p, ResolvedType onType) {
    ...
    if (!newParents.isEmpty()) {
      didSomething = true;
      final BcelObjectType classType = BcelWorld.getBcelObjectType(onType);
      ...
      for (ResolvedType newParent : newParents) {
        ...
        final ResolvedTypeMunger newParentMunger = new NewParentTypeMunger(newParent, p.getDeclaringType());
  ...
  private boolean applyDeclareAtType(DeclareAnnotation decA, ResolvedType onType, boolean reportProblems) {
    ...
    if (decA.matches(onType)) {
      ...
      final boolean isOK = checkTargetOK(decA, onType, annoX);
  ...
  private boolean applyDeclareAtField(DeclareAnnotation deca, ResolvedType type) {
    ...
    final ResolvedMember[] fields = type.getDeclaredFields();
    for (ResolvedMember field : fields) {
      if (deca.matches(field, this)) {
        final AnnotationAJ anno = deca.getAnnotation();
  ...
  static class WeavingXmlConfig {
    ...
    public void addScopedAspect(String aspectName, String scope) {
      ...
      try {
        final TypePattern scopePattern = new PatternParser(scope).parseTypePattern();
      ...
      if (!initialized) {
        try {
          ...
          for (Definition definition : definitions) {
            final List<String> aspectNames = definition.getAspectClassNames();
            for (String name : aspectNames) {
              ...
              // }
              final String scope = definition.getScopeForAspect(name);
              if (scope != null) {
            ...
            try {
              final List<String> includePatterns = definition.getIncludePatterns();
              ...
              for (String includePattern : includePatterns) {
                if (includePattern.endsWith("..*")) {
                  ...
                  final TypePattern includedPattern = new PatternParser(includePattern).parseTypePattern();
                  includedPatterns.add(includedPattern);
                }
              }
              final List<String> excludePatterns = definition.getExcludePatterns();
              ...
              for (String excludePattern : excludePatterns) {
                if (excludePattern.endsWith("..*")) {
                  ...
                  final TypePattern excludedPattern = new PatternParser(excludePattern).parseTypePattern();
