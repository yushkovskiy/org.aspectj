...
public final class BcelWeaver {
  ...
  public BcelWeaver(@NotNull BcelWorld world) {
    super();
  ...
  public ResolvedType addLibraryAspect(String aspectName) {
    ...
    // 1 - resolve as is
    final UnresolvedType unresolvedT = UnresolvedType.forName(aspectName);
    ...
    if (type.isMissing()) {
      ...
      while (hasDot > 0) {
        ...
        // fixedName);
        final char[] fixedNameChars = fixedName.toCharArray();
        fixedNameChars[hasDot] = '$';
        fixedName = new String(fixedNameChars);
        hasDot = fixedName.lastIndexOf('.');
        final UnresolvedType ut = UnresolvedType.forName(fixedName);
    ...
    if (type.isAspect()) {
      ...
      final WeaverStateInfo wsi = type.getWeaverState();
      if (wsi != null && wsi.isReweavable()) {
        final BcelObjectType classType = getClassType(type.getName());
        final JavaClass wovenJavaClass = classType.getJavaClass();
        final byte[] bytes = wsi.getUnwovenClassFileData(wovenJavaClass.getBytes());
        final JavaClass unwovenJavaClass = Utility.makeJavaClass(wovenJavaClass.getFileName(), bytes);
      ...
      final RuntimeException ex = new RuntimeException("Cannot register non aspect: " + type.getName() + " , " + aspectName);
      if (trace.isTraceEnabled()) {
   ...
   */
  public void addLibraryJarFile(@NotNull File inFile) throws IOException {
  ...
  public List<UnwovenClassFile> addDirectoryContents(File inFile, File outDir) throws IOException {
    final List<UnwovenClassFile> addedClassFiles = new ArrayList<UnwovenClassFile>();

    // Get a list of all files (i.e. everything that isnt a directory)
    final File[] files = FileUtil.listFiles(inFile, new FileFilter() {
      @Override
      public boolean accept(File f) {
        final boolean accept = !f.isDirectory();
        return accept;
      }
    });
   ...
   */
  @NotNull
  public List<UnwovenClassFile> addJarFile(@NotNull File inFile, File outDir, boolean canBeDirectory) {
    ...
    try {
      ...
      if (inFile.isDirectory() && canBeDirectory) {
        ...
        try {
          ...
          final Enumeration entries = inJar.entries();

          while (entries.hasMoreElements()) {
            final JarEntry entry = (JarEntry) entries.nextElement();
            final InputStream inStream = inJar.getInputStream(entry);

            final byte[] bytes = FileUtil.readAsByteArray(inStream);
            final String filename = entry.getName();
            // System.out.println("? addJarFile() filename='" + filename
            // + "'");
            final UnwovenClassFile classFile = new UnwovenClassFile(new File(outDir, filename).getAbsolutePath(), bytes);

            if (filename.endsWith(".class")) {
              final ReferenceType type = this.addClassFile(classFile, false);
      ...
      final IMessage message = new Message("Could not find input jar file " + inFile.getPath() + ", ignoring", new SourceLocation(
          inFile, 0), false);
      world.getMessageHandler().handleMessage(message);
    } catch (IOException ex) {
      final IMessage message = new Message("Could not read input jar file " + inFile.getPath() + "(" + ex.getMessage() + ")",
      ...
      if (inJar != null) {
        try {
          ...
          final IMessage message = new Message("Could not close input jar file " + inFile.getPath() + "(" + ex.getMessage()
              + ")", new SourceLocation(inFile, 0), true);
          world.getMessageHandler().handleMessage(message);
        }
      }
  ...
  public ReferenceType addClassFile(UnwovenClassFile classFile, boolean fromInpath) {
    ...
    final ReferenceType type = world.addSourceObjectType(classFile.getJavaClass(), false).getResolvedTypeX();
    if (fromInpath) {
  ...
  public UnwovenClassFile addClassFile(File classFile, File inPathDir, File outDir) throws IOException {
    ...
    final byte[] bytes = FileUtil.readAsByteArray(fis);
    // String relativePath = files[i].getPath();

    // ASSERT:
    // files[i].getAbsolutePath().startsWith(inFile.getAbsolutePath()
    // or we are in trouble...
    final String filename = classFile.getAbsolutePath().substring(inPathDir.getAbsolutePath().length() + 1);
    final UnwovenClassFile ucf = new UnwovenClassFile(new File(outDir, filename).getAbsolutePath(), bytes);
    if (filename.endsWith(".class")) {
      // System.err.println(
      ...
      sb.append("!");
      sb.append(filename);
      final ReferenceType type = this.addClassFile(ucf, false);
      type.setBinaryPath(sb.toString());
    }
    fis.close();
    return ucf;
  ...
  public void prepareForWeave() {
    ...
    // update mungers
    for (final Iterator<UnwovenClassFile> i = addedClasses.iterator(); i.hasNext(); ) {
      final UnwovenClassFile jc = i.next();
      final String name = jc.getClassName();
    ...
    for (final Iterator<String> i = deletedTypenames.iterator(); i.hasNext(); ) {
  ...
  public void setCustomMungerFactory(@Nullable CustomMungerFactory factory) {
  ...
  public Manifest getManifest(boolean shouldCreate) {
    if (manifest == null && shouldCreate) {
      final String WEAVER_MANIFEST_VERSION = "1.0";
      final Attributes.Name CREATED_BY = new Name("Created-By");
      final String WEAVER_CREATED_BY = "AspectJ Compiler";
  ...
  @TestOnly
  @NotNull
  public Collection<String> weave(@NotNull File file) throws IOException {
    final OutputStream os = FileUtil.makeOutputStream(file);
    this.zipOutputStream = new ZipOutputStream(os);
    prepareForWeave();
    final Collection<String> c = weave(new IClassFileProvider() {
  ...
  @NotNull
  public Collection<String> weave(@NotNull IClassFileProvider input) throws IOException {
    if (trace.isTraceEnabled()) {
      trace.enter("weave", this, input);
    }
    final ContextToken weaveToken = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.WEAVING, "");
    final Collection<String> wovenClassNames = new ArrayList<String>();
    final IWeaveRequestor requestor = input.getRequestor();
    ...
    if (world.getModel() != null && !isBatchWeave) {
      final AsmManager manager = world.getModelAsAsmManager();
      for (final Iterator<UnwovenClassFile> i = input.getClassFileIterator(); i.hasNext(); ) {
        final UnwovenClassFile classFile = i.next();
    ...
    for (final Iterator<UnwovenClassFile> i = input.getClassFileIterator(); i.hasNext(); ) {
      ...
      final ResolvedType theType = world.resolve(className);
      if (theType != null) {
    ...
    if (input.isApplyAtAspectJMungersOnly()) {
      final ContextToken atAspectJMungersOnly = CompilationAndWeavingContext.enteringPhase(
      ...
      for (final Iterator<UnwovenClassFile> i = input.getClassFileIterator(); i.hasNext(); ) {
        ...
        if (theType.isAnnotationStyleAspect()) {
          final BcelObjectType classType = BcelWorld.getBcelObjectType(theType);
          if (classType == null) {
            throw new BCException("Can't find bcel delegate for " + className + " type=" + theType.getClass());
          }
          final LazyClassGen clazz = classType.getLazyClassGen();
          final BcelPerClauseAspectAdder selfMunger = new BcelPerClauseAspectAdder(theType, theType.getPerClause().getKind());
          selfMunger.forceMunge(clazz, true);
          classType.finishedWith();
          final UnwovenClassFile[] newClasses = getClassFilesFor(clazz);
    ...
    final ContextToken reweaveToken = CompilationAndWeavingContext.enteringPhase(
    ...
    for (final Iterator<UnwovenClassFile> i = input.getClassFileIterator(); i.hasNext(); ) {
      final UnwovenClassFile classFile = i.next();
      final String className = classFile.getClassName();
      final BcelObjectType classType = getClassType(className);
    ...
    final ContextToken typeMungingToken = CompilationAndWeavingContext.enteringPhase(
    ...
    final List<String> typesToProcess = new ArrayList<String>();
    for (final Iterator<UnwovenClassFile> iter = input.getClassFileIterator(); iter.hasNext(); ) {
      final UnwovenClassFile clf = iter.next();
    ...
    requestor.weavingAspects();
    final ContextToken aspectToken = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.WEAVING_ASPECTS, "");
    ...
    for (final Iterator<UnwovenClassFile> i = input.getClassFileIterator(); i.hasNext(); ) {
      ...
      if (theType.isAspect()) {
        ...
        if (classType == null) {
          ...
          final ReferenceTypeDelegate theDelegate = ((ReferenceType) theType).getDelegate();
    ...
    final ContextToken classToken = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.WEAVING_CLASSES, "");
  ...
  public void processReweavableStateIfPresent(String className, BcelObjectType classType) {
    ...
    final WeaverStateInfo wsi = classType.getWeaverState();
    ...
    if (wsi != null && wsi.isReweavable()) { // Check all necessary types
      ...
      final Set<String> aspectsPreviouslyInWorld = wsi.getAspectsAffectingType();
      // keep track of them just to ensure unique missing aspect error
      // reporting
      final Set<String> alreadyConfirmedReweavableState = new HashSet<String>();
      for (String requiredTypeSignature : aspectsPreviouslyInWorld) {
        ...
        if (!alreadyConfirmedReweavableState.contains(requiredTypeSignature)) {
          final ResolvedType rtx = world.resolve(UnresolvedType.forSignature(requiredTypeSignature), true);
          final boolean exists = !rtx.isMissing();
          if (!exists) {
      ...
      if (!world.isOverWeaving()) {
        final byte[] bytes = wsi.getUnwovenClassFileData(classType.getJavaClass().getBytes());
        final WeaverVersionInfo wvi = classType.getWeaverVersionAttribute();
        final JavaClass newJavaClass = Utility.makeJavaClass(classType.getJavaClass().getFileName(), bytes);
  ...
  public UnwovenClassFile[] getClassFilesFor(LazyClassGen clazz) {
    final List<UnwovenClassFile.ChildClass> childClasses = clazz.getChildClasses(world);
    final UnwovenClassFile[] ret = new UnwovenClassFile[1 + childClasses.size()];
    ret[0] = new UnwovenClassFile(clazz.getFileName(), clazz.getClassName(), clazz.getJavaClassBytesIncludingReweavable(world));
    int index = 1;
    for (final Iterator<UnwovenClassFile.ChildClass> iter = childClasses.iterator(); iter.hasNext(); ) {
      final UnwovenClassFile.ChildClass element = iter.next();
      final UnwovenClassFile childClass = new UnwovenClassFile(clazz.getFileName() + "$" + element.name, element.bytes);
   ...
   */
  public void weaveParentTypeMungers(@NotNull ResolvedType onType) {
    ...
    for (DeclareParents decp : declareParentsList) {
      final boolean typeChanged = applyDeclareParents(decp, onType);
      if (typeChanged) {
    ...
    for (DeclareAnnotation decA : xcutSet.getDeclareAnnotationOnTypes()) {
      final boolean typeChanged = applyDeclareAtType(decA, onType, true);
      if (typeChanged) {
    ...
    while ((aParentChangeOccurred || anAnnotationChangeOccurred) && !decpToRepeat.isEmpty()) {
      ...
      final List<DeclareParents> decpToRepeatNextTime = new ArrayList<DeclareParents>();
      for (final Iterator<DeclareParents> iter = decpToRepeat.iterator(); iter.hasNext(); ) {
        final DeclareParents decp = iter.next();
        final boolean typeChanged = applyDeclareParents(decp, onType);
        if (typeChanged) {
          aParentChangeOccurred = true;
        } else {
      ...
      for (DeclareAnnotation decA : xcutSet.getDeclareAnnotationOnTypes()) {
        final boolean typeChanged = applyDeclareAtType(decA, onType, false);
        if (typeChanged) {
          anAnnotationChangeOccurred = true;
        }
      }
  ...
  public void weaveNormalTypeMungers(@NotNull ResolvedType onType) {
    final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.PROCESSING_TYPE_MUNGERS,
        onType.getName());
    if (onType.isRawType() || onType.isParameterizedType()) {
      onType = onType.getGenericType();
    }
  ...
  @TestOnly
  public void setShadowMungers(@NotNull List<ShadowMunger> shadowMungers) {
  ...
  LazyClassGen weave(UnwovenClassFile classFile, BcelObjectType classType) throws IOException {
    final LazyClassGen ret = weave(classFile, classType, true);
    return ret;
  }

  @NotNull
  private List<ResolvedType> addAspectsFromJarFile(File inFile) throws FileNotFoundException, IOException {
    final ZipInputStream inStream = new ZipInputStream(new FileInputStream(inFile)); // ??? buffered
    final List<ResolvedType> addedAspects = new ArrayList<ResolvedType>();
    try {
      while (true) {
        ...
        final ClassParser parser = new ClassParser(new ByteArrayInputStream(FileUtil.readAsByteArray(inStream)), entry.getName());
        final JavaClass jc = parser.parse();
        inStream.closeEntry();

        final ResolvedType type = world.addSourceObjectType(jc, false).getResolvedTypeX();
  ...
  private List<ResolvedType> addAspectsFromDirectory(File directory) throws FileNotFoundException, IOException {
    final List<ResolvedType> addedAspects = new ArrayList<ResolvedType>();
    final File[] classFiles = FileUtil.listFiles(directory, new FileFilter() {
      @Override
    ...
    for (File classFile : classFiles) {
      final FileInputStream fis = new FileInputStream(classFile);
      final byte[] classBytes = FileUtil.readAsByteArray(fis);
      final ResolvedType aspectType = isAspect(classBytes, classFile.getAbsolutePath(), directory);
      if (aspectType != null) {
   ...
   * @param dir        directory which contained the class file
  ...
  private ResolvedType isAspect(byte[] classbytes, String name, File dir) throws IOException {
    final ClassParser parser = new ClassParser(new ByteArrayInputStream(classbytes), name);
    final JavaClass jc = parser.parse();
    final ResolvedType type = world.addSourceObjectType(jc, false).getResolvedTypeX();
    final String typeName = type.getName().replace('.', File.separatorChar);
    final int end = name.lastIndexOf(typeName + ".class");
  ...
  private void addCustomMungers() {
    if (customMungerFactory != null) {
      for (final Iterator<UnwovenClassFile> i = addedClasses.iterator(); i.hasNext(); ) {
        ...
        final ResolvedType type = world.resolve(name);
        if (type.isAspect()) {
          final Collection<ShadowMunger> shadowMungers = customMungerFactory.createCustomShadowMungers(type);
          if (shadowMungers != null) {
            shadowMungerList.addAll(shadowMungers);
          }
          final Collection<ConcreteTypeMunger> typeMungers = customMungerFactory.createCustomTypeMungers(type);
          if (typeMungers != null) {
  ...
  private void rewritePointcuts(List<ShadowMunger> shadowMungers) {
    final PointcutRewriter rewriter = new PointcutRewriter();
    for (ShadowMunger munger : shadowMungers) {
      final Pointcut p = munger.getPointcut();
      final Pointcut newP = rewriter.rewrite(p);
      ...
      if (munger instanceof Advice) {
        ...
        if (advice.getSignature() != null) {
          ...
          if ((advice.getConcreteAspect().isAnnotationStyleAspect() && advice.getDeclaringAspect() != null && advice
            ...
            final int numArgs = advice.getSignature().getParameterTypes().length;
            if (numFormals > 0) {
    ...
    // pc identity.
    final Map<Pointcut, Pointcut> pcMap = new HashMap<Pointcut, Pointcut>();
    for (ShadowMunger munger : shadowMungers) {
      final Pointcut p = munger.getPointcut();
      final Pointcut newP = shareEntriesFromMap(p, pcMap);
      newP.m_ignoreUnboundBindingForNames = p.m_ignoreUnboundBindingForNames;
      munger.setPointcut(newP);
    }
  }
  ...
  @NotNull
  private Pointcut shareEntriesFromMap(@NotNull Pointcut p, Map<Pointcut, Pointcut> pcMap) {
    ...
    if (p instanceof AndPointcut) {
      final AndPointcut apc = (AndPointcut) p;
      final Pointcut left = shareEntriesFromMap(apc.getLeft(), pcMap);
      final Pointcut right = shareEntriesFromMap(apc.getRight(), pcMap);
      return new AndPointcut(left, right);
    } else if (p instanceof OrPointcut) {
      final OrPointcut opc = (OrPointcut) p;
      final Pointcut left = shareEntriesFromMap(opc.getLeft(), pcMap);
      final Pointcut right = shareEntriesFromMap(opc.getRight(), pcMap);
      return new OrPointcut(left, right);
    } else if (p instanceof NotPointcut) {
      final NotPointcut npc = (NotPointcut) p;
      final Pointcut not = shareEntriesFromMap(npc.getNegatedPointcut(), pcMap);
  ...
  private void validateBindings(Pointcut dnfPointcut, Pointcut userPointcut, int numFormals, String[] names) {
    ...
    if (dnfPointcut instanceof OrPointcut) {
      final OrPointcut orBasedDNFPointcut = (OrPointcut) dnfPointcut;
      final Pointcut[] leftBindings = new Pointcut[numFormals];
      final Pointcut[] rightBindings = new Pointcut[numFormals];
      validateOrBranch(orBasedDNFPointcut, userPointcut, numFormals, names, leftBindings, rightBindings);
    } else {
      final Pointcut[] bindings = new Pointcut[numFormals];
  ...
  private void validateOrBranch(OrPointcut pc, Pointcut userPointcut, int numFormals, String[] names, Pointcut[] leftBindings,
    ...
    final Pointcut left = pc.getLeft();
    final Pointcut right = pc.getRight();
    if (left instanceof OrPointcut) {
      final Pointcut[] newRightBindings = new Pointcut[numFormals];
    ...
    if (right instanceof OrPointcut) {
      final Pointcut[] newLeftBindings = new Pointcut[numFormals];
    ...
    final int kindsInCommon = left.couldMatchKinds() & right.couldMatchKinds();
  ...
  private void validateSingleBranch(Pointcut pc, Pointcut userPointcut, int numFormals, String[] names, Pointcut[] bindings) {
    final boolean[] foundFormals = new boolean[numFormals];
  ...
  private void validateSingleBranchRecursion(Pointcut pc, Pointcut userPointcut, boolean[] foundFormals, String[] names,
    ...
    if (pc instanceof NotPointcut) {
      ...
      final NotPointcut not = (NotPointcut) pc;
      if (not.getNegatedPointcut() instanceof NameBindingPointcut) {
        final NameBindingPointcut nnbp = (NameBindingPointcut) not.getNegatedPointcut();
      ...
      final AndPointcut and = (AndPointcut) pc;
      ...
      final List/* BindingTypePattern */btps = ((NameBindingPointcut) pc).getBindingTypePatterns();
      for (final Iterator iter = btps.iterator(); iter.hasNext(); ) {
        final BindingTypePattern btp = (BindingTypePattern) iter.next();
        final int index = btp.getFormalIndex();
        bindings[index] = pc;
      ...
      final List/* BindingPattern */baps = ((NameBindingPointcut) pc).getBindingAnnotationTypePatterns();
      for (final Iterator iter = baps.iterator(); iter.hasNext(); ) {
        final BindingPattern bap = (BindingPattern) iter.next();
        final int index = bap.getFormalIndex();
        bindings[index] = pc;
        if (foundFormals[index]) {
          raiseAmbiguousBindingError(names[index], userPointcut);
        } else {
      ...
      final ConcreteCflowPointcut cfp = (ConcreteCflowPointcut) pc;
      final int[] slots = cfp.getUsedFormalSlots();
  ...
  private boolean couldEverMatchSameJoinPoints(Pointcut left, Pointcut right) {
    ...
    if (left instanceof OrPointcut) {
      final OrPointcut leftOrPointcut = (OrPointcut) left;
    ...
    if (right instanceof OrPointcut) {
      final OrPointcut rightOrPointcut = (OrPointcut) right;
    ...
    // look for withins
    final WithinPointcut leftWithin = (WithinPointcut) findFirstPointcutIn(left, WithinPointcut.class);
    final WithinPointcut rightWithin = (WithinPointcut) findFirstPointcutIn(right, WithinPointcut.class);
    ...
    // look for kinded
    final KindedPointcut leftKind = (KindedPointcut) findFirstPointcutIn(left, KindedPointcut.class);
    final KindedPointcut rightKind = (KindedPointcut) findFirstPointcutIn(right, KindedPointcut.class);
  ...
  private Pointcut findFirstPointcutIn(Pointcut toSearch, Class toLookFor) {
    ...
    if (toSearch instanceof AndPointcut) {
      final AndPointcut apc = (AndPointcut) toSearch;
      final Pointcut left = findFirstPointcutIn(apc.getLeft(), toLookFor);
      if (left != null) {
        return left;
      }
   ...
   * @param userPointcut
   */
  private void raiseNegationBindingError(@NotNull Pointcut userPointcut) {
  ...
  private void raiseAmbiguityInDisjunctionError(Pointcut userPointcut, List<String> names) {
    final StringBuffer formalNames = new StringBuffer(names.get(0).toString());
  ...
  private void warnOnUnmatchedAdvice() {
    ...
    class AdviceLocation {
      ...
      public boolean equals(Object obj) {
        ...
        final AdviceLocation other = (AdviceLocation) obj;
    ...
    if (world.isInJava5Mode() && world.getLint().adviceDidNotMatch.isEnabled()) {
      final List l = world.getCrosscuttingMembersSet().getShadowMungers();
      final Set<AdviceLocation> alreadyWarnedLocations = new HashSet<AdviceLocation>();

      for (final Iterator iter = l.iterator(); iter.hasNext(); ) {
        final ShadowMunger element = (ShadowMunger) iter.next();
        // This will stop us incorrectly reporting deow checkers:
        if (element instanceof BcelAdvice) {
          final BcelAdvice ba = (BcelAdvice) element;
          ...
          if (!ba.hasMatchedSomething()) {
            ...
            if (ba.getSignature() != null) {
              ...
              final AdviceLocation loc = new AdviceLocation(ba);
  ...
  private void weaveParentsFor(List<String> typesForWeaving, String typeToWeave, ResolvedType resolvedTypeToWeave) {
    ...
    final ResolvedType superclassType = resolvedTypeToWeave.getSuperclass();
    final String superclassTypename = (superclassType == null ? null : superclassType.getName());
    ...
    final ResolvedType[] interfaceTypes = resolvedTypeToWeave.getDeclaredInterfaces();
    for (ResolvedType resolvedSuperInterface : interfaceTypes) {
      if (!resolvedSuperInterface.isTypeHierarchyComplete()) {
        final String interfaceTypename = resolvedSuperInterface.getName();
    ...
    final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.PROCESSING_DECLARE_PARENTS,
  ...
  private void weaveAndNotify(UnwovenClassFile classFile, BcelObjectType classType, IWeaveRequestor requestor) throws IOException {
    ...
    final ContextToken tok = CompilationAndWeavingContext.enteringPhase(CompilationAndWeavingContext.WEAVING_TYPE, classType
        .getResolvedTypeX().getName());
    final LazyClassGen clazz = weaveWithoutDump(classFile, classType);
    classType.finishedWith();
  ...
  private boolean applyDeclareAtType(DeclareAnnotation decA, ResolvedType onType, boolean reportProblems) {
    ...
    if (decA.matches(onType)) {
      final AnnotationAJ theAnnotation = decA.getAnnotation();
      ...
      final AnnotationAJ annoX = decA.getAnnotation();

      // check the annotation is suitable for the target
      final boolean problemReported = verifyTargetIsOK(decA, onType, annoX, reportProblems);
      ...
      if (!problemReported) {
        ...
        didSomething = true;
        final ResolvedTypeMunger newAnnotationTM = new AnnotationOnTypeMunger(annoX);
  ...
  private boolean applyDeclareParents(DeclareParents p, ResolvedType onType) {
    boolean didSomething = false;
    final List<ResolvedType> newParents = p.findMatchingNewParents(onType, true);
    if (!newParents.isEmpty()) {
      ...
      for (ResolvedType newParent : newParents) {
        ...
        final NewParentTypeMunger newParentMunger = new NewParentTypeMunger(newParent, p.getDeclaringType());
        if (p.isMixin()) {
  ...
  private LazyClassGen weave(UnwovenClassFile classFile, BcelObjectType classType, boolean dump) throws IOException {
    try {
      ...
      final ReferenceType resolvedClassType = classType.getResolvedTypeX();
      ...
      final List<ShadowMunger> shadowMungers = fastMatch(shadowMungerList, resolvedClassType);
      final List<ConcreteTypeMunger> typeMungers = classType.getResolvedTypeX().getInterTypeMungers();

      resolvedClassType.checkInterTypeMungers();

      // Decide if we need to do actual weaving for this class
      final boolean mightNeedToWeave = shadowMungers.size() > 0 || typeMungers.size() > 0 || classType.isAspect()
      ...
      // affected by ITDs
      final boolean mightNeedBridgeMethods = world.isInJava5Mode() && !classType.isInterface()
      ...
      if (mightNeedToWeave || mightNeedBridgeMethods) {
        ...
        try {
          ...
          final String messageText = "trouble in: \n" + classDebugInfo;
      ...
      final AsmManager model = world.getModelAsAsmManager();
      if (world.isMinimalModel() && model != null && !classType.isAspect()) {
        final AspectJElementHierarchy hierarchy = (AspectJElementHierarchy) model.getHierarchy();
        final String pkgname = classType.getResolvedTypeX().getPackageName();
        final String tname = classType.getResolvedTypeX().getSimpleBaseName();
        final IProgramElement typeElement = hierarchy.findElementForType(pkgname, tname);
        ...
        if (typeElement != null && !hasInnerType(typeElement)) {
          final IProgramElement parent = typeElement.getParent();
  ...
  private void walkUpRemovingEmptyTypesAndPossiblyEmptySourceFile(AspectJElementHierarchy hierarchy, String tname,
    ...
    if (candidatesForRemoval.contains(typeThatHasChildRemoved) && !hasInnerType(typeThatHasChildRemoved)) {
      ...
      final IProgramElement parent = typeThatHasChildRemoved.getParent();
      if (parent != null) {
  ...
  private void removeSourceFileIfNoMoreTypeDeclarationsInside(AspectJElementHierarchy hierarchy, IProgramElement typeElement,
    ...
    final IProgramElement compilationUnit = sourceFileNode;
    boolean anyOtherTypeDeclarations = false;
    for (IProgramElement child : compilationUnit.getChildren()) {
      final IProgramElement.Kind k = child.getKind();
      if (k.isType()) {
    ...
    if (!anyOtherTypeDeclarations) {
      final IProgramElement cuParent = compilationUnit.getParent();
      if (cuParent != null) {
  ...
  private boolean hasInnerType(IProgramElement typeNode) {
    for (IProgramElement child : typeNode.getChildren()) {
      final IProgramElement.Kind kind = child.getKind();
      if (kind.isType()) {
        return true;
      }
      ...
      if (kind.isType() || kind == IProgramElement.Kind.METHOD || kind == IProgramElement.Kind.CONSTRUCTOR) {
        final boolean b = hasInnerType(child);
        if (b) {
          return b;
        }
      }
  ...
  private void checkDeclareTypeErrorOrWarning(BcelWorld world2, BcelObjectType classType) {
    final List<DeclareTypeErrorOrWarning> dteows = world.getDeclareTypeEows();
  ...
  private void dump(UnwovenClassFile classFile, LazyClassGen clazz) throws IOException {
    if (zipOutputStream != null) {
      final String mainClassName = classFile.getJavaClass().getClassName();
      writeZipEntry(getEntryName(mainClassName), clazz.getJavaClass(world).getBytes());
      final List<UnwovenClassFile.ChildClass> childClasses = clazz.getChildClasses(world);
      if (!childClasses.isEmpty()) {
        for (final Iterator<UnwovenClassFile.ChildClass> i = childClasses.iterator(); i.hasNext(); ) {
          final UnwovenClassFile.ChildClass c = i.next();
  ...
  private void writeZipEntry(String name, byte[] bytes) throws IOException {
    final ZipEntry newEntry = new ZipEntry(name); // ??? get compression scheme
    // right
  ...
  private List<ShadowMunger> fastMatch(List<ShadowMunger> list, ResolvedType type) {
    ...
    final WeaverStateInfo typeWeaverState = (isOverweaving ? type.getWeaverState() : null);
    ...
    // weaving
    final FastMatchInfo info = new FastMatchInfo(type, null, world);

    final List<ShadowMunger> result = new ArrayList<ShadowMunger>();
    ...
    if (world.areInfoMessagesEnabled() && world.isTimingEnabled()) {
      for (ShadowMunger munger : list) {
        if (typeWeaverState != null) { // will only be null if overweaving is ON and there is weaverstate
          final ResolvedType declaringAspect = munger.getDeclaringType();
          if (typeWeaverState.isAspectAlreadyApplied(declaringAspect)) {
            continue;
          }
        }
        final Pointcut pointcut = munger.getPointcut();
        final long starttime = System.nanoTime();
        final FuzzyBoolean fb = pointcut.fastMatch(info);
      ...
      for (ShadowMunger munger : list) {
        if (typeWeaverState != null) { // will only be null if overweaving is ON and there is weaverstate
          final ResolvedType declaringAspect = munger.getConcreteAspect();// getDeclaringType();
          if (typeWeaverState.isAspectAlreadyApplied(declaringAspect)) {
            continue;
          }
        }
