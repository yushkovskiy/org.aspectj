...
final class BcelField extends ResolvedMemberImpl {
  ...
  private void unpackAttributes(World world) {
    ...
    if (attrs != null && attrs.length > 0) {
      final ISourceContext sourceContext = getSourceContext(world);
      final List<AjAttribute> as = Utility.readAjAttributes(getDeclaringType().getClassName(), attrs, sourceContext, world,
  ...
  public Field getField(ConstantPool cpool) {
    ...
    final FieldGen newFieldGen = new FieldGen(field, cpool);
  ...
  private void unpackGenericSignature() {
    ...
    final String gSig = field.getGenericSignature();
    if (gSig != null) {
      // get from generic
      final GenericSignature.FieldTypeSignature fts = new GenericSignatureParser().parseAsFieldSignature(gSig);
      final GenericSignature.ClassSignature genericTypeSig = bcelObjectType.getGenericClassTypeSignature();

      final GenericSignature.FormalTypeParameter[] parentFormals = bcelObjectType.getAllFormals();
      final GenericSignature.FormalTypeParameter[] typeVars = ((genericTypeSig == null) ? new GenericSignature.FormalTypeParameter[0]
