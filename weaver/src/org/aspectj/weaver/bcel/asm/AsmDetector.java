...
public class AsmDetector {
  ...
  static {
    try {
      final Class<?> reader = Class.forName("aj.org.objectweb.asm.ClassReader");
      final Class<?> visitor = Class.forName("aj.org.objectweb.asm.ClassVisitor");
      final Method m = reader.getMethod("accept", new Class[]{visitor, Integer.TYPE});
      isAsmAround = m != null;
    } catch (Exception e) {
      isAsmAround = false;
    }
