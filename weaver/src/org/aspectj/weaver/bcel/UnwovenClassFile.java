...
public class UnwovenClassFile implements IUnwovenClassFile {
  ...
  public String makeInnerFileName(String innerName) {
    final String prefix = filename.substring(0, filename.length() - 6); // strip the .class
  ...
  public void writeWovenBytes(byte[] bytes, List childClasses) throws IOException {
    ...
    final BufferedOutputStream os = FileUtil.makeOutputStream(new File(filename));
    os.write(bytes);
    os.close();
  ...
  private void writeChildClassFile(String innerName, byte[] bytes) throws IOException {
    final BufferedOutputStream os = FileUtil.makeOutputStream(new File(makeInnerFileName(innerName)));
    os.write(bytes);
    os.close();
  }
  ...
  protected void deleteChildClassFile(String innerName) {
    final File childClassFile = new File(makeInnerFileName(innerName));
  ...
  public static class ChildClass {
    ...
    public boolean equals(Object other) {
      if (!(other instanceof ChildClass))
        return false;
      final ChildClass o = (ChildClass) other;
