...
public class AnnotationAccessVar extends BcelVar {
  ...
  private InstructionList createLoadInstructions(ResolvedType toType, InstructionFactory fact) {
    ...
    final Type jlClass = BcelWorld.makeBcelType(UnresolvedType.JL_CLASS);
    final Type jlString = BcelWorld.makeBcelType(UnresolvedType.JL_STRING);
    final Type jlClassArray = BcelWorld.makeBcelType(UnresolvedType.JAVA_LANG_CLASS_ARRAY);
    final Type jlaAnnotation = BcelWorld.makeBcelType(UnresolvedType.JAVA_LANG_ANNOTATION);

    final Instruction pushConstant = fact.createConstant(new ObjectType(toType.getName()));
    ...
    if (kind == Shadow.MethodCall || kind == Shadow.MethodExecution || kind == Shadow.PreInitialization
      ...
      final Type jlrMethod = BcelWorld.makeBcelType(UnresolvedType.forSignature("Ljava/lang/reflect/Method;"));
      final Type jlAnnotation = BcelWorld.makeBcelType(UnresolvedType.forSignature("Ljava/lang/annotation/Annotation;"));
      final Type[] paramTypes = BcelWorld.makeBcelTypes(member.getParameterTypes());
      ...
      if (kind == Shadow.MethodCall
        ...
        final Field annotationCachingField = shadow.getEnclosingClass().getAnnotationCachingField(shadow, toType, isWithin);
        ...
        final InstructionHandle ifNullElse = il.append(InstructionConstants.NOP);
        ifNonNull.setTarget(ifNullElse);

      } else { // init/preinit/ctor-call/ctor-exec
        il.append(fact.createConstant(BcelWorld.makeBcelType(containingType)));
        buildArray(il, fact, jlClass, paramTypes, 1);
        final Type jlrCtor = BcelWorld.makeBcelType(UnresolvedType.JAVA_LANG_REFLECT_CONSTRUCTOR);
  ...
  private void generateBytecodeToAccessAnnotationAtFieldGetSetShadow(ResolvedType toType, InstructionFactory fact,
    ...
    final Type jlString = BcelWorld.makeBcelType(UnresolvedType.JL_STRING);
    final Type jlaAnnotation = BcelWorld.makeBcelType(UnresolvedType.JAVA_LANG_ANNOTATION);
    final Type jlrField = BcelWorld.makeBcelType(UnresolvedType.JAVA_LANG_REFLECT_FIELD);

    final LazyClassGen shadowEnclosingClass = shadow.getEnclosingClass();

    // The annotation for the field of interest is cached, check cached value before fetching it
    final Field annotationCachingField = shadowEnclosingClass.getAnnotationCachingField(shadow, toType, isWithin);
    final String annotationCachingFieldName = annotationCachingField.getName();

    // Basic idea here is to check if the cached field is null, if it is then initialize it, otherwise use it
    il.append(fact.createGetStatic(shadowEnclosingClass.getName(), annotationCachingFieldName, jlaAnnotation));
    il.appendDUP();
    final InstructionBranch ifNonNull = new InstructionBranch(Constants.IFNONNULL, null);
    il.append(ifNonNull);
    il.appendPOP();
    ...
    final InstructionHandle ifNullElse = il.appendNOP();
    ifNonNull.setTarget(ifNullElse);
  }
