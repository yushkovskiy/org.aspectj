...
public class BcelShadow extends Shadow {
  ...
  public BcelShadow copyInto(LazyMethodGen recipient, BcelShadow enclosing) {
    final BcelShadow s = new BcelShadow(world, getKind(), getSignature(), recipient, enclosing);
    if (mungers.size() > 0) {
      final List<ShadowMunger> src = mungers;
      if (s.mungers == Collections.EMPTY_LIST) {
        s.mungers = new ArrayList<ShadowMunger>();
      }
      final List<ShadowMunger> dest = s.mungers;
      for (final Iterator<ShadowMunger> i = src.iterator(); i.hasNext(); ) {
        dest.add(i.next());
      }
    }
    return s;
  ...
  private boolean deleteNewAndDup() {
    ...
    final InstructionHandle newHandle = ih;
    InstructionHandle endHandle = newHandle.getNext();
    final InstructionHandle nextHandle;
    if (endHandle.getInstruction().opcode == Constants.DUP) {
      ...
      final InstructionHandle dupHandle = endHandle;
  ...
  private void retargetFrom(InstructionHandle old, InstructionHandle fresh) {
    for (InstructionTargeter targeter : old.getTargetersCopy()) {
      if (targeter instanceof ExceptionRange) {
        final ExceptionRange it = (ExceptionRange) targeter;
  ...
  protected void prepareForMungers() {
    ...
    if (getKind() == ConstructorCall) {
      ...
      final ShadowRange range = getRange();
      range.insert(InstructionConstants.NOP, Range.InsideAfter);
    } else if (getKind() == ExceptionHandler) {

      final ShadowRange range = getRange();
      final InstructionList body = range.getBody();
      final InstructionHandle start = range.getStart();
      ...
      final UnresolvedType tx = getArgType(0);
      argVars[0] = genTempVar(tx, "ajc$arg0");
      final InstructionHandle insertedInstruction = range.insert(argVars[0].createStore(getFactory()), Range.OutsideBefore);
    ...
    if (thisJoinPointVar != null && !isThisJoinPointLazy && badAdvice != null && badAdvice.size() > 1) {
      ...
      int valid = 0;
      for (final Iterator<BcelAdvice> iter = badAdvice.iterator(); iter.hasNext(); ) {
        final BcelAdvice element = iter.next();
        final ISourceLocation sLoc = element.getSourceLocation();
        if (sLoc != null && sLoc.getLine() > 0) {
          valid++;
        }
      }
      if (valid != 0) {
        final ISourceLocation[] badLocs = new ISourceLocation[valid];
        int i = 0;
        for (final Iterator<BcelAdvice> iter = badAdvice.iterator(); iter.hasNext(); ) {
          final BcelAdvice element = iter.next();
          final ISourceLocation sLoc = element.getSourceLocation();
  ...
  public static BcelShadow makeConstructorExecution(BcelWorld world, LazyMethodGen enclosingMethod,
    ...
    final BcelShadow s = new BcelShadow(world, ConstructorExecution, world.makeJoinPointSignatureFromMethod(enclosingMethod,
  ...
  public static BcelShadow makeStaticInitialization(BcelWorld world, LazyMethodGen enclosingMethod) {
    ...
    if (clinitStart.getInstruction() instanceof InvokeInstruction) {
      final InvokeInstruction ii = (InvokeInstruction) clinitStart.getInstruction();
    ...
    final InstructionHandle clinitEnd = body.getEnd();
    ...
    final BcelShadow s = new BcelShadow(world, StaticInitialization, world.makeJoinPointSignatureFromMethod(enclosingMethod,
  ...
  public static BcelShadow makeExceptionHandler(BcelWorld world, ExceptionRange exceptionRange, LazyMethodGen enclosingMethod,
    ...
    final InstructionList body = enclosingMethod.getBody();
    final UnresolvedType catchType = exceptionRange.getCatchType();
    final UnresolvedType inType = enclosingMethod.getEnclosingClass().getType();

    final ResolvedMemberImpl sig = MemberImpl.makeExceptionHandlerSignature(inType, catchType);
    sig.setParameterNames(new String[]{findHandlerParamName(startOfHandler)});

    final BcelShadow s = new BcelShadow(world, ExceptionHandler, sig, enclosingMethod, enclosingShadow);
    final ShadowRange r = new ShadowRange(body);
    r.associateWithShadow(s);
    final InstructionHandle start = Range.genStart(body, startOfHandler);
    final InstructionHandle end = Range.genEnd(body, start);
  ...
  private static String findHandlerParamName(InstructionHandle startOfHandler) {
    if (startOfHandler.getInstruction().isStoreInstruction() && startOfHandler.getNext() != null) {
      final int slot = startOfHandler.getInstruction().getIndex();
      // System.out.println("got store: " + startOfHandler.getInstruction() + ", " + index);
      final Iterator<InstructionTargeter> tIter = startOfHandler.getNext().getTargeters().iterator();
      while (tIter.hasNext()) {
        final InstructionTargeter targeter = tIter.next();
        if (targeter instanceof LocalVariableTag) {
          final LocalVariableTag t = (LocalVariableTag) targeter;
          if (t.getSlot() == slot) {
            return t.getName();
          }
        }
  ...
  public static BcelShadow makeIfaceInitialization(BcelWorld world, LazyMethodGen constructor,
    ...
    final BcelShadow s = new BcelShadow(world, Initialization, interfaceConstructorSignature, constructor, null);
    // s.fallsThrough = true;
  ...
  public void initIfaceInitializer(InstructionHandle end) {
    ...
    final InstructionHandle nop = body.insert(end, InstructionConstants.NOP);
  ...
  public static BcelShadow makeUnfinishedInitialization(BcelWorld world, LazyMethodGen constructor) {
    final BcelShadow ret = new BcelShadow(world, Initialization, world.makeJoinPointSignatureFromMethod(constructor,
  ...
  public static BcelShadow makeUnfinishedPreinitialization(BcelWorld world, LazyMethodGen constructor) {
    final BcelShadow ret = new BcelShadow(world, PreInitialization, world.makeJoinPointSignatureFromMethod(constructor,
        Member.CONSTRUCTOR), constructor, null);
    if (constructor.getEffectiveSignature() != null) {
      ret.setMatchingSignature(constructor.getEffectiveSignature().getEffectiveSignature());
    }
  ...
  public static BcelShadow makeMethodExecution(BcelWorld world, LazyMethodGen enclosingMethod, boolean lazyInit) {
    ...
    final BcelShadow s = new BcelShadow(world, MethodExecution, enclosingMethod.getMemberView(), enclosingMethod, null);
  ...
  public static BcelShadow makeShadowForMethod(BcelWorld world, LazyMethodGen enclosingMethod, Shadow.Kind kind, Member sig) {
    final InstructionList body = enclosingMethod.getBody();
    final BcelShadow s = new BcelShadow(world, kind, sig, enclosingMethod, null);
    final ShadowRange r = new ShadowRange(body);
    r.associateWithShadow(s);
  ...
  public static BcelShadow makeAdviceExecution(BcelWorld world, LazyMethodGen enclosingMethod) {
    final InstructionList body = enclosingMethod.getBody();
    final BcelShadow s = new BcelShadow(world, AdviceExecution,
  ...
  public static BcelShadow makeConstructorCall(BcelWorld world, LazyMethodGen enclosingMethod, InstructionHandle callHandle,
    ...
    final Member sig = world.makeJoinPointSignatureForMethodInvocation(enclosingMethod.getEnclosingClass(),
        (InvokeInstruction) callHandle.getInstruction());

    final BcelShadow s = new BcelShadow(world, ConstructorCall, sig, enclosingMethod, enclosingShadow);
    final ShadowRange r = new ShadowRange(body);
    r.associateWithShadow(s);
  ...
  public static BcelShadow makeArrayConstructorCall(BcelWorld world, LazyMethodGen enclosingMethod,
    ...
    final InstructionList body = enclosingMethod.getBody();
    final Member sig = world.makeJoinPointSignatureForArrayConstruction(enclosingMethod.getEnclosingClass(), arrayInstruction);
    final BcelShadow s = new BcelShadow(world, ConstructorCall, sig, enclosingMethod, enclosingShadow);
    final ShadowRange r = new ShadowRange(body);
    r.associateWithShadow(s);
  ...
  public static BcelShadow makeMonitorEnter(BcelWorld world, LazyMethodGen enclosingMethod, InstructionHandle monitorInstruction,
                                            BcelShadow enclosingShadow) {
    final InstructionList body = enclosingMethod.getBody();
    final Member sig = world.makeJoinPointSignatureForMonitorEnter(enclosingMethod.getEnclosingClass(), monitorInstruction);
    final BcelShadow s = new BcelShadow(world, SynchronizationLock, sig, enclosingMethod, enclosingShadow);
    final ShadowRange r = new ShadowRange(body);
    r.associateWithShadow(s);
  ...
  public static BcelShadow makeMonitorExit(BcelWorld world, LazyMethodGen enclosingMethod, InstructionHandle monitorInstruction,
                                           BcelShadow enclosingShadow) {
    final InstructionList body = enclosingMethod.getBody();
    final Member sig = world.makeJoinPointSignatureForMonitorExit(enclosingMethod.getEnclosingClass(), monitorInstruction);
    final BcelShadow s = new BcelShadow(world, SynchronizationUnlock, sig, enclosingMethod, enclosingShadow);
    final ShadowRange r = new ShadowRange(body);
    r.associateWithShadow(s);
    r.associateWithTargets(Range.genStart(body, monitorInstruction), Range.genEnd(body, monitorInstruction));
    retargetAllBranches(monitorInstruction, r.getStart());
  ...
  public static BcelShadow makeMethodCall(BcelWorld world, LazyMethodGen enclosingMethod, InstructionHandle callHandle,
                                          BcelShadow enclosingShadow) {
    final InstructionList body = enclosingMethod.getBody();
    final BcelShadow s = new BcelShadow(world, MethodCall, world.makeJoinPointSignatureForMethodInvocation(
  ...
  public static BcelShadow makeShadowForMethodCall(BcelWorld world, LazyMethodGen enclosingMethod, InstructionHandle callHandle,
    ...
    final InstructionList body = enclosingMethod.getBody();
    final BcelShadow s = new BcelShadow(world, kind, sig, enclosingMethod, enclosingShadow);
    final ShadowRange r = new ShadowRange(body);
    r.associateWithShadow(s);
    r.associateWithTargets(Range.genStart(body, callHandle), Range.genEnd(body, callHandle));
    retargetAllBranches(callHandle, r.getStart());
  ...
  public static BcelShadow makeFieldGet(BcelWorld world, ResolvedMember field, LazyMethodGen enclosingMethod,
    ...
    final InstructionList body = enclosingMethod.getBody();
    final BcelShadow s = new BcelShadow(world, FieldGet, field,
  ...
  public static BcelShadow makeFieldSet(BcelWorld world, ResolvedMember field, LazyMethodGen enclosingMethod,
    ...
    final InstructionList body = enclosingMethod.getBody();
    final BcelShadow s = new BcelShadow(world, FieldSet, field,
  ...
  void initializeThisJoinPoint() {
    ...
    if (isThisJoinPointLazy) {
      ...
      final InstructionFactory fact = getFactory();
      final InstructionList il = createThisJoinPoint();
      il.append(thisJoinPointVar.createStore(fact));
      range.insert(il, Range.OutsideBefore);
    }
  }
  ...
  private boolean checkLazyTjp() {
    ...
    for (final Iterator i = mungers.iterator(); i.hasNext(); ) {
      final ShadowMunger munger = (ShadowMunger) i.next();
  ...
  InstructionList loadThisJoinPoint() {
    ...
    if (isThisJoinPointLazy) {
      ...
      if (lazyTjpConsumers > 1) {
        ...
        final InstructionHandle end = il.append(thisJoinPointVar.createLoad(fact));
  ...
  InstructionList createThisJoinPoint() {
    ...
    final BcelVar staticPart = getThisJoinPointStaticPartBcelVar();
  ...
  public BcelVar getThisJoinPointStaticPartBcelVar(final boolean isEnclosingJp) {
    if (thisJoinPointStaticPartVar == null) {
      final Field field = getEnclosingClass().getTjpField(this, isEnclosingJp);
  ...
  private InstructionList makeArgsObjectArray() {
    final InstructionFactory fact = getFactory();
    final BcelVar arrayVar = genTempVar(UnresolvedType.OBJECTARRAY);
    final InstructionList il = new InstructionList();
    final int alen = getArgCount();
  ...
  public UnresolvedType ensureTargetTypeIsCorrect(UnresolvedType tx) {
    ...
    final Member msig = getSignature();
    if (msig.getArity() == 0 && getKind() == MethodCall && msig.getName().charAt(0) == 'c' && tx.equals(ResolvedType.OBJECT)
      ...
      if (searchPtr.getInstruction().isLoadInstruction()) {
        final LocalVariableTag lvt = LazyMethodGen.getLocalVariableTag(searchPtr, searchPtr.getInstruction().getIndex());
        if (lvt != null) {
      ...
      if (searchPtr.getInstruction() instanceof FieldInstruction) {
        final FieldInstruction si = (FieldInstruction) searchPtr.getInstruction();
        final Type t = si.getFieldType(getEnclosingClass().getConstantPool());
      ...
      if (searchPtr.getInstruction() instanceof MULTIANEWARRAY) {
        final MULTIANEWARRAY ana = (MULTIANEWARRAY) searchPtr.getInstruction();
        // Type t = ana.getType(getEnclosingClass().getConstantPool());
  ...
  public void ensureInitializedArgVar(int argNumber) {
    ...
    final int positionOffset = (hasTarget() ? 1 : 0) + ((hasThis() && !getKind().isTargetSameAsThis()) ? 1 : 0);
    ...
    if (getKind().argsOnStack()) {
      ...
      for (int i = len - 1; i >= 0; i--) {
        final UnresolvedType type = getArgType(i);
        final BcelVar tmp = genTempVar(type, "ajc$arg" + i);
  ...
  public void initializeTargetAnnotationVars() {
    ...
    if (getKind().isTargetSameAsThis()) {
      ...
      final ResolvedType[] rtx = this.getTargetType().resolve(world).getAnnotationTypes(); // what about annotations we havent
      // gotten yet but we will get in
      // subclasses?
      for (int i = 0; i < rtx.length; i++) {
        final ResolvedType typeX = rtx[i];
  ...
  protected ResolvedMember getRelevantMember(ResolvedMember foundMember, Member relevantMember, ResolvedType relevantType) {
    ...
    final List<ConcreteTypeMunger> mungers = relevantType.resolve(world).getInterTypeMungers();
    for (ConcreteTypeMunger typeMunger : mungers) {
      if (typeMunger.getMunger() instanceof NewMethodTypeMunger || typeMunger.getMunger() instanceof NewConstructorTypeMunger) {
        final ResolvedMember fakerm = typeMunger.getSignature();
  ...
  protected ResolvedType[] getAnnotations(ResolvedMember foundMember, Member relevantMember, ResolvedType relevantType) {
    if (foundMember == null) {
      ...
      for (final Iterator<ConcreteTypeMunger> iter = mungers.iterator(); iter.hasNext(); ) {
        final Object munger = iter.next();
        final ConcreteTypeMunger typeMunger = (ConcreteTypeMunger) munger;
        if (typeMunger.getMunger() instanceof NewMethodTypeMunger
            || typeMunger.getMunger() instanceof NewConstructorTypeMunger) {
          final ResolvedMember fakerm = typeMunger.getSignature();
          // if (fakerm.hasAnnotations())

          final ResolvedMember ajcMethod = (getSignature().getKind() == ResolvedMember.CONSTRUCTOR ? AjcMemberMaker
          ...
          final ResolvedMember rmm = findMethod(typeMunger.getAspectType(), ajcMethod);
          if (fakerm.getName().equals(getSignature().getName())
              && fakerm.getParameterSignature().equals(getSignature().getParameterSignature())) {
  ...
  public void initializeKindedAnnotationVars() {
    ...
    final Member shadowSignature = getSignature();
    ...
    if (getKind() == Shadow.StaticInitialization) {
      ...
      final ResolvedMember foundMember = findMethod2(relevantType.resolve(world).getDeclaredMethods(), getSignature());
      ...
      if (annotationHolder == null) {
        ...
        for (ConcreteTypeMunger typeMunger : mungers) {
          if (typeMunger.getMunger() instanceof NewFieldTypeMunger) {
            final ResolvedMember fakerm = typeMunger.getSignature();
            // if (fakerm.hasAnnotations())
            final ResolvedMember ajcMethod = AjcMemberMaker.interFieldInitializer(fakerm, typeMunger.getAspectType());
            final ResolvedMember rmm = findMethod(typeMunger.getAspectType(), ajcMethod);
      ...
      final ResolvedMember foundMember = findMethod2(relevantType.getDeclaredMethods(), getSignature());
      annotations = getAnnotations(foundMember, shadowSignature, relevantType);
      annotationHolder = getRelevantMember(foundMember, annotationHolder, relevantType);
      final UnresolvedType ut = annotationHolder.getDeclaringType();
      ...
      final ResolvedMember found = findMethod2(relevantType.getDeclaredMethods(), getSignature());
    ...
    for (ResolvedType annotationType : annotations) {
      final AnnotationAccessVar accessVar = new AnnotationAccessVar(this, getKind(), annotationType.resolve(world), relevantType,
  ...
  private ResolvedMember findMethod2(ResolvedMember members[], Member sig) {
    final String signatureName = sig.getName();
    final String parameterSignature = sig.getParameterSignature();
  ...
  private ResolvedMember findField(ResolvedMember[] members, Member lookingFor) {
    for (int i = 0; i < members.length; i++) {
      final ResolvedMember member = members[i];
  ...
  public void initializeWithinAnnotationVars() {
    ...
    final ResolvedType[] annotations = getEnclosingType().resolve(world).getAnnotationTypes();
    for (int i = 0; i < annotations.length; i++) {
      final ResolvedType ann = annotations[i];
      final Kind k = Shadow.StaticInitialization;
  ...
  public void initializeWithinCodeAnnotationVars() {
    ...
    final ResolvedType[] annotations = getEnclosingMethod().getMemberView().getAnnotationTypes();
    for (int i = 0; i < annotations.length; i++) {
      final ResolvedType ann = annotations[i];
      final Kind k = (getEnclosingMethod().getMemberView().getKind() == Member.CONSTRUCTOR ? Shadow.ConstructorExecution
  ...
  public void weaveAfterReturning(BcelAdvice munger) {
    final List<InstructionHandle> returns = findReturnInstructions();
    final boolean hasReturnInstructions = !returns.isEmpty();

    // list of instructions that handle the actual return from the join point
    final InstructionList retList = new InstructionList();
    ...
    final InstructionList advice = getAfterReturningAdviceDispatchInstructions(munger, retList.getStart());

    if (hasReturnInstructions) {
      final InstructionHandle gotoTarget = advice.getStart();
      for (final Iterator<InstructionHandle> i = returns.iterator(); i.hasNext(); ) {
        final InstructionHandle ih = i.next();
  ...
  private List<InstructionHandle> findReturnInstructions() {
    final List<InstructionHandle> returns = new ArrayList<InstructionHandle>();
  ...
  private BcelVar generateReturnInstructions(List<InstructionHandle> returns, InstructionList returnInstructions) {
    ...
    if (this.hasANonVoidReturnType()) {
      ...
      while (newReturnInstruction == null && i >= 0) {
        final InstructionHandle ih = returns.get(i);
      ...
      final InstructionHandle lastReturnHandle = returns.get(returns.size() - 1);
      final Instruction newReturnInstruction = Utility.copyInstruction(lastReturnHandle.getInstruction());
      returnInstructions.append(newReturnInstruction);
    }
  ...
  private InstructionList getAfterReturningAdviceDispatchInstructions(BcelAdvice munger,
    ...
    final InstructionList advice = new InstructionList();
  ...
  private BcelVar insertAdviceInstructionsForBindingReturningParameter(InstructionList advice) {
    final BcelVar tempVar;
    final UnresolvedType tempVarType = getReturnType();
  ...
  public void weaveAfterThrowing(BcelAdvice munger, UnresolvedType catchType) {
    ...
    final InstructionFactory fact = getFactory();
    final InstructionList handler = new InstructionList();
    final BcelVar exceptionVar = genTempVar(catchType);
    ...
    if (this.getEnclosingMethod().getName().equals("<clinit>")) {
      final ResolvedType eiieType = world.resolve("java.lang.ExceptionInInitializerError");
      final ObjectType eiieBcelType = (ObjectType) BcelWorld.makeBcelType(eiieType);
      final InstructionList ih = new InstructionList(InstructionConstants.NOP);
      handler.append(exceptionVar.createLoad(fact));
      handler.append(fact.createInstanceOf(eiieBcelType));
      final InstructionBranch bi = InstructionFactory.createBranchInstruction(Constants.IFEQ, ih.getStart());
      handler.append(bi);
      handler.append(exceptionVar.createLoad(fact));
    ...
    final InstructionList endHandler = new InstructionList(exceptionVar.createLoad(fact));
    handler.append(munger.getAdviceInstructions(this, exceptionVar, endHandler.getStart()));
    handler.append(endHandler);
    handler.append(InstructionConstants.ATHROW);
    final InstructionHandle handlerStart = handler.getStart();

    if (isFallsThrough()) {
      final InstructionHandle jumpTarget = handler.append(InstructionConstants.NOP);
      handler.insert(InstructionFactory.createBranchInstruction(Constants.GOTO, jumpTarget));
    }
    final InstructionHandle protectedEnd = handler.getStart();
  ...
  public void weaveSoftener(BcelAdvice munger, UnresolvedType catchType) {
    ...
    final InstructionFactory fact = getFactory();
    final InstructionList handler = new InstructionList();
    final InstructionList rtExHandler = new InstructionList();
    final BcelVar exceptionVar = genTempVar(catchType);
    ...
    final InstructionHandle handlerStart = rtExHandler.getStart();

    if (isFallsThrough()) {
      final InstructionHandle jumpTarget = range.getEnd();// handler.append(fact.NOP);
      rtExHandler.insert(InstructionFactory.createBranchInstruction(Constants.GOTO, jumpTarget));
    }

    rtExHandler.append(handler);

    final InstructionHandle protectedEnd = rtExHandler.getStart();
  ...
  public void weavePerObjectEntry(final BcelAdvice munger, final BcelVar onVar) {
    ...
    final InstructionList entryInstructions = new InstructionList();
    final InstructionList entrySuccessInstructions = new InstructionList();
    ...
    final InstructionList testInstructions = munger.getTestInstructions(this, entrySuccessInstructions.getStart(),
  ...
  public void weavePerTypeWithinAspectInitialization(final BcelAdvice munger, UnresolvedType t) {
    ...
    final String aspectname = munger.getConcreteAspect().getName();

    final String ptwField = NameMangler.perTypeWithinFieldForTarget(munger.getConcreteAspect());
  ...
  public void weaveCflowEntry(final BcelAdvice munger, final Member cflowField) {
    ...
    final InstructionList entryInstructions = new InstructionList();
    {
      ...
      if (isPer) {
        ...
        final BcelVar[] cflowStateVars = munger.getExposedStateAsBcelVars(false);
        ...
        if (cflowStateVars.length == 0) {
          ...
          final int alen = cflowStateVars.length;
    ...
    weaveAfter(new BcelAdvice(null, null, null, 0, 0, 0, null, munger.getConcreteAspect()) {
      ...
      public InstructionList getAdviceInstructions(BcelShadow s, BcelVar extraArgVar, InstructionHandle ifNoAdvice) {
        final InstructionList exitInstructions = new InstructionList();
        if (munger.hasDynamicTests()) {
  ...
  public void weaveAroundInline(BcelAdvice munger, boolean hasDynamicTest) {
    ...
    if (mungerSig instanceof ResolvedMember) {
      final ResolvedMember rm = (ResolvedMember) mungerSig;
      if (rm.hasBackingGenericMember()) {
        mungerSig = rm.getBackingGenericMember();
      }
    }
    final ResolvedType declaringAspectType = world.resolve(mungerSig.getDeclaringType(), true);
    ...
    final ResolvedType rt = (declaringAspectType.isParameterizedType() ? declaringAspectType.getGenericType() : declaringAspectType);
    final BcelObjectType ot = BcelWorld.getBcelObjectType(rt);
    final LazyMethodGen adviceMethod = ot.getLazyClassGen().getLazyMethodGen(mungerSig);
    ...
    final LazyClassGen shadowClass = getEnclosingClass();
    ...
    final String extractedShadowMethodName = NameMangler.aroundShadowMethodName(getSignature(), shadowClass.getNewGeneratedNameTag());
    final List<String> parameterNames = new ArrayList<String>();
    final LazyMethodGen extractedShadowMethod = extractShadowInstructionsIntoNewMethod(extractedShadowMethodName, Modifier.PRIVATE,
        munger.getSourceLocation(), parameterNames);

    final List<BcelVar> argsToCallLocalAdviceMethodWith = new ArrayList<BcelVar>();
    final List<BcelVar> proceedVarList = new ArrayList<BcelVar>();
    ...
    final Type[] adviceParameterTypes = BcelWorld.makeBcelTypes(munger.getSignature().getParameterTypes());
    ...
    final Type[] extractedMethodParameterTypes = extractedShadowMethod.getArgumentTypes();

    final Type[] parameterTypes = new Type[extractedMethodParameterTypes.length + adviceParameterTypes.length + 1];
    ...
    final String localAdviceMethodName = NameMangler.aroundAdviceMethodName(getSignature(), shadowClass.getNewGeneratedNameTag());
    final LazyMethodGen localAdviceMethod = new LazyMethodGen(Modifier.PRIVATE | (world.useFinal() ? Modifier.FINAL : 0)
    ...
    final int nVars = adviceMethod.getMaxLocals() + extraParamOffset;
    final IntMap varMap = IntMap.idMap(nVars);
    ...
    {
      for (final Iterator<BcelVar> i = argsToCallLocalAdviceMethodWith.iterator(); i.hasNext(); ) {
        final BcelVar var = i.next();
        var.appendLoad(advice, fact);
      }
      // ??? we don't actually need to push NULL for the closure if we take care
      final boolean isAnnoStyleConcreteAspect = munger.getConcreteAspect().isAnnotationStyleAspect();
      final boolean isAnnoStyleDeclaringAspect = munger.getDeclaringAspect() != null ? munger.getDeclaringAspect().resolve(world)
    ...
    if (!hasDynamicTest) {
      ...
      final InstructionList afterThingie = new InstructionList(InstructionConstants.NOP);
      final InstructionList callback = makeCallToCallback(extractedShadowMethod);
    ...
    if (!munger.getDeclaringType().isAnnotationStyleAspect()) {
      final String proceedName = NameMangler.proceedMethodName(munger.getSignature().getName());

      InstructionHandle curr = localAdviceMethod.getBody().getStart();
      final InstructionHandle end = localAdviceMethod.getBody().getEnd();
      final ConstantPool cpg = localAdviceMethod.getEnclosingClass().getConstantPool();
      while (curr != end) {
        final InstructionHandle next = curr.getNext();
        final Instruction inst = curr.getInstruction();
      ...
      while (curr != end) {
        ...
        if ((inst instanceof INVOKEINTERFACE) && "proceed".equals(((INVOKEINTERFACE) inst).getMethodName(cpg))) {
          ...
          final InstructionList insteadProceedIl = getRedoneProceedCallForAnnotationStyle(fact, extractedShadowMethod, munger,
    ...
    final Type[] args = localAdviceMethod.getArgumentTypes();
    int argNumber = 0;
    for (int slot = 0; slot < extraParamOffset; argNumber++) { // slot will increase by the argument size each time
      ...
      final String argumentSignature = args[argNumber].getSignature();
      final LocalVariableTag lvt = new LocalVariableTag(argumentSignature, argumentName, slot, 0);
  ...
  private boolean isAnnotationStylePassingProceedingJoinPointOutOfAdvice(BcelAdvice munger, boolean hasDynamicTest,
    ...
    if (munger.getConcreteAspect().isAnnotationStyleAspect()) {
      ...
      final InstructionHandle end = adviceMethod.getBody().getEnd();
      final ConstantPool cpg = adviceMethod.getEnclosingClass().getConstantPool();
      while (curr != end) {
        final InstructionHandle next = curr.getNext();
        final Instruction inst = curr.getInstruction();
  ...
  private InstructionList getRedoneProceedCall(InstructionFactory fact, LazyMethodGen callbackMethod, BcelAdvice munger,
    ...
    final BcelVar[] adviceVars = munger.getExposedStateAsBcelVars(true);
    final IntMap proceedMap = makeProceedArgumentMap(adviceVars);
    ...
    if (munger.getBaseParameterCount() + 1 < proceedParamTypes.length) {
      final int len = munger.getBaseParameterCount() + 1;
      final ResolvedType[] newTypes = new ResolvedType[len];
      System.arraycopy(proceedParamTypes, 0, newTypes, 0, len);
      proceedParamTypes = newTypes;
    }

    // System.out.println("stateTypes: " + Arrays.asList(stateTypes));
    final BcelVar[] proceedVars = Utility.pushAndReturnArrayOfVars(proceedParamTypes, ret, fact, localAdviceMethod);

    final Type[] stateTypes = callbackMethod.getArgumentTypes();
    // System.out.println("stateTypes: " + Arrays.asList(stateTypes));

    for (int i = 0, len = stateTypes.length; i < len; i++) {
      final Type stateType = stateTypes[i];
      final ResolvedType stateTypeX = BcelWorld.fromBcel(stateType).resolve(world);
  ...
  private InstructionList getRedoneProceedCallForAnnotationStyle(InstructionFactory fact, LazyMethodGen callbackMethod,
    ...
    if (isProceedWithArgs) {
      ...
      final Type objectArrayType = Type.OBJECT_ARRAY;
      final int theObjectArrayLocalNumber = localAdviceMethod.allocateLocal(objectArrayType);
      ...
      final Type proceedingJpType = Type.getType("Lorg/aspectj/lang/ProceedingJoinPoint;");
      final int pjpLocalNumber = localAdviceMethod.allocateLocal(proceedingJpType);
      ...
      final boolean pointcutBindsThis = bindsThis(munger);
      final boolean pointcutBindsTarget = bindsTarget(munger);
      final boolean targetIsSameAsThis = getKind().isTargetSameAsThis();
      ...
      if (hasTarget()) {
        if (pointcutBindsTarget) {
          if (getKind().isTargetSameAsThis()) {
            ...
            final int position = (hasThis()/* && pointcutBindsThis */ ? 1 : 0);
            ret.append(InstructionFactory.createLoad(objectArrayType, theObjectArrayLocalNumber));
      ...
      final int indexIntoObjectArrayForArguments = (pointcutBindsThis ? 1 : 0) + (pointcutBindsTarget ? 1 : 0);

      final int len = callbackMethod.getArgumentTypes().length;
      for (int i = nextArgumentToProvideForCallback; i < len; i++) {
        final Type stateType = callbackMethod.getArgumentTypes()[i];
      ...
      final Type proceedingJpType = Type.getType("Lorg/aspectj/lang/ProceedingJoinPoint;");
      final int localJp = localAdviceMethod.allocateLocal(proceedingJpType);
  ...
  private boolean bindsThis(BcelAdvice munger) {
    final UsesThisVisitor utv = new UsesThisVisitor();
    munger.getPointcut().accept(utv, null);
    return utv.usesThis;
  }

  private boolean bindsTarget(BcelAdvice munger) {
    final UsesTargetVisitor utv = new UsesTargetVisitor();
    munger.getPointcut().accept(utv, null);
  ...
  public void weaveAroundClosure(BcelAdvice munger, boolean hasDynamicTest) {
    ...
    final int linenumber = getSourceLine();
    // MOVE OUT ALL THE INSTRUCTIONS IN MY SHADOW INTO ANOTHER METHOD!
    final LazyMethodGen callbackMethod = extractShadowInstructionsIntoNewMethod(
    ...
    final String closureClassName = NameMangler.makeClosureClassName(getEnclosingClass().getType(), getEnclosingClass()
        .getNewGeneratedNameTag());

    final Member constructorSig = new MemberImpl(Member.CONSTRUCTOR, UnresolvedType.forName(closureClassName), 0, "<init>",
        "([Ljava/lang/Object;)V");
    ...
    final InstructionList closureInstantiation = makeClosureInstantiation(constructorSig, closureHolder);

		/* LazyMethodGen constructor = */
    makeClosureClassAndReturnConstructor(closureClassName, callbackMethod, makeProceedArgumentMap(adviceVars));

    final InstructionList returnConversionCode;
    if (getKind() == PreInitialization) {
      returnConversionCode = new InstructionList();

      final BcelVar stateTempVar = genTempVar(UnresolvedType.OBJECTARRAY);
      ...
      final Type[] stateTypes = getSuperConstructorParameterTypes();

      returnConversionCode.append(InstructionConstants.ALOAD_0); // put "this" back on the stack
      for (int i = 0, len = stateTypes.length; i < len; i++) {
        final UnresolvedType bcelTX = BcelWorld.fromBcel(stateTypes[i]);
        final ResolvedType stateRTX = world.resolve(bcelTX, true);
      ...
      final UnresolvedType returnType = mungerSignature.getReturnType();
    ...
    if (!hasDynamicTest) {
      ...
      final InstructionList callback = makeCallToCallback(callbackMethod);
      final InstructionList postCallback = new InstructionList();
      if (terminatesWithReturn()) {
  ...
  InstructionList makeCallToCallback(LazyMethodGen callbackMethod) {
    final InstructionFactory fact = getFactory();
    final InstructionList callback = new InstructionList();
    if (thisVar != null) {
  ...
  private InstructionList makeClosureInstantiation(Member constructor, BcelVar holder) {
    ...
    // final Type objectArrayType = new ArrayType(Type.OBJECT, 1);
    final InstructionList il = new InstructionList();
    final int alen = getArgCount() + (thisVar == null ? 0 : 1) + ((targetVar != null && targetVar != thisVar) ? 1 : 0)
  ...
  private IntMap makeProceedArgumentMap(BcelVar[] adviceArgs) {
    ...
    final IntMap ret = new IntMap();
    for (int i = 0, len = adviceArgs.length; i < len; i++) {
      final BcelVar v = adviceArgs[i];
      if (v == null) {
        continue; // XXX we don't know why this is required
      }
      final int pos = v.getPositionInAroundState();
  ...
  private LazyMethodGen makeClosureClassAndReturnConstructor(String closureClassName, LazyMethodGen callbackMethod,
                                                             IntMap proceedMap) {
    final String superClassName = "org.aspectj.runtime.internal.AroundClosure";
    final Type objectArrayType = new ArrayType(Type.OBJECT, 1);

    final LazyClassGen closureClass = new LazyClassGen(closureClassName, superClassName, getEnclosingClass().getFileName(),
        Modifier.PUBLIC, new String[]{}, getWorld());
    final InstructionFactory fact = new InstructionFactory(closureClass.getConstantPool());

    // constructor
    final LazyMethodGen constructor = new LazyMethodGen(Modifier.PUBLIC, Type.VOID, "<init>", new Type[]{objectArrayType},
        new String[]{}, closureClass);
    final InstructionList cbody = constructor.getBody();
    ...
    // method
    final LazyMethodGen runMethod = new LazyMethodGen(Modifier.PUBLIC, Type.OBJECT, "run", new Type[]{objectArrayType},
        new String[]{}, closureClass);
    final InstructionList mbody = runMethod.getBody();
    final BcelVar proceedVar = new BcelVar(UnresolvedType.OBJECTARRAY.resolve(world), 1);
    // int proceedVarIndex = 1;
    final BcelVar stateVar = new BcelVar(UnresolvedType.OBJECTARRAY.resolve(world), runMethod.allocateLocal(1));
  ...
  LazyMethodGen extractShadowInstructionsIntoNewMethod(String extractedMethodName, int extractedMethodVisibilityModifier,
    ...
    final LazyMethodGen newMethod = createShadowMethodGen(extractedMethodName, extractedMethodVisibilityModifier, parameterNames);
    final IntMap remapper = makeRemap();
  ...
  private void addPreInitializationReturnCode(LazyMethodGen extractedMethod, Type[] superConstructorTypes) {
    final InstructionList body = extractedMethod.getBody();
    final InstructionFactory fact = getFactory();

    final BcelVar arrayVar = new BcelVar(world.getCoreType(UnresolvedType.OBJECTARRAY), extractedMethod.allocateLocal(1));

    final int len = superConstructorTypes.length;
  ...
  private Type[] getSuperConstructorParameterTypes() {
    ...
    final InstructionHandle superCallHandle = getRange().getEnd().getNext();
    final InvokeInstruction superCallInstruction = (InvokeInstruction) superCallHandle.getInstruction();
  ...
  private LazyMethodGen createShadowMethodGen(String newMethodName, int visibilityModifier, List<String> parameterNames) {
    ...
    final int modifiers = (world.useFinal() ? Modifier.FINAL : 0) | Modifier.STATIC | visibilityModifier;
    if (targetVar != null && targetVar != thisVar) {
      ...
      final ResolvedMember resolvedMember = getSignature().resolve(world);
    ...
    if (thisVar != null) {
      final UnresolvedType thisType = getThisType();
    ...
    if (this.getKind() == Shadow.FieldSet || this.getKind() == Shadow.FieldGet) {
      ...
      final String[] pnames = getSignature().getParameterNames(world);
      if (pnames != null) {
    ...
    final UnresolvedType returnType;
    if (getKind() == PreInitialization) {
  ...
  public BcelVar genTempVar(UnresolvedType utype) {
    final ResolvedType rtype = utype.resolve(world);
  ...
  public BcelVar genTempVar(UnresolvedType typeX, String localName) {
    final BcelVar tv = genTempVar(typeX);
  ...
  public ISourceLocation getSourceLocation() {
    final int sourceLine = getSourceLine();
