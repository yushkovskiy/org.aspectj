...
public class BcelPerClauseAspectAdder extends BcelTypeMunger {
  ...
  private void doAggressiveInner(LazyClassGen gen) {
    ...
    if (!hasGeneratedInner) {
      if (kind == PerClause.PEROBJECT) {// redundant test - see constructor, but safer
        // inner class
        final UnresolvedType interfaceTypeX = AjcMemberMaker.perObjectInterfaceType(aspectType);
        final LazyClassGen interfaceGen = new LazyClassGen(interfaceTypeX.getName(), "java.lang.Object", null,
  ...
  private boolean hasPerClauseMembersAlready(LazyClassGen classGen) {
    final ResolvedMember[] methods = classGen.getBcelObjectType().getDeclaredMethods();
  ...
  private void generatePerClauseMembers(LazyClassGen classGen) {
    ...
    final ResolvedMember failureFieldInfo = AjcMemberMaker.initFailureCauseField(aspectType);
    if (kind == PerClause.SINGLETON) {
      classGen.addField(makeFieldGen(classGen, failureFieldInfo), null);
    }

    if (kind == PerClause.SINGLETON) {
      final ResolvedMember perSingletonFieldInfo = AjcMemberMaker.perSingletonField(aspectType);
      ...
      final ResolvedMember perCflowFieldInfo = AjcMemberMaker.perCflowField(aspectType);
      classGen.addField(makeFieldGen(classGen, perCflowFieldInfo), null);
    } else if (kind == PerClause.PERTYPEWITHIN) {
      final ResolvedMember perTypeWithinForField = AjcMemberMaker.perTypeWithinWithinTypeField(aspectType, aspectType);
  ...
  private void generatePerSingletonAspectOfMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perSingletonAspectOfMethod(aspectType));
    flagAsSynthetic(method, false);
    classGen.addMethodGen(method);

    final InstructionList il = method.getBody();
    il.append(Utility.createGet(factory, AjcMemberMaker.perSingletonField(aspectType)));
    final InstructionBranch ifNotNull = InstructionFactory.createBranchInstruction(Constants.IFNONNULL, null);
    il.append(ifNotNull);
    ...
    final InstructionHandle ifElse = il.append(Utility.createGet(factory, AjcMemberMaker.perSingletonField(aspectType)));
  ...
  private void generatePerSingletonHasAspectMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perSingletonHasAspectMethod(aspectType));
    flagAsSynthetic(method, false);
    classGen.addMethodGen(method);

    final InstructionList il = method.getBody();
    il.append(Utility.createGet(factory, AjcMemberMaker.perSingletonField(aspectType)));
    final InstructionBranch ifNull = InstructionFactory.createBranchInstruction(Constants.IFNULL, null);
    il.append(ifNull);
    il.append(InstructionFactory.PUSH(classGen.getConstantPool(), true));
    il.append(InstructionFactory.createReturn(Type.INT));
    final InstructionHandle ifElse = il.append(InstructionFactory.PUSH(classGen.getConstantPool(), false));
    il.append(InstructionFactory.createReturn(Type.INT));
  ...
  private void generatePerSingletonAjcClinitMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.ajcPostClinitMethod(aspectType));
    ...
    final LazyMethodGen clinit = classGen.getStaticInitializer();
    il = new InstructionList();
    final InstructionHandle tryStart = il.append(factory.createInvoke(aspectType.getName(), NameMangler.AJC_POST_CLINIT_NAME,
        Type.VOID, Type.NO_ARGS, Constants.INVOKESTATIC));
    final InstructionBranch tryEnd = InstructionFactory.createBranchInstruction(Constants.GOTO, null);
    il.append(tryEnd);
    final InstructionHandle handler = il.append(InstructionConstants.ASTORE_0);
  ...
  private void generatePerObjectAspectOfMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final ReferenceType interfaceType = (ReferenceType) BcelWorld.makeBcelType(AjcMemberMaker.perObjectInterfaceType(aspectType));
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perObjectAspectOfMethod(aspectType));
    flagAsSynthetic(method, false);
    classGen.addMethodGen(method);
    ...
    final InstructionBranch ifEq = InstructionFactory.createBranchInstruction(Constants.IFEQ, null);
    il.append(ifEq);
    il.append(InstructionConstants.ALOAD_0);
    ...
    final InstructionHandle ifNullElse = il.append(InstructionConstants.POP);
    ifNull.setTarget(ifNullElse);
    final InstructionHandle ifEqElse = il.append(factory.createNew(AjcMemberMaker.NO_ASPECT_BOUND_EXCEPTION.getName()));
  ...
  private void generatePerObjectHasAspectMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final ReferenceType interfaceType = (ReferenceType) BcelWorld.makeBcelType(AjcMemberMaker.perObjectInterfaceType(aspectType));
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perObjectHasAspectMethod(aspectType));
    flagAsSynthetic(method, false);
    classGen.addMethodGen(method);
    ...
    il.append(InstructionFactory.createReturn(Type.INT));
    final InstructionHandle ifEqElse = il.append(InstructionConstants.ICONST_0);
    ifEq.setTarget(ifEqElse);
  ...
  private void generatePerObjectBindMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final ReferenceType interfaceType = (ReferenceType) BcelWorld.makeBcelType(AjcMemberMaker.perObjectInterfaceType(aspectType));
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perObjectBind(aspectType));
    flagAsSynthetic(method, true);
    classGen.addMethodGen(method);
    ...
    final InstructionHandle end = il.append(InstructionFactory.createReturn(Type.VOID));
    ifEq.setTarget(end);
  ...
  private void generatePerCflowAspectOfMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perCflowAspectOfMethod(aspectType));
    flagAsSynthetic(method, false);
    classGen.addMethodGen(method);
  ...
  private void generatePerCflowHasAspectMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perCflowHasAspectMethod(aspectType));
    flagAsSynthetic(method, false);
    classGen.addMethodGen(method);
  ...
  private void generatePerCflowPushMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perCflowPush(aspectType));
    flagAsSynthetic(method, true);
    classGen.addMethodGen(method);
  ...
  private void generatePerCflowAjcClinitMethod(LazyClassGen classGen) {
    ...
    final LazyMethodGen method = classGen.getAjcPreClinit(); // Creates a clinit if there isn't one
  ...
  private void generatePerTWAspectOfMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perTypeWithinAspectOfMethod(aspectType, classGen.getWorld()
        .isInJava5Mode()));
    flagAsSynthetic(method, false);
    classGen.addMethodGen(method);

    final InstructionList il = method.getBody();
    final InstructionHandle tryStart = il.append(InstructionConstants.ALOAD_0);
    ...
    il.append(factory.createInvoke(AjcMemberMaker.NO_ASPECT_BOUND_EXCEPTION.getName(), "<init>", Type.VOID, new Type[]{
        Type.STRING, new ObjectType("java.lang.Throwable")}, Constants.INVOKESPECIAL));
    il.append(InstructionConstants.ATHROW);
    final InstructionHandle ifElse = il.append(InstructionConstants.ALOAD_1);
    ifNonNull.setTarget(ifElse);
    il.append(InstructionFactory.createReturn(Type.OBJECT));

    final InstructionHandle handler = il.append(InstructionConstants.ASTORE_1);
    il.append(factory.createNew(AjcMemberMaker.NO_ASPECT_BOUND_EXCEPTION.getName()));
    il.append(InstructionConstants.DUP);
    il.append(factory.createInvoke(AjcMemberMaker.NO_ASPECT_BOUND_EXCEPTION.getName(), "<init>", Type.VOID, Type.NO_ARGS,
        Constants.INVOKESPECIAL));
  ...
  private void generatePerTWGetWithinTypeNameMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perTypeWithinGetWithinTypeNameMethod(aspectType, classGen
  ...
  private void generatePerTWHasAspectMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perTypeWithinHasAspectMethod(aspectType, classGen.getWorld()
        .isInJava5Mode()));
    flagAsSynthetic(method, false);
    classGen.addMethodGen(method);
    ...
    final InstructionHandle ifElse = il.append(InstructionConstants.ICONST_0);
    ifNull.setTarget(ifElse);
    il.append(InstructionConstants.IRETURN);
  ...
  private void generatePerTWGetInstanceMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perTypeWithinGetInstance(aspectType));
    flagAsSynthetic(method, true);
    classGen.addMethodGen(method);
  ...
  private void generatePerTWCreateAspectInstanceMethod(LazyClassGen classGen) {
    final InstructionFactory factory = classGen.getFactory();
    final LazyMethodGen method = makeMethodGen(classGen, AjcMemberMaker.perTypeWithinCreateAspectInstance(aspectType));
    flagAsSynthetic(method, true);
    classGen.addMethodGen(method);
