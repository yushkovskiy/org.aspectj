 ...
 */
public final class AtAjAttributes {
  @NotNull
  ...
  private static class AjAttributeStruct {
     ...
     */
    @NotNull
    final List<AjAttribute> ajAttributes = new ArrayList<AjAttribute>();
    ...
    public AjAttributeStruct(@NotNull ResolvedType type, @NotNull ISourceContext sourceContext, @NotNull IMessageHandler messageHandler) {
      enclosingType = type;
  ...
  private static class AjAttributeMethodStruct extends AjAttributeStruct {
    ...
    public AjAttributeMethodStruct(@NotNull Method method, @NotNull BcelMethod bMethod, @NotNull ResolvedType type,
                                   @NotNull ISourceContext sourceContext, @NotNull IMessageHandler messageHandler) {
  ...
  private static class AjAttributeFieldStruct extends AjAttributeStruct {
    ...
    public AjAttributeFieldStruct(@NotNull Field field, BcelField bField, @NotNull ResolvedType type,
                                  @NotNull ISourceContext sourceContext, @NotNull IMessageHandler messageHandler) {
      super(type, sourceContext, messageHandler);
      this.field = field;
      // this.bField = bField;
   ...
   */
  public static boolean acceptAttribute(@NotNull Attribute attribute) {
   ...
   */
  @NotNull
  public static List<AjAttribute> readAj5ClassAttributes(@Nullable AsmManager model, @NotNull JavaClass javaClass,
                                                         @NotNull ReferenceType type, @NotNull ISourceContext context,
                                                         @NotNull IMessageHandler msgHandler, boolean isCodeStyleAspect) {
    final boolean ignoreThisClass = javaClass.getClassName().charAt(0) == 'o'
    ...
    final Constant[] cpool = javaClass.getConstantPool().getConstantPool();
    for (int i = 0; i < cpool.length; i++) {
      final Constant constant = cpool[i];
      if (constant != null && constant.getTag() == Constants.CONSTANT_Utf8) {
        final String constantValue = ((ConstantUtf8) constant).getValue();
        if (constantValue.length() > 28 && constantValue.charAt(1) == 'o') {
          if (constantValue.startsWith("Lorg/aspectj/lang/annotation")) {
            ...
            if ("Lorg/aspectj/lang/annotation/DeclareAnnotation;".equals(constantValue)) {
              if (!AtAjAttributes.class.getName().equals(javaClass.getClassName()))
    ...
    final AjAttributeStruct struct = new AjAttributeStruct(type, context, msgHandler);
    ...
    for (int i = 0; i < attributes.length; i++) {
      ...
      if (acceptAttribute(attribute)) {
        final RuntimeAnnos rvs = (RuntimeAnnos) attribute;
    ...
    for (int i = attributes.length - 1; i >= 0; i--) {
      ...
      if (attribute.getName().equals(WeaverVersionInfo.AttributeName)) {
        try {
          final VersionedDataInputStream s = new VersionedDataInputStream(new ByteArrayInputStream(
    ...
    if (wvinfo == null) {
      ...
      final ReferenceTypeDelegate delegate = type.getDelegate();
    ...
    for (int i = 0; i < javaClass.getMethods().length; i++) {
      final Method method = javaClass.getMethods()[i];
      ...
      final Attribute[] mattributes = method.getAttributes();
      for (int j = 0; j < mattributes.length; j++) {
        final Attribute mattribute = mattributes[j];
    ...
    for (int i = 0; i < fs.length; i++) {
      ...
      // FIXME alex optimize, this method struct will gets recreated for
      // advice extraction
      final AjAttributeFieldStruct fstruct = new AjAttributeFieldStruct(field, null, type, context, msgHandler);
      final Attribute[] fattributes = field.getAttributes();

      for (int j = 0; j < fattributes.length; j++) {
        final Attribute fattribute = fattributes[j];
        if (acceptAttribute(fattribute)) {
          final RuntimeAnnos frvs = (RuntimeAnnos) fattribute;
   ...
   */
  @NotNull
  public static List<AjAttribute> readAj5MethodAttributes(@NotNull Method method, @NotNull BcelMethod bMethod,
                                                          @NotNull ResolvedType type, @Nullable ResolvedPointcutDefinition preResolvedPointcut,
                                                          @NotNull ISourceContext context, @NotNull IMessageHandler msgHandler) {
    if (method.getName().startsWith(NameMangler.PREFIX)) {
      return Collections.emptyList(); // already dealt with by ajc...
    }

    final AjAttributeMethodStruct struct = new AjAttributeMethodStruct(method, bMethod, type, context, msgHandler);
   ...
   */
  @NotNull
  public static List<AjAttribute> readAj5FieldAttributes(@NotNull Field field, @NotNull BcelField bField,
                                                         @NotNull ResolvedType type,
                                                         @NotNull ISourceContext context, @NotNull IMessageHandler msgHandler) {
   ...
   */
  private static boolean handleAspectAnnotation(@NotNull RuntimeAnnos runtimeAnnotations, @NotNull AjAttributeStruct struct) {
    final AnnotationGen aspect = getAnnotation(runtimeAnnotations, AjcMemberMaker.ASPECT_ANNOTATION);
    if (aspect != null) {
      ...
      final NameValuePair aspectPerClause = getAnnotationElement(aspect, VALUE);
      ...
      if (aspectPerClause == null) {
        ...
        final String perX = aspectPerClause.getValue().stringifyValue();
      ...
      if (perClause == null) {
        ...
        final AjAttribute.Aspect aspectAttribute = new AjAttribute.Aspect(perClause);
   ...
   */
  @Nullable
  private static PerClause parsePerClausePointcut(@NotNull String perClauseString, @NotNull AjAttributeStruct struct) {
   ...
   * @return true if found
   */
  private static boolean handlePrecedenceAnnotation(@NotNull RuntimeAnnos runtimeAnnotations, @NotNull AjAttributeStruct struct) {
    final AnnotationGen aspect = getAnnotation(runtimeAnnotations, AjcMemberMaker.DECLAREPRECEDENCE_ANNOTATION);
    if (aspect != null) {
      final NameValuePair precedence = getAnnotationElement(aspect, VALUE);
      if (precedence != null) {
        final String precedencePattern = precedence.getValue().stringifyValue();
        final PatternParser parser = new PatternParser(precedencePattern);
        final DeclarePrecedence ajPrecedence = parser.parseDominates();
   ...
   * @return true if found
   */
  private static boolean handleDeclareParentsAnnotation(@NotNull RuntimeAnnos runtimeAnnotations, @NotNull AjAttributeFieldStruct struct) {// ,
    // ResolvedPointcutDefinition
    // preResolvedPointcut)
    // {
    final AnnotationGen decp = getAnnotation(runtimeAnnotations, AjcMemberMaker.DECLAREPARENTS_ANNOTATION);
    if (decp != null) {
      final NameValuePair decpPatternNVP = getAnnotationElement(decp, VALUE);
      final String decpPattern = decpPatternNVP.getValue().stringifyValue();
      if (decpPattern != null) {
        TypePattern typePattern = parseTypePattern(decpPattern, struct);
        final ResolvedType fieldType = UnresolvedType.forSignature(struct.field.getSignature()).resolve(
            struct.enclosingType.getWorld());
        if (fieldType.isInterface()) {
          final TypePattern parent = parseTypePattern(fieldType.getName(), struct);
          final FormalBinding[] bindings = new org.aspectj.weaver.patterns.FormalBinding[0];
          final IScope binding = new BindingScope(struct.enclosingType, struct.context, bindings);
          // first add the declare implements like
          final List<TypePattern> parents = new ArrayList<TypePattern>(1);
          parents.add(parent);
          final DeclareParents dp = new DeclareParents(typePattern, parents, false);
          ...
          final NameValuePair defaultImplNVP = getAnnotationElement(decp, "defaultImpl");
          if (defaultImplNVP != null) {
            final ClassElementValue defaultImpl = (ClassElementValue) defaultImplNVP.getValue();
            ...
            if (defaultImplClassName.equals("org.aspectj.lang.annotation.DeclareParents")) {
              ...
              final ResolvedType impl = struct.enclosingType.getWorld().resolve(defaultImplClassName, false);
              final ResolvedMember[] mm = impl.getDeclaredMethods();
              final int implModifiers = impl.getModifiers();
              final boolean defaultVisibilityImpl = !(Modifier.isPrivate(implModifiers)
              ...
              for (int i = 0; i < mm.length; i++) {
                final ResolvedMember resolvedMember = mm[i];
          ...
          final Iterator<ResolvedMember> methodIterator = fieldType.getMethodsIncludingIntertypeDeclarations(false, true);
          while (methodIterator.hasNext()) {
            final ResolvedMember method = methodIterator.next();
            ...
            if (method.isAbstract()) {
              ...
              final MethodDelegateTypeMunger mdtm = new MethodDelegateTypeMunger(method, struct.enclosingType,
          ...
          if (hasAtLeastOneMethod && defaultImplClassName != null) {
            final ResolvedMember fieldHost = AjcMemberMaker.itdAtDeclareParentsField(null, fieldType, struct.enclosingType);
   ...
   */
  @NotNull
  public static String getMethodForMessage(@NotNull AjAttributeMethodStruct methodstructure) {
    ...
    sb.append("(");
    final Type[] args = methodstructure.method.getArgumentTypes();
    if (args != null) {
   ...
   * <br>
   */
  private static boolean handleDeclareMixinAnnotation(@NotNull RuntimeAnnos runtimeAnnotations, @NotNull AjAttributeMethodStruct struct) {
    final AnnotationGen declareMixinAnnotation = getAnnotation(runtimeAnnotations, AjcMemberMaker.DECLAREMIXIN_ANNOTATION);
    if (declareMixinAnnotation == null) {
      // No annotation found
      return false;
    }

    final Method annotatedMethod = struct.method;
    final World world = struct.enclosingType.getWorld();
    final NameValuePair declareMixinPatternNameValuePair = getAnnotationElement(declareMixinAnnotation, VALUE);

    // declareMixinPattern could be of the form "Bar*" or "A || B" or "Foo+"
    final String declareMixinPattern = declareMixinPatternNameValuePair.getValue().stringifyValue();
    ...
    final ResolvedType methodReturnType = UnresolvedType.forSignature(annotatedMethod.getReturnType().getSignature()).resolve(world);
    ...
    final NameValuePair interfaceListSpecified = getAnnotationElement(declareMixinAnnotation, "interfaces");

    final List<TypePattern> newParents = new ArrayList<TypePattern>(1);
    final List<ResolvedType> newInterfaceTypes = new ArrayList<ResolvedType>(1);
    if (interfaceListSpecified != null) {
      final ArrayElementValue arrayOfInterfaceTypes = (ArrayElementValue) interfaceListSpecified.getValue();
      final int numberOfTypes = arrayOfInterfaceTypes.getElementValuesArraySize();
      final ElementValue[] theTypes = arrayOfInterfaceTypes.getElementValuesArray();
      for (int i = 0; i < numberOfTypes; i++) {
        final ClassElementValue interfaceType = (ClassElementValue) theTypes[i];
        // Check: needs to be resolvable
        // TODO crappy replace required
        final ResolvedType ajInterfaceType = UnresolvedType.forSignature(interfaceType.getClassString().replace("/", "."))
            .resolve(world);
        ...
        final TypePattern newParent = parseTypePattern(ajInterfaceType.getName(), struct);
      ...
      final TypePattern newParent = parseTypePattern(methodReturnType.getName(), struct);
    ...
    final DeclareParents dp = new DeclareParentsMixin(targetTypePattern, newParents);
    dp.resolve(binding);
    ...
    for (final Iterator<ResolvedType> iterator = newInterfaceTypes.iterator(); iterator.hasNext(); ) {
      final ResolvedType typeForDelegation = iterator.next();
      ...
      final ResolvedMember[] methods = typeForDelegation.getMethodsWithoutIterator(true, false, false).toArray(
          new ResolvedMember[0]);
      for (int i = 0; i < methods.length; i++) {
        ResolvedMember method = methods[i];
        if (method.isAbstract()) {
    ...
    if (hasAtLeastOneMethod) {
      final ResolvedMember fieldHost = AjcMemberMaker.itdAtDeclareParentsField(null, methodReturnType, struct.enclosingType);
   ...
   * @return true if found
   */
  private static boolean handleBeforeAnnotation(@NotNull RuntimeAnnos runtimeAnnotations, @NotNull AjAttributeMethodStruct struct,
                                                @Nullable ResolvedPointcutDefinition preResolvedPointcut) {
    final AnnotationGen before = getAnnotation(runtimeAnnotations, AjcMemberMaker.BEFORE_ANNOTATION);
    if (before != null) {
      final NameValuePair beforeAdvice = getAnnotationElement(before, VALUE);
      if (beforeAdvice != null) {
        // this/target/args binding
        final String argumentNames = getArgNamesValue(before);
        ...
        final int extraArgument = extractExtraArgument(struct.method);
        ...
        final ISourceLocation sl = struct.context.makeSourceLocation(struct.bMethod.getDeclarationLineNumber(),
  ...
  private static boolean handleAfterAnnotation(RuntimeAnnos runtimeAnnotations, AjAttributeMethodStruct struct,
    ...
    final AnnotationGen after = getAnnotation(runtimeAnnotations, AjcMemberMaker.AFTER_ANNOTATION);
    if (after != null) {
      final NameValuePair afterAdvice = getAnnotationElement(after, VALUE);
      if (afterAdvice != null) {
        // this/target/args binding
        FormalBinding[] bindings = new org.aspectj.weaver.patterns.FormalBinding[0];
        final String argumentNames = getArgNamesValue(after);
        if (argumentNames != null) {
          struct.unparsedArgumentNames = argumentNames;
        }
        try {
  ...
  private static boolean handleAfterReturningAnnotation(RuntimeAnnos runtimeAnnotations, AjAttributeMethodStruct struct,
    ...
    final AnnotationGen after = getAnnotation(runtimeAnnotations, AjcMemberMaker.AFTERRETURNING_ANNOTATION);
    if (after != null) {
      final NameValuePair annValue = getAnnotationElement(after, VALUE);
      final NameValuePair annPointcut = getAnnotationElement(after, POINTCUT);
      final NameValuePair annReturned = getAnnotationElement(after, RETURNING);
      ...
      if (annReturned != null) {
        ...
        if (isNullOrEmpty(returned)) {
          ...
          // the advice
          final String[] pNames = owningMethod.getParameterNames();
  ...
  private static boolean handleAfterThrowingAnnotation(RuntimeAnnos runtimeAnnotations, AjAttributeMethodStruct struct,
    ...
    final AnnotationGen after = getAnnotation(runtimeAnnotations, AjcMemberMaker.AFTERTHROWING_ANNOTATION);
    if (after != null) {
      final NameValuePair annValue = getAnnotationElement(after, VALUE);
      final NameValuePair annPointcut = getAnnotationElement(after, POINTCUT);
      final NameValuePair annThrown = getAnnotationElement(after, THROWING);
  ...
  private static boolean handleAroundAnnotation(RuntimeAnnos runtimeAnnotations, AjAttributeMethodStruct struct,
                                                ResolvedPointcutDefinition preResolvedPointcut) {
    final AnnotationGen around = getAnnotation(runtimeAnnotations, AjcMemberMaker.AROUND_ANNOTATION);
    if (around != null) {
      final NameValuePair aroundAdvice = getAnnotationElement(around, VALUE);
      if (aroundAdvice != null) {
        // this/target/args binding
        final String argumentNames = getArgNamesValue(around);
        if (argumentNames != null) {
          struct.unparsedArgumentNames = argumentNames;
        }
        FormalBinding[] bindings = new org.aspectj.weaver.patterns.FormalBinding[0];
  ...
  private static boolean handlePointcutAnnotation(RuntimeAnnos runtimeAnnotations, AjAttributeMethodStruct struct) {
    final AnnotationGen pointcut = getAnnotation(runtimeAnnotations, AjcMemberMaker.POINTCUT_ANNOTATION);
    if (pointcut == null) {
      return false;
    }
    final NameValuePair pointcutExpr = getAnnotationElement(pointcut, VALUE);
    ...
    final String argumentNames = getArgNamesValue(pointcut);
    if (argumentNames != null) {
      struct.unparsedArgumentNames = argumentNames;
    }
    // this/target/args binding
    ...
    final UnresolvedType[] argumentTypes = new UnresolvedType[struct.method.getArgumentTypes().length];
   ...
   * @return true if found
   */
  private static boolean handleDeclareErrorOrWarningAnnotation(@Nullable AsmManager model, @NotNull RuntimeAnnos runtimeAnnotations,
                                                               @NotNull AjAttributeFieldStruct struct) {
    final AnnotationGen error = getAnnotation(runtimeAnnotations, AjcMemberMaker.DECLAREERROR_ANNOTATION);
    boolean hasError = false;
    if (error != null) {
      final NameValuePair declareError = getAnnotationElement(error, VALUE);
      if (declareError != null) {
        ...
        final Pointcut pc = parsePointcut(declareError.getValue().stringifyValue(), struct, false);
        if (pc == null) {
          hasError = false;// cannot parse pointcut
        } else {
          final DeclareErrorOrWarning deow = new DeclareErrorOrWarning(true, pc, struct.field.getConstantValue().toString());
    ...
    final AnnotationGen warning = getAnnotation(runtimeAnnotations, AjcMemberMaker.DECLAREWARNING_ANNOTATION);
    boolean hasWarning = false;
    if (warning != null) {
      final NameValuePair declareWarning = getAnnotationElement(warning, VALUE);
      if (declareWarning != null) {
        if (!STRING_DESC.equals(struct.field.getSignature()) || struct.field.getConstantValue() == null) {
          reportError("@DeclareWarning used on a non String constant field", struct);
          return false;
        }
        final Pointcut pc = parsePointcut(declareWarning.getValue().stringifyValue(), struct, false);
        if (pc == null) {
          hasWarning = false;// cannot parse pointcut
        } else {
          final DeclareErrorOrWarning deow = new DeclareErrorOrWarning(false, pc, struct.field.getConstantValue().toString());
          setDeclareErrorOrWarningLocation(model, deow, struct);
          struct.ajAttributes.add(new AjAttribute.DeclareAttribute(deow));
   ...
   * @param struct
   */
  private static void setDeclareErrorOrWarningLocation(@Nullable AsmManager model, @NotNull DeclareErrorOrWarning deow, @NotNull AjAttributeFieldStruct struct) {
    final IHierarchy top = (model == null ? null : model.getHierarchy());
    if (top != null && top.getRoot() != null) {
      final IProgramElement ipe = top.findElementForLabel(top.getRoot(), IProgramElement.Kind.FIELD, struct.field.getName());
      if (ipe != null && ipe.getSourceLocation() != null) {
        final ISourceLocation sourceLocation = ipe.getSourceLocation();
        final int start = sourceLocation.getOffset();
        final int end = start + struct.field.getName().length();
  ...
  private static FormalBinding[] extractBindings(AjAttributeMethodStruct struct) throws UnreadableDebugInfoException {
    final Method method = struct.method;
    final String[] argumentNames = struct.getArgumentNames();
    ...
    final List<FormalBinding> bindings = new ArrayList<FormalBinding>();
    for (int i = 0; i < argumentNames.length; i++) {
      final String argumentName = argumentNames[i];
      final UnresolvedType argumentType = UnresolvedType.forSignature(method.getArgumentTypes()[i].getSignature());
  ...
  private static FormalBinding[] extractBindings(AjAttributeMethodStruct struct, String excludeFormal)
    ...
    final FormalBinding[] bindings = extractBindings(struct);
    // int excludeIndex = -1;
    for (int i = 0; i < bindings.length; i++) {
      final FormalBinding binding = bindings[i];
  ...
  private static int extractExtraArgument(Method method) {
    final Type[] methodArgs = method.getArgumentTypes();
    final String[] sigs = new String[methodArgs.length];
   ...
   */
  @Nullable
  private static AnnotationGen getAnnotation(@NotNull RuntimeAnnos rvs, @NotNull UnresolvedType annotationType) {
  ...
  private static String getArgNamesValue(AnnotationGen anno) {
    final List<NameValuePair> elements = anno.getValues();
   ...
   */
  @NotNull
  private static String[] getMethodArgumentNames(@NotNull Method method, @NotNull String argNamesFromAnnotation,
                                                 @NotNull AjAttributeMethodStruct methodStruct) {
    ...
    final LocalVariableTable lt = method.getLocalVariableTable();
    if (lt != null) {
      final LocalVariable[] lvt = lt.getLocalVariableTable();
      for (int j = 0; j < lvt.length; j++) {
        final LocalVariable localVariable = lvt[j];
        if (localVariable != null) { // pr348488
          ...
          final String typename = (methodStruct.enclosingType != null ? methodStruct.enclosingType.getName() : "");
      ...
      if (argNamesFromAnnotation != null) {
        final StringTokenizer st = new StringTokenizer(argNamesFromAnnotation, " ,");
        final List<String> args = new ArrayList<String>();
        while (st.hasMoreTokens()) {
          args.add(st.nextToken());
        }
        if (args.size() != method.getArgumentTypes().length) {
          final StringBuilder shortString = new StringBuilder().append(lastbit(method.getReturnType().toString())).append(" ")
  ...
  private static class MethodArgument {
    ...
    public MethodArgument(@NotNull String name, int indexOnStack) {
      this.name = name;
  ...
  private static void setIgnoreUnboundBindingNames(Pointcut pointcut, FormalBinding[] bindings) {
    ...
    for (int i = 0; i < bindings.length; i++) {
      final FormalBinding formalBinding = bindings[i];
  ...
  private static Pointcut parsePointcut(String pointcutString, AjAttributeStruct struct, boolean allowIf) {
    try {
      final PatternParser parser = new PatternParser(pointcutString, struct.context);
      final Pointcut pointcut = parser.parsePointcut();
      parser.checkEof();
  ...
  private static boolean hasIf(Pointcut pointcut) {
    final IfFinder visitor = new IfFinder();
  ...
  private static TypePattern parseTypePattern(String patternString, AjAttributeStruct location) {
    try {
      final TypePattern typePattern = new PatternParser(patternString).parseTypePattern();
