...
public class TypeXTestCase extends TestCase {
  ...
  public void testUnresolvedTypes() {
    ...
    final int[] sizes = new int[]{1, 2, 1, 1, 1, 1, 0};
  ...
  public void testNameAndSigWithInners() {
    final UnresolvedType t = UnresolvedType.forName("java.util.Map$Entry");
...
//		System.err.println(tx.dump());
  ...
  public void testTypeXForParameterizedTypes() {
    if (LangUtil.is15VMOrGreater()) { // no funny types pre 1.5
      final World world = new BcelWorld();
      final UnresolvedType stringType = UnresolvedType.forName("java/lang/String");
      final ResolvedType listOfStringType =
  ...
  public void testTypeFactoryForParameterizedTypes() {
    if (LangUtil.is15VMOrGreater()) { // no funny types pre 1.5
      final UnresolvedType enumOfSimpleType =
      ...
      final UnresolvedType enumOfNestedType =
      ...
      final UnresolvedType nestedTypeOfParameterized =
      ...
      final UnresolvedType doublyNestedTypeSignatures =
  ...
  private void checkTX(UnresolvedType tx, boolean shouldBeParameterized, int numberOfTypeParameters) {
    ...
    if (numberOfTypeParameters == 0) {
      final UnresolvedType[] params = tx.getTypeParameters();
  ...
  private void isPrimitiveTest(UnresolvedType[] types, boolean[] isPrimitives) {
    for (int i = 0, len = types.length; i < len; i++) {
      final UnresolvedType type = types[i];
      final boolean b = isPrimitives[i];
  ...
  private void nameSignatureTest(String[] ns, String[] ss) {
    for (int i = 0, len = ns.length; i < len; i++) {
      final String n = ns[i];
      final String s = ss[i];
      final UnresolvedType tn = UnresolvedType.forName(n);
      final UnresolvedType ts = UnresolvedType.forSignature(s);
      ...
      for (int j = 0; j < len; j++) {
        if (i == j) continue;
        final UnresolvedType tn1 = UnresolvedType.forName(ns[j]);
        final UnresolvedType ts1 = UnresolvedType.forSignature(ss[j]);
