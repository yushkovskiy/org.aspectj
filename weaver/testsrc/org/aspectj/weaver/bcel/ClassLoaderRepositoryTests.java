...
//		ClassLoaderRepository.sharedCacheCompactFrequency = 16384;
  ...
  private ClassLoaderRepository setupRepository() throws Exception {
    ...
    final ClassLoader res = new URLClassLoader(new URL[]{f.toURL()}, cl);
    final ClassLoaderRepository rep = new ClassLoaderRepository(res);
    return rep;
  }

  private void compareTwoRepositories() throws Exception {
    final ClassLoaderRepository rep1 = setupRepository();
    final ClassLoaderRepository rep2 = setupRepository();
    int i = 0;
    while (entries.hasMoreElements()) {
      final ZipEntry zfe = (ZipEntry) entries.nextElement();
      final String classfileName = zfe.getName();
      if (classfileName.endsWith(".class")) {
        final String clazzname = classfileName.substring(0, classfileName.length() - 6).replace('/', '.');
...
//		 ClassLoaderRepository.sharedCacheCompactFrequency = 200;
    //loadOnce();
    final TestThread[] threads = new TestThread[6];
...
//		ClassLoaderRepository.compactSharedCache();
  ...
  private Field getSharedMapField() throws Exception {
    final Field field = ClassLoaderRepository.class.getDeclaredField("sharedCache");
