...
public abstract class AbstractCacheBackingTestSupport extends TestCase {
  ...
  public static File getClassContainerLocationFile(Class<?> clazz)
    ...
    try {
      final URI uri = getClassContainerLocationURI(clazz);
  ...
  public static URI getClassContainerLocationURI(Class<?> clazz) throws URISyntaxException {
    final URL url = getClassContainerLocationURL(clazz);
  ...
  public static final String getURLSource(String externalForm) {
    ...
    final int sepPos = url.indexOf(RESOURCE_SUBPATH_SEPARATOR);
    if (sepPos < 0) {
  ...
  protected static final void writeIndex(File indexFile, Collection<? extends IndexEntry> entries) throws IOException {
    final File indexDir = indexFile.getParentFile();
    if ((!indexDir.exists()) && (!indexDir.mkdirs())) {
      throw new IOException("Failed to create path to " + indexFile.getAbsolutePath());
    }

    final int numEntries = LangUtil.isEmpty(entries) ? 0 : entries.size();
    final IndexEntry[] entryValues = (numEntries <= 0) ? null : entries.toArray(new IndexEntry[numEntries]);
    ...
    final ObjectOutputStream oos = new ObjectOutputStream(new BufferedOutputStream(new FileOutputStream(indexFile), 4096));
    try {
  ...
  public static final void assertArrayEquals(String msg, byte[] expected, byte[] actual) {
    final int eLen = LangUtil.isEmpty(expected) ? 0 : expected.length;
    final int aLen = LangUtil.isEmpty(actual) ? 0 : expected.length;
    assertEquals(msg + "[mismatched length]", eLen, aLen);

    for (int index = 0; index < eLen; index++) {
      final byte eb = expected[index];
      final byte ab = actual[index];
      if (eb != ab) {
