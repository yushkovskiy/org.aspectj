...
public class WeavedClassCacheTest extends AbstractCacheBackingTestSupport {
  ...
  public void testGetCachingClassHandler() throws Exception {
    final WeavedClassCache cache = createCache();
    final GeneratedClassHandler newHandle = cache.getCachingClassHandler();
  ...
  public void testCache() throws Exception {
    reset();
    final WeavedClassCache cache = createCache();
    final CacheStatistics stats = cache.getStats();
  ...
  public void testRemove() throws Exception {
    reset();
    final WeavedClassCache cache = createCache();
    final CachedClassReference ref = cache.createCacheKey(FAKE_CLASS, FAKE_BYTES);
    assertNull(cache.get(ref, FAKE_BYTES));
    cache.put(ref, FAKE_BYTES, FAKE_BYTES);
    assertNotNull(cache.get(ref, FAKE_BYTES));
    cache.remove(ref);
