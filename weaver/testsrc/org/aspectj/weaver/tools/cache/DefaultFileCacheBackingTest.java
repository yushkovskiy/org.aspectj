...
public class DefaultFileCacheBackingTest extends AbstractCacheBackingTestSupport {
  ...
  public void testCreateBacking() throws Exception {
    final CacheBacking backing = DefaultFileCacheBacking.createBacking(root);
  ...
  public void testPut() throws Exception {
    final CacheBacking backing = DefaultFileCacheBacking.createBacking(root);
    backing.put(new CachedClassEntry(fakeRef, FAKE_BYTES, CachedClassEntry.EntryType.WEAVED), FAKE_BYTES);
    final File cachedFile = new File(root, fakeKey);
  ...
  public void testGet() throws Exception {
    final DefaultFileCacheBacking backing = DefaultFileCacheBacking.createBacking(root);
    assertNull(backing.get(fakeRef, FAKE_BYTES));
    backing.put(new CachedClassEntry(fakeRef, FAKE_BYTES, CachedClassEntry.EntryType.WEAVED), FAKE_BYTES);
    final File cachedFile = new File(root, fakeKey);
    assertTrue(cachedFile.isFile());
    ...
    final CachedClassEntry entry = backing.get(fakeRef, FAKE_BYTES);
  ...
  public void testMultiFile() throws Exception {
    ...
    final DefaultFileCacheBacking backing = DefaultFileCacheBacking.createBacking(root);
    // add weaved
    final CachedClassReference wref = resolver.weavedKey(FAKE_CLASS + "WEAVED", FAKE_BYTES);
    ...
    // add generated
    final CachedClassReference gref = resolver.generatedKey(FAKE_CLASS + "GENERATED");
    ...
    // add ignored
    final CachedClassReference iref = resolver.generatedKey(FAKE_CLASS + "IGNORED");
  ...
  public void testOriginalClassBytesChanged() {
    ...
    final byte[] newBytes = new byte[FAKE_BYTES.length];
  ...
  private boolean indexEntryExists(AbstractIndexedFileCacheBacking cache, String key, long expectedCRC) throws Exception {
    long storedCRC = 0L;
    final IndexEntry[] index = cache.readIndex(new File(root, AbstractIndexedFileCacheBacking.INDEX_FILE));
