...
public class PointcutDesignatorHandlerTests extends TestCase {
  ...
  public void testParseWithHandler() {
    if (needToSkip) return;
    final PointcutParser parser = PointcutParser.getPointcutParserSupportingAllPrimitivesAndUsingContextClassloaderForResolution();
    final BeanDesignatorHandler beanHandler = new BeanDesignatorHandler();
...
//        PointcutExpression pc = 
  ...
  public void testParsingBeanInReferencePointcut02() throws Exception {
    ...
    // This should be found and resolved
    final PointcutExpression pc = parser.parsePointcutExpression("CounterAspect.testBean1toString()");
  ...
  public void testStaticMatch() throws Exception {
    ...
    final BeanDesignatorHandler beanHandler = new BeanDesignatorHandler();
    parser.registerPointcutDesignatorHandler(beanHandler);
    final PointcutExpression pc = parser.parsePointcutExpression("bean(myBean)");
    final DefaultMatchingContext context = new DefaultMatchingContext();
  ...
  public void testDynamicMatch() throws Exception {
    ...
    parser.registerPointcutDesignatorHandler(beanHandler);
    final PointcutExpression pc = parser.parsePointcutExpression("bean(myBean)");
    final ShadowMatch sm = pc.matchesMethodExecution(Object.class.getMethod("toString", new Class[0]));
    final DefaultMatchingContext context = new DefaultMatchingContext();
