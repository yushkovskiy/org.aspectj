...
public class ReadingAttributes extends TestCase {
  ...
  public void testWeaverStateInfo() throws ClassNotFoundException, IOException {
    ...
    final JavaClass jc = getClassFrom(new File("n:/temp"), "com.springsource.petclinic.domain.Visit");
    assertNotNull(jc);
    final Attribute[] attrs = jc.getAttributes();
    for (int i = 0; i < attrs.length; i++) {
      System.out.println(attrs[i].getName());
      if (attrs[i].getName().endsWith("WeaverState")) {
        final Unknown u = (Unknown) attrs[i];
        final VersionedDataInputStream vdis = new VersionedDataInputStream(new ByteArrayInputStream(u.getBytes()), null);
        // WeaverStateInfo wsi =
  ...
  public SyntheticRepository createRepos(File cpentry) {
    final ClassPath cp = new ClassPath(cpentry + File.pathSeparator + System.getProperty("java.class.path"));
